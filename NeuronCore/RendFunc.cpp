...
void pie_BuildTransTable(UDWORD tableNo)
{
  ...
  for (i = 0; i < 256; i++)
  {
    ...
    case TRANS_BRITE:
      if (static_cast<UDWORD>(psPalette[i].r) + 50 > 255)
        red = 255;
      else
        red = (psPalette[i].r + 50);

      if (static_cast<UDWORD>(psPalette[i].b) + 50 > 255)
        blue = 255;
      else
        blue = (psPalette[i].b + 50);

      if (static_cast<UDWORD>(psPalette[i].g) + 50 > 255)
        green = 255;
      else
    ...
    default: ASSERT((FALSE,"Invalid transparency filter selection"));
      break;
    }
