...
static BLOCK_HEAP* psBlockList = nullptr;
...
BOOL blkCreate(BLOCK_HEAP** ppsHeap, SDWORD init, SDWORD ext)
{
  ...
  *ppsHeap = static_cast<BLOCK_HEAP*>(RMALLOC(sizeof(BLOCK_HEAP)));
  if (!*ppsHeap)
  {
    DBERROR(("blkCreate: Out of memory"));
    return FALSE;
  }
  (*ppsHeap)->psBlocks = static_cast<BLOCK_HEAP_MEM*>(RMALLOC(sizeof(BLOCK_HEAP_MEM)));
  if (!(*ppsHeap)->psBlocks)
  {
    DBERROR(("blkCreate: Out of memory"));
    return FALSE;
  }
  (*ppsHeap)->psBlocks->pMem = static_cast<UBYTE*>(RMALLOC(init));
  ...
  (*ppsHeap)->psBlocks->psNext = nullptr;

#ifdef DEBUG_BLOCK
  (*ppsHeap)->pFileName = pCallFileName;
  (*ppsHeap)->line = callLine;
  (*ppsHeap)->psMemTreap = nullptr;
  (*ppsHeap)->free = FALSE;
...
void blkPrintDetails(BLOCK_HEAP* psHeap)
{
  ...
  {
#ifdef DEBUG_BLOCK
    UDWORD Left = static_cast<UDWORD>((psHeap->psBlocks->pMem) + (psHeap->psBlocks->size) - (psHeap->psBlocks->pFree));
    DBPRINTF(
    ("ptr=%p init=%d ext=%d used=%d (Start=$%x Free=$%x Left=%d)\n",psHeap,psHeap->init,psHeap->ext,psHeap->TotalAllocated,psHeap->
      psBlocks->pMem,psHeap->psBlocks->pFree,Left));
...
BOOL blkSpecialFree(BLOCK_HEAP* psHeap, void* Ptr)
{
  ...
  for (psCurr = psHeap->psBlocks; psCurr; psCurr = psCurr->psNext) { if ((UDWORD)psCurr->pLastAllocated == RequestedFreeMem) { 
#ifdef DEBUG_BLOCK

UDWORD BlockSize = ((UDWORD)psCurr->pFree) - RequestedFreeMem;DBPRINTF(("FREED %d block bytes\n",BlockSize)); // /// del me now !
psHeap->TotalAllocated-=BlockSize;
#endif

psCurr->pFree= psCurr->pLastAllocated; psCurr->pLastAllocated=0; // remove pointer so that it cant be allocated againreturn(TRUE);
// able to return mem}
	}
...
void* blkAlloc(BLOCK_HEAP* psHeap, SDWORD size)
{
  ...
  if ((psCurr == nullptr) && (psHeap->ext != 0))
  {
    ...
    if (size < psHeap->ext)
    {
      psNew->pMem = static_cast<UBYTE*>(RMALLOC(psHeap->ext));
      psNew->size = psHeap->ext;
    }
    else
    {
      psNew->pMem = static_cast<UBYTE*>(RMALLOC(size));
      psNew->size = size;
    }
    if (!psNew->pMem)
    {
    ...
    for (psCurr = psHeap->psBlocks; psCurr->psNext != nullptr; psCurr = psCurr->psNext);
    psCurr->psNext = psNew;
  }
  ...
  psNode = static_cast<MEM_NODE*>(pAlloc);
  ...
  memset((UBYTE*)(psNode) + sizeof(MEM_NODE), SAFETY_ZONE_BYTE, SAFETY_ZONE_SIZE);
  memset((UBYTE*)(psNode) + sizeof(MEM_NODE) + SAFETY_ZONE_SIZE + allocSize, SAFETY_ZONE_BYTE, SAFETY_ZONE_SIZE);
#if MEMORY_SET
  /* The PC initialises malloc'ed memory already, no need to do it again */
  memset((UBYTE*)(psNode) + sizeof(MEM_NODE) + SAFETY_ZONE_SIZE, INITIALISE_BYTE, allocSize);
#endif

  // offset the return value by the header size
  pAlloc = (static_cast<UBYTE*>(pAlloc) + sizeof(MEM_NODE) + SAFETY_ZONE_SIZE);
#endif

  psCurr->pLastAllocated = static_cast<UBYTE*>(pAlloc);
...
#ifdef DEBUG
    if (bRes == TRUE) { DBPRINTF(("blkFree called - memory successfully released\n")); }
    else {}
#endif
#endif
  }
...
BLOCK_HEAP* blkFind(void* pPtr)
{
  ...
  for (psHeap = psBlockList; psHeap; psHeap = psHeap->psNext)
  {
    ...
    {
      if (static_cast<UBYTE*>(pPtr) >= psMem->pMem && static_cast<UBYTE*>(pPtr) < psMem->pMem + psMem->size)
        return psHeap;
    }
  }
...
BOOL blkPointerValid(BLOCK_HEAP* psHeap, void* pData, SDWORD size)
{
  ...
  // Create a dummy node for the search
  // This is only looked at by memBlockCmp so only need to set the object and size
  sNode.pObj = static_cast<UBYTE*>(pData) - sizeof(MEM_NODE) - SAFETY_ZONE_SIZE;
  sNode.size = size;
  ...
  return FALSE;
#else
  psHeap = psHeap; pData = pData; size = size; return TRUE;
#endif
}
...
BOOL blkPointerValidAll(void* pData, SDWORD size)
{
  ...
  return FALSE;
#else
  pData = pData; size = size; return TRUE;
#endif
}

BLOCK_HEAP* psSuspendedHeap = nullptr;
...
void blockCurrentBlockInfo(void)
{
  ...
  if (psCurHeap == nullptr)
    DBPRINTF(("*** No current block defined\n"));
  else
  {
    UDWORD Left = static_cast<UDWORD>((psCurHeap->psBlocks->pMem) + (psCurHeap->psBlocks->size) - (psCurHeap->psBlocks->pFree));

    DBPRINTF(
    ("ptr=%p init=%d ext=%d used=%d (Start=$%x Free=$%x Left=%d)\n",psCurHeap,psCurHeap->init,psCurHeap->ext,psCurHeap->TotalAllocated,
      psCurHeap->psBlocks->pMem,psCurHeap->psBlocks->pFree,Left));
  }
#endif
}
