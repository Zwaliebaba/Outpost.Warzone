...
using BINARY_HDR = struct _binary_hdr
{
  UBYTE aType[4];
  UDWORD version;
  UDWORD size;
...
void scriptSetCallbackTab(CALLBACK_SYMBOL* psCallTab) { asScrCallbackTab = psCallTab; }void scriptSetTypeTab(TYPE_SYMBOL* psTypeTab)
{
...
BOOL scriptSaveProg(SCRIPT_CODE* psProg, UDWORD* pSize, UBYTE** ppData)
{
  ...
  alignVars = (psProg->numGlobals % 2) == 0 ? psProg->numGlobals : psProg->numGlobals + 1;
  ...
  *pSize = sizeof(BINARY_HDR) + psProg->size + sizeof(INTERP_TYPE) * psProg->numGlobals + sizeof(ARRAY_DATA) * psProg->numArrays + sizeof(
    TRIGGER_DATA) * psProg->numTriggers + TabSize;

  debugInfo = psProg->psDebug != nullptr;
  if (debugInfo)
  {
    *pSize = *pSize + sizeof(SCRIPT_DEBUG) * psProg->debugEntries;
    for (i = 0; i < psProg->numGlobals; i++) { *pSize += sizeof(STORAGE_TYPE) + strlen(psProg->psVarDebug[i].pIdent) + 1; }
    for (i = 0; i < psProg->numArrays; i++) { *pSize += sizeof(STORAGE_TYPE) + strlen(psProg->psArrayDebug[i].pIdent) + 1; }
  }

  // Allocate the save buffer
  *ppData = static_cast<UBYTE*>(MALLOC(*pSize));
  if (!*ppData)
  {
  ...
  while (((UBYTE*)ip) < pPos + psProg->size)
  {
    ...
    case OP_CALL:
      ...
      saveFunc = 0;
      MatchFound = FALSE;
      for (psFunc = asScrInstinctTab; psFunc->pFunc != nullptr; psFunc++)
      {
    ...
    case OP_VARCALL:
      ...
      saveFunc = 0;
      MatchFound = FALSE;
      for (psVar = asScrObjectVarTab; psVar->pIdent != nullptr; psVar++)
      {
        ...
        if (((UDWORD)psVar->get) == fptr && psVar->index == data)
        {
          saveFunc = psVar - asScrObjectVarTab;
      ...
      for (psVar = asScrExternalTab; psVar->pIdent != nullptr; psVar++)
      {
        if (((UDWORD)psVar->set) == fptr && psVar->index == data)
        {
  ...
  if (debugInfo)
  {
    ...
    for (i = 0; i < psProg->debugEntries; i++)
    {
      ...
      SavedDebug->pLabel = nullptr; // remove the ascii label pointers from the saved binary info
      SavedDebug++;
      IngameDebug++;
    }
    ...
    {
      *pPos = psProg->psVarDebug[i].storage;
      ...
      if (psProg->psVarDebug[i].pIdent == nullptr)
      {
    ...
    {
      *pPos = psProg->psArrayDebug[i].storage;
      pPos += sizeof(STORAGE_TYPE);

      DbgISize += sizeof(STORAGE_TYPE);

      // We need to be able to handle NULL strings because the bastarding script compiler sometime generates them !!!!1
      if (psProg->psArrayDebug[i].pIdent == nullptr)
      {
        *pPos = 0; // Null string ... just store a zero
        pPos++; // Move it on by one
        DbgISize++;
...
BOOL scriptLoadProg(UDWORD size, UBYTE* pData, SCRIPT_CODE** ppsProg)
{
  ...
  psProg = static_cast<SCRIPT_CODE*>(MALLOC(sizeof(SCRIPT_CODE)));
  if (!psProg)
  {
  ...
  {
    psProg->pGlobals = static_cast<INTERP_TYPE*>(MALLOC(sizeof(INTERP_TYPE) * psProg->numGlobals));
    if (!psProg->pGlobals)
    {
      DBERROR(("scriptLoadProg: out of memory"));
      return FALSE;
    ...
    psProg->pGlobals = nullptr;

  if (psProg->numArrays > 0)
  {
    // Load the array data
    psProg->psArrayInfo = static_cast<ARRAY_DATA*>(MALLOC(sizeof(ARRAY_DATA) * psProg->numArrays));
  ...
  psProg->psTriggerData = static_cast<TRIGGER_DATA*>(MALLOC(sizeof(TRIGGER_DATA) * psProg->numTriggers));
  ...
  psProg->pTriggerTab = static_cast<UWORD*>(MALLOC(sizeof(UWORD) * (psProg->numTriggers+1)));
  ...
  psProg->pEventTab = static_cast<UWORD*>(MALLOC(sizeof(UWORD) * (psProg->numEvents+1)));
  if (!psProg->pEventTab)
  {
    DBERROR(("scriptLoadProg: out of memory"));
    return FALSE;
  ...
  psProg->pEventLinks = static_cast<SWORD*>(MALLOC(sizeof(SWORD) * psProg->numEvents));
  ...
  psProg->pCode = static_cast<UDWORD*>(MALLOC(psProg->size));
  if (!psProg->pCode)
  {
    DBERROR(("scriptLoadProg: out of memory"));
    return FALSE;
  ...
  if (debugInfo)
  {
    ...
    psProg->psDebug = static_cast<SCRIPT_DEBUG*>(MALLOC(sizeof(SCRIPT_DEBUG) * psProg->debugEntries));
    if (!psProg->psDebug)
    {
      DBERROR(("scriptLoadProg: out of memory"));
      return FALSE;
    ...
    if (psProg->numGlobals > 0)
    {
      psProg->psVarDebug = static_cast<VAR_DEBUG*>(MALLOC(sizeof(VAR_DEBUG) * psProg->numGlobals));
    ...
    {
      psProg->psVarDebug = nullptr;
      psProg->pGlobals = nullptr;
    }
    if (psProg->numArrays > 0)
    {
      psProg->psArrayDebug = static_cast<ARRAY_DEBUG*>(MALLOC(sizeof(ARRAY_DEBUG) * psProg->numArrays));
    ...
    {
      psProg->psArrayInfo = nullptr;
      psProg->psArrayDebug = nullptr;
    }
    ...
    for (i = 0; i < psProg->numGlobals; i++)
    {
      ...
      psProg->psVarDebug[i].storage = *pPos;
      pPos += sizeof(STORAGE_TYPE);
      ...
      if (*pPos == 0)
      {
        psProg->psVarDebug[i].pIdent = nullptr;
        StringSize = 1;
      }
      else
      {
        ...
        psProg->psVarDebug[i].pIdent = static_cast<STRING*>(MALLOC(StringSize));
    ...
    for (i = 0; i < psProg->numArrays; i++)
    {
      ...
      psProg->psArrayDebug[i].storage = *pPos;
      pPos += sizeof(STORAGE_TYPE);
      DbgSize += sizeof(STORAGE_TYPE);
      Tot += sizeof(STORAGE_TYPE);
      ...
      if (*pPos == 0)
      {
        psProg->psArrayDebug[i].pIdent = nullptr;
        StringSize = 1;
      }
      else
      {
        ...
        psProg->psArrayDebug[i].pIdent = static_cast<STRING*>(MALLOC(StringSize));
...
void scriptFreeCode(SCRIPT_CODE* psCode)
{
  ...
  FREE(psCode->pCode);
  if (psCode->pTriggerTab) { FREE(psCode->pTriggerTab); }
  if (psCode->psTriggerData) { FREE(psCode->psTriggerData); }
  FREE(psCode->pEventTab);
  FREE(psCode->pEventLinks);
  if (psCode->pGlobals != nullptr) { FREE(psCode->pGlobals); }
  if (psCode->psArrayInfo != nullptr) { FREE(psCode->psArrayInfo); }
  if (psCode->psDebug)
  {
    for (i = 0; i < psCode->debugEntries; i++) { if (psCode->psDebug[i].pLabel) { FREE(psCode->psDebug[i].pLabel); } }
    FREE(psCode->psDebug);
  }
  if (psCode->psVarDebug)
  {
    for (i = 0; i < psCode->numGlobals; i++) { if (psCode->psVarDebug[i].pIdent) { FREE(psCode->psVarDebug[i].pIdent); } }
    FREE(psCode->psVarDebug);
  }
  if (psCode->psArrayDebug)
  {
    for (i = 0; i < psCode->numArrays; i++) { if (psCode->psArrayDebug[i].pIdent) { FREE(psCode->psArrayDebug[i].pIdent); } }
