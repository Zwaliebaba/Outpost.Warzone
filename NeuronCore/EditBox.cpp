...
{
  ASSERT((PTRVALID(psWidget, sizeof(W_EDITBOX)), "editBoxInitialise: Invalid edit box pointer"));
...
static void fitStringEnd(STRING* pBuffer, UDWORD boxWidth, UWORD* pStart, UWORD* pCount, UWORD* pCharWidth)
{
  UDWORD len;
  UWORD printWidth, printChars, width;
  STRING* pCurr;
  ...
  /* Return the number of characters and their width */
  *pStart = static_cast<UWORD>(len - printChars);
  *pCount = printChars;
  *pCharWidth = printWidth;
}
...
void editBoxRun(W_EDITBOX* psWidget, W_CONTEXT* psContext)
{
  ...
  mx = psContext->mx;
  my = psContext->my;
  if (mousePressed(MOUSE_LMB) && (mx < psWidget->x || (mx > psWidget->x + psWidget->width) || my < psWidget->y || (my > psWidget->y +
    psWidget->height)))
  {
  ...
  for (key = inputGetKey(); key != 0 && !done; key = inputGetKey())
  {
    ...
    case INPBUF_RIGHT:
      ...
      /* If the cursor has gone off the right of the edit box,
			 * need to update the printable text.
			 */
      if (pos > static_cast<UDWORD>(printStart + printChars))
      {
        printStart += WEDB_CHARJUMP;
        if (printStart >= len)
          printStart = static_cast<UWORD>(len - 1);
        fitStringStart(pBuffer + printStart, psWidget->width, &printChars, &printWidth);
      }
      break;
    case INPBUF_UP:
    ...
    case INPBUF_BKSPACE:
      ...
      else { fitStringStart(pBuffer + printStart, psWidget->width, &printChars, &printWidth); }
      break;
    case INPBUF_TAB:
      break;
    case INPBUF_CR:
    ...
    default:
      ...
      if (editState == WEDBS_INSERT)
        insertChar(pBuffer, &pos, static_cast<STRING>(key));
      else
        overwriteChar(pBuffer, &pos, static_cast<STRING>(key));
  ...
  psWidget->insPos = static_cast<UWORD>(pos);
...
{
  ASSERT((PTRVALID(psWidget, sizeof(W_EDITBOX)), "editBoxSetString: Invalid edit box pointer"));
...
void editBoxReleased(W_EDITBOX* psWidget) { (void)psWidget; }
...
#if CURSOR_BLINK
  blink = (GetTickCount() / WEDB_BLINKRATE) % 2; if ((psEdBox->state & WEDBS_MASK) == WEDBS_INSERT && blink)
#else
