...
#if COPY_FILE_STRING
  pCFile = (STRING*)MALLOC(strlen(pFileName) + 1); if (!pCFile) { return; } strcpy(pCFile, pFileName);
#else
  pCFile = pFileName;
#endif
}
...
BOOL heapCreate(OBJ_HEAP** ppsHeap, UDWORD size, UDWORD init, UDWORD ext)
{
  ...
  int Heap;
#endif
  DEBUG_ASSERT_TEXT(size >= sizeof(FREE_OBJECT), "heapCreate: object is too small to be stored in free list");
  ...
  *ppsHeap = static_cast<OBJ_HEAP*>(MALLOC(sizeof(OBJ_HEAP)));
  if (*ppsHeap == nullptr)
  {
    DBERROR(("heapCreate: Out of memory"));
    return FALSE;
  }
  (*ppsHeap)->pMemory = static_cast<UBYTE*>(MALLOC(size * init));
...
BOOL heapAlloc(OBJ_HEAP* psHeap, void** ppObject)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psHeap, sizeof(OBJ_HEAP)), "heapAlloc: Invalid heap pointer");

  if (psHeap->psFree == nullptr)
  {
...
#endif
    psNew = static_cast<HEAP_EXTENSION*>(MALLOC(sizeof(HEAP_EXTENSION)));
    if (psNew == nullptr)
    {
      /* Out of memory */
      return FALSE;
    }
    psNew->pMemory = static_cast<UBYTE*>(MALLOC(psHeap->objSize * psHeap->extAlloc));
    if (psNew->pMemory == nullptr)
    {
      /* Out of memory */
      FREE(psNew);
      return FALSE;
  ...
  *ppObject = psHeap->psFree;
  ...
  {
    for (pBase = static_cast<UBYTE*>(*ppObject) + sizeof(FREE_OBJECT); pBase < static_cast<UBYTE*>(*ppObject) + psHeap->objSize; pBase++)
    {
      if (*pBase != FREE_BYTE)
        break;
    }
    DEBUG_ASSERT_TEXT(pBase == static_cast<UBYTE*>(*ppObject) + psHeap->objSize, "heapAlloc: unallocated object memory has been overwritten");
  }

  /* Store the call position */
  psHdr = static_cast<HEAP_OBJHDR*>(*ppObject);
  psHdr->pFile = pCFile;
  psHdr->line = cPos;
  *ppObject = static_cast<UBYTE*>(*ppObject) + sizeof(HEAP_OBJHDR);
...
BOOL heapFree(OBJ_HEAP* psHeap, void* pObject)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psHeap, sizeof(OBJ_HEAP)), "heapFree: Invalid heap pointer");

#if DEBUG_HEAP
  /* Adjust the pointer to include the call position */
  pObject = static_cast<UBYTE*>(pObject) - sizeof(HEAP_OBJHDR);
  /* Free the string memory for the file name */
  psHdr = static_cast<HEAP_OBJHDR*>(pObject);
#if COPY_FILE_STRING
  if (psHdr->pFile) { FREE(psHdr->pFile); }
#endif
  ...
  DEBUG_ASSERT_TEXT(found, "heapFree: object not allocated on this heap");

  /* Check the object hasn't been freed already */
  found = FALSE;
  for (psFree = psHeap->psFree; psFree != nullptr; psFree = psFree->psNext)
  {
    if (pObject == static_cast<void*>(psFree))
      found = TRUE;
  }
  DEBUG_ASSERT_TEXT(!found, "heapFree: Object has already been freed");
  ...
  psFree = static_cast<FREE_OBJECT*>(pObject);
...
void heapDestroy(OBJ_HEAP* psHeap)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psHeap, sizeof(OBJ_HEAP)), "heapDestroy: invalid heap pointer");
...
#if COPY_FILE_STRING
    if (psCurrHdr->pFile) { FREE(psCurrHdr->pFile); }
#endif
  }
  ...
  DEBUG_ASSERT_TEXT(clean, "heapDestroy: unallocated memory has been overwritten");
...
#if COPY_FILE_STRING
  if (psHeap->pFile) { FREE(psHeap->pFile); }
#endif
  ...
  for (psExt = psHeap->psExt; psExt != nullptr; psExt = psNext)
  {
    psNext = psExt->psNext;
    FREE(psExt->pMemory);
    FREE(psExt);
...
BOOL heapIntegrityCheck(OBJ_HEAP* psHeap)
{
  ...
  for (psCurr = psHeap->psFree; psCurr; psCurr = psCurr->psNext)
  {
    ...
    for (pBase = (UBYTE*)psCurr + sizeof(FREE_OBJECT); pBase < (UBYTE*)psCurr + psHeap->objSize; pBase++)
    {
      if (*pBase != FREE_BYTE)
        break;
    }
    DEBUG_ASSERT_TEXT(pBase == (UBYTE *)psCurr + psHeap->objSize, "heapIntegrityCheck: unallocated object memory has been overwritten");
  }
#else
  psHeap = psHeap;
#endif
