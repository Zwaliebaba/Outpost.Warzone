...
{
#ifdef LOG
  time_t aclock; struct tm* newtime; time(&aclock); /* Get time in seconds */
  newtime = localtime(&aclock); /* Convert time to struct */

  pFileHandle = fopen("c:\\netplay.log", "w"); // open the file
  if (!pFileHandle) { return FALSE; } fprintf(pFileHandle, "NETPLAY log: %s\n", asctime(newtime));
#endif
  return TRUE;
}
...
{
#ifdef LOG
  if (fclose(pFileHandle) != 0) { return FALSE; }
#endif
  return TRUE;
}
