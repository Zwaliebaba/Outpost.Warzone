...
void barGraphInitialise(W_BARGRAPH* psWidget) { (void)psWidget; }
...
void widgSetBarSize(W_SCREEN* psScreen, UDWORD id, UDWORD iValue)
{
  ...
  if (iValue < psBGraph->iRange)
    psBGraph->iValue = static_cast<UWORD>(iValue);
  else
    psBGraph->iValue = psBGraph->iRange;

  size = WBAR_SCALE * psBGraph->iValue / psBGraph->iRange;

  psBGraph->majorSize = static_cast<UWORD>(size);
}
...
void widgSetMinorBarSize(W_SCREEN* psScreen, UDWORD id, UDWORD iValue)
{
  ...
  psBGraph->minorSize = static_cast<UWORD>(size);
}
...
void barGraphHiLiteLost(W_BARGRAPH* psWidget) { tipStop((WIDGET*)psWidget); }

/* The simple bar graph display function */
void barGraphDisplay(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  SDWORD x0, y0, x1, y1;
  W_BARGRAPH* psBGraph;
...
void barGraphDisplayDouble(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
...
void barGraphDisplayTrough(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
