...
static FILE* pDebugFile = nullptr;
static BOOL StringOut = TRUE;

// message and assert callbacks
static DB_MBCALLBACK mbCallback = nullptr;
static DB_MBCALLBACK errorCallback = nullptr;
static DB_MBCALLBACK assertCallback = nullptr;

// Set the message box callback
void dbg_SetMessageBoxCallback(DB_MBCALLBACK callback) { mbCallback = callback; }

// set the error box callback
void dbg_SetErrorBoxCallback(DB_MBCALLBACK callback) { errorCallback = callback; }

// Set the assert box callback
void dbg_SetAssertCallback(DB_MBCALLBACK callback) { assertCallback = callback; }
 ...
 */
void dbg_SetOutputString(void) { StringOut = TRUE; }
 ...
 */
void dbg_NoOutputString(void) { StringOut = FALSE; }
...
void dbg_Assert(BOOL Expression, SBYTE* pFormat, ...)
{
  ...
  if (!Expression)
  {
    ...
    {
      {
        Result = MessageBox(frameGetWinHandle(), aBuffer, "Assertion Failure", MB_YESNOCANCEL | MB_ICONWARNING);
      }
      /*			else
            {
    ...
    if (retVal == DBR_YES || Result == IDYES)
      ExitProcess(3);
    if (retVal == DBR_NO || Result == IDNO)
    {
