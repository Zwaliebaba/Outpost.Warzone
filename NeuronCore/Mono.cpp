...
void dbg_MONO_ClearRectangle(SDWORD leftedge, SDWORD topedge, SDWORD width, SDWORD height)
{
  UBYTE *pub_rectchar, ub_newwidth, ub_newheight, ub_i, ub_j, ub_leftedge, ub_topedge, ub_width, ub_height;

  ub_leftedge = static_cast<UBYTE>(leftedge);
  ub_topedge = static_cast<UBYTE>(topedge);
  ub_width = static_cast<UBYTE>(width);
  ub_height = static_cast<UBYTE>(height);
...
void dbg_MONO_ClearScreen(void) { memset((char*)MONO_SCREEN_ADDR, '\0', (MONO_SCREEN_WIDTH * 2) * MONO_SCREEN_HEIGHT); }
...
void dbg_MONO_PrintString(SDWORD ub_leftedge, SDWORD ub_topedge, SBYTE* pub_formatstring, ...)
{
  va_list val_arglist;
  UBYTE *pub_formatstringptr, aub_percentstring[256], ub_percentstringindex, aub_currentcharacter[2], ub_attribute, aub_expandedstring[256],
        ub_numprinted;
  BOOL bool_percent, bool_printchar;
    ...
    while (*pub_formatstringptr)
    {
      ...
      if (aub_currentcharacter[0] == '%')
      {
        ...
        if (bool_percent == TRUE)
        {
          ...
          ub_leftedge += static_cast<UBYTE>(strlen((const char*)&aub_expandedstring[0]));
          ub_numprinted += static_cast<UBYTE>(strlen((const char*)&aub_expandedstring[0]));
...
static void DBug_DumpString(SDWORD ub_leftedge, SDWORD ub_topedge, UBYTE* pub_string, UBYTE ub_attr)
{
  UDWORD ul_stringlength, ul_i;
  UBYTE *pub_stringptr, *pub_screenptr;

  /* init the string pointer */
  pub_stringptr = pub_string;

  /* is there a string? */
  if (pub_stringptr != nullptr)
  {
...
static UBYTE DBug_ExpandString(UBYTE* pub_stringbuffer, UBYTE* pub_percentstring, UBYTE ub_oldattribute, va_list val_arglist,
                               UBYTE ub_numprinted)
{
  UBYTE ub_percentchar, ub_newattribute, ub_stringlength, *pub_numberchars;

  /* init the variables */
  ub_newattribute = ub_oldattribute;

  /* find the last character */
  ub_stringlength = static_cast<UBYTE>(strlen((const char*)pub_percentstring)) - 1;
