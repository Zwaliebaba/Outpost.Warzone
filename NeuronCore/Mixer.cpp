...
static SDWORD g_iMixerLineCD, g_iMixerLineWav;
static DWORD g_dwCDControlID, g_dwWavControlID, g_iVolRangeCD, g_iVolRangeWav, g_iWinVolWav, g_iWinVolCD, g_iInGameVolWav, g_iInGameVolCD;
...
static BOOL mixer_GetVolumeControlID(DWORD dwComponentType, DWORD* pdwControlID, DWORD* piVolRange)
{
  ...
  MIXERCONTROL* aMixerControl = nullptr;
  ...
  {
    if (aMixerControl[i].dwControlType == MIXERCONTROL_CONTROLTYPE_VOLUME)
    {
...
BOOL mixer_Open(void)
{
  ...
  bMixerOK = mixer_GetVolumeControlID(MIXERLINE_COMPONENTTYPE_SRC_COMPACTDISC, &g_dwCDControlID, &g_iVolRangeCD);
  if (bMixerOK == TRUE) { bMixerOK = mixer_GetVolumeControlID(MIXERLINE_COMPONENTTYPE_SRC_WAVEOUT, &g_dwWavControlID, &g_iVolRangeWav); }
...
static SDWORD mixer_GetVolume(DWORD dwControlID, DWORD iVolRange)
{
  ...
  if (bMixerOK == TRUE)
  {
    ...
    if ((mmRes = mixerGetControlDetails((HMIXEROBJ)g_hMixer, &mixerCntlDetails, MIXER_GETCONTROLDETAILSF_VALUE)) != MMSYSERR_NOERROR)
      return 0;
...
static void mixer_SetVolume(DWORD dwControlID, DWORD iVolRange, SDWORD iVol)
{
  ...
  if (bMixerOK == TRUE)
  {
    ...
    mixerCntlDetails.cbDetails = sizeof(MIXERCONTROLDETAILS_UNSIGNED);
    mixerCntlDetails.paDetails = &mxCntrlDetUnSigned;
    if ((mmRes = mixerSetControlDetails((HMIXEROBJ)g_hMixer, &mixerCntlDetails, MIXER_GETCONTROLDETAILSF_VALUE)) != MMSYSERR_NOERROR)
      return;
  }
}
...
SDWORD mixer_GetCDVolume(void) { return mixer_GetVolume(g_dwCDControlID, g_iVolRangeCD); }

/***************************************************************************/

void mixer_SetCDVolume(SDWORD iVol) { mixer_SetVolume(g_dwCDControlID, g_iVolRangeCD, iVol); }

/***************************************************************************/

SDWORD mixer_GetWavVolume(void) { return mixer_GetVolume(g_dwWavControlID, g_iVolRangeWav); }

/***************************************************************************/

void mixer_SetWavVolume(SDWORD iVol) { mixer_SetVolume(g_dwWavControlID, g_iVolRangeWav, iVol); }
