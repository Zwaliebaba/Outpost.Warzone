...
#define yyval	scr_yyval
#define yypvt	scr_yypvt
#define yyerrflag	scr_yyerrflag
#define yynerrs	scr_yynerrs
...
} yyNamedType;typedef struct yyTypedRules_tag
{
  /* Typed rule table */
...
using CODE_ERROR = enum _code_error
{
  CE_OK,
  // No error
  CE_MEMORY,
...
static STRING* pCurrFuncIdent = nullptr;
                              ...
                              CODE_BLOCK** ppsCBlock) // The generated code block
{
  ...
  ASSERT_TEXT(ppsCBlock != NULL, "scriptCodeFunction: Invalid generated code block pointer");
 ...
 * types match.
 */
CODE_ERROR scriptCodeCallbackParams(CALLBACK_SYMBOL* psCBSymbol, // The callback being called
                                    ...
                                    TRIGGER_DECL** ppsTDecl) // The generated code block
{
  ...
  ASSERT_TEXT(ppsTDecl != NULL, "scriptCodeCallbackParams: Invalid generated code block pointer");
  ASSERT_TEXT(psCBSymbol->pFunc != NULL, "scriptCodeCallbackParams: Expected function pointer for callback symbol");
                                ...
                                CODE_BLOCK** ppsBlock) // Generated code
{
  ...
  ASSERT_TEXT(psVariable != NULL, "scriptCodeAssignment: Invalid variable symbol pointer");
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeAssignment: Invalid generated code block pointer");

  size = psValue->size + sizeof(OPCODE);
  if (psVariable->storage == ST_EXTERN)
  {
    // Check there is a set function
    if (psVariable->set == nullptr)
    {
  ...
  {
  case ST_PUBLIC:
  case ST_PRIVATE: PUT_PKOPCODE(ip, OP_POPGLOBAL, psVariable->index);
    break;
  case ST_EXTERN: PUT_PKOPCODE(ip, OP_VARCALL, psVariable->index);
...
CODE_ERROR scriptCodeObjAssignment(OBJVAR_BLOCK* psVariable, // The variable to assign to
                                   ...
                                   CODE_BLOCK** ppsBlock) // Generated code
{
  ASSERT_TEXT(psVariable->psObjVar != NULL, "scriptCodeObjAssignment: Invalid object variable symbol pointer");
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeObjAssignment: Invalid generated code block pointer");

  // Check there is an access function for the variable
  if (psVariable->psObjVar->set == nullptr)
  {
                            ...
                            CODE_BLOCK** ppsBlock) // Generated code
{
  ...
  // Check there is an access function for the variable
  if (psVariable->psObjVar->get == nullptr)
  {
                                     ...
                                     CODE_BLOCK** ppsBlock) // Generated code
{
  ...
  ASSERT_TEXT(psVariable->psArrayVar != NULL, "scriptCodeObjAssignment: Invalid object variable symbol pointer");
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeObjAssignment: Invalid generated code block pointer");
  ...
  PUT_PKOPCODE(ip, OP_POPARRAYGLOBAL,
               ((psVariable->psArrayVar->dimensions << ARRAY_DIMENSION_SHIFT) & ARRAY_DIMENSION_MASK) | (psVariable->psArrayVar->index &
                 ARRAY_BASE_MASK));
...
CODE_ERROR scriptCodeConditional(COND_BLOCK* psCondBlock, // The intermediate conditional code
                                 CODE_BLOCK** ppsBlock) // The final conditional code
{
  UDWORD i;

  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeConditional: Invalid generated code block pointer");
...
{
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeParameter: Invalid generated code block pointer");
...
{
  ASSERT_TEXT(psVar != NULL, "scriptCodeObjectVariable: Invalid variable symbol pointer");
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeObjectVariable: Invalid generated code block pointer");
...
CODE_ERROR scriptCodeConstant(CONST_SYMBOL* psConst, // The object variable symbol
                              CODE_BLOCK** ppsBlock) // Generated code
{
  ASSERT_TEXT(psConst != NULL, "scriptCodeConstant: Invalid constant symbol pointer");
  ASSERT_TEXT(ppsBlock != NULL, "scriptCodeConstant: Invalid generated code block pointer");
  ...
  switch (psConst->type)
  {
  case VAL_BOOL: PUT_PKOPCODE(ip, OP_PUSH, VAL_BOOL);
    PUT_DATA(ip, psConst->bval);
    break;
  case VAL_INT: PUT_PKOPCODE(ip, OP_PUSH, VAL_INT);
  ...
  /*	case VAL_FLOAT:
      ...
      break;*/
  default: PUT_PKOPCODE(ip, OP_PUSH, psConst->type);
                            ...
                            CODE_BLOCK** ppsBlock) // Generated code
{
  ...
  {
  case ST_PUBLIC:
  case ST_PRIVATE: PUT_PKOPCODE(ip, OP_PUSHGLOBAL, psVariable->index);
    break;
  case ST_EXTERN: PUT_PKOPCODE(ip, OP_VARCALL, psVariable->index);
                            ...
                            PARAM_BLOCK** ppsBlock) // Generated code
{
  ...
  {
  case ST_PUBLIC:
  case ST_PRIVATE: PUT_PKOPCODE(ip, OP_PUSHREF, (*ppsBlock)->aParams[0]);
...
CODE_ERROR scriptCodeTrigger(STRING* pIdent, CODE_BLOCK* psCode)
{
  ...
  if (genDebugInfo)
  {
    ...
    psNewBlock->psDebug[psNewBlock->debugEntries].offset = ip - psNewBlock->pCode;
...
{
  {"&00: %35 &00", 0}, {"%38:", 0}, {"%35: %36 %37 %38 %39 %40", 0}, {"%36:", 0}, {"%36: %41", 0}, {"%36: %36 %41", 0},
  {"%41: &09 &33 &60", 0}, {"%37:", 0}, {"%37: %32 &60", 0}, {"%37: %37 %32 &60", 0}, {"%31: &34 &33", 18}, {"%31: &34 &03", 18},
  {"%31: &34 &04", 18}, {"%28: &61 &31 &62", 17}, {"%29: %28", 17}, {"%29: %29 &61 &31 &62", 17}, {"%30: &35", 17}, {"%30: &35 %29", 17},
  {"%32: %31 %30", 18}, {"%32: %32 &63 %30", 18}, {"%39:", 0}, {"%39: %42", 0}, {"%39: %39 %42", 0}, {"%33: %02 &63 &31", 19},
  {"%33: &05 &63 &31", 19}, {"%33: &06 &63 &31", 19}, {"%33: &08", 19}, {"%33: &59", 19}, {"%33: &59 &63 %20", 19},
  {"%42: &03 &35 &64 %33 &65 &60", 0}, {"%40: %43", 0}, {"%40: %40 %43", 0}, {"%34: &04 &35", 10}, {"%34: &04 &58", 10},
  {"%43: %34 &60", 0}, {"%43: %34 &64 &57 &65 &66 %16 &67", 0}, {"%44:", 0}, {"%43: %34 &64 %33 &65 %44 &66 %16 &67", 0},
  {"%43: %34 &64 &07 &65 &66 %16 &67", 0}, {"%16:", 12}, {"%16: %17", 12}, {"%16: %16 %17", 12}, {"%17: %18 &60", 12}, {"%17: %19 &60", 12},
  {"%17: %23", 12}, {"%17: %27", 12}, {"%17: &14 &60", 12}, {"%17: &15 &64 &31 &65 &60", 12}, {"%18: &38 &68 %01", 12},
  {"%18: &37 &68 %02", 12}, {"%18: &39 &68 %03", 12}, {"%18: &36 &68 %05", 12}, {"%18: %06 &68 %01", 12}, {"%18: %07 &68 %02", 12},
  {"%18: %08 &68 %05", 12}, {"%18: %09 &68 %03", 12}, {"%18: %12 &68 %01", 12}, {"%18: %13 &68 %02", 12}, {"%18: %14 &68 %05", 12},
  {"%18: %15 &68 %03", 12}, {"%19: &54 &64 %20 &65", 12}, {"%19: &53 &64 %20 &65", 12}, {"%19: &55 &64 %20 &65", 12},
  {"%19: &56 &64 %20 &65", 12}, {"%19: &52 &64 %20 &65", 12}, {"%20:", 15}, {"%20: %21", 15}, {"%20: %20 &63 %21", 15}, {"%21: %01", 15},
  {"%21: %02", 15}, {"%21: %05", 15}, {"%21: %03", 15}, {"%21: %22", 15}, {"%22: &10 &38", 15}, {"%22: &10 &37", 15}, {"%22: &10 &36", 15},
  {"%22: &10 &39", 15}, {"%23: %26", 12}, {"%23: %26 &13 %25", 12}, {"%26: %24", 16}, {"%26: %26 &13 %24", 16}, {"%25: &66 %16 &67", 16},
  {"%45:", 0}, {"%24: &12 &64 %02 &65 %45 &66 %16 &67", 16}, {"%46:", 0}, {"%27: &11 &64 %02 &65 %46 &66 %16 &67", 12},
  {"%01: %01 &26 %01", 12}, {"%01: %01 &25 %01", 12}, {"%01: %01 &27 %01", 12}, {"%01: %01 &28 %01", 12}, {"%01: &25 %01", 12},
  {"%01: &64 %01 &65", 12}, {"%01: &54 &64 %20 &65", 12}, {"%01: &38", 12}, {"%01: &49", 12}, {"%01: %06", 12}, {"%01: %12", 12},
  {"%01: &31", 12}, {"%02: %02 &22 %02", 12}, {"%02: %02 &23 %02", 12}, {"%02: %02 &16 %02", 12}, {"%02: %02 &17 %02", 12},
  {"%02: &24 %02", 12}, {"%02: &64 %02 &65", 12}, {"%02: &53 &64 %20 &65", 12}, {"%02: &37", 12}, {"%02: &48", 12}, {"%02: %07", 12},
  {"%02: %13", 12}, {"%02: &30", 12}, {"%02: %01 &16 %01", 12}, {"%02: %05 &16 %05", 12}, {"%02: %03 &16 %03", 12},
  {"%02: %01 &17 %01", 12}, {"%02: %05 &17 %05", 12}, {"%02: %03 &17 %03", 12}, {"%02: %01 &19 %01", 12}, {"%02: %01 &18 %01", 12},
  {"%02: %01 &20 %01", 12}, {"%02: %01 &21 %01", 12}, {"%05: &36", 12}, {"%05: &50", 12}, {"%05: %08", 12}, {"%05: %14", 12},
  {"%05: &55 &64 %20 &65", 12}, {"%05: &57", 12}, {"%05: &07", 12}, {"%05: &58", 12}, {"%03: &39", 12}, {"%03: &51", 12},
  {"%03: &56 &64 %20 &65", 12}, {"%03: %09", 12}, {"%03: %15", 12}, {"%04: %03 &69", 12}, {"%06: %04 &45", 13}, {"%07: %04 &44", 13},
  {"%08: %04 &46", 13}, {"%09: %04 &47", 13}, {"%10: &61 %01 &62", 14}, {"%11: %10", 14}, {"%11: %11 &61 %01 &62", 14},
  {"%12: &42 %11", 14}, {"%13: &41 %11", 14}, {"%15: &43 %11", 14}, {"%14: &40 %11", 14}, {"$accept", 0}, {"error", 0}
};yyNamedType yyTokenTypes[] = {
  {"$end", 0, 0}, {"error", 256, 0}, {"FUNCTION", 257, 0}, {"TRIGGER", 258, 0}, {"EVENT", 259, 0}, {"WAIT", 260, 0}, {"EVERY", 261, 0},
  {"INACTIVE", 262, 0}, {"INITIALISE", 263, 0}, {"LINK", 264, 0}, {"REF", 265, 0}, {"WHILE", 266, 0}, {"IF", 267, 0}, {"ELSE", 268, 0},
  {"EXIT", 269, 0}, {"PAUSE", 270, 0}, {"BOOLEQUAL", 271, 0}, {"NOTEQUAL", 272, 0}, {"GREATEQUAL", 273, 0}, {"LESSEQUAL", 274, 0},
  {"GREATER", 275, 0}, {"LESS", 276, 0}, {"_AND", 277, 0}, {"_OR", 278, 0}, {"_NOT", 279, 0}, {"'-'", 45, 0}, {"'+'", 43, 0},
  {"'*'", 42, 0}, {"'/'", 47, 0}, {"UMINUS", 280, 0}, {"BOOLEAN", 281, 1}, {"INTEGER", 282, 2}, {"QTEXT", 283, 3}, {"TYPE", 284, 4},
  {"STORAGE", 285, 5}, {"IDENT", 286, 3}, {"VAR", 287, 6}, {"BOOL_VAR", 288, 6}, {"NUM_VAR", 289, 6}, {"OBJ_VAR", 290, 6},
  {"VAR_ARRAY", 291, 6}, {"BOOL_ARRAY", 292, 6}, {"NUM_ARRAY", 293, 6}, {"OBJ_ARRAY", 294, 6}, {"BOOL_OBJVAR", 295, 6},
  {"NUM_OBJVAR", 296, 6}, {"USER_OBJVAR", 297, 6}, {"OBJ_OBJVAR", 298, 6}, {"BOOL_CONSTANT", 299, 7}, {"NUM_CONSTANT", 300, 7},
  {"USER_CONSTANT", 301, 7}, {"OBJ_CONSTANT", 302, 7}, {"FUNC", 303, 8}, {"BOOL_FUNC", 304, 8}, {"NUM_FUNC", 305, 8}, {"USER_FUNC", 306, 8},
  {"OBJ_FUNC", 307, 8}, {"TRIG_SYM", 308, 9}, {"EVENT_SYM", 309, 10}, {"CALLBACK_SYM", 310, 11}, {"';'", 59, 0}, {"'['", 91, 0},
  {"']'", 93, 0}, {"','", 44, 0}, {"'('", 40, 0}, {"')'", 41, 0}, {"'{'", 123, 0}, {"'}'", 125, 0}, {"'='", 61, 0}, {"'.'", 46, 0}
};
#endif
static short yydef[] = {
  65535, 65527, 65521, 65517, 5, 65511, 6, 4, 7, 56, 57, 41, 40, 39, 38, 65507, 65501, 65495, 65489, 65483, 24, 23, 22, 21, 33, 31, 37, 36,
  44, 19, 18, 17, 16, 65283, 65282, 115, 65284, 65271, 65291, 65290, 90, 65289, 65496, 65272, 65252, 65529, 65255, 65254, 190, 65342, 65346,
  65344, 3, 65377, 65373, 178, 65353, 65533, 65534, 65532, 65530, 65349, 65528, 65243, 65244, 1, 65241, 65242, 186, 65239, 65240, 7, 65418,
  65471, 65470, -1
};
static short yypgo[] = {
  0, 0, 0, 513, 510, 500, 500, 507, 507, 446, 465, 465, 465, 465, 465, 465, 465, 465, 465, 466, 476, 487, 487, 487, 487, 490, 29, 29, 72,
  72, 72, 72, 72, 72, 72, 72, 72, 72, 353, 378, 428, 403, 337, 337, 333, 320, 295, 270, 245, 172, 115, 115, 115, 115, 115, 216, 216, 216,
  216, 216, 216, 216, 216, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 29, 29, 29, 29, 29, 29, 29, 29, 29, 29, 496, 527, 492, 526, 494,
  495, 495, 490, 489, 489, 489, 489, 487, 476, 476, 466, 466, 466, 466, 465, 465, 465, 458, 458, 458, 458, 458, 458, 446, 446, 523, 523,
  525, 523, 509, 509, 520, 507, 507, 507, 507, 504, 504, 498, 498, 497, 502, 502, 502, 512, 512, 517, 511, 511, 514, 514, 515, 515, 523,
  514, 512, 511, 0
};
static short yyrlen[] = {
  0, 0, 0, 0, 5, 1, 2, 1, 3, 0, 3, 3, 3, 3, 3, 3, 3, 3, 3, 4, 0, 1, 1, 1, 1, 1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 4, 1, 3, 2,
  2, 2, 2, 2, 1, 1, 4, 1, 1, 1, 1, 1, 4, 1, 1, 1, 1, 3, 3, 1, 1, 1, 1, 1, 4, 3, 2, 3, 3, 1, 1, 1, 1, 1, 4, 3, 2, 3, 3, 8, 0, 8, 0, 3, 3, 1,
  3, 2, 2, 2, 2, 1, 3, 1, 4, 4, 4, 4, 3, 3, 3, 5, 2, 1, 1, 2, 2, 2, 1, 7, 8, 0, 7, 2, 2, 6, 1, 3, 3, 3, 3, 2, 4, 1, 3, 2, 2, 2, 3, 2, 3, 1,
  2, 1, 2, 1, 2, 2, 0, 0, 0, 2
};
#define YYS0	0
#define YYDELTA	154
...
char* yysvar[] =
{
  "$accept", "expression", "boolexp", "objexp", "objexp_dot", "userexp", "num_objvar", "bool_objvar", "user_objvar", "obj_objvar",
  "array_index", "array_index_list", "num_array_var", "bool_array_var", "user_array_var", "obj_array_var", "statement_list", "statement",
  "assignment", "func_call", "param_list", "parameter", "var_ref", "conditional", "cond_clause", "terminal_cond", "cond_clause_list",
  "loop", "array_sub_decl", "array_sub_decl_list", "variable_ident", "variable_decl_head", "variable_decl", "trigger_subdecl",
  "event_subdecl", "script", "header", "var_list", "$1", "trigger_list", "event_list", "header_decl", "trigger_decl", "event_decl", "$36",
  "$82", "$84", 0
};short yyrmap[] =
{
  145, 146, 147, 1, 2, 16, 17, 27, 28, 39, 48, 49, 50, 52, 53, 55, 56, 57, 59, 63, 65, 68, 69, 70, 71, 77, 86, 87, 98, 99, 110, 112, 113,
  115, 116, 117, 118, 119, 141, 142, 143, 144, 140, 139, 138, 137, 136, 135, 134, 133, 132, 131, 130, 129, 128, 127, 126, 125, 124, 123,
  122, 121, 120, 114, 111, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 85, 84, 83, 82, 81, 80,
  79, 78, 76, 75, 74, 73, 72, 67, 66, 64, 62, 61, 60, 58, 54, 51, 47, 46, 45, 44, 43, 42, 41, 40, 38, 37, 36, 35, 33, 32, 29, 26, 25, 24,
  23, 19, 18, 15, 14, 13, 12, 11, 10, 9, 8, 6, 4, 5, 21, 22, 30, 31, 34, 20, 7, 3, 0
};short yysmap[] = {
  0, 3, 4, 10, 13, 20, 21, 36, 83, 88, 90, 91, 94, 95, 96, 101, 102, 108, 110, 128, 143, 144, 145, 146, 151, 152, 161, 162, 163, 164, 165,
  166, 169, 170, 172, 177, 178, 180, 182, 197, 203, 207, 212, 223, 230, 231, 232, 233, 234, 257, 265, 267, 268, 269, 271, 272, 274, 275,
  276, 283, 292, 293, 295, 294, 289, 288, 287, 281, 277, 264, 263, 262, 261, 260, 256, 255, 253, 247, 246, 245, 244, 243, 242, 241, 240,
  239, 238, 237, 236, 235, 229, 228, 227, 224, 222, 220, 219, 216, 215, 214, 213, 211, 210, 209, 208, 206, 205, 204, 202, 201, 200, 199,
  198, 195, 194, 190, 183, 181, 160, 157, 156, 153, 150, 149, 147, 141, 140, 139, 138, 136, 135, 134, 133, 132, 131, 130, 129, 127, 126,
  125, 124, 123, 122, 121, 120, 119, 118, 117, 115, 114, 113, 112, 111, 107, 106, 105, 104, 93, 89, 87, 86, 85, 82, 81, 80, 79, 78, 72, 71,
  65, 64, 59, 53, 50, 49, 48, 47, 46, 40, 39, 37, 33, 32, 31, 30, 29, 28, 26, 23, 19, 17, 8, 7, 6, 5, 1, 225, 92, 184, 97, 98, 99, 100, 103,
  51, 52, 189, 54, 55, 56, 57, 58, 191, 60, 61, 62, 63, 154, 155, 66, 67, 68, 69, 70, 192, 158, 109, 175, 176, 73, 74, 75, 76, 77, 193, 159,
  116, 167, 168, 296, 279, 297, 280, 290, 258, 196, 259, 185, 186, 187, 188, 142, 226, 148, 282, 284, 285, 286, 266, 270, 273, 291, 248,
  217, 218, 249, 250, 251, 221, 252, 278, 137, 254, 42, 43, 171, 84, 173, 174, 179, 24, 12, 45, 22, 38, 14, 15, 16, 25, 18, 11, 2, 9, 27,
  35, 34, 44, 41
};int yyntoken = 70;int yynvar = 47;int yynstate = 298;int yynrule = 148;
#endif
...
extern void yyShowState YY_ARGS((yyTraceItems *));extern void yyShowReduce YY_ARGS((yyTraceItems *));extern void yyShowGoto
YY_ARGS((yyTraceItems *));extern void yyShowShift YY_ARGS((yyTraceItems *));extern void yyShowErrRecovery YY_ARGS((yyTraceItems *));extern
void yyShowErrDiscard YY_ARGS((yyTraceItems *));extern void yyShowRead YY_ARGS((int));
#endif
...
#ifdef YYDYNAMIC
#define YYALLOC
char* getenv();int atoi();int yysinc = -1; /* stack size increment, <0 = double, 0 = none, >0 = fixed */
#endif
...
extern int yynstate, yynvar, yyntoken, yynrule;extern int yyGetType YY_ARGS((int)); /* token type */
...
static void scriptResetTables(void)
{
  ...
  for (psFCurr = psFunctions; psFCurr != nullptr; psFCurr = psFNext)
  {
...
BOOL scriptLookUpVariable(STRING* pIdent, VAR_SYMBOL** ppsSym)
{
  ...
  {
    for (psCurr = asScrObjectVarTab; psCurr->pIdent != nullptr; psCurr++)
    {
      if (interpCheckEquiv(psCurr->objType, objVarContext) && strcmp(psCurr->pIdent, pIdent) == 0)
      {
        *ppsSym = psCurr;
        return TRUE;
      }
  ...
  if (asScrExternalTab)
  {
    for (psCurr = asScrExternalTab; psCurr->pIdent != nullptr; psCurr++)
    {
  ...
  for (psCurr = psLocalVars; psCurr != nullptr; psCurr = psCurr->psNext)
  {
    if (strcmp(psCurr->pIdent, pIdent) == 0)
    {
      *ppsSym = psCurr;
  ...
  if (!localVariableDef)
  {
    for (psCurr = psGlobalVars; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      if (strcmp(psCurr->pIdent, pIdent) == 0)
      {
        *ppsSym = psCurr;
    ...
    for (psCurr = psGlobalArrays; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      if (strcmp(psCurr->pIdent, pIdent) == 0)
      {
        *ppsSym = psCurr;
...
BOOL scriptDeclareEvent(STRING* pIdent, EVENT_SYMBOL** ppsEvent)
{
  ...
  psEvent->pCode = nullptr;
  psEvent->size = 0;
  psEvent->psDebug = nullptr;
  psEvent->debugEntries = 0;
  psEvent->index = numEvents++;
  psEvent->psNext = nullptr;
...
BOOL scriptDefineEvent(EVENT_SYMBOL* psEvent, CODE_BLOCK* psCode, SDWORD trigger)
{
  ...
  if (genDebugInfo)
  {
    ...
    memcpy(psEvent->psDebug, psCode->psDebug, sizeof(SCRIPT_DEBUG) * psCode->debugEntries);
...
BOOL scriptLookUpFunction(STRING* pIdent, FUNC_SYMBOL** ppsSym)
{
  ...
  for (psCurr = psFunctions; psCurr != nullptr; psCurr = psCurr->psNext)
  {
    if (strcmp(psCurr->pIdent, pIdent) == 0)
    {
      *ppsSym = psCurr;
...
void scriptSetTypeTab(TYPE_SYMBOL* psTypeTab)
{
  ...
  {
    ASSERT_TEXT(psTypeTab[i].typeID == type, "scriptSetTypeTab: ID's must be >= VAL_USERTYPESTART and sequential");
    type += 1;
  }
#endif
...
void scriptSetCallbackTab(CALLBACK_SYMBOL* psCallTab)
{
  ...
  {
    ASSERT_TEXT(psCallTab[i].type == type, "scriptSetCallbackTab: ID's must be >= VAL_CALLBACKSTART and sequential");
    type += 1;
  }
#endif
...
int scr_parse()
{
  ...
  hRes_table = LoadResource(hInst, FindResource(hInst, "UD_RES_yyYACC", "yyYACCTBL"));
...

{
  ...
  short* yytp; int yyruletype = 0;
#endif
#ifdef YYSTATIC
  static short yys[YYSSIZE + 1]; static YYSTYPE yyv[YYSSIZE + 1];
#if YYDEBUG
  static short yytypev[YYSSIZE + 1]; /* type assignments */
#endif
#else /* ! YYSTATIC */
#ifdef YYALLOC
  YYSTYPE* yyv; short* yys;
#if YYDEBUG
  short* yytypev;
#endif
  YYSTYPE save_yylval; YYSTYPE save_yyval; YYSTYPE* save_yypvt; int save_yychar, save_yyerrflag, save_yynerrs; int retval;
  /* return value holder */
#else
  short yys[YYSSIZE + 1];
...
#endif
#ifdef YYALLOC
  yys = (short*)malloc((yyssize + 1) * sizeof(short)); yyv = (YYSTYPE*)malloc((yyssize + 1) * sizeof(YYSTYPE));
#if YYDEBUG
  ...
  if (yys == (short*)0 || yyv == (YYSTYPE*)0
    ...
    scr_error(m_textmsg(4967, "Not enough space for parser stacks", "E"));
    return 1;
  } save_yylval = scr_lval; save_yyval = yyval; save_yypvt = yypvt; save_yychar = scr_char; save_yyerrflag = yyerrflag; save_yynerrs =
    yynerrs;
#endif
...
#if YYDEBUG
  yytp = yytypev; yyi = yyj = 0; /* silence compiler warnings */
#endif

yyStack: yyassert((unsigned)yystate < yynstate, m_textmsg(587, "state %d\n", ""), yystate);
#ifdef YYDYNAMIC
  if (++yyps > &yys[yyssize])
  {
    int yynewsize;
  ...
  if (yysinc == 0)
  {
    ...
    yynewsize = yyssize + yysinc; if (yynewsize < yyssize)
  {
    scr_error(m_textmsg(4967, "Not enough space for parser stacks", "E"));
    YYABORT;
  } yyssize = yynewsize; yys = (short*)realloc(yys, (yyssize + 1) * sizeof(short)); yyps = yys + yysindex; yyv = (YYSTYPE*)realloc(
    yyv, (yyssize + 1) * sizeof(YYSTYPE)); yypv = yyv + yyvindex;
#if YYDEBUG
  yytypev = (short*)realloc(yytypev, (yyssize + 1) * sizeof(short)); yytp = yytypev + yytindex;
#endif
  if (yys == (short*)0 || yyv == (YYSTYPE*)0
#if YYDEBUG
			|| yytypev == (short*)0
  ...
  *++yytp = yyruletype; /* ... and type */
  ...
  if (scr_debug) YY_TRACE(yyShowState)
#endif
...
yyEncore:
...
#ifdef YACC_WINDOWS
      yyassert((unsigned)yyneg(yyi) < Sizeof_yyex, m_textmsg(2825, "exception %d\n", "I num"), yystate);
#else /* YACC_WINDOWS */
      yyassert((unsigned)yyneg(yyi) < sizeof yyex/sizeof yyex[0], m_textmsg(2825, "exception %d\n", "I num"), yystate);
#endif /* YACC_WINDOWS */
      yyp = &yyex[yyneg(yyi)];
#ifndef YYSYNC
      YYREAD;
#endif
      while ((yyi = *yyp) >= 0 && yyi != scr_char) { yyp += 2; }
      yyi = yyp[1];
      yyassert(yyi >= 0, m_textmsg(2826, "Ex table not reduce %d\n", "I num"), yyi);
    }
  }
  ...
  yyj = yyrlen[yyi];
#if YYDEBUG
  if (scr_debug) YY_TRACE(yyShowReduce) yytp -= yyj;
#endif
  yyps -= yyj; /* pop stacks */
  yypvt = yypv; /* save top */
  yypv -= yyj;
  ...
  case YYr2:
    {
      /* script :  header var_list $1 trigger_list event_list */
      ...
      ALLOC_PROG(psFinalProg, size, psCurrBlock->pCode, numVars, numArrays, numTriggers, numEvents);
      ...
      for (psTrig = psTriggers; psTrig; psTrig = psTrig->psNext)
      {
        ...
        psFinalProg->pTriggerTab[i] = static_cast<UWORD>(ip - psFinalProg->pCode);
        if (psTrig->pCode != nullptr)
        {
          // Store the label
          DEBUG_LABEL(psFinalProg, psFinalProg->debugEntries, psTrig->pIdent);
          // Store debug info
        ...
        // Store the data
        psFinalProg->psTriggerData[i].type = static_cast<UWORD>(psTrig->type);
      ...
      for (psEvent = psEvents; psEvent; psEvent = psEvent->psNext)
      {
        ...
        if (psEvent->pCode == nullptr)
        {
          scr_error("Event %s declared without being defined", psEvent->pIdent);
          YYABORT;
        }

        // Store the event offset
        psFinalProg->pEventTab[i] = static_cast<UWORD>(ip - psFinalProg->pCode);
        // Store the trigger link
        psFinalProg->pEventLinks[i] = static_cast<SWORD>(psEvent->trigger);
        // Store the label
        DEBUG_LABEL(psFinalProg, psFinalProg->debugEntries, psEvent->pIdent);
        // Store debug info
      ...
      if (genDebugInfo)
      {
        ...
        if (numArrays > 0)
        {
          ...
          psFinalProg->psArrayDebug = nullptr;
      }
      else
      {
        psFinalProg->psVarDebug = nullptr;
      ...
      for (psCurr = psGlobalArrays; psCurr != nullptr; psCurr = psCurr->psNext)
      {
        ...
        psFinalProg->psArrayInfo[i].type = static_cast<UBYTE>(psCurr->type);
        psFinalProg->psArrayInfo[i].dimensions = static_cast<UBYTE>(psCurr->dimensions);
        for (dimension = 0; dimension < psCurr->dimensions; dimension += 1)
          psFinalProg->psArrayInfo[i].elements[dimension] = static_cast<UBYTE>(psCurr->elements[dimension]);
  ...
  case YYr9:
    {
      /* var_list :  var_list variable_decl ';' */
  ...
  case YYr11:
    {
      /* variable_decl_head :  STORAGE TRIGGER */
  ...
  case YYr12:
    {
      /* variable_decl_head :  STORAGE EVENT */
  ...
  case YYr15:
    {
      /* array_sub_decl_list :  array_sub_decl_list '[' INTEGER ']' */
  ...
  case YYr18:
    {
      ...
      if (!scriptAddVariable(yypvt[-1].vdecl, yypvt[0].videcl)) { YYABORT; }
  ...
  case YYr19:
    {
      /* variable_decl :  variable_decl ',' variable_ident */

      if (!scriptAddVariable(yypvt[-2].vdecl, yypvt[0].videcl)) { YYABORT; }
  ...
  case YYr28:
    {
      /* trigger_subdecl :  CALLBACK_SYM ',' param_list */
  ...
  case YYr29:
    {
      ...
      if (!scriptAddTrigger(yypvt[-4].sval, yypvt[-2].tdecl, static_cast<UDWORD>(line))) { YYABORT; }
  ...
  case YYr32:
    {
      ...
      EVENT_SYMBOL* psEvent;
      if (!scriptDeclareEvent(yypvt[0].sval, &psEvent)) { YYABORT; }
  ...
  case YYr35:
    {
      /* event_decl :  event_subdecl '(' TRIG_SYM ')' '{' statement_list '}' */

      if (!scriptDefineEvent(yypvt[-6].eSymbol, yypvt[-1].cblock, yypvt[-4].tSymbol->index)) { YYABORT; }
  ...
  case YYr36:
    {
      /* event_decl :  event_subdecl '(' trigger_subdecl ')' */
  ...
  case YYr37:
    {
      /* event_decl :  event_subdecl '(' trigger_subdecl ')' $36 '{' statement_list '}' */

      // Create a trigger for this event
      if (!scriptAddTrigger("", yypvt[-5].tdecl, debugLine)) { YYABORT; }
      FREE_TSUBDECL(yypvt[-5].tdecl);

      if (!scriptDefineEvent(yypvt[-7].eSymbol, yypvt[-1].cblock, numTriggers - 1)) { YYABORT; }
  ...
  case YYr38:
    {
      /* event_decl :  event_subdecl '(' INACTIVE ')' '{' statement_list '}' */

      if (!scriptDefineEvent(yypvt[-6].eSymbol, yypvt[-1].cblock, -1)) { YYABORT; }
  ...
  case YYr41:
    {
      ...
      ALLOC_DEBUG(psCurrBlock, yypvt[-1].cblock->debugEntries + yypvt[0].cblock->debugEntries);
  ...
  case YYr52:
    {
      /* assignment :  num_objvar '=' expression */
  ...
  case YYr53:
    {
      /* assignment :  bool_objvar '=' boolexp */
  ...
  case YYr54:
    {
      /* assignment :  user_objvar '=' userexp */
  ...
  case YYr56:
    {
      /* assignment :  num_array_var '=' expression */
  ...
  case YYr57:
    {
      /* assignment :  bool_array_var '=' boolexp */
  ...
  case YYr58:
    {
      /* assignment :  user_array_var '=' userexp */
  ...
  case YYr59:
    {
      /* assignment :  obj_array_var '=' objexp */
  ...
  case YYr67:
    {
      ...
      memcpy(psCurrPBlock->aParams, yypvt[-2].pblock->aParams, yypvt[-2].pblock->numParams * sizeof(INTERP_TYPE));
      memcpy(psCurrPBlock->aParams + yypvt[-2].pblock->numParams, yypvt[0].pblock->aParams,
  ...
  case YYr78:
    {
      /* conditional :  cond_clause_list ELSE terminal_cond */

      ALLOC_CONDBLOCK(psCondBlock, yypvt[-2].condBlock->numOffsets + yypvt[0].condBlock->numOffsets,
      ...
      memcpy(psCondBlock->aOffsets, yypvt[-2].condBlock->aOffsets, yypvt[-2].condBlock->numOffsets * sizeof(SDWORD));
  ...
  case YYr80:
    {
      /* cond_clause_list :  cond_clause_list ELSE cond_clause */
      ...
      memcpy(psCondBlock->aOffsets, yypvt[-2].condBlock->aOffsets, yypvt[-2].condBlock->numOffsets * sizeof(SDWORD));
      psCondBlock->aOffsets[yypvt[-2].condBlock->numOffsets] = yypvt[0].condBlock->aOffsets[0] + yypvt[-2].condBlock->size / sizeof(UDWORD);
  ...
  case YYr83:
    {
      /* cond_clause :  IF '(' boolexp ')' $82 '{' statement_list '}' */

      ALLOC_CONDBLOCK(psCondBlock, 1, yypvt[-5].cblock->size + yypvt[-1].cblock->size + sizeof(OPCODE)*2);
  ...
  case YYr85:
    {
      /* loop :  WHILE '(' boolexp ')' $84 '{' statement_list '}' */
      ...
      PUT_PKOPCODE(ip, OP_JUMP, static_cast<SWORD>(-(SWORD)(psCurrBlock->size / sizeof(UDWORD)) + 1));
  ...
  case YYr92:
    {
      /* expression :  NUM_FUNC '(' param_list ')' */
  ...
  case YYr100:
    {
      /* boolexp :  boolexp BOOLEQUAL boolexp */
  ...
  case YYr101:
    {
      /* boolexp :  boolexp NOTEQUAL boolexp */
  ...
  case YYr110:
    {
      /* boolexp :  expression BOOLEQUAL expression */
  ...
  case YYr111:
    {
      /* boolexp :  userexp BOOLEQUAL userexp */
  ...
  case YYr112:
    {
      /* boolexp :  objexp BOOLEQUAL objexp */
  ...
  case YYr113:
    {
      /* boolexp :  expression NOTEQUAL expression */
  ...
  case YYr114:
    {
      /* boolexp :  userexp NOTEQUAL userexp */
  ...
  case YYr115:
    {
      /* boolexp :  objexp NOTEQUAL objexp */
  ...
  case YYr116:
    {
      /* boolexp :  expression LESSEQUAL expression */
  ...
  case YYr117:
    {
      /* boolexp :  expression GREATEQUAL expression */
  ...
  case YYr119:
    {
      /* boolexp :  expression LESS expression */
  ...
  case YYr140:
    {
      /* array_index_list :  array_index_list '[' expression ']' */
...
#if YYDEBUG
  if (scr_debug) YY_TRACE(yyShowGoto)
#endif
  goto yyStack;
...
yyError:
  ...
  case 2: yyerrflag = 3; /* need 3 valid shifts to recover */
  ...
  case 3:
    ...
    if (scr_char == 0) /* but not EOF */
      break;
#if YYDEBUG
    if (scr_debug) YY_TRACE(yyShowErrDiscard)
#endif
    yyclearin;
...
#ifdef YYALLOC
yyReturn:
  scr_lval = save_yylval; yyval = save_yyval; yypvt = save_yypvt; scr_char = save_yychar; yyerrflag = save_yyerrflag; yynerrs =
    save_yynerrs; free((char*)yys); free((char*)yyv);
#if YYDEBUG
  free((char*)yytypev);
#endif
  return (retval);
...
static char* yygetState(num)int num;
{
  int size;
 ...
 */
int yyExpandName(num, isrule, buf, len)int num, isrule;char* buf;int len;
{
  int i, n, cnt, type;
  char *endp, *cp;
  char* s;
 ...
 */
void yyShowState(tp)yyTraceItems* tp;
{
  short* p;
  YYSTYPE* q;

  printf(m_textmsg(2828, "state %d (%d), char %s (%d)\n", "I num1 num2 char num3"), yysmap[tp->state], tp->state, yyptok(tp->lookahead),
         tp->lookahead);
}
/*
 * show results of reduction
 */
void yyShowReduce(tp)yyTraceItems* tp;

{
  printf("reduce %d (%d), pops %d (%d)\n", yyrmap[tp->rule], tp->rule, tp->states[tp->nstates - tp->npop],
         yysmap[tp->states[tp->nstates - tp->npop]]);
}void yyShowRead(val)int val; { printf(m_textmsg(2829, "read %s (%d)\n", "I token num"), yyptok(val), val); }void yyShowGoto(tp)yyTraceItems
* tp; { printf(m_textmsg(2830, "goto %d (%d)\n", "I num1 num2"), yysmap[tp->state], tp->state); }void yyShowShift(tp)yyTraceItems* tp;
{
  printf(m_textmsg(2831, "shift %d (%d)\n", "I num1 num2"), yysmap[tp->state], tp->state);
}void yyShowErrRecovery(tp)yyTraceItems* tp;
{
  short* top = tp->states + tp->nstates - 1;

  printf(m_textmsg(2832, "Error recovery pops state %d (%d), uncovers %d (%d)\n", "I num1 num2 num3 num4"), yysmap[*top], *top,
         yysmap[*(top - 1)], *(top - 1));
}void yyShowErrDiscard(tp)yyTraceItems* tp;
{
  printf(m_textmsg(2833, "Error recovery discards %s (%d), ", "I token num"), yyptok(tp->lookahead), tp->lookahead);
}
#endif	/* ! YYTRACE */
#endif	/* YYDEBUG */
