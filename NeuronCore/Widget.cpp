...
void widgReleaseWidgetList(WIDGET* psWidgets)
{
  ...
  for (psCurr = psWidgets; psCurr; psCurr = psNext)
  {
    ...
    case WIDG_SLIDER:
      ...
      break;
    default: ASSERT((FALSE,"widgReleaseWidgetList: Unknown widget type"));
      break;
    }
  }
}
...
void widgRelease(WIDGET* psWidget)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgRelease: Unknown widget type"));
    break;
  }
}
...
BOOL widgAddForm(W_SCREEN* psScreen, W_FORMINIT* psInit)
{
  ...
  else
  {
    ...
    {
      ASSERT((FALSE, "widgAddForm: Could not find parent form from formID"));
      return FALSE;
    }
  }
...
BOOL widgAddLabel(W_SCREEN* psScreen, W_LABINIT* psInit)
{
  ...
  else
  {
    ...
    {
      ASSERT((FALSE, "widgAddLabel: Could not find parent form from formID"));
      return FALSE;
    }
  }
...
BOOL widgAddButton(W_SCREEN* psScreen, W_BUTINIT* psInit)
{
  ...
  else
  {
    ...
    if (psForm == nullptr || psForm->type != WIDG_FORM)
    {
      ASSERT((FALSE, "widgAddButton: Could not find parent form from formID"));
      return FALSE;
    }
  }
...
BOOL widgAddEditBox(W_SCREEN* psScreen, W_EDBINIT* psInit)
{
  ...
  else
  {
    ...
    {
      ASSERT((FALSE, "widgAddEditBox: Could not find parent form from formID"));
      return FALSE;
    }
  }
...
BOOL widgAddBarGraph(W_SCREEN* psScreen, W_BARINIT* psInit)
{
  ...
  else
  {
    ...
    if (!psForm || psForm->type != WIDG_FORM)
    {
      ASSERT((FALSE, "widgAddBarGraph: Could not find parent form from formID"));
      return FALSE;
    }
  }
...
BOOL widgDeleteFromForm(W_FORM* psForm, UDWORD id, W_CONTEXT* psContext)
{
  ...
  if ((psForm->psLastHiLite != nullptr) && (psForm->psLastHiLite->id == id))
  {
    widgHiLiteLost(psForm->psLastHiLite, psContext);
    psForm->psLastHiLite = nullptr;
  }
...
static void widgStartForm(W_FORM* psForm)
{
  ...
  while (psCurr != nullptr)
  {
    ...
    case WIDG_SLIDER:
      ...
      break;
    default: ASSERT((FALSE,"widgStartScreen: Unknown widget type"));
      break;
    }
...
{
  psScreen->psFocus = nullptr;
  widgStartForm((W_FORM*)psScreen->psForm);
}

/* Clean up after a screen has been run */
void widgEndScreen(W_SCREEN* psScreen) { (void)psScreen; }
...
void widgSetTip(W_SCREEN* psScreen, UDWORD id, STRING* pTip)
{
  ...
  if (psWidget)
  {
    ...
    case WIDG_EDITBOX: ASSERT((FALSE, "widgSetTip: edit boxes do not have a tip"));
      break;

    default: ASSERT((FALSE,"widgSetTip: Unknown widget type"));
      break;
    }
  }
}
...
STRING* widgGetString(W_SCREEN* psScreen, UDWORD id)
{
  ...
  if (psWidget != nullptr)
  {
    switch (psWidget->type)
    {
    case WIDG_FORM: ASSERT((FALSE, "widgGetString: Forms do not have a string"));
    ...
    case WIDG_EDITBOX:
      ...
      break;
    case WIDG_BARGRAPH: ASSERT((FALSE, "widgGetString: Bar Graphs do not have a string"));
      aStringRetBuffer[0] = 0;
      break;
    case WIDG_SLIDER: ASSERT((FALSE, "widgGetString: Sliders do not have a string"));
      aStringRetBuffer[0] = 0;
      break;
    default: ASSERT((FALSE,"widgGetString: Unknown widget type"));
      aStringRetBuffer[0] = 0;
      break;
    }
  }
...
void widgSetString(W_SCREEN* psScreen, UDWORD id, STRING* pText)
{
  ...
  if (psWidget != nullptr)
  {
    switch (psWidget->type)
    {
    case WIDG_FORM: ASSERT((FALSE, "widgSetString: forms do not have a string"));
      break;
    case WIDG_LABEL:
    ...
    case WIDG_BUTTON:
#if W_USE_STRHEAP
      if (((W_BUTTON*)psWidget)->pText) { widgCopyString(((W_BUTTON*)psWidget)->pText, pText); }
      else { widgAllocCopyString(&((W_BUTTON*)psWidget)->pText, pText); }
#else
    ...
    case WIDG_SLIDER: ASSERT((FALSE, "widgGetString: Sliders do not have a string"));
      break;
    default: ASSERT((FALSE,"widgSetString: Unknown widget type"));
      break;
    }
  }
  else
...
static void widgProcessForm(W_CONTEXT* psContext)
{
  ...
  omx = mx - xOrigin;
  omy = my - yOrigin;
  if (mx >= 0 && mx <= psForm->width && my >= 0 && my <= psForm->height)
  {
    ...
    for (psCurr = formGetWidgets(psForm); psCurr; psCurr = psCurr->psNext)
    {
      ...
      if (omx >= psCurr->x && omy >= psCurr->y && omx <= psCurr->x + psCurr->width && omy <= psCurr->y + psCurr->height)
      {
    ...
    if (pressed != WKEY_NONE && (psOver == nullptr || (psForm->style & WFORM_CLICKABLE)))
    {
      /* Tell the form it has been clicked */
      widgClicked((WIDGET*)psForm, pressed, psContext);
    }
    if (released != WKEY_NONE && (psOver == nullptr || (psForm->style & WFORM_CLICKABLE)))
    {
  ...
  if (psForm->psLastHiLite != psOver)
  {
    ...
    if (psForm->psLastHiLite != nullptr)
...
void widgSetReturn(WIDGET* psWidget) { psRetWidget = psWidget; }
...
static void _widgDisplayForm(W_FORM* psForm, UDWORD xOffset, UDWORD yOffset)
{
  ...
  {
    if ((psForm->style & WFORM_CLICKABLE) && (((W_CLICKFORM*)psForm)->state & (WCLICK_DOWN | WCLICK_LOCKED | WCLICK_CLICKLOCK)))
    {
      xOffset += 1;
      yOffset += 1;
    }
...
static void widgDisplayForm(W_FORM* psForm, UDWORD xOffset, UDWORD yOffset) { _widgDisplayForm(psForm, xOffset, yOffset); }
...
void widgFocusLost(WIDGET* psWidget)
{
  ...
  case WIDG_SLIDER:
    break;
  default: ASSERT((FALSE,"widgFocusLost: Unknown widget type"));
    break;
  }
}
...
void widgHiLite(WIDGET* psWidget, W_CONTEXT* psContext)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgHiLite: Unknown widget type"));
    break;
  }
}
...
void widgHiLiteLost(WIDGET* psWidget, W_CONTEXT* psContext)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgHiLiteLost: Unknown widget type"));
    break;
  }
}
...
static void widgClicked(WIDGET* psWidget, UDWORD key, W_CONTEXT* psContext)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgClicked: Unknown widget type"));
    break;
  }
}
...
static void widgReleased(WIDGET* psWidget, UDWORD key, W_CONTEXT* psContext)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgReleased: Unknown widget type"));
    break;
  }
}
...
static void widgRun(WIDGET* psWidget, W_CONTEXT* psContext)
{
  ...
  case WIDG_SLIDER:
    ...
    break;
  default: ASSERT((FALSE,"widgRun: Unknown widget type"));
    break;
  }
}
...
WIDGET_AUDIOCALLBACK WidgGetAudioCallback(void) { return AudioCallback; }

SWORD WidgGetHilightAudioID(void) { return HilightAudioID; }

SWORD WidgGetClickedAudioID(void) { return ClickedAudioID; }

void setWidgetsStatus(BOOL var) { bWidgetsActive = var; }

BOOL getWidgetsStatus(void) { return (bWidgetsActive); }
