...
using D3DDRIVER = struct D3DDRIVER
{
...
using D3DAPPMODE = struct D3DAPPMODE
{
  int w; /* width */
  int h; /* height */
...
using TEXTUREFORMAT = struct TEXTUREFORMAT
{
...
using DISP3DGLOBALS = struct DISP3DGLOBALS
{
...
static void* g_pCurTexture = nullptr;
...
void D3DDrawPoly(int nVerts, D3DTLVERTEX* psVert)
{
  ...
  {
    hResult = g_sD3DGlob.psD3DDevice3->lpVtbl->DrawPrimitive(g_sD3DGlob.psD3DDevice3, D3DPT_TRIANGLEFAN, D3DFVF_TLVERTEX,
                                                             static_cast<LPVOID>(psVert), nVerts, CLIP_STATUS);
  }
  ...
  {
    hResult = g_sD3DGlob.psD3DDevice3->lpVtbl->DrawPrimitive(g_sD3DGlob.psD3DDevice3, D3DPT_LINELIST, D3DFVF_TLVERTEX,
                                                             static_cast<LPVOID>(psVert), nVerts, CLIP_STATUS);
  }
  else //point
  {
    hResult = g_sD3DGlob.psD3DDevice3->lpVtbl->DrawPrimitive(g_sD3DGlob.psD3DDevice3, D3DPT_POINTLIST, D3DFVF_TLVERTEX,
                                                             static_cast<LPVOID>(psVert), nVerts, CLIP_STATUS);
  }
...
void D3DInitDesc(LPD3DDEVICEDESC psD3DDevDesc)
{
...
void D3DGetCaps(void)
{
  LPDIRECT3DDEVICE3 psDev = g_sD3DGlob.psD3DDevice3;
  HRESULT hRes;
  D3DDEVICEDESC sD3DHWDevDesc, sD3DHELDevDesc;
...
void D3DEnableFog(BOOL bEnable)
{
  ...
  if (bFirst || (bEnableLast != bEnable)) { ATTEMPTD3D((hRes = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_FOGENABLE, bEnable))); }
...
void D3DSetFogColour(D3DCOLOR dwColor)
{
  ...
  ATTEMPTD3D((hRes = psDev->lpVtbl->SetRenderState(psDev, D3DRENDERSTATE_FOGCOLOR, dwColor)));
...
void D3DSetTexelOffsetState(BOOL bOffsetOn) { g_bTexelOffsetOn = bOffsetOn; }

/***************************************************************************/

SDWORD D3DCheckCard(void)
{
  ...
  case 0x10DE:
    if (did.dwDeviceId == 0x20 || did.dwDeviceId == 0x28 || did.dwDeviceId == 0x2C)
      return RIVA_TNT;
    break;
...
void D3DSetCardSpecificParams(void)
{
...
void D3DSetTranslucencyMode(TRANSLUCENCY_MODE transMode)
{
  ...
  static DWORD dwAlphaOpLast = D3DTOP_DISABLE, dwAlphaArg1Last = -1, dwAlphaArg2Last = -1;
  DWORD dwAlphaOp = D3DTOP_DISABLE, dwAlphaArg1 = -1, dwAlphaArg2 = -1;
  ...
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_SRCBLEND, srcBlend)));
  }

  if (bFirst || (destBlend != destBlendLast))
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_DESTBLEND, destBlend)));
  }

  if (bFirst || (bBlendEnable != bBlendEnableLast))
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_ALPHABLENDENABLE, bBlendEnable )));
  }

  if (bFirst || (dwAlphaOp != dwAlphaOpLast))
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetTextureStageState( psDev, 0, D3DTSS_ALPHAOP, dwAlphaOp )));
  }

  if ((bFirst || (dwAlphaArg1 != dwAlphaArg1Last)) && (dwAlphaArg1 != -1))
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetTextureStageState( psDev, 0, D3DTSS_ALPHAARG1, dwAlphaArg1 )));
  }

  if ((bFirst || (dwAlphaArg2 != dwAlphaArg2Last)) && (dwAlphaArg2 != -1))
  {
    ATTEMPTD3D((hResult = psDev->lpVtbl->SetTextureStageState( psDev, 0, D3DTSS_ALPHAARG2, dwAlphaArg2 )));
  }
...
void D3DSetCulling(BOOL bCullingOn)
{
  HRESULT hResult;
  LPDIRECT3DDEVICE3 psDev = g_sD3DGlob.psD3DDevice3;
  D3DCULL cullMode;
...
void D3DSetClipWindow(SDWORD xMin, SDWORD yMin, SDWORD xMax, SDWORD yMax)
{
  LPDIRECT3DDEVICE3 psDev = g_sD3DGlob.psD3DDevice3;
  ...
  status.dwStatus = 0;
  status.maxx = static_cast<float>(xMax);
  status.minx = static_cast<float>(xMin);
  status.maxy = static_cast<float>(yMax);
  status.miny = static_cast<float>(yMin);
  status.maxz = 0.0f;
  status.minz = 0.0f;
...
BOOL SetRenderState(void)
{
  HRESULT hResult;
  LPDIRECT3DDEVICE3 psDev = g_sD3DGlob.psD3DDevice3;
  D3DCLIPSTATUS status = {D3DCLIPSTATUS_EXTENTS2, 0, 640.0f, 0.0f, 480.0f, 0.0f, 0.0f, 0.0f};
  ...
  if (D3DGetAlphaKey() == TRUE)
  {
    ATTEMPTD3D(hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_ALPHAFUNC, D3DCMP_NOTEQUAL));
    ATTEMPTD3D(hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_ALPHAREF, 0x00000000));
    ATTEMPTD3D(hResult = psDev->lpVtbl->SetRenderState( psDev, D3DRENDERSTATE_ALPHATESTENABLE, TRUE ));
  }
...
void D3DFreeTexturePage(TEXPAGE_D3D* psTexPage)
{
  if (psTexPage != nullptr)
  {
...
DWORD BitsPerPixelToBitDepth(int iBpp)
{
  BOOL bValFound = TRUE;
...
BOOL GetWindowsMode(void)
{
  ...
  /* get current mode */
  hResult = g_sD3DGlob.psDD4->lpVtbl->GetDisplayMode(g_sD3DGlob.psDD4, &ddsd);
...
static HRESULT WINAPI EnumDeviceFunc(LPGUID lpGuid, LPSTR lpDeviceDescription, LPSTR lpDeviceName, LPD3DDEVICEDESC lpHWDesc,
                                     LPD3DDEVICEDESC lpHELDesc, LPVOID lpContext)
{
  ...
  */
  ...
  if (lpHWDesc->dcmColorModel)
  {
    ...
    memcpy(&g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].Desc, lpHWDesc, sizeof(D3DDEVICEDESC));
  }
  else
  {
    g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].bIsHardware = FALSE;
    memcpy(&g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].Desc, lpHELDesc, sizeof(D3DDEVICEDESC));
  }

  /*
   * Does this driver do texture mapping?
   */
  g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].bDoesTextures = (g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].Desc.dpcTriCaps.dwTextureCaps &
   ...
   */
  g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].bDoesZBuffer = g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].Desc.dwDeviceZBufferBitDepth
                                                              ? TRUE
                                                              : FALSE;
  ...
  /*
     ...
     */
  g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].bCanDoWindow = (g_sD3DGlob.Driver[g_sD3DGlob.uwNumDrivers].Desc.dwDeviceRenderBitDepth &
                                                              BitsPerPixelToBitDepth(g_sD3DGlob.WindowsDisplay.bpp))
                                                              ? TRUE
                                                              : FALSE;
...
HRESULT GetSurfDesc(LPDDSURFACEDESC2 lpDDSurfDesc, LPDIRECTDRAWSURFACE4 lpDDSurf)
{
  HRESULT result;
...
static HRESULT WINAPI EnumZBufferCallback(DDPIXELFORMAT* pddpf, VOID* pddpfDesired)
{
  ...
  {
    if (pddpf->dwZBufferBitDepth == static_cast<DDPIXELFORMAT*>(pddpfDesired)->dwZBufferBitDepth)
    {
...
BOOL CreateZBuffer(int w, int h, int driver)
{
  ...
  hResult = g_sD3DGlob.psD3D3->lpVtbl->EnumZBufferFormats(g_sD3DGlob.psD3D3, (REFCLSID)pDeviceGUID, //(REFCLSID)
                                                          EnumZBufferCallback, &ddpfZBuffer);
  ...
  //create the zBuffer
  hResult = g_sD3DGlob.psDD4->lpVtbl->CreateSurface(g_sD3DGlob.psDD4, &ddsd, &g_sD3DGlob.psZBuffer, nullptr);
   ...
   */
  hResult = g_sD3DGlob.psBack4->lpVtbl->AddAttachedSurface(g_sD3DGlob.psBack4, g_sD3DGlob.psZBuffer);
  ...
  g_sD3DGlob.bZBufferInVideo = (ddsd.ddsCaps.dwCaps & DDSCAPS_VIDEOMEMORY) ? TRUE : FALSE;
...
exit_with_error: RELEASE(g_sD3DGlob.psZBuffer);
...
static HRESULT CALLBACK EnumTextureFormatsCallback(LPDDPIXELFORMAT lpDDPF, LPVOID lpContext)
{
  unsigned long m;
  int r, g, b;
  auto lpStartFormat = static_cast<int*>(lpContext);
  BOOL bOK;

  /* Record the DDSURFACEDESC2 of this texture format */
  memset(&g_sD3DGlob.TextureFormat[g_sD3DGlob.uwNumTextureFormats], 0, sizeof(TEXTUREFORMAT));
  memcpy(&g_sD3DGlob.TextureFormat[g_sD3DGlob.uwNumTextureFormats].ddsd, lpDDPF, sizeof(DDSURFACEDESC2));
  ...
  else
  {
    ...
    for (r = 0, m = lpDDPF->dwRBitMask; !(m & 1); r++, m >>= 1);
    for (r = 0; m & 1; r++, m >>= 1);
    for (g = 0, m = lpDDPF->dwGBitMask; !(m & 1); g++, m >>= 1);
    for (g = 0; m & 1; g++, m >>= 1);
    for (b = 0, m = lpDDPF->dwBBitMask; !(m & 1); b++, m >>= 1);
    for (b = 0; m & 1; b++, m >>= 1);
...
extern BOOL d3d_bHardware(void) { return g_sD3DGlob.Driver[g_sD3DGlob.uwCurrDriver].bIsHardware; }
...
LPDIRECT3D3 d3d_GetpsD3D(void) { return g_sD3DGlob.psD3D3; }

LPDIRECTDRAW4 d3d_GetpsDD4(void) { return g_sD3DGlob.psDD4; }
/***************************************************************************/
/*
 * CreateDevice
 *
...
BOOL CreateDevice(int iDriver)
{
  ...
  g_sD3DGlob.bBackBufferInVideo = (sDDSurfDesc.ddsCaps.dwCaps & DDSCAPS_VIDEOMEMORY) ? TRUE : FALSE;
  ...
  /* create D3D device */
  hResult = g_sD3DGlob.psD3D3->lpVtbl->CreateDevice(g_sD3DGlob.psD3D3, g_sD3DGlob.Driver[g_sD3DGlob.uwCurrDriver].Guid, g_sD3DGlob.psBack4,
                                                    &g_sD3DGlob.psD3DDevice3, nullptr);
...
exit_with_error: RELEASE(g_sD3DGlob.psD3DDevice3);
...
BOOL ChooseDriver(void)
{
  ...
  hResult = g_sD3DGlob.psD3D3->lpVtbl->EnumDevices(g_sD3DGlob.psD3D3, EnumDeviceFunc, nullptr);
  ...
  // choose driver 
  while ((uwDriver < g_sD3DGlob.uwNumDrivers) && (bDriverFound == FALSE))
  {
...
exit_with_error: RELEASE(g_sD3DGlob.psD3DViewport3);
...
BOOL CreateMaterial(LPDIRECT3D3 lpD3D, D3DMATERIALHANDLE* hMat, LPDIRECT3DMATERIAL3* ppsMat, D3DTEXTUREHANDLE hTexture)
{
  D3DMATERIAL sMat;
  HRESULT hResult;
...
exit_with_error: RELEASE((*ppsMat));
...
BOOL rend_InitD3D(void)
{
  ...
  {
    /* Create the Z-buffer */
    ATTEMPT(CreateZBuffer( pie_GetVideoBufferWidth(), pie_GetVideoBufferHeight(), g_sD3DGlob.uwCurrDriver ));
  }
  ...
  /* create viewpoint */
  ATTEMPT(CreateViewport( pie_GetVideoBufferWidth(), pie_GetVideoBufferHeight()));
...
exit_with_error: RELEASE(g_sD3DGlob.psD3DDevice3);
  ...
  if (g_sD3Dinfo.bHardware == TRUE) { RELEASE(g_sD3DGlob.psZBuffer); }
...
void D3DTestCooperativeLevel(BOOL bGotFocus)
{
  ...
  hRes = screenGetDDObject()->lpVtbl->TestCooperativeLevel(screenGetDDObject());
