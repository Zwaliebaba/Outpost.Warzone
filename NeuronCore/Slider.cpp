...
BOOL sliderCreate(W_SLIDER** ppsWidget, W_SLDINIT* psInit)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "sliderCreate: Unknown style");
    return FALSE;
  }

  if (psInit->orientation < WSLD_LEFT || psInit->orientation > WSLD_BOTTOM)
  {
    DEBUG_ASSERT_TEXT(FALSE, "sliderCreate: Unknown orientation");
    return FALSE;
  }

  if (((psInit->orientation == WSLD_LEFT || psInit->orientation == WSLD_RIGHT) && psInit->numStops > (psInit->width - psInit->barSize)) || (
    (psInit->orientation == WSLD_TOP || psInit->orientation == WSLD_BOTTOM) && psInit->numStops > (psInit->height - psInit->barSize)))
  {
    DEBUG_ASSERT_TEXT(FALSE, "sliderCreate: Too many stops for slider length");
    return FALSE;
  }

  if (psInit->pos > psInit->numStops)
  {
    DEBUG_ASSERT_TEXT(FALSE, "sliderCreate: slider position greater than stops");
    return FALSE;
  }

  if (((psInit->orientation == WSLD_LEFT || psInit->orientation == WSLD_RIGHT) && psInit->barSize > psInit->width) || ((psInit->orientation
    == WSLD_TOP || psInit->orientation == WSLD_BOTTOM) && psInit->barSize > psInit->height))
  {
    DEBUG_ASSERT_TEXT(FALSE, "sliderCreate: slider bar is larger than slider width");
    return FALSE;
  }
...
static void sliderGetBarBox(W_SLIDER* psSlider, SWORD* pX, SWORD* pY, UWORD* pWidth, UWORD* pHeight)
{
  switch (psSlider->orientation)
  {
  case WSLD_LEFT:
    *pX = static_cast<SWORD>((psSlider->width - psSlider->barSize) * psSlider->pos / psSlider->numStops);
    *pY = 0;
    *pWidth = psSlider->barSize;
    *pHeight = psSlider->height;
    break;
  case WSLD_RIGHT:
    *pX = static_cast<SWORD>(psSlider->width - psSlider->barSize - (psSlider->width - psSlider->barSize) * psSlider->pos / psSlider->
      numStops);
    *pY = 0;
    *pWidth = psSlider->barSize;
    *pHeight = psSlider->height;
  ...
  case WSLD_TOP:
    *pX = 0;
    *pY = static_cast<SWORD>((psSlider->height - psSlider->barSize) * psSlider->pos / psSlider->numStops);
    *pWidth = psSlider->width;
    *pHeight = psSlider->barSize;
    break;
  case WSLD_BOTTOM:
    *pX = 0;
    *pY = static_cast<SWORD>(psSlider->height - psSlider->barSize - (psSlider->height - psSlider->barSize) * psSlider->pos / psSlider->
      numStops);
    *pWidth = psSlider->width;
    *pHeight = psSlider->barSize;
    break;
  ...
  else if (psWidget->state & SLD_DRAG)
  {
    ...
    case WSLD_LEFT:
      ...
      else
      {
        ...
        psWidget->pos = static_cast<UWORD>((mx + stopSize / 2 - psWidget->barSize / 2) * psWidget->numStops / (psWidget->width - psWidget->
          barSize));
      }
      break;
    case WSLD_RIGHT:
      ...
      else
      {
        /* Mouse is in the middle of the slider, calculate which stop */
        stopSize = (psWidget->width - psWidget->barSize) / psWidget->numStops;
        psWidget->pos = static_cast<UWORD>(psWidget->numStops - (mx + stopSize / 2 - psWidget->barSize / 2) * psWidget->numStops / (psWidget
          ->width - psWidget->barSize));
      }
      break;
    case WSLD_TOP:
      ...
      else
      {
        ...
        psWidget->pos = static_cast<UWORD>((my + stopSize / 2 - psWidget->barSize / 2) * psWidget->numStops / (psWidget->height - psWidget->
          barSize));
      }
      break;
    case WSLD_BOTTOM:
      ...
      else
      {
        /* Mouse is in the middle of the slider, calculate which stop */
        stopSize = (psWidget->height - psWidget->barSize) / psWidget->numStops;
        psWidget->pos = static_cast<UWORD>(psWidget->numStops - (my + stopSize / 2 - psWidget->barSize / 2) * psWidget->numStops / (psWidget
          ->height - psWidget->barSize));
      }
      break;
    }
  }
...
void sliderClicked(W_SLIDER* psWidget, W_CONTEXT* psContext)
{
  if (DragEnabled)
  {
    if (psContext->mx >= psWidget->x && psContext->mx <= psWidget->x + psWidget->width && psContext->my >= psWidget->y && psContext->my <=
      psWidget->y + psWidget->height)
      psWidget->state |= SLD_DRAG;
  }
}

/* Respond to a mouse up */
void sliderReleased(W_SLIDER* psWidget) { (void)psWidget; }

/* Respond to a mouse moving over a slider */
void sliderHiLite(W_SLIDER* psWidget) { psWidget->state |= SLD_HILITE; }

/* Respond to the mouse moving off a slider */
void sliderHiLiteLost(W_SLIDER* psWidget) { psWidget->state &= ~SLD_HILITE; }

/* The slider display function */
void sliderDisplay(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  W_SLIDER* psSlider;
  SWORD x0, y0, x1, y1;
  UWORD width, height;
  ...
  case WSLD_RIGHT:
    ...
    x0 = static_cast<SWORD>(x0 + psSlider->x + xOffset);
    y0 = static_cast<SWORD>(y0 + psSlider->y + yOffset);
  ...
  {
    x0 = static_cast<SWORD>(psWidget->x + xOffset - 2);
