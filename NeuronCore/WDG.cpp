...
} PRIMBUFFERCATALOG;PRIMBUFFERCATALOG* PrimBufferCatalog = NULL; // NULL indicates invalid data
...
BOOL WDG_SetCurrentWDG(char* filename)
{
  ...
  assert(PrimBufferCatalog->Check1==CHECK1); assert(PrimBufferCatalog->Check2==CHECK2); BytesRead = DISK_ReadPos(
    headersize, PrimBufferCatalog->primWRFCatalog, sizeof(WDGINFO)*NumberOfWRFfiles, pFileHandle);
#else
...
#ifdef PRIMCATALOG
  assert(PrimBufferCatalog); // make sure the cache is valid
  assert(PrimBufferCatalog->Check1==CHECK1); assert(PrimBufferCatalog->Check2==CHECK2); CurrentWRF = PrimBufferCatalog->primWRFCatalog;
#else
...
#ifdef PRIMCATALOG
  assert(PrimBufferCatalog); // make sure the cache is valid
  assert(PrimBufferCatalog->Check1==CHECK1); assert(PrimBufferCatalog->Check2==CHECK2); assert(CurrentWRF->filecount < MAXFILESINWRF);
...
BOOL WDG_ProcessWRF(char* WRFname, BOOL UseDataFromWDG)
{
  ...
  if (FILE_IsCatalogValid() == FALSE) { FILE_RestoreCache(); }
#endif
...
#ifdef PRIMCATALOG
  assert(PrimBufferCatalog); // make sure the cache is valid
  assert(PrimBufferCatalog->Check1==CHECK1); assert(PrimBufferCatalog->Check2==CHECK2); CurrentFile = PrimBufferCatalog->
    primWRFfilesCatalog;
#else
  ...
  */
  for (File = 0; File < CurrentWRF->filecount; File++)
  {
    ...
    {
      if (!wdgGetFileFromOtherWDG(CurrentWRF->WRFname, CurrentFile->type, CurrentFile->name, &CurrentFile, &pRetreivedFile))
        return FALSE;
    }
    else
    {
      ...
      pRetreivedFile = FILE_Retrieve(pFileHandle, FileOffset, CurrentFile->filesize);
      // Somehow get from the wdg the required data and return a pointer to it
    }
    ...
    if (FILE_ProcessFile(CurrentFile, pRetreivedFile) == FALSE)
      return FALSE;
...
BOOL WDG_AllowWRFs(void) { return TRUE; }

BOOL WDG_AllowWDGs(void) { return TRUE; }

/** private functions **/
BOOL FILE_ShutdownCache(void)
{
  if (Cache.pBufferStart != nullptr)
  {
...
{
#ifdef PRIMCATALOG
  if (PrimBufferCatalog) { if (PrimBufferCatalog->Check1 == CHECK1 && PrimBufferCatalog->Check2 == CHECK2) { return TRUE; } } return FALSE;
#else
  return TRUE;
#endif
}
...
SDWORD FILE_AmountPending(void) { return DataPendingSize; }
...
UBYTE* FILE_Retrieve(FILE* pFileHandle, UDWORD offsetInWDG, UDWORD filesize)
{
  ...
  return nullptr; // error condition ... cache not set up
}
...
UBYTE* FILE_RetreivePending(UDWORD* SizeLoaded)
{
  ...
  *SizeLoaded = 0;
  if (DataPendingSize <= 0)
    return nullptr; // nothing to do

  if (static_cast<UDWORD>(DataPendingSize) > Cache.BufferSize)
...
void ScanString(char* InString, UBYTE* pSoftwareFlag, SBYTE* pTexturePage)
{
  ...
  if (strstr(String, "soft") != nullptr) //and this is a software textpage
    SoftwareFlag |= TPAGE_SOFTWARE;

  if (strstr(String, "hard") != nullptr) //and this is a software textpage
  ...
  if (strncmp(String, "page-", 5) == 0)
  {
    ...
    for (i = 5; i < static_cast<SDWORD>(strlen(String)); i++)
    {
      if (isdigit(String[i]))
      {
        Value *= 10;
...
void SetLastFnameExtra(UBYTE SoftwareFlag, SBYTE TexturePage)
{
  ...
  if (SoftwareFlag & TPAGE_SOFTWARE)
    strcpy(TypeString, "soft");

  if (SoftwareFlag & TPAGE_HARDWARE)
    strcpy(TypeString, "hard");
...
BOOL loadFileFromWDG(STRING* pFileName, UBYTE** ppFileData, UDWORD* pFileSize, UBYTE AllocateMem)
{
  ...
  // find the file
  if (!wdgFindFirstFile(HashStringIgnoreCase("MISCDATA"), HashString("MISCDATA"), HashStringIgnoreCase(pFileName), &sFindFile))
    return FALSE;
