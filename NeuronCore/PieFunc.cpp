...
/* ---------------------------------------------------------------------------------- */
void pie_CornerBox(SDWORD x0, SDWORD y0, SDWORD x1, SDWORD y1, UDWORD colour, UBYTE a, UBYTE b, UBYTE c, UBYTE d) {}
...
void pie_DrawBoundingDisc(iIMDShape* shape, int pieFlag)
{
  ...
  for (n = 0; n < 8; n++)
  {
    ...
    //cull triangles with off screen points
    if (scrPoints[n].d3dy > static_cast<float>(LONG_TEST))
      renderPoly.flags = 0;

    d3dVrts[n].sz = pPixels->d3dz * INV_MAX_Z;
    d3dVrts[n].rhw = static_cast<float>(1.0) / d3dVrts[n].sz;
    d3dVrts[n].tu = static_cast<float>(0.0);
    d3dVrts[n].tv = static_cast<float>(0.0);
    d3dVrts[n].color = colour;
    d3dVrts[n].specular = specular;
  }
  renderPoly.nVrts = 8;
  renderPoly.pVrts = &d3dVrts[0];
  renderPoly.pTexAnim = nullptr;
...
void pie_Blit(SDWORD texPage, SDWORD x0, SDWORD y0, SDWORD x1, SDWORD y1)
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    {
    case 0:
      d3dVrts[i].sx = static_cast<float>(x0);
      d3dVrts[i].sy = static_cast<float>(y0);
      d3dVrts[i].tu = static_cast<float>(0.0);
      d3dVrts[i].tv = static_cast<float>(0.0);
      break;
    case 1:
      d3dVrts[i].sx = static_cast<float>(x1);
      d3dVrts[i].sy = static_cast<float>(y0);
      d3dVrts[i].tu = static_cast<float>(1.0);
      d3dVrts[i].tv = static_cast<float>(0.0);
      break;
    case 2:
      d3dVrts[i].sx = static_cast<float>(x1);
      d3dVrts[i].sy = static_cast<float>(y1);
      d3dVrts[i].tu = static_cast<float>(1.0);
      d3dVrts[i].tv = static_cast<float>(1.0);
      break;
    case 3: default:
      d3dVrts[i].sx = static_cast<float>(x0);
      d3dVrts[i].sy = static_cast<float>(y1);
    ...
    d3dVrts[i].sz = static_cast<float>(0.001); //scrPoints[*index].d3dz;
    d3dVrts[i].rhw = static_cast<float>(1000.0);
    d3dVrts[i].color = static_cast<D3DCOLOR>((255 << 24) + (255 << 16) + (255 << 8) + 255);
...
void pie_Sky(SDWORD texPage, PIEVERTEX* aSky)
{
  ...
  for (i = 0; i < 4; i++)
  {
    d3dVrts[i].sx = static_cast<float>(aSky[i].sx);
    d3dVrts[i].sy = static_cast<float>(aSky[i].sy);
    if (d3dVrts[i].sy == -9999)
      renderPoly.flags = 0;
    d3dVrts[i].tu = static_cast<float>(aSky[i].tu) * INV_TEX_SIZE;
    d3dVrts[i].tv = static_cast<float>(aSky[i].tv) * INV_TEX_SIZE;
    d3dVrts[i].sz = static_cast<float>(aSky[i].sz) * INV_MAX_Z; //scr Points[*index].d3dz;
    d3dVrts[i].rhw = static_cast<float>(1.0) / d3dVrts[i].sz;
    d3dVrts[i].color = static_cast<D3DCOLOR>((255 << 24) + (255 << 16) + (255 << 8) + 255);
    d3dVrts[i].specular = 0;
  }
  pie_SetTexturePage(texPage);
...
void pie_Water(SDWORD texPage, SDWORD x0, SDWORD y0, SDWORD x1, SDWORD y1, SDWORD height, SDWORD translucency)
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    case 2:
      ...
      vertex.z = y1;
      d3dVrts[i].tu = static_cast<float>(1.0);
      d3dVrts[i].tv = static_cast<float>(0.5);
      break;
    case 3: default:
      vertex.x = x0;
      vertex.z = y1;
    ...
    if ((tzx <= 0) || (tzy <= 0))
    {
      pPixel.d3dx = static_cast<float>(LONG_WAY); //just along way off screen
      pPixel.d3dy = static_cast<float>(LONG_WAY);
    }
    ...
    d3dVrts[i].sx = pPixel.d3dx;
    d3dVrts[i].sy = pPixel.d3dy;
    if (d3dVrts[i].sy > LONG_TEST)
      renderPoly.flags = 0;
    d3dVrts[i].sz = pPixel.d3dz * INV_MAX_Z;
    d3dVrts[i].rhw = static_cast<float>(1.0) / d3dVrts[i].sz;
    d3dVrts[i].color = static_cast<D3DCOLOR>((translucency << 24) + (translucency << 16) + (translucency << 8) + translucency);
    d3dVrts[i].specular = 0;
  }
  pie_SetTexturePage(texPage);
  pie_D3DPoly(&renderPoly); // draw the polygon ... this is an inline function
