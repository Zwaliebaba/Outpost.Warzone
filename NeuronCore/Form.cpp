...
static BOOL formCreatePlain(W_FORM** ppsWidget, W_FORMINIT* psInit)
{
  ...
  *ppsWidget = new (std::nothrow) W_FORM;
  ...
  (*ppsWidget)->psWidgets = nullptr;
  (*ppsWidget)->psLastHiLite = nullptr;
...
static BOOL formCreateTabbed(W_TABFORM** ppsWidget, W_FORMINIT* psInit)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreateTabbed: Must have at least one major tab on a tabbed form");
    return FALSE;
  }
  if (psInit->majorPos != 0 && psInit->majorPos == psInit->minorPos)
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreateTabbed: Cannot have major and minor tabs on same side");
    return FALSE;
  }
  if (psInit->numMajor >= WFORM_MAXMAJOR)
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreateTabbed: Too many Major tabs");
    return FALSE;
  }
  for (major = 0; major < psInit->numMajor; major++)
  {
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "formCreateTabbed: Too many Minor tabs for Major {}", major);
      return FALSE;
    }
    if (psInit->aNumMinors[major] == 0)
    {
      DEBUG_ASSERT_TEXT(FALSE, "formCreateTabbed: Must have at least one Minor tab for each major");
      return FALSE;
    }
  }
  ...
  *ppsWidget = new (std::nothrow) W_TABFORM;
  ...
  for (major = 0; major < psInit->numMajor; major++)
  {
    ...
    for (minor = 0; minor < psInit->aNumMinors[major]; minor++)
    {
      ...
      {
#if W_USE_STRHEAP
        (void)widgAllocCopyString(&(psMajor->asMinor[minor].pTip), psInit->apMinorTips[major][minor]);
#else
...
{
#if W_USE_STRHEAP
  UDWORD minor, major; W_MAJORTAB* psMajor; psMajor = psForm->asMajor; for (major = 0; major < psForm->numMajor; major++)
  {
    if (psMajor->pTip) { widgFreeString(psMajor->pTip); }
    for (minor = 0; minor < psMajor->numMinor; minor++)
    {
      if (psMajor->asMinor[minor].pTip) { widgFreeString(psMajor->asMinor[minor].pTip); }
    }
    psMajor++;
  }
#else
...
BOOL formCreate(W_FORM** ppsWidget, W_FORMINIT* psInit)
{
  ...
  if (psInit->style & ~(WFORM_TABBED | WFORM_INVISIBLE | WFORM_CLICKABLE | WFORM_NOCLICKMOVE | WFORM_NOPRIMARY | WFORM_SECONDARY |
    WIDG_HIDDEN))
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreate: Unknown style bit");
    return FALSE;
  }
  if ((psInit->style & WFORM_TABBED) && (psInit->style & (WFORM_INVISIBLE | WFORM_CLICKABLE)))
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreate: Tabbed form cannot be invisible or clickable");
    return FALSE;
  }
  if ((psInit->style & WFORM_INVISIBLE) && (psInit->style & WFORM_CLICKABLE))
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreate: Cannot have an invisible clickable form");
    return FALSE;
  }
  if (!(psInit->style & WFORM_CLICKABLE) && ((psInit->style & WFORM_NOPRIMARY) || (psInit->style & WFORM_SECONDARY)))
  {
    DEBUG_ASSERT_TEXT(FALSE, "formCreate: Cannot set keys if the form isn't clickable");
    return FALSE;
  }
...
BOOL formAddWidget(W_FORM* psForm, WIDGET* psWidget, W_INIT* psInit)
{
  ...
  if (psForm->style & WFORM_TABBED)
  {
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "formAddWidget: Major tab does not exist");
      return FALSE;
    }
    psMajor = psTabForm->asMajor + psInit->majorID;
    if (psInit->minorID >= psMajor->numMinor)
    {
      DEBUG_ASSERT_TEXT(FALSE, "formAddWidget: Minor tab does not exist");
      return FALSE;
    }
...
WIDGET* formGetAllWidgets(W_FORMGETALL* psCtrl)
{
  ...
  else
  {
    ...
    psRetList = nullptr;
    while (psRetList == nullptr && psCtrl->GAWMajor < psCtrl->psGAWForm->numMajor)
    {
...
void widgSetTabs(W_SCREEN* psScreen, UDWORD id, UWORD major, UWORD minor)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "widgSetTabs: invalid major or minor id");
    return;
  }
...
void widgSetColour(W_SCREEN* psScreen, UDWORD id, UDWORD colour, UBYTE red, UBYTE green, UBYTE blue)
{
  W_TABFORM* psForm;
...
void formInitialise(W_FORM* psWidget)
{
  ...
  if (psWidget->style & WFORM_TABBED)
  {
    ...
    psTabForm->majorT = 0;
    psTabForm->minorT = 0;
    psTabForm->tabHiLite = static_cast<UWORD>(-1);
...
static BOOL formPickHTab(TAB_POS* psTabPos, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD gap, UDWORD number, SDWORD fx,
                         SDWORD fy)
{
  SDWORD x, y1;
  UDWORD i;
  ...
  for (i = 0; i < number; i++)
  {
    ...
    if (fx >= x && fx <= x + static_cast<SDWORD>(width) && fy >= y0 && fy <= y1)
    {
...
static BOOL formPickVTab(TAB_POS* psTabPos, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD gap, UDWORD number, SDWORD fx,
                         SDWORD fy)
{
  SDWORD x1, y;
  UDWORD i;
  ...
  x1 = x0 + width;
  y = y0;
  for (i = 0; i < static_cast<SDWORD>(number); i++)
  {
    if (fx >= x0 && fx <= x1 && fy >= y && fy <= y + static_cast<SDWORD>(height))
...
static BOOL formPickTab(W_TABFORM* psForm, UDWORD fx, UDWORD fy, TAB_POS* psTabPos)
{
  SDWORD x0, y0, x1, y1;
  W_MAJORTAB* psMajor;
  ...
  {
  case WFORM_TABTOP:
    if (formPickHTab(psTabPos, x0 + psForm->majorOffset - xOffset, y0 - psForm->tabMajorThickness, psForm->majorSize,
                     psForm->tabMajorThickness, psForm->tabMajorGap, psForm->numMajor, fx, fy))
      return TRUE;
    yOffset2 = -psForm->tabVertOffset;
    break;
  case WFORM_TABBOTTOM:
    if (formPickHTab(psTabPos, x0 + psForm->majorOffset - xOffset, y1, psForm->majorSize, psForm->tabMajorThickness, psForm->tabMajorGap,
                     psForm->numMajor, fx, fy))
      return TRUE;
    break;
  case WFORM_TABLEFT:
    if (formPickVTab(psTabPos, x0 - psForm->tabMajorThickness, y0 + psForm->majorOffset - yOffset, psForm->tabMajorThickness,
                     psForm->majorSize, psForm->tabMajorGap, psForm->numMajor, fx, fy))
      return TRUE;
    xOffset2 = psForm->tabHorzOffset;
    break;
  case WFORM_TABRIGHT:
    if (formPickVTab(psTabPos, x1, y0 + psForm->majorOffset - yOffset, psForm->tabMajorThickness, psForm->majorSize, psForm->tabMajorGap,
                     psForm->numMajor, fx, fy))
      return TRUE;
    break;
  case WFORM_TABNONE: DEBUG_ASSERT_TEXT(FALSE, "formDisplayTabbed: Cannot have a tabbed form with no major tabs");
    break;
  }
  ...
  {
  case WFORM_TABTOP:
    if (formPickHTab(psTabPos, x0 + psForm->minorOffset - xOffset2, y0 - psForm->tabMinorThickness, psForm->minorSize,
                     psForm->tabMinorThickness, psForm->tabMinorGap, psMajor->numMinor, fx, fy))
    {
    ...
    break;
  case WFORM_TABBOTTOM:
    if (formPickHTab(psTabPos, x0 + psForm->minorOffset - xOffset2, y1, psForm->minorSize, psForm->tabMinorThickness, psForm->tabMinorGap,
    ...
    break;
  case WFORM_TABLEFT:
    if (formPickVTab(psTabPos, x0 + xOffset - psForm->tabMinorThickness, y0 + psForm->minorOffset - yOffset2, psForm->tabMinorThickness,
                     psForm->minorSize, psForm->tabMinorGap, psMajor->numMinor, fx, fy))
    {
      psTabPos->index += psForm->numMajor;
      return TRUE;
    }
    break;
  case WFORM_TABRIGHT:
    if (formPickVTab(psTabPos, x1 + xOffset, y0 + psForm->minorOffset - yOffset2, psForm->tabMinorThickness, psForm->minorSize,
                     psForm->tabMinorGap, psMajor->numMinor, fx, fy))
    {
      psTabPos->index += psForm->numMajor;
      return TRUE;
    }
...
void formRun(W_FORM* psWidget, W_CONTEXT* psContext)
{
  ...
  if (psWidget->style & WFORM_TABBED)
  {
    ...
    if (mx >= 0 && mx <= psTabForm->width && my >= 0 && my <= psTabForm->height)
    {
      if (formPickTab(psTabForm, mx, my, &sTabPos))
      {
        if (psTabForm->tabHiLite != static_cast<UWORD>(sTabPos.index))
        {
          /* Got a new tab - start the tool tip if there is one */
          psTabForm->tabHiLite = static_cast<UWORD>(sTabPos.index);
          if (sTabPos.index >= psTabForm->numMajor)
          {
            pTip = psTabForm->asMajor[psTabForm->majorT].asMinor[sTabPos.index - psTabForm->numMajor].pTip;
          }
          else
          ...
          if (pTip)
          {
            ...
            tipStart((WIDGET*)psTabForm, pTip, psContext->psScreen->TipFontID, (UDWORD*)psTabForm->aColours, sTabPos.x + psContext->xOffset,
                     sTabPos.y + psContext->yOffset, sTabPos.width, sTabPos.height);
          }
          else
          {
...
void formClicked(W_FORM* psWidget, UDWORD key)
{
  ...
  if (psWidget->style & WFORM_CLICKABLE)
  {
    ...
    if (!(((W_CLICKFORM*)psWidget)->state & (WCLICK_GREY | WCLICK_LOCKED)))
    {
      ...
      if ((!(psWidget->style & WFORM_NOPRIMARY) && key == WKEY_PRIMARY) || ((psWidget->style & WFORM_SECONDARY) && key == WKEY_SECONDARY))
      {
  ...
  if (psWidget->style & WFORM_TABBED)
  {
    ...
    if (formPickTab(psTabForm, psContext->mx, psContext->my, &sTabPos))
    {
      if (sTabPos.index >= psTabForm->numMajor)
      {
        ...
        psTabForm->minorT = static_cast<UWORD>(sTabPos.index - psTabForm->numMajor);
      ...
      else
      {
        ...
        psTabForm->majorT = static_cast<UWORD>(sTabPos.index);
...
void formHiLite(W_FORM* psWidget, W_CONTEXT* psContext)
{
  ...
  if (psWidget->style & WFORM_CLICKABLE)
  {
    ...
    if (psClickForm->pTip)
    {
      tipStart((WIDGET*)psClickForm, psClickForm->pTip, psContext->psScreen->TipFontID, (UDWORD*)psContext->psForm->aColours,
...
void formHiLiteLost(W_FORM* psWidget, W_CONTEXT* psContext)
{
  ...
  if (psWidget->psLastHiLite != nullptr)
    widgHiLiteLost(psWidget->psLastHiLite, psContext);
  if (psWidget->style & WFORM_TABBED)
    ((W_TABFORM*)psWidget)->tabHiLite = static_cast<UWORD>(-1);
  if (psWidget->style & WFORM_CLICKABLE)
...
void formDisplay(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  if (!(psWidget->style & WFORM_INVISIBLE))
  {
    ...
    pie_Line(x1, y1, x0, y1, *(pColours + WCOL_DARK));
  }
}
...
void formDisplayClickable(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  {
    /* Form down */
    pie_Line(x0, y1, x0, y0, *(pColours + WCOL_DARK));
    pie_Line(x0, y0, x1, y0, *(pColours + WCOL_DARK));
    pie_Line(x1, y0, x1, y1, *(pColours + WCOL_LIGHT));
    pie_Line(x1, y1, x0, y1, *(pColours + WCOL_LIGHT));
  }
  else
  {
    /* Form up */
...
/* Draw top tabs */
static void formDisplayTTabs(W_TABFORM* psForm, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD number, UDWORD selected,
                             UDWORD hilite, UDWORD* pColours, UDWORD TabType, UDWORD TabGap)
{
  SDWORD x, x1, y1;
  UDWORD i;
  ...
  for (i = 0; i < number; i++)
  {
    ...
    else
    {
      if (i == selected)
      {
        ...
        /* Draw the outline */
        pie_Line(x, y0 + 2, x, y1 - 1, *(pColours + WCOL_LIGHT));
        pie_Line(x, y0 + 2, x + 2, y0, *(pColours + WCOL_LIGHT));
        pie_Line(x + 2, y0, x1 - 1, y0, *(pColours + WCOL_LIGHT));
        pie_Line(x1, y0 + 1, x1, y1, *(pColours + WCOL_DARK));
      }
      else
      {
        /* Fill in the tab */
        ...
        /* Draw the outline */
        pie_Line(x, y0 + 3, x, y1 - 1, *(pColours + WCOL_LIGHT));
        pie_Line(x, y0 + 3, x + 2, y0 + 1, *(pColours + WCOL_LIGHT));
        pie_Line(x + 2, y0 + 1, x1 - 1, y0 + 1, *(pColours + WCOL_LIGHT));
        pie_Line(x1, y0 + 2, x1, y1 - 1, *(pColours + WCOL_DARK));
      }
      if (i == hilite)
      {
        /* Draw the hilite box */
        pie_Box(x + 2, y0 + 4, x1 - 3, y1 - 3, *(pColours + WCOL_HILITE));
      }
    }
    x += width + TabGap;
    x1 += width + TabGap;
...
/* Draw bottom tabs */
static void formDisplayBTabs(W_TABFORM* psForm, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD number, UDWORD selected,
                             UDWORD hilite, UDWORD* pColours, UDWORD TabType, UDWORD TabGap)
{
  SDWORD x, x1, y1;
  UDWORD i;
  ...
  for (i = 0; i < number; i++)
  {
    ...
    else
    {
      if (i == selected)
      {
        ...
        /* Draw the outline */
        pie_Line(x, y0, x, y1 - 1, *(pColours + WCOL_LIGHT));
        pie_Line(x, y1, x1 - 3, y1, *(pColours + WCOL_DARK));
        pie_Line(x1 - 2, y1, x1, y1 - 2, *(pColours + WCOL_DARK));
        pie_Line(x1, y1 - 3, x1, y0 + 1, *(pColours + WCOL_DARK));
      }
      else
      {
        /* Fill in the tab */
        ...
        /* Draw the outline */
        pie_Line(x, y0 + 1, x, y1 - 1, *(pColours + WCOL_LIGHT));
        pie_Line(x + 1, y1 - 1, x1 - 3, y1 - 1, *(pColours + WCOL_DARK));
        pie_Line(x1 - 2, y1 - 1, x1, y1 - 3, *(pColours + WCOL_DARK));
        pie_Line(x1, y1 - 4, x1, y0 + 1, *(pColours + WCOL_DARK));
      }
      if (i == hilite)
      {
        /* Draw the hilite box */
        pie_Box(x + 2, y0 + 3, x1 - 3, y1 - 4, *(pColours + WCOL_HILITE));
      }
    }
    x += width + TabGap;
    x1 += width + TabGap;
...
/* Draw left tabs */
static void formDisplayLTabs(W_TABFORM* psForm, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD number, UDWORD selected,
                             UDWORD hilite, UDWORD* pColours, UDWORD TabType, UDWORD TabGap)
{
  SDWORD x1, y, y1;
  UDWORD i;
  ...
  for (i = 0; i < static_cast<SDWORD>(number); i++)
  {
    ...
    else
    {
      if (i == selected)
      {
        ...
        pie_Line(x0, y + 1, x0, y1 - 2, *(pColours + WCOL_LIGHT));
        pie_Line(x0 + 1, y1 - 1, x0 + 2, y1, *(pColours + WCOL_DARK));
        pie_Line(x0 + 3, y1, x1, y1, *(pColours + WCOL_DARK));
      }
      else
      {
        /* Fill in the tab */
        ...
        pie_Line(x0 + 1, y + 1, x0 + 1, y1 - 2, *(pColours + WCOL_LIGHT));
        pie_Line(x0 + 2, y1 - 1, x0 + 3, y1, *(pColours + WCOL_DARK));
        pie_Line(x0 + 4, y1, x1 - 1, y1, *(pColours + WCOL_DARK));
      }
      if (i == hilite)
        pie_Box(x0 + 4, y + 2, x1 - 2, y1 - 3, *(pColours + WCOL_HILITE));
    }
    y += height + TabGap;
    y1 += height + TabGap;
  }
...
/* Draw right tabs */
static void formDisplayRTabs(W_TABFORM* psForm, SDWORD x0, SDWORD y0, UDWORD width, UDWORD height, UDWORD number, UDWORD selected,
                             UDWORD hilite, UDWORD* pColours, UDWORD TabType, UDWORD TabGap)
{
  SDWORD x1, y, y1;
  UDWORD i;
  ...
  for (i = 0; i < static_cast<SDWORD>(number); i++)
  {
    ...
    else
    {
      if (i == selected)
      {
        ...
        pie_Line(x1 - 1, y1 - 1, x1 - 2, y1, *(pColours + WCOL_DARK));
        pie_Line(x1 - 3, y1, x0, y1, *(pColours + WCOL_DARK));
      }
      else
      {
        /* Fill in the tab */
        ...
        pie_Line(x1 - 2, y1 - 1, x1 - 3, y1, *(pColours + WCOL_DARK));
        pie_Line(x1 - 4, y1, x0 + 1, y1, *(pColours + WCOL_DARK));
      }
      if (i == hilite)
        pie_Box(x0 + 2, y + 2, x1 - 4, y1 - 3, *(pColours + WCOL_HILITE));
    }
    y += height + TabGap;
    y1 += height + TabGap;
  }
...
void formDisplayTabbed(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  {
  case WFORM_TABTOP:
    formDisplayTTabs(psForm, x0 + psForm->majorOffset, y0 - psForm->tabMajorThickness + psForm->tabVertOffset, psForm->majorSize,
                     psForm->tabMajorThickness, psForm->numMajor, psForm->majorT, psForm->tabHiLite, pColours,TAB_MAJOR,
                     psForm->tabMajorGap);
    break;
  case WFORM_TABBOTTOM:
    formDisplayBTabs(psForm, x0 + psForm->majorOffset, y1 + psForm->tabVertOffset, psForm->majorSize, psForm->tabMajorThickness,
                     psForm->numMajor, psForm->majorT, psForm->tabHiLite, pColours,TAB_MAJOR, psForm->tabMajorGap);
    break;
  case WFORM_TABLEFT:
    formDisplayLTabs(psForm, x0 - psForm->tabMajorThickness + psForm->tabHorzOffset, y0 + psForm->majorOffset, psForm->tabMajorThickness,
                     psForm->majorSize, psForm->numMajor, psForm->majorT, psForm->tabHiLite, pColours,TAB_MAJOR, psForm->tabMajorGap);
    break;
  case WFORM_TABRIGHT:
  ...
  {
  case WFORM_TABTOP:
    formDisplayTTabs(psForm, x0 + psForm->minorOffset, y0 - psForm->tabMinorThickness + psForm->tabVertOffset, psForm->minorSize,
                     psForm->tabMinorThickness, psMajor->numMinor, psForm->minorT, psForm->tabHiLite - psForm->numMajor, pColours,TAB_MINOR,
                     psForm->tabMinorGap);
    break;
  case WFORM_TABBOTTOM:
    formDisplayBTabs(psForm, x0 + psForm->minorOffset, y1 + psForm->tabVertOffset, psForm->minorSize, psForm->tabMinorThickness,
                     psMajor->numMinor, psForm->minorT, psForm->tabHiLite - psForm->numMajor, pColours,TAB_MINOR, psForm->tabMinorGap);
    break;
  case WFORM_TABLEFT:
    formDisplayLTabs(psForm, x0 - psForm->tabMinorThickness + psForm->tabHorzOffset + psForm->minorOffset, y0 + psForm->minorOffset,
                     psForm->tabMinorThickness, psForm->minorSize, psMajor->numMinor, psForm->minorT, psForm->tabHiLite - psForm->numMajor,
                     pColours,TAB_MINOR, psForm->tabMinorGap);
    break;
  case WFORM_TABRIGHT:
    formDisplayRTabs(psForm, x1 + psForm->tabHorzOffset, y0 + psForm->minorOffset, psForm->tabMinorThickness, psForm->minorSize,
                     psMajor->numMinor, psForm->minorT, psForm->tabHiLite - psForm->numMajor, pColours,TAB_MINOR, psForm->tabMinorGap);
    break;
    /* case WFORM_TABNONE - no minor tabs so nothing to display */
  }
