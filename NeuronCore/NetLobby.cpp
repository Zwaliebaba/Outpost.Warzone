...
BOOL NETsetRegistryEntries(char* name, char* guid, char* file, char* cline, char* path, char* cdir)
{
  ...
  HKEY ghGameKey = nullptr; // duel registry key handle
...
BOOL NETconnectToLobby(LPNETPLAY lpNetPlay)
{
  LPDIRECTPLAY2A lpDirectPlay2A = nullptr;
  LPDIRECTPLAY4A lpDirectPlay4A = nullptr;
  LPDIRECTPLAYLOBBYA lpDPlob = nullptr;
  LPDPLCONNECTION lpConnSettings = nullptr;
  DPID dpidPlayer;
  DWORD dwSize;
  HRESULT hr;
  ...
  lpConnSettings = static_cast<LPDPLCONNECTION>(GlobalAllocPtr(GHND, dwSize)); // allocate memory for connection settings
  ...
  lpDirectPlay4A = nullptr; // set to NULL here so won't release below
  lpDPlob = nullptr;
  goto SUCCESS;
...
FAILURE: DBPRINTF(("NETPLAY:lobby connect failed\n"));
  ...
  if (lpDirectPlay4A)
    IDirectPlayX_Release(lpDirectPlay4A);
  if (lpDPlob)
    IDirectPlayLobby_Release(lpDPlob);
  if (lpConnSettings)
    GlobalFreePtr(lpConnSettings);
  return FALSE;
SUCCESS:
  if (lpDirectPlay2A)
    IDirectPlay2_Release(lpDirectPlay2A);
