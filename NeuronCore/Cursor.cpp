...
BOOL cursorInitialise(SDWORD width, SDWORD height)
{
  ...
  // Create the Thread
  hCursorThread = CreateThread(nullptr, 1024, cursorThreadUpdate, nullptr, 0, //CREATE_SUSPENDED,
                               &cursorThreadID);
...
BOOL cursorLoad(CURSOR_LOAD* psLoadData)
{
  ...
  if (!surfLoadFrom8Bit(psCursorSurface, psLoadData->imgWidth, psLoadData->imgHeight, psLoadData->pImageData, psLoadData->psPalette))
    return FALSE;
...
static void setRects(RECT* psScreen, RECT* psCursor, SDWORD x, SDWORD y, SDWORD width, SDWORD height)
{
  SDWORD diff;
  ...
  if (psScreen->right >= static_cast<SDWORD>(screenWidth))
  {
    diff = psScreen->right - static_cast<SDWORD>(screenWidth) + 1;
    psScreen->right -= diff;
    psCursor->right -= diff;
  }
  if (psScreen->bottom >= static_cast<SDWORD>(screenHeight))
  {
    diff = psScreen->bottom - static_cast<SDWORD>(screenHeight) + 1;
...
static DWORD WINAPI cursorThreadUpdate(LPVOID param)
{
  ...
  while (!cursorExitThread)
  {
    ...
    setRects(&sScreenRect, &sSaveRect, mx, my, CURSOR_SAVEWIDTH, CURSOR_SAVEHEIGHT);
    ...
    if (cursorFrames > 0)
    {
      setRects(&sScreenRect, &sCursorRect, mx, my, cursorWidth, cursorHeight);
...
BOOL cursorDisplay(void) { return TRUE; }

// Stop displaying the cursor
BOOL cursorHide(void) { return TRUE; }
