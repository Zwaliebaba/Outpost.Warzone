...
static BOOL sendAudioBuffer(BOOL bNow)
{
  ...
  if (lastread <= read) // non wrapped.
    size = static_cast<UWORD>(read - lastread);
  else // wrapped
    size = static_cast<UWORD>(read + (captureBuff.dwBufferBytes - lastread));
  ...
  hr = IDirectSoundCaptureBuffer_Lock(lpDirectSoundCaptureBuffer, //Obtain mem add. of write block.
                                      lastread, // start at
                                      size, // read this much
                                      &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0 //DSCBLOCK_ENTIREBUFFER	
    );
  ...
  if (lpvPtr2 != nullptr) // second part.
    memcpy(((UBYTE*)msg.body) + dwBytes1, lpvPtr2, dwBytes2); // buffer contents			

  // unlock area.
  hr = IDirectSoundCaptureBuffer_Unlock(lpDirectSoundCaptureBuffer, lpvPtr1, dwBytes1, lpvPtr2, dwBytes2);
  if (hr != DS_OK)
    return FALSE;
  ...
  msg.type = AUDIOMSG;
  msg.size = static_cast<UWORD>(dwBytes1 + dwBytes2);
  NETbcast(&msg,FALSE);
...
BOOL NETinitAudioCapture(VOID)
{
  ...
  lpDirectSoundCaptureBuffer = nullptr;
  ...
  hr = IDirectSoundCapture_CreateCaptureBuffer(lpDirectSoundCapture, &captureBuff, &lpDirectSoundCaptureBuffer, NULL);
  if (hr != DS_OK)
  {
    if (hr == DSERR_INVALIDPARAM)
...
static BOOL setupPlayBuffer(VOID)
{
  ...
  playbackBuff.dwFlags = DSBCAPS_CTRLFREQUENCY | DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME;
...
BOOL NETinitPlaybackBuffer(VOID* pDs)
{
  ...
  if (pDs == nullptr) // init the pointers.
  {
    ourDSPointer = TRUE;
  ...
  {
    ourDSPointer = FALSE;
    lpDirectSound = static_cast<LPDIRECTSOUND>(pDs);
  }
...
static VOID stopIncomingAudio(VOID)
{
  ...
  hr = IDirectSoundBuffer_Lock(lpDirectSoundBuffer, // clear out and start again
                               0, 0, &lpvPtr1, &dwBytes1, NULL, NULL, DSBLOCK_ENTIREBUFFER);
  if (DS_OK == hr)
  {
    ZeroMemory(lpvPtr1, playbackBuff.dwBufferBytes); // remove old stuff
...
BOOL NETqueueIncomingAudio(LPBYTE lpbSoundData, DWORD dwSoundBytes, BOOL bStream)
{
  LPVOID lpvPtr1, lpvPtr2 = nullptr;
  ...
  if (!bStream)
  {
    ...
    hr = IDirectSoundBuffer_Lock(lpDirectSoundBuffer, 0, //offset
                                 dwSoundBytes, //size (ignored)
                                 &lpvPtr1, &dwBytes1, NULL, NULL, DSBLOCK_ENTIREBUFFER);
  }
  else // stream nicely
  {
    hr = IDirectSoundBuffer_Lock(lpDirectSoundBuffer, 0, //offset
  ...
  if (DS_OK == hr)
  {
    ...
    else // not streaming
    {
      ...
      ZeroMemory(static_cast<UBYTE*>(lpvPtr1)+dwSoundBytes, (playbackBuff.dwBufferBytes)-dwSoundBytes); // remove old stuff
    }
...
void NETplayIncomingAudio(NETMSG* pMsg)
{
  ...
  stopIncomingAudio(); IDirectSoundBuffer_SetCurrentPosition(lpDirectSoundBuffer, 0); // reset the play position
