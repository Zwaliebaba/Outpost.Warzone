...
{
  ASSERT((psWDGCache == NULL, "wdgMultiInit: the WDG cache has not been reset"));
  psWDGCache = nullptr;
  psWDGCacheRev = nullptr;
  psWDGCacheStore = nullptr;
  psWDGCacheStoreRev = nullptr;
...
BOOL wdgCheckDependancies(void)
{
  ...
  for (psCheck = psWDGCache; psCheck != nullptr; psCheck = psCheck->psNext)
  {
    for (dep = 0; dep < psCheck->numDep; dep++)
    {
      for (psSearch = psWDGCache; psSearch != nullptr; psSearch = psSearch->psNext)
      {
        if ((psSearch->sequence == psCheck->asDep[dep].sequence) && (strcmp(psSearch->aName, psCheck->asDep[dep].aName) == 0))
          break;
      }
...
BOOL wdgLoadCompleteCatalog(char* pWDGName)
{
  ...
  psPrev = nullptr;
  for (psCurr = psWDGCache; psCurr != nullptr; psCurr = psCurr->psNext)
  {
...
BOOL wdgFindNextWRF(WDG_FINDWRF* psFindData)
{
  ...
  if (psFindData->psCurrCache == nullptr)
    return FALSE;

  // search each WDG cache
  psCurrCache = psFindData->psCurrCache->psNext;
  for (; psCurrCache != nullptr; psCurrCache = psCurrCache->psNext)
  {
...
found:
  ...
  psFindData->psCurrCache = nullptr;
...
BOOL wdgFindFirstFile(UDWORD WRFName, UDWORD type, UDWORD FileName, WDG_FINDFILE* psFindData)
{
  ...
  {
    if ((asFileData[i].name == psFindData->FileNameHash) && (asFileData[i].type == psFindData->FileType) && (asFileData[i].offset !=
      UDWORD_MAX))
    {
...
BOOL wdgFindNextFile(WDG_FINDFILE* psFindData)
{
  ...
  while (psFindData->psCurrCache != nullptr)
  {
    // find the next WRF
...
void wdgSetCurrentWDGFromFind(WDG_FINDWRF* psFindData)
{
  ...
  WDG_SetCurrentWDGCatalog(psCache->aFileName, psCache->numWRF, psCache->asWRFCatalog);
...
BOOL wdgFindNextWRFRev(WDG_FINDWRF* psFindData)
{
  ...
  for (; psCurrCache != nullptr; psCurrCache = psCurrCache->psPrev)
  {
    for (i = 0; i < psCurrCache->numWRF; i++)
    {
      if (psCurrCache->asWRFCatalog[i].WRFname == psFindData->WRFNameHash)
