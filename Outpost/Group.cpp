...
BOOL grpCreate(DROID_GROUP** ppsGroup)
{
  ...
  (*ppsGroup)->refCount = 0;
  (*ppsGroup)->psList = nullptr;
  (*ppsGroup)->psCommander = nullptr;
...
void grpJoin(DROID_GROUP* psGroup, DROID* psDroid)
{
  ...
  if (psDroid != nullptr)
  {
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "grpJoin: Cannot have more than one players droids in a group");
      return;
    }
    ...
    {
      DEBUG_ASSERT_TEXT((psGroup->type == GT_NORMAL), "grpJoin: Cannot have two transporters in a group");
    ...
    {
      DEBUG_ASSERT_TEXT((psGroup->type == GT_NORMAL) && (psGroup->psCommander == NULL), "grpJoin: Cannot have two command droids in a group");
...
void grpLeave(DROID_GROUP* psGroup, DROID* psDroid)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "grpLeave: droid group does not match");
    return;
  }
  ...
  if (psDroid != nullptr && (psDroid->droidType != DROID_COMMAND || psGroup->type != GT_COMMAND))
  {
    psPrev = nullptr;
    for (psCurr = psGroup->psList; psCurr; psCurr = psCurr->psGrpNext)
    {
    ...
    DEBUG_ASSERT_TEXT(psCurr != NULL, "grpLeave: droid not found");
    if (psCurr != nullptr)
    {
      if (psPrev)
  ...
  if (psDroid)
  {
    ...
    if ((psDroid->droidType == DROID_COMMAND) && (psGroup->type == GT_COMMAND))
    {
      psGroup->type = GT_NORMAL;
      psGroup->psCommander = nullptr;
    }
    else if ((psDroid->droidType == DROID_TRANSPORTER) && (psGroup->type == GT_TRANSPORTER))
      psGroup->type = GT_NORMAL;
  }
...
void orderGroupLoc(DROID_GROUP* psGroup, DROID_ORDER order, UDWORD x, UDWORD y)
{
  ...
  if (bMultiPlayer)
  {
    SendGroupOrderGroup(psGroup, order, x, y, nullptr);
    bMultiPlayer = FALSE;
