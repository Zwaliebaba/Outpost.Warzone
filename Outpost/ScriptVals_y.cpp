...
BOOL scrvCheckArrayIndex(SDWORD base, ARRAY_INDEXES* psIndexes, UDWORD* pIndex)
{
  ...
  {
    if ((psIndexes->elements[i] < 0) || (psIndexes->elements[i] >= psCurrScript->psArrayInfo[base].elements[i]))
    {
...
yyTypedRules yyRules[] = {
  {"&00: %06 &00", 0}, {"%06: %07", 0}, {"%06: %06 %07", 0}, {"%08:", 0}, {"%07: %01 &11 %08 &12 %09 &13", 0}, {"%10:", 0},
  {"%07: %01 &10 &05 %10 &12 %09 &13", 0}, {"%01: &09 &05", 3}, {"%09:", 0}, {"%09: %11", 0}, {"%09: %09 %11", 0}, {"%11: %03 &06 %02", 0},
  {"%04: &14 &03 &15", 7}, {"%05: %04", 7}, {"%05: %05 &14 &03 &15", 7}, {"%03: &07", 5}, {"%03: &08 %05", 5}, {"%02: &02", 6},
  {"%02: &03", 6}, {"%02: &05", 6}, {"$accept", 0}, {"error", 0}
};yyNamedType yyTokenTypes[] = {
  {"$end", 0, 0}, {"error", 256, 0}, {"BOOLEAN", 257, 1}, {"INTEGER", 258, 2}, {"IDENT", 259, 3}, {"QTEXT", 260, 3}, {"TYPE", 261, 4},
  {"VAR", 262, 5}, {"ARRAY", 263, 5}, {"SCRIPT", 264, 0}, {"STORE", 265, 0}, {"RUN", 266, 0}, {"'{'", 123, 0}, {"'}'", 125, 0},
  {"'['", 91, 0}, {"']'", 93, 0}
};
#endif
static short yydef[] = {65535, 65531, 65527, 3};
static short yyex[] = {0, 0, 65535, 1, 125, 21, 65535, 1, 125, 21, 65535, 1};
static short yyact[] = {
  65518, 264, 65508, 260, 65520, 65505, 266, 265, 65507, 260, 65534, 123, 65533, 123, 65513, 65523, 263, 262, 65527, 91, 65528, 261, 65506,
  65513, 65523, 263, 262, 125, 65502, 65513, 65523, 263, 262, 125, 65529, 91, 65530, 258, 65514, 65515, 65516, 260, 258, 257, 65531, 258,
  65510, 93, 65512, 93, -1
};
static short yypact[] = {1, 16, 16, 35, 49, 47, 45, 41, 37, 31, 25, 21, 19, 13, 11, 9, 6, 3, 1};
static short yygo[] = {
  65519, 65509, 65524, 65511, 65532, 65535, 65503, 65504, 0, 65521, 65526, 65525, 2, 65522, 65500, 65500, 65501, 10, 9, -1
};
static short yypgo[] = {0, 0, 0, 2, 1, 1, 1, 2, 4, 4, 3, 16, 0, 13, 7, 9, 5, 5, 7, 11, 11, 11, 0};
static short yyrlen[] = {0, 0, 0, 2, 1, 1, 1, 1, 4, 1, 3, 3, 2, 0, 6, 0, 1, 2, 7, 1, 2, 0, 2};
#define YYS0	18
#define YYDELTA	15
#define YYNPACT	19
#define YYNDEF	4
...
#if YYDEBUG
char* yysvar[] = {
  "$accept", "script_name", "var_value", "var_entry", "array_index", "array_index_list", "val_file", "script_entry", "$3", "var_init_list",
  "$5", "var_init", 0
};short yyrmap[] = {20, 21, 22, 16, 19, 18, 17, 15, 14, 13, 12, 11, 7, 5, 4, 3, 1, 2, 6, 9, 10, 8, 0};short yysmap[] = {
  4, 12, 13, 20, 33, 28, 27, 23, 22, 19, 18, 16, 14, 11, 10, 6, 2, 1, 0, 29, 30, 31, 15, 35, 21, 34, 32, 5, 9, 25, 7, 3, 8, 26, 17, 24
};int yyntoken = 16;int yynvar = 12;int yynstate = 36;int yynrule = 23;
#endif
...

{
  ...
  if (yys == (short*)0 || yyv == (YYSTYPE*)0
    ...
    scrv_error(m_textmsg(4967, "Not enough space for parser stacks", "E"));
    return 1;
  } save_yylval = scrv_lval; save_yyval = yyval; save_yypvt = yypvt; save_yychar = scrv_char; save_yyerrflag = yyerrflag; save_yynerrs =
    yynerrs;
#endif
...
yyStack: yyassert((unsigned)yystate < yynstate, m_textmsg(587, "state %d\n", ""), yystate);
  ...
  *++yytp = yyruletype; /* ... and type */
  ...
  if (scrv_debug) YY_TRACE(yyShowState)
#endif
...
yyEncore:
  ...
  else
  {
    ...
    if ((yyi = yydef[yystate]) < 0)
    {
      ...
      YYREAD;
#endif
      while ((yyi = *yyp) >= 0 && yyi != scrv_char) { yyp += 2; }
      yyi = yyp[1];
  ...
  yyj = yyrlen[yyi];
#if YYDEBUG
  if (scrv_debug) YY_TRACE(yyShowReduce) yytp -= yyj;
#endif
  yyps -= yyj; /* pop stacks */
  yypvt = yypv; /* save top */
  yypv -= yyj;
  ...
  case YYr4:
    {
      /* script_entry :  script_name RUN $3 '{' var_init_list '}' */

      if (!eventRunContext(psCurrContext, gameTime / SCR_TICKRATE)) { YYABORT; }
    }
    break;

  case YYr5:
    {
      /* script_entry :  script_name STORE QTEXT */
  ...
  case YYr7:
    {
      ...
      if (strncmp(&stringname[extpos], "blo", 3) == 0)
      {
        if (resPresent("BLO", stringname) == TRUE)
          psCurrScript = static_cast<SCRIPT_CODE*>(resGetData("BLO", stringname));
        else
        {
          // change extension to "slo"
          stringname[extpos] = 's';
          psCurrScript = static_cast<SCRIPT_CODE*>(resGetData("SCRIPT", stringname));
        }
      }
  ...
  case YYr11:
    {
      ...
      {
      case VAL_INT:
        if (yypvt[0].sInit.type != IT_INDEX || !eventSetContextVar(psCurrContext, yypvt[-2].vindex, yypvt[-1].tval,
                                                                   static_cast<UDWORD>(yypvt[0].sInit.index)))
        {
      ...
      case ST_DROID:
        ...
        if (!scrvGetBaseObj(static_cast<UDWORD>(yypvt[0].sInit.index), &psObj))
        {
          scrv_error("Droid id %d not found", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
        if (psObj->type != OBJ_DROID)
        {
          scrv_error("Object id %d is not a droid", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
      ...
      case ST_STRUCTURE:
        ...
        if (!scrvGetBaseObj(static_cast<UDWORD>(yypvt[0].sInit.index), &psObj))
        {
          scrv_error("Structure id %d not found", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
        if (psObj->type != OBJ_STRUCTURE)
        {
          scrv_error("Object id %d is not a structure", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
        if (!eventSetContextVar(psCurrContext, yypvt[-2].vindex, yypvt[-1].tval, (UDWORD)psObj))
        {
      ...
      case ST_FEATURE:
        ...
        if (!scrvGetBaseObj(static_cast<UDWORD>(yypvt[0].sInit.index), &psObj))
        {
          scrv_error("Feature id %d not found", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
        if (psObj->type != OBJ_FEATURE)
        {
          scrv_error("Object id %d is not a feature", static_cast<UDWORD>(yypvt[0].sInit.index));
          YYABORT;
        }
        if (!eventSetContextVar(psCurrContext, yypvt[-2].vindex, yypvt[-1].tval, (UDWORD)psObj))
        {
      ...
      case ST_FEATURESTAT:
        ...
        break;
      case VAL_BOOL:
        if (yypvt[0].sInit.type != IT_BOOL || !eventSetContextVar(psCurrContext, yypvt[-2].vindex, yypvt[-1].tval,
                                                                  static_cast<UDWORD>(yypvt[0].sInit.index)))
        {
          scrv_error("Typemismatch for variable %d", yypvt[-2].vindex);
          YYABORT;
      ...
      case ST_STRUCTUREID:
        ...
        if (!eventSetContextVar(psCurrContext, yypvt[-2].vindex, yypvt[-1].tval, static_cast<UDWORD>(yypvt[0].sInit.index)))
        {
          scrv_error("Set Value Failed for %s", yypvt[-2].vindex);
          YYABORT;
        }
  ...
  case YYr14:
    {
      /* array_index_list :  array_index_list '[' INTEGER ']' */
  ...
  case YYr16:
    {
      /* var_entry :  ARRAY array_index_list */

      UDWORD index;

      if (!scrvCheckArrayIndex(yypvt[-1].vindex, yypvt[0].arrayIndex, &index)) { YYABORT; }
...
#if YYDEBUG
  if (scrv_debug) YY_TRACE(yyShowGoto)
#endif
  goto yyStack;
...
yyError:
  ...
  case 3:
    ...
    if (scrv_char == 0) /* but not EOF */
      break;
#if YYDEBUG
    if (scrv_debug) YY_TRACE(yyShowErrDiscard)
#endif
    yyclearin;
...
#ifdef YYALLOC
yyReturn:
  scrv_lval = save_yylval; yyval = save_yyval; yypvt = save_yypvt; scrv_char = save_yychar; yyerrflag = save_yyerrflag; yynerrs =
