...
void initLighting(UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2)
{
  ...
  for (i = x1; i < x2; i++)
  {
    for (j = y1; j < y2; j++)
    {
      ...
      //	Basically darkens down the tiles that are outside the scroll
      //	limits - thereby emphasising the cannot-go-there-ness of them 
      if (static_cast<SDWORD>(i) < (scrollMinX + 4) OR static_cast<SDWORD>(i) > (scrollMaxX - 4) OR static_cast<SDWORD>(j) < (scrollMinY +
        4) OR static_cast<SDWORD>(j) > (scrollMaxY - 4))
...
void calcTileIllum(UDWORD tileX, UDWORD tileY)
{
  ...
  */
  ...
  if (val > 254)
    val = 254;
  mapTile(tileX, tileY)->illumination = static_cast<UBYTE>(val);
}
...
void normalsOnTile(UDWORD tileX, UDWORD tileY, UDWORD quadrant)
{
  ...
  case 3:
    ...
    break;
  default: ASSERT_TEXT(FALSE,"Invalid quadrant in lighting code");
  } // end switch
}
...
void processLight(LIGHT* psLight)
{
  ...
  rangeSkip *= 2;
  rangeSkip = static_cast<UDWORD>(sqrt(rangeSkip));
  ...
  if (startX < 0)
    startX = 0;
  else if (startX > static_cast<SDWORD>(mapWidth - 1))
    startX = mapWidth - 1;
  if (endX < 0)
    endX = 0;
  else if (endX > static_cast<SDWORD>(mapWidth - 1))
    endX = mapWidth - 1;

  /* Clip to grid limits */
  if (startY < 0)
    startY = 0;
  else if (startY > static_cast<SDWORD>(mapHeight - 1))
    startY = mapHeight - 1;
  if (endY < 0)
    endY = 0;
  else if (endY > static_cast<SDWORD>(mapHeight - 1))
    endY = mapHeight - 1;
  ...
  for (i = startX; i <= endX; i++)
  {
    for (j = startY; j <= endY; j++)
    {
      ...
      if (distToCorner < static_cast<SDWORD>(psLight->range))
      {
        ...
        if (xIndex >= 0 AND yIndex >= 0 AND xIndex < static_cast<SDWORD>(visibleXTiles) AND yIndex < static_cast<SDWORD>(visibleYTiles))
          colourTile(xIndex, yIndex, static_cast<LIGHT_COLOUR>(psLight->colour), static_cast<UBYTE>(2 * percent));
      }
    }
  }
}
...
UDWORD calcDistToTile(UDWORD tileX, UDWORD tileY, iVector* pos)
{
  ...
  return static_cast<UDWORD>(sqrt(total));
}
...
void colourTile(SDWORD xIndex, SDWORD yIndex, LIGHT_COLOUR colour, UBYTE percent)
{
  ...
  case LIGHT_WHITE:
    ...
    break;
  default: ASSERT_TEXT(FALSE,"Weirdy colour of light attempted");
    break;
  }
}
...
UDWORD lightDoFogAndIllumination(UBYTE brightness, SDWORD dx, SDWORD dz, UDWORD* pSpecular)
{
  ...
  distance = static_cast<SDWORD>(sqrt(dx * dx + dz * dz));
  ...
  if (player.p.x < penumbraRadius) //fog lefthand edge
  {
    ...
    {
      if (player.p.x - dx + penumbraRadius < 0) { edge = 0; }
      else if (player.p.x - dx + penumbraRadius > UMBRA_RADIUS) { edge = 255; }
      else { edge = (((player.p.x - dx + penumbraRadius) * 255) / (UMBRA_RADIUS)); }
    }
    else { edge = 255; }
  }
  else { edge = 255; }
...
*/
  if ((fogStatus & FOG_BACKGROUND) && (pie_GetFogEnabled()))
  {
    ...
    if (depth > static_cast<float>(-0.1))
    {
      fog = fog + (255 - umbra);
      if (fog > 255)
        fog = 255;
...
#ifdef EDGE_FOG
    fog = fog + (255 - edge); if (fog > 255) { fog = 255; } if (fog < 0) { fog = 0; }
#endif
  }
  else
    brightness = pie_ByteScale(brightness, static_cast<UBYTE>(umbra));
  ...
  else
  {
    ...
    specular.byte.a = static_cast<UBYTE>(fog);
    specular.byte.r = pie_ByteScale(static_cast<UBYTE>(fog), fogColour.byte.r);
    specular.byte.g = pie_ByteScale(static_cast<UBYTE>(fog), fogColour.byte.g);
    specular.byte.b = pie_ByteScale(static_cast<UBYTE>(fog), fogColour.byte.b);
    *pSpecular = specular.argb;

    //calculate new brightness
    colour = 256 - fog;
    brightness = pie_ByteScale(static_cast<UBYTE>(colour), brightness);
    lighting.byte.a = UBYTE_MAX;
    lighting.byte.r = brightness;
    lighting.byte.g = brightness;
    lighting.byte.b = brightness;
