...
static CDSPAN_CALLBACK g_fpOKCallback = nullptr;
static CDSPAN_CALLBACK g_fpCancelCallback = nullptr;

static BOOL g_bBoxVisible = FALSE;
static W_SCREEN* g_psCurWScreen = nullptr;
...
BOOL cdspan_GetDriveName(BOOL bCheckAny, STRING szDriveName[], ...)
{
  UINT uiRet;
  STRING szDrives[MAX_STR], szVolNameList[MAX_VOL_NAMES][MAX_STR], seps[] = " ,\t\n", *token = nullptr;
  SDWORD i, j, iLabels;
  BOOL bFound = FALSE;
  va_list vArgList;
  ...
  if (GetLogicalDriveStrings(MAX_STR, szDrives))
  {
    ...
    while (token != nullptr && bFound == FALSE)
    {
      ...
      if (uiRet == DRIVE_CDROM)
      {
        ...
        if (GetVolumeInformation(token, szVolName, MAX_STR, &dwVolSerialNum, &dwMaxlen, &dwFlags, szSysName, MAX_STR))
        {
          for (j = 0; j < iLabels; j++)
          {
...
BOOL cdspan_GetCDLetter(STRING szDriveName[], CD_INDEX index)
{
  switch (index)
  {
  case DISC_ONE:
  case DISC_TWO:
    return cdspan_GetDriveName(FALSE, szDriveName, cdspan_GetDiskLabel(index), "");
  case DISC_EITHER:
    return cdspan_GetDriveName(FALSE, szDriveName, cdspan_GetDiskLabel(DISC_ONE), cdspan_GetDiskLabel(DISC_TWO), "");
  case DISC_INVALID:
...
static BOOL cdspan_GetCDLabel(STRING* szLabel)
{
  char szVolName[MAX_STR], szSysName[MAX_STR];
  DWORD dwVolSerialNum, dwFlags, dwMaxlen;

  if (GetVolumeInformation(g_szCurDriveName, szVolName, MAX_STR, &dwVolSerialNum, &dwMaxlen, &dwFlags, szSysName, MAX_STR))
  {
...
static BOOL cdspan_CheckForCD(STRING szVolumeLabel[])
{
  STRING szVolName[MAX_STR];

  if (cdspan_GetCDLabel(szVolName) && stricmp(szVolumeLabel, szVolName) == 0)
    return TRUE;
  return FALSE;
...
STRING* cdspan_GetCDStrFromIndex(CD_INDEX index)
{
  if (index == DISC_ONE)
    return LABEL1;
  if (index == DISC_TWO)
...
CD_INDEX cdspan_GetCDIndexFromStr(STRING* pStr)
{
...
BOOL cdIsValid(CD_INDEX cdRequired)
{
  ...
  ASSERT_TEXT(cdRequired == DISC_ONE OR cdRequired == DISC_TWO, "Invalid CD name request for cdIsValid");
...
BOOL showChangeCDBox(W_SCREEN* psCurWScreen, CD_INDEX CDrequired, CDSPAN_CALLBACK fpOKCallback, CDSPAN_CALLBACK fpCancelCallback)
{
  ...
  sFormInit.x = static_cast<SWORD>(CDLEFT);
  sFormInit.y = static_cast<SWORD>(TOP);
  sFormInit.width = static_cast<UWORD>((CDSPAN_BOX_WIDTH));
  sFormInit.height = static_cast<UWORD>((CDSPAN_BOX_DEPTH));
  ...
  else { sprintf(szMessage, strresGetString(psStringRes, STR_CD_CHANGE), getCDRealNumberIndex(CDrequired)); }
...
BOOL cdspan_initialCDcheck()
{
  ...
  if (cdspan_CheckForCD(LABEL1) == TRUE || cdspan_CheckForCD(LABEL2) == TRUE)
    return TRUE;
  else
    return FALSE;
}
