...
static DROID* psLastDroid = nullptr;
...
void processDemoCam(void)
{
  ...
  else if (gameTime > (lastDroidSending + GAME_TICKS_PER_SEC))
  {
    ...
    if (!bSkipOrder)
    {
      ...
      while (apsDroidLists[firstPlayer] == nullptr)
      {
      ...
      while (apsStructLists[otherPlayer] == nullptr OR otherPlayer == firstPlayer)
      {
        /* Nope, so choose another one until we get one with droids */
        otherPlayer = rand() % MAX_PLAYERS;
      }
      ...
      {
        if ((orderState(psDroid, DORDER_NONE) == TRUE) OR ((orderState(psDroid, DORDER_GUARD) == TRUE) AND (psDroid->action ==
          DACTION_NONE)))
        {
          /* Make the droid attack the building - it'll indirectly route there too */
          orderDroidLoc(psDroid, DORDER_SCOUT, apsStructLists[otherPlayer]->x, apsStructLists[otherPlayer]->y);
        }
      }
    }
  }
...
// -------------------------------------------------------------------------
void setFindNewTarget(void) { lastCameraMove = 0; }
// -------------------------------------------------------------------------
/* Flips on/off */
void toggleDemoStatus(void)
{
...
void findSomethingInteresting(void)
{
  ...
  case TAR_SEEK:
    ...
    /* Have they got any droids? */
    while (apsDroidLists[player] == nullptr)
    {
      /* Nope, so choose another one until we get one with droids */
      player = rand() % MAX_PLAYERS - 2;
    }
    ...
    /* Have they got any structures? Make sure it's not their own we're checking! */
    while (apsStructLists[otherPlayer] == nullptr OR otherPlayer == player)
    {
      /* Nope, so choose another one until we get one with droids */
      otherPlayer = rand() % MAX_PLAYERS;
    }
    ...
    if (psDroid AND apsStructLists[otherPlayer])
    {
      ...
      orderDroidLoc(psDroid, DORDER_MOVE, apsStructLists[otherPlayer]->x, apsStructLists[otherPlayer]->y);
  ...
  for (i = 0; i < mapWidth; i++)
  {
    for (j = 0; j < mapHeight; j++)
    {
      ...
      if (TILE_HAS_STRUCTURE(psTile))
      {
        ...
        ASSERT((psStructure != NULL, "demoProcessTilesIn: no structure at tile %i %i\n", i, j));
        if (psStructure)
        {
...
BOOL tooNearEdge(UDWORD x, UDWORD y)
{
  if ((x > ((visibleXTiles / 2) * TILE_UNITS)) AND (x < ((mapWidth - (visibleXTiles / 2)) * TILE_UNITS)) AND (y > ((visibleYTiles / 2) *
    TILE_UNITS)) AND (y < ((mapHeight - (visibleYTiles / 2)) * TILE_UNITS)))
    return (FALSE);
  return (TRUE);
}
