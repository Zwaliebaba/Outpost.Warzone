...
void intUpdateProgressBar(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  psObj = static_cast<BASE_OBJECT*>(BarGraph->pUserData); // Get the object associated with this widget.
  ...
  case OBJ_STRUCTURE: // If it's a structure and...
    ...
    else if (StructureIsResearching(Structure))
    {
      ...
      pPlayerRes = asPlayerResList[selectedPlayer] + ((RESEARCH*)Research->psSubject - asResearch);
  ...
  default: DEBUG_ASSERT_TEXT(FALSE, "intUpdateProgressBar: invalid object type");
  }
}
...
void intUpdateQuantity(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  psObj = static_cast<BASE_OBJECT*>(Label->pUserData); // Get the object associated with this widget.
  Structure = (STRUCTURE*)psObj;

  if ((psObj != nullptr) && (psObj->type == OBJ_STRUCTURE) && (StructureIsManufacturing(Structure)))
  {
    DEBUG_ASSERT_TEXT(!psObj->died, "intUpdateQuantity: object is dead");
...
void intAddProdQuantity(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  if (psStat != nullptr)
  {
    ...
    if (psObj != nullptr AND psObj->type == OBJ_STRUCTURE)
      psStructure = (STRUCTURE*)psObj;

    if (psStructure != nullptr AND StructIsFactory(psStructure))
...
void intAddLoopQuantity(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  {
    psFactory = (FACTORY*)static_cast<STRUCTURE*>(Label->pUserData)->pFunctionality;
...
void intUpdateCommandSize(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  if (psObj != nullptr)
  {
    ...
    DEBUG_ASSERT_TEXT(psDroid->droidType == DROID_COMMAND, "intUpdateCommandSize: droid is not a command droid");
...
void intDisplayStatusButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  Object = nullptr;
  Image = -1;
  psObj = static_cast<BASE_OBJECT*>(Buffer->Data); // Get the object associated with this widget.
  ...
  if (psObj)
  {
    ...
    case OBJ_DROID: // If it's a droid...
      ...
      else if (DroidGoingToBuild(Droid))
      {
        ...
        DEBUG_ASSERT_TEXT(Stats!=NULL, "intDisplayStatusButton : NULL Stats pointer.");
          ...
          if (Stats)
          {
            ...
            if (psResGraphic)
            {
              ...
              else
              {
                ...
                {
                  DEBUG_ASSERT_TEXT(FALSE, "intDisplayStatsButton:Invalid Stat for research button");
                  Object = nullptr;
                  IMDType = IMDTYPE_RESEARCH;
                }
              }
    ...
    default: DEBUG_ASSERT_TEXT(FALSE, "intDisplayObjectButton: invalid structure type");
    }
  }
  else
    RENDERBUTTON_INITIALISED(Buffer);
...
void intDisplayObjectButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  if (psObj)
  {
    ...
    default: DEBUG_ASSERT_TEXT(FALSE, "intDisplayStatusButton: invalid structure type");
    }
  }
...
void intDisplayStatsButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  Stat = static_cast<BASE_STATS*>(Buffer->Data);
...
void intDisplayButtonFlash(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  DEBUG_ASSERT_TEXT(psWidget->type == WIDG_BUTTON, "intDisplayButtonFlash : Not a button");
...
void intDisplayReticuleButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  DEBUG_ASSERT_TEXT(psWidget->type == WIDG_BUTTON, "intDisplayReticuleButton : Not a button");
...
void intDisplayDPButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  if (psStruct)
  {
    DEBUG_ASSERT_TEXT(StructIsFactory(psStruct), "intDisplayDPButton: structure is not a factory");
...
void intDisplayNumber(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  if (Label->pUserData != nullptr)
  {
    psStruct = static_cast<STRUCTURE*>(Label->pUserData);
                     ...
                     UDWORD buttonType)
{
  ...
  if ((IMDType == IMDTYPE_DROID) || (IMDType == IMDTYPE_DROIDTEMPLATE))
  {
    ...
    DEBUG_ASSERT_TEXT(Radius <= 128, "create PIE button big component found");
...
BOOL DroidIsDemolishing(DROID* Droid)
{
  ...
  if (!(droidType(Droid) == DROID_CONSTRUCT OR droidType(Droid) == DROID_CYBORG_CONSTRUCT))
    return FALSE;
...
BOOL DroidIsRepairing(DROID* Droid)
{
  ...
  if (!(droidType(Droid) == DROID_REPAIR OR droidType(Droid) == DROID_CYBORG_REPAIR))
    return FALSE;
...
{
  return ((Structure->pStructureType->type == REF_FACTORY OR Structure->pStructureType->type == REF_CYBORG_FACTORY OR Structure->
...
iIMDShape* StructureGetIMD(STRUCTURE* Structure) { return Structure->pStructureType->pIMD; }
...
BOOL StatGetComponentIMD(BASE_STATS* Stat, SDWORD compID, iIMDShape** CompIMD, iIMDShape** MountIMD)
{
  ...
  *MountIMD = nullptr;
  ...
  default:
    ...
    DEBUG_ASSERT_TEXT(FALSE, "StatGetComponent : Unknown component");
  }
...
void drawRadarBlips()
{
  ...
  for (psProxDisp = apsProxDisp[selectedPlayer]; psProxDisp != nullptr; psProxDisp = psProxDisp->psNext)
  {
    ...
    if (psProxDisp->radarX > 0 AND psProxDisp->radarX < VisWidth AND psProxDisp->radarY > 0 AND psProxDisp->radarY < VisHeight)
    {
...
void intDisplayProximityBlips(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  DEBUG_ASSERT_TEXT(psMsg->type == MSG_PROXIMITY, "Invalid message type");
