...
// -------------------------------------------------------------------------------
using ENVIRON_DATA = struct environ_data
{
  UBYTE bProcess;
  UBYTE type;
  FRACT val;
...
// -------------------------------------------------------------------------------
ENVIRON_DATA* pEnvironData = nullptr;
...
// -------------------------------------------------------------------------------
BOOL waterOnMap(void) { return (bWaterOnMap); }
// -------------------------------------------------------------------------------
...
void environReset(void)
{
  ...
  if (pEnvironData == nullptr) // loading map preview..
    return;
  ...
  */
  ...
  if (psMapTiles[tileX + tileYOffset].texture & TILE_TRIFLIP)
  {
    if ((ox + oy) > TILE_UNITS) //tile split top right to bottom left object if in bottom right half
    {
      ...
      retVal = static_cast<UDWORD>(hxy + dx + dy);
      return (retVal * 4);
    }
    ...
    retVal = static_cast<UDWORD>(h0 + dx + dy);
    return (retVal * 4);
  }
