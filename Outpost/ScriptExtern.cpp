...
BOOL scrGenExternGet(UDWORD index)
{
  ...
  case EXTID_GAMETIME:
    type = VAL_INT;
    val = static_cast<SDWORD>(gameTime / SCR_TICKRATE);
    break;
  case EXTID_TUTORIAL:
    type = VAL_BOOL;
    val = bInTutorial;
  ...
  case EXTID_MULTIGAMEBASETYPE:
    ...
    val = game.base;
    break;
  default: ASSERT((FALSE, "scrGenExternGet: unknown variable index"));
    return FALSE;
    break;
  }
...
BOOL scrGenExternSet(UDWORD index)
{
  ...
  case EXTID_EXTRAFAILFLAG:
    ...
    bExtraFailFlag = val;
    break;
  default: ASSERT((FALSE, "scrGenExternSet: unknown variable index"));
    return FALSE;
    break;
  }
