...
BOOL sendBuildStarted(STRUCTURE* psStruct, DROID* psDroid)
{
  NETMSG msg;
  UDWORD zero = 0;
  NetAddType(msg, 0, UBYTE, psDroid->player); //player
  ...
  NetAddType(msg, 19, UBYTE, psDroid->order); // building id to create
...
BOOL recvBuildStarted(NETMSG* pMsg)
{
  ...
  for (typeindex = 0; // find structure target
       (typeindex < numStructureStats) && (asStructureStats[typeindex].ref != structStat); typeindex++);
...
BOOL SendBuildFinished(STRUCTURE* psStruct)
{
  ...
  m.body[14] = static_cast<char>(psStruct->player); // player
...
BOOL recvBuildFinished(NETMSG* m)
{
  ...
  for (typeindex = 0; // find structure target
       (typeindex < numStructureStats) && (asStructureStats[typeindex].ref != type); typeindex++);
  psStr = nullptr;
...
BOOL recvDemolishFinished(NETMSG* m)
{
  ...
  if (psStruct)
  {
    ...
    if (psDroid && psDroid->psTarStats)
      psDroid->psTarStats = nullptr; // update droid if reqd.
  }
  return TRUE;
}
...
BOOL sendLasSat(UBYTE player, STRUCTURE* psStruct, BASE_OBJECT* psObj)
{
  ...
  NetAddType(msg, 9, UBYTE, psObj->player);
