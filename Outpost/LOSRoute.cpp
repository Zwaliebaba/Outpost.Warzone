...
using FP_POINT = struct _fp_point
{
  SDWORD x, y;
  SDWORD dist;
};
...
using TLOS_STATE = enum _tlos_state
{
  TLOS_NOOBST,
  // no obstruction
  TLOS_NEARSIDE,
  // found a clear tile on the near side of obstruction
  TLOS_FARSIDE,
  // found a clear tile on the far side of obstruction
  TLOS_REPEAT,
  // found a second obstruction
...
using FP_HUGPOINT = struct _fp_hugpoint
{
  SDWORD x, y;
...
using STACK_TYPE = enum _stack_type
{
  ST_START,
  // first entry in a route
  ST_INTERMEDIATE,
  // in the middle of a route
  ST_END,
  // final entry in a route

  ST_FAILED,
  // route failed this way
};

// store the current position in the search
using FP_STACK = struct _fp_stack
{
...
using ROUTE_STATE = enum _route_state
{
  RS_SEARCHING,
  // looking for a route
  RS_SUCCESS,
...
// dir 0 => x = 0, y = -1
static POINT aDirOffset[NUM_DIR] = {0, 1, -1, 0, 0, -1, 1, 0,};
...
void fpathPointsToHugDir(SDWORD clearx, SDWORD cleary, SDWORD obstx, SDWORD obsty, SDWORD* pLeftDir, SDWORD* pRightDir)
{
  SDWORD xdiff, ydiff;
  SDWORD leftDir, rightDir;
  ...
  ASSERT_TEXT((xdiff >= -1 && xdiff <= 1 && ydiff >= -1 && ydiff <= 1), "fpathPointsToHugDir: points are more than one tile apart");
  ASSERT_TEXT(xdiff != 0 || ydiff != 0, "fpathPointsToHugDir: points are on same tile");
  ...
  case 11: // xdiff == 1, ydiff == 1
    ...
    rightDir = 0;
    break;
  default: ASSERT_TEXT(FALSE, "fpathPointsToHugDir: unexpected point relationship");
    leftDir = 0;
    rightDir = 2;
    break;
  }
  ...
  case TLOS_NOOBST:
    if (fpathBlockingTile(x >> TILE_SHIFT, y >> TILE_SHIFT))
    {
      ...
      if (nearPoints < FPATH_HUGPOINTMAX)
      {
        ...
        fpathPointsToHugDir(sCurrBlock.x, sCurrBlock.y, x, y, &leftHugDir, &rightHugDir);
        ...
        // DEBUG
#ifdef TEST_BED
        psTile = mapTile(sCurrBlock.x >> TILE_SHIFT, sCurrBlock.y >> TILE_SHIFT); psTile->tileInfoBits |= START;
#endif
      }
      else
      {
  ...
  case TLOS_NEARSIDE:
    if (!fpathBlockingTile(x >> TILE_SHIFT, y >> TILE_SHIFT))
    {
      ...
      if (farPoints < FPATH_HUGPOINTMAX)
      {
        ...
        // decide which direction the obstruction was hit for wall hugging
        fpathPointsToHugDir(x, y, sCurrBlock.x, sCurrBlock.y, &leftHugDir, &rightHugDir);
        ...
        // DEBUG
#ifdef TEST_BED
        psTile = mapTile(x >> TILE_SHIFT, y >> TILE_SHIFT); psTile->tileInfoBits |= FINISH;
#endif
      }
      else
      {
    ...
    break;
  default: ASSERT_TEXT(FALSE,"fpathObstructionCallback: unknown state");
    break;
  }
...
BOOL fpathTileLOSCallback(SDWORD x, SDWORD y, SDWORD dist)
{
  ...
  case TLOS_FARSIDE:
    ...
    break;
  default: ASSERT_TEXT(FALSE,"fpathTileLOSCallback: unknown state");
    break;
  }
...
BOOL fpathNextHugDir(SDWORD x, SDWORD y, SDWORD* pDir, SDWORD hugDir)
{
  ...
  for (i = 0; i < NUM_DIR; i++)
  {
    if (!fpathBlockingTile(x + aDirOffset[nextDir].x, y + aDirOffset[nextDir].y))
    {
      *pDir = nextDir;
      blocked = FALSE;
      break;
                         ...
                         FP_POINT* asPoints, SDWORD* pNumPoints) // wall hug points
{
  ...
  while (!hugFinished)
  {
    ...
    ASSERT_TEXT(!fpathBlockingTile(x,y), "fpathWallHug: wall hugged onto a blocking tile");

    // see if the route has got to a finish point
    for (i = 0; i < farPoints; i++)
    {
      if (x == asFarPoints[i].x >> TILE_SHIFT && y == asFarPoints[i].y >> TILE_SHIFT)
      {
        testDir = dir;
        if (hugDir == LEFT_TURN && fpathNextHugDir(x, y, &testDir, hugDir) && testDir == asFarPoints[i].rightDir)
        {
          *pFarIndex = i;
          hugFinished = TRUE;
          break;
        }
        if (hugDir == RIGHT_TURN && fpathNextHugDir(x, y, &testDir, hugDir) && testDir == asFarPoints[i].leftDir)
        {
          *pFarIndex = i;
          hugFinished = TRUE;
          break;
    ...
    {
      if (x == asNearPoints[i].x >> TILE_SHIFT && y == asNearPoints[i].y >> TILE_SHIFT)
      {
        testDir = dir;
        if (hugDir == LEFT_TURN && fpathNextHugDir(x, y, &testDir, hugDir) && testDir == asNearPoints[i].rightDir)
        {
          *pFarIndex = i - 1;
          hugFinished = TRUE;
          break;
        }
        if (hugDir == RIGHT_TURN && fpathNextHugDir(x, y, &testDir, hugDir) && testDir == asNearPoints[i].leftDir)
        {
          *pFarIndex = i - 1;
          hugFinished = TRUE;
          break;
...
SDWORD fpathHugDistance(SDWORD sx, SDWORD sy, SDWORD fx, SDWORD fy, FP_POINT* asPoints, SDWORD numPoints)
{
  ...
  ASSERT_TEXT(numPoints != 0, "fpathHugDistance: no points to optimise");
  ...
  while (search >= 0)
  {
    ...
    do
    {
      ...
      rayCast(raySX, raySY, ray, RAY_MAXLEN, fpathTileVisCallback);
      ...
      rayCast(rayFX, rayFY, ray, RAY_MAXLEN, fpathTileVisCallback);
    ...
    {
#ifdef DEBUG
      ASSERT_TEXT(lastIndex != search, "fpathHugDist: LOS failed for neighbouring tile");
      lastIndex = search;
#endif
...
// Optimise a route
void fpathOptimiseRoute(SDWORD sx, SDWORD sy, SDWORD fx, SDWORD fy, FP_POINT* asPoints, SDWORD* pNumPoints, SDWORD* pDist)
{
  SDWORD currIndex, search;
  SDWORD ray;
  SDWORD raySX, raySY, rayFX, rayFY; // ray start and end coords
  ...
  ASSERT_TEXT(*pNumPoints != 0, "fpathOptimiseRoute: no points to optimise");
  ...
  while (search >= 0)
  {
    ...
    {
#ifdef DEBUG
      ASSERT_TEXT(lastIndex != search, "fpathOptimiseRoute: LOS failed for neighbouring tile");
      lastIndex = search;
#endif
...
#ifdef TEST_BED
      mapTile(asPoints[currIndex].x >> TILE_SHIFT, asPoints[currIndex].y >> TILE_SHIFT)->tileInfoBits |= ROUTE;
#endif
    }
  }
  ...
  *pDist = 0;
  ...
  {
    *pDist += fpathCalcDistance(cx, cy, asPoints[point].x, asPoints[point].y);
    cx = asPoints[point].x;
    cy = asPoints[point].y;
  }
...
BOOL fpathBuildRouteRec(SDWORD index)
{
  ...
  if (success)
  {
    ...
    if (asRouteStack[index].dir & ST_HUGLEFT)
    {
      ...
      success = fpathWallHug(asNearPoints[nearIndex].x >> TILE_SHIFT, asNearPoints[nearIndex].y >> TILE_SHIFT, &hugIndex,
                             asNearPoints[nearIndex].leftDir, RIGHT_TURN, asLeftHug, &hugPoints);
    ...
    else if (asRouteStack[index].dir & ST_HUGRIGHT)
    {
      ...
      success = fpathWallHug(asNearPoints[nearIndex].x >> TILE_SHIFT, asNearPoints[nearIndex].y >> TILE_SHIFT, &hugIndex,
                             asNearPoints[nearIndex].rightDir, LEFT_TURN, asRightHug, &hugPoints);
...
BOOL fpathRouteValid(SDWORD index)
{
  ...
  while (index > 0)
  {
    ...
    for (search = asRouteStack[index].prev; search >= 0; search = asRouteStack[search].prev)
    {
...
BOOL fpathTileRoute(MOVE_CONTROL* psMoveCntl, SDWORD sx, SDWORD sy, SDWORD fx, SDWORD fy)
{
  ...
  nearPoints = 0;
  farPoints = 0;
  rayCast(sx, sy, ray, RAY_MAXLEN, fpathObstructionCallback);
  ...
  {
    // wall hug to the left
    leftSuccess = fpathWallHug(asNearPoints[nearIndex].x >> TILE_SHIFT, asNearPoints[nearIndex].y >> TILE_SHIFT, &leftIndex,
                               asNearPoints[nearIndex].leftDir, RIGHT_TURN, asLeftHug, &leftPoints);
    // wall hug to the right
    rightSuccess = fpathWallHug(asNearPoints[nearIndex].x >> TILE_SHIFT, asNearPoints[nearIndex].y >> TILE_SHIFT, &rightIndex,
                                asNearPoints[nearIndex].rightDir, LEFT_TURN, asRightHug, &rightPoints);
    ...
    if (leftSuccess)
    {
      ...
      do
      {
        ...
        fpathOptimiseRoute(sx, sy, optFX, optFY, asRoutePoints, &pathPoints, &leftDist);
      }
      ...
      if (stackTop < FPATH_STACKMAX)
      {
        ...
        asRouteStack[stackTop].dir = static_cast<UBYTE>(asNearPoints[nearIndex].leftDir & ST_DIRMASK);
        asRouteStack[stackTop].dir |= static_cast<UBYTE>(ST_HUGLEFT);
        asRouteStack[stackTop].start = static_cast<SWORD>(nearIndex);
        asRouteStack[stackTop].end = static_cast<SWORD>(leftIndex);
        asRouteStack[stackTop].dist = leftDist; // + routeDist;
        asRouteStack[stackTop].prev = static_cast<SWORD>(stackCurr);
    ...
    if (rightSuccess)
    {
      ...
      do
      {
        ...
        fpathOptimiseRoute(sx, sy, optFX, optFY, asRightHug, &rightPoints, &rightDist);
      }
      ...
      if (stackTop < FPATH_STACKMAX)
      {
        // store the new route end
        asRouteStack[stackTop].type = ST_END;
        asRouteStack[stackTop].dir = static_cast<UBYTE>(asNearPoints[nearIndex].rightDir & ST_DIRMASK);
        asRouteStack[stackTop].dir |= static_cast<UBYTE>(ST_HUGRIGHT);
        asRouteStack[stackTop].start = static_cast<SWORD>(nearIndex);
        asRouteStack[stackTop].end = static_cast<SWORD>(rightIndex);
        asRouteStack[stackTop].dist = static_cast<SWORD>(rightDist); // + routeDist;
        asRouteStack[stackTop].prev = static_cast<SWORD>(stackCurr);
    ...
    if (routeState == RS_SEARCHING)
    {
      ...
      {
        if (asRouteStack[i].type == ST_END && asRouteStack[i].dist < stackMinDist)
        {
  ...
  if (routeState == RS_SUCCESS)
  {
    ...
    {
      prevPoints = pathPoints;
      fpathOptimiseRoute(sx, sy, fx, fy, asRoutePoints, &pathPoints, &finalDist);
    }
    while (pathPoints != 0 && prevPoints != pathPoints);
    ...
    for (i = 0; i < pathPoints; i++)
    {
      psMoveCntl->asPath[i].x = static_cast<UBYTE>(asRoutePoints[i].x);
      psMoveCntl->asPath[i].y = static_cast<UBYTE>(asRoutePoints[i].y);
    }
    psMoveCntl->numPoints = static_cast<UBYTE>(pathPoints);
  }
  else
