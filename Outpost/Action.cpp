...
BOOL actionVTOLLandingPos(DROID* psDroid, UDWORD* _px, UDWORD* _py);

/* Check if a target is at correct range to attack */
BOOL actionInAttackRange(DROID* _psDroid, const BASE_OBJECT* _psObj)
{
  SDWORD rangeSq, longRange, state;

  if (_psDroid->asWeaps[0].nStat == 0)
    return FALSE;

  WEAPON_STATS* psStats = asWeaponStats + _psDroid->asWeaps[0].nStat;

  SDWORD dx = static_cast<SDWORD>(_psDroid->x) - static_cast<SDWORD>(_psObj->x);
  SDWORD dy = static_cast<SDWORD>(_psDroid->y) - static_cast<SDWORD>(_psObj->y);
  SDWORD dz = static_cast<SDWORD>(_psDroid->z) - static_cast<SDWORD>(_psObj->z);

  SDWORD radSq = dx * dx + dy * dy;

  if ((_psDroid->order == DORDER_ATTACKTARGET) && secondaryGetState(_psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_HOLD))
  {
  ...
  {
    switch (_psDroid->secondaryOrder & DSS_ARANGE_MASK)
    {
    case DSS_ARANGE_DEFAULT:
      if (weaponShortHit(psStats, _psDroid->player) > weaponLongHit(psStats, _psDroid->player))
...
BOOL actionInRange(const DROID* _psDroid, const BASE_OBJECT* _psObj)
{
  if (_psDroid->asWeaps[0].nStat == 0)
    return FALSE;
  ...
  SDWORD longRange = proj_GetLongRange(psStats, dz);
  SDWORD rangeSq = longRange * longRange;
...
BOOL actionInsideMinRange(const DROID* _psDroid, const BASE_OBJECT* _psObj)
{
  if (_psDroid->asWeaps[0].nStat == 0)
    return FALSE;
  ...
  SDWORD minRange = static_cast<SDWORD>(psStats->minRange);
  SDWORD rangeSq = minRange * minRange;
                        ...
                        BOOL bInvert)
{
  ...
  if (psWeapStats != nullptr && (proj_Direct(psWeapStats) || ((psAttacker->type == OBJ_DROID) && !proj_Direct(psWeapStats) &&
    actionInsideMinRange(psDroid, psDroid->psActionTarget))))
  {
    ...
    SDWORD dx = psTarget->x - psAttacker->x; //muzzle.x;
    SDWORD dy = psTarget->y - psAttacker->y; //muzzle.y;
...
{
  if ((psDroid->asWeaps[0].nStat == 0) || vtolDroid(psDroid))
    return visibleObject((BASE_OBJECT*)psDroid, psTarget);

  WEAPON_STATS* psStats = asWeaponStats + psDroid->asWeaps[0].nStat;
...
void actionAddVtolAttackRun(DROID* psDroid)
{
  ...
  else if (psDroid->psTarget != nullptr)
    psTarget = psDroid->psTarget;
  else
    return;

  /* get normal vector from droid to target */
  SDWORD iVNx = psTarget->x - psDroid->x;
  ...
  SDWORD iX = psTarget->x + (iVNx * VTOL_ATTACK_LENGTH / iA);
  SDWORD iY = psTarget->y + (iVNy * VTOL_ATTACK_LENGTH / iA);

  if (iX <= 0 || iY <= 0 || iX > static_cast<SDWORD>(GetWidthOfMap() << TILE_SHIFT) || iY > static_cast<SDWORD>(GetHeightOfMap() <<
    TILE_SHIFT))
    DebugTrace("*** actionAddVtolAttackRun: run off map! ***\n");
  else
...
void actionUpdateTransporter(DROID* psDroid)
{
  ...
  if (psDroid->psActionTarget != nullptr AND psDroid->player == psDroid->psActionTarget->player)
    ...
    if (psDroid->psActionTarget != nullptr)
    {
      if (visibleObject((BASE_OBJECT*)psDroid, psDroid->psActionTarget))
      {
        ...
        if (actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget, &(psDroid->turretRotation), &(psDroid->turretPitch),
                               &asWeaponStats[psDroid->asWeaps->nStat], TRUE))
        {
          // In range - fire !!!
...
void actionCalcPullBackPoint(BASE_OBJECT* psObj, BASE_OBJECT* psTarget, SDWORD* px, SDWORD* py)
{
  ...
  SDWORD xdiff = static_cast<SDWORD>(psObj->x) - static_cast<SDWORD>(psTarget->x);
  SDWORD ydiff = static_cast<SDWORD>(psObj->y) - static_cast<SDWORD>(psTarget->y);
  SDWORD len = static_cast<SDWORD>(static_cast<float>(std::sqrt(xdiff * xdiff + ydiff * ydiff)));
  ...
  // create the position
  *px = static_cast<SDWORD>(psObj->x) + xdiff * PULL_BACK_DIST;
  *py = static_cast<SDWORD>(psObj->y) + ydiff * PULL_BACK_DIST;
}
...
BOOL actionReachedBuildPos(DROID* psDroid, SDWORD x, SDWORD y, BASE_STATS* psStats)
{
  ...
  SDWORD tx = x >> (TILE_SHIFT);
  SDWORD ty = y >> (TILE_SHIFT);
...
BOOL actionDroidOnBuildPos(DROID* psDroid, SDWORD x, SDWORD y, BASE_STATS* psStats)
{
  ...
  SDWORD tx = (x >> (TILE_SHIFT)) - (width / 2);
  SDWORD ty = (y >> (TILE_SHIFT)) - (breadth / 2);
...
void actionHomeBasePos(SDWORD player, SDWORD* px, SDWORD* py)
{
  ...
  for (STRUCTURE* psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
  {
    ...
    {
      *px = static_cast<SDWORD>(psStruct->x);
      *py = static_cast<SDWORD>(psStruct->y);
      return;
    }
  }
...
BOOL actionRouteBlockingPos(DROID* psDroid, SDWORD tx, SDWORD ty)
{
  ...
  if (vtolDroid(psDroid) || ((psDroid->order != DORDER_MOVE) && (psDroid->order != DORDER_SCOUT)))
    return FALSE;

  // see if there is a wall to attack around the location
  STRUCTURE* psWall = nullptr;
  for (SDWORD i = tx - 1; i <= tx + 1; i++)
  {
    for (SDWORD j = ty - 1; j <= ty + 1; j++)
    {
      if (tileOnMap(i, j))
      {
        psTile = mapTile(i, j);
        ...
        {
          psWall = getTileStructure(static_cast<UDWORD>(i), static_cast<UDWORD>(j));
...
void actionUpdateDroid(DROID* psDroid)
{
  ...
  if (!cyborgDroid(psDroid) && psPropStats->propulsionType == LIFT)
    bInvert = TRUE;
  else
    bInvert = FALSE;
  ...
  {
    psDroid->psActionTarget = nullptr;
    if ((psDroid->action != DACTION_MOVEFIRE) && (psDroid->action != DACTION_TRANSPORTIN) && (psDroid->action != DACTION_TRANSPORTOUT))
    {
      psDroid->action = DACTION_NONE;
  ...
  case DACTION_WAITDURINGREPAIR:
    ...
    if (DROID_STOPPED(psDroid) && !actionReachedBuildPos(psDroid, psDroid->psTarget->x, psDroid->psTarget->y,
  ...
  case DACTION_TRANSPORTWAITTOFLYIN:
    ...
    if (psDroid->player == selectedPlayer AND getDroidsToSafetyFlag())
    {
      if (static_cast<SDWORD>(mission.ETA - (gameTime - missionGetReinforcementTime())) <= 0)
      {
        ...
        psDroid->x = static_cast<UWORD>(droidX);
        psDroid->y = static_cast<UWORD>(droidY);
        //fly Transporter back to get some more droids
        orderDroidLoc(psDroid, DORDER_TRANSPORTIN, getLandingX(selectedPlayer), getLandingY(selectedPlayer));
      }
      else
      {
  ...
  case DACTION_MOVE:
    ...
    else if (!vtolDroid(psDroid) && psDroid->asWeaps[0].nStat > 0 && psWeapStats->rotate && psWeapStats->fireOnMove != FOM_NO &&
      aiNearestTarget(psDroid, &psDroid->psActionTarget))
    {
  ...
  case DACTION_MOVEFIRE:
    ...
    else if ((psDroid->psActionTarget == nullptr) || !validTarget((BASE_OBJECT*)psDroid, psDroid->psActionTarget) || (
    ...
    //check the target hasn't become one the same player ID - Electronic Warfare
    else if (electronicDroid(psDroid) && (psDroid->player == psDroid->psActionTarget->player))
    {
      psDroid->psActionTarget = nullptr;
      psDroid->action = DACTION_NONE;
    }
  ...
  case DACTION_ATTACK: DEBUG_ASSERT_TEXT(psDroid->psActionTarget != NULL, "actionUpdateUnit: target is NULL while attacking");
    ...
    //check the target hasn't become one the same player ID - Electronic Warfare
    if ((electronicDroid(psDroid) && (psDroid->player == psDroid->psActionTarget->player)) || !validTarget(
      (BASE_OBJECT*)psDroid, psDroid->psActionTarget)) // ||
    //			(secondaryGetState(psDroid, DSO_ATTACK_LEVEL, &state) && (state != DSS_ALEV_ALWAYS)))
    {
      psDroid->psActionTarget = nullptr;
      psDroid->action = DACTION_NONE;
    }
    else if (actionVisibleTarget(psDroid, psDroid->psActionTarget) && actionInAttackRange(psDroid, psDroid->psActionTarget))
    {
      ...
      if (dirDiff > FIXED_TURRET_DIR)
      {
        ...
        /*actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget,
                  ...
                  proj_Direct(&asWeaponStats[psDroid->asWeaps->nStat]),
                  bInvert))*/
        actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget, &(psDroid->turretRotation), &(psDroid->turretPitch),
                           &asWeaponStats[psDroid->asWeaps->nStat], bInvert))
      {
        /* In range - fire !!! */
        combFire(psDroid->asWeaps, (BASE_OBJECT*)psDroid, psDroid->psActionTarget);
    ...
    else
    {
      if (((psDroid->order == DORDER_ATTACKTARGET || psDroid->order == DORDER_FIRESUPPORT) &&
        secondaryGetState(psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_HOLD)) || (!vtolDroid(psDroid) &&
  ...
  case DACTION_VTOLATTACK:
    //check if vtol that its armed
    if ((vtolEmpty(psDroid)) || (psDroid->psActionTarget == nullptr) ||
      //check the target hasn't become one the same player ID - Electronic Warfare
      (electronicDroid(psDroid) && (psDroid->player == psDroid->psActionTarget->player)) || !validTarget(
        (BASE_OBJECT*)psDroid, psDroid->psActionTarget)) // ||
    //			 (secondaryGetState(psDroid, DSO_ATTACK_LEVEL, &state) && (state != DSS_ALEV_ALWAYS)))
    {
      moveToRearm(psDroid);
    ...
    else
    {
      ...
      xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psDroid->psActionTarget->x);
      ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psDroid->psActionTarget->y);
      ...
      if (rangeSq < (VTOL_ATTACK_TARDIST * VTOL_ATTACK_TARDIST))
      {
        ...
        xdiff = psDroid->sMove.DestinationX - static_cast<SDWORD>(psDroid->psActionTarget->x);
        ydiff = psDroid->sMove.DestinationY - static_cast<SDWORD>(psDroid->psActionTarget->y);
      ...
      else if (rangeSq > static_cast<SDWORD>(psWeapStats->longRange * psWeapStats->longRange))
      {
  ...
  case DACTION_MOVETOATTACK:
    ...
    if (vtolDroid(psDroid) && vtolEmpty(psDroid))
    {
      moveToRearm(psDroid);
      break;
    }
    ...
    else
    {
      if (actionVisibleTarget(psDroid, psDroid->psActionTarget))
      {
        ...
        bChaseBloke = FALSE;
        if (!vtolDroid(psDroid) && psDroid->psActionTarget->type == OBJ_DROID && ((DROID*)psDroid->psActionTarget)->droidType ==
          DROID_PERSON && psWeapStats->fireOnMove != FOM_NO)
          bChaseBloke = TRUE;

        if (actionInAttackRange(psDroid, psDroid->psActionTarget) && !bChaseBloke)
        {
      ...
      {
        if ((psDroid->turretRotation != 0) || (psDroid->turretPitch != 0))
      ...
      if (DROID_STOPPED(psDroid) && psDroid->action != DACTION_ATTACK)
      {
        ...
        else if (actionInsideMinRange(psDroid, psDroid->psActionTarget))
        {
          if (proj_Direct(psWeapStats))
          {
            ...
            moveDroidTo(psDroid, static_cast<UDWORD>(pbx), static_cast<UDWORD>(pby));
          }
          else
          {
            if (psWeapStats->rotate)
  ...
  case DACTION_MOVETOBUILD:
    ...
    if (actionReachedBuildPos(psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats) && !actionDroidOnBuildPos(
      psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats))
    {
      moveStopDroid(psDroid);
      /*			xdiff = (SDWORD)psDroid->x - (SDWORD)psDroid->actionX;
            ydiff = (SDWORD)psDroid->y - (SDWORD)psDroid->actionY;
            ...
            else*/
      {
        ...
        if (psDroid->order == DORDER_BUILD && (psDroid->psTarget == nullptr))
        //					&&  ( 
        ...
        //psDroid->order == DORDER_LINEBUILD)
        {
          ...
          tlx = static_cast<SDWORD>(psDroid->orderX) - static_cast<SDWORD>(psStructStats->baseWidth * TILE_UNITS) / 2;
          tly = static_cast<SDWORD>(psDroid->orderY) - static_cast<SDWORD>(psStructStats->baseBreadth * TILE_UNITS) / 2;
          ...
          else if (TILE_HAS_STRUCTURE(mapTile(psDroid->orderX >> TILE_SHIFT, psDroid->orderY >> TILE_SHIFT)))
          {
            ...
            else if ((psStruct->pStructureType->type == REF_WALL || psStruct->pStructureType->type == REF_WALLCORNER) && ((STRUCTURE_STATS*)
              psDroid->psTarStats)->type == REF_DEFENSE)
            {
        ...
        else if ((psDroid->order == DORDER_LINEBUILD || psDroid->order == DORDER_BUILD) && (psStructStats->type == REF_WALL || psStructStats
          ->type == REF_WALLCORNER || psStructStats->type == REF_DEFENSE))
        {
          // building a wall.
          psTile = mapTile(psDroid->orderX >> TILE_SHIFT, psDroid->orderY >> TILE_SHIFT);
          if ((psDroid->psTarget == nullptr) && (TILE_HAS_STRUCTURE(psTile) || TILE_HAS_FEATURE(psTile)))
          {
    ...
    else if (DROID_STOPPED(psDroid))
    {
      if (actionDroidOnBuildPos(psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats))
      {
        ...
        moveDroidToNoFormation(psDroid, static_cast<UDWORD>(pbx), static_cast<UDWORD>(pby));
      }
      else
  ...
  case DACTION_BUILD:
    ...
    if (DROID_STOPPED(psDroid) && !actionReachedBuildPos(psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats))
      moveDroidToNoFormation(psDroid, psDroid->actionX, psDroid->actionY);
    else if (!DROID_STOPPED(psDroid) && psDroid->sMove.Status != MOVETURNTOTARGET && psDroid->sMove.Status != MOVESHUFFLE &&
      actionReachedBuildPos(psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats))
      moveStopDroid(psDroid);
    if (droidUpdateBuild(psDroid))
    {
      ...
      actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget, &(psDroid->turretRotation), &(psDroid->turretPitch), nullptr,
                         FALSE);
    }
    else
      psDroid->action = DACTION_NONE;
  ...
  case DACTION_MOVETORESTORE:
    {
      ...
      if (actionReachedBuildPos(psDroid, static_cast<SDWORD>(psDroid->actionX), static_cast<SDWORD>(psDroid->actionY), psDroid->psTarStats)
        && !actionDroidOnBuildPos(psDroid, static_cast<SDWORD>(psDroid->actionX), static_cast<SDWORD>(psDroid->actionY),
                                  psDroid->psTarStats))
      {
        moveStopDroid(psDroid);
      ...
      {
        if (actionDroidOnBuildPos(psDroid, static_cast<SDWORD>(psDroid->actionX), static_cast<SDWORD>(psDroid->actionY),
                                  psDroid->psTarStats))
        {
          actionHomeBasePos(psDroid->player, &pbx, &pby);
          moveDroidToNoFormation(psDroid, static_cast<UDWORD>(pbx), static_cast<UDWORD>(pby));
  ...
  case DACTION_RESTORE:
    ...
    if (DROID_STOPPED(psDroid) && !actionReachedBuildPos(psDroid, static_cast<SDWORD>(psDroid->actionX),
                                                         static_cast<SDWORD>(psDroid->actionY), psDroid->psTarStats))
      moveDroidToNoFormation(psDroid, psDroid->actionX, psDroid->actionY);
    else if (!DROID_STOPPED(psDroid) && psDroid->sMove.Status != MOVETURNTOTARGET && psDroid->sMove.Status != MOVESHUFFLE &&
      actionReachedBuildPos(psDroid, static_cast<SDWORD>(psDroid->actionX), static_cast<SDWORD>(psDroid->actionY), psDroid->psTarStats))
      moveStopDroid(psDroid);
  ...
  case DACTION_MOVETOREPAIRPOINT:
                              ...
                              (BASE_STATS*)((STRUCTURE*)psDroid->psActionTarget)->pStructureType))
    {
      ...
      moveDroidToNoFormation(psDroid, psDroid->psActionTarget->x, psDroid->psActionTarget->y);
    break;
  case DACTION_BUILD_FOUNDATION:
    ...
    else
    {
      ...
      if ((psDroid->psTarget == nullptr) && (TILE_HAS_STRUCTURE(psTile) || TILE_HAS_FEATURE(psTile)))
      {
        ...
        else if (!validLocation(psDroid->psTarStats, tlx >> TILE_SHIFT, tly >> TILE_SHIFT, psDroid->player, FALSE))
          psDroid->action = DACTION_NONE;
      }

      //ready to start building the structure
      if (psDroid->action != DACTION_NONE && droidStartBuild(psDroid))
      {
        psDroid->action = DACTION_BUILD;
  ...
  case DACTION_OBSERVE:
    // align the turret
    actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget, &(psDroid->turretRotation), &(psDroid->turretPitch), nullptr,FALSE);
    ...
    else
    {
      ...
      rangeSq = static_cast<SDWORD>(psDroid->sensorRange);
      rangeSq = rangeSq * rangeSq;
      if (!visibleObject((BASE_OBJECT*)psDroid, psDroid->psActionTarget) || xdiff * xdiff + ydiff * ydiff >= rangeSq)
      {
  ...
  case DACTION_MOVETOOBSERVE:
    ...
    if (visibleObject((BASE_OBJECT*)psDroid, psDroid->psActionTarget))
    {
      ...
      rangeSq = static_cast<SDWORD>(psDroid->sensorRange);
      rangeSq = rangeSq * rangeSq;
      if ((xdiff * xdiff + ydiff * ydiff < rangeSq) && !DROID_STOPPED(psDroid))
      {
  ...
  case DACTION_FIRESUPPORT:
    ...
    DEBUG_ASSERT_TEXT(
      (psDroid->psTarget->type == OBJ_DROID OR psDroid->psTarget->type == OBJ_STRUCTURE) && (psDroid->psTarget->player == psDroid->player),
    ...
    //move (indirect weapon)droids attached to a sensor
    {
      ...
      if (!vtolDroid(psDroid) && (psDroid->psTarget->type != OBJ_STRUCTURE))
      {
        ...
        else
        {
          if (!DROID_STOPPED(psDroid))
          {
            xdiff = static_cast<SDWORD>(psDroid->psTarget->x) - psDroid->sMove.DestinationX;
            ydiff = static_cast<SDWORD>(psDroid->psTarget->y) - psDroid->sMove.DestinationY;
          }

          if (DROID_STOPPED(psDroid) || (xdiff * xdiff + ydiff * ydiff > rangeSq))
          {
            if (secondaryGetState(psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_HOLD))
            {
  ...
  case DACTION_MOVETODROIDREPAIR:
    ...
    if (xdiff * xdiff + ydiff * ydiff < REPAIR_RANGE)
    {
      ...
      if (actionTargetTurret((BASE_OBJECT*)psDroid, psDroid->psActionTarget, &(psDroid->turretRotation), &(psDroid->turretPitch), nullptr,
                             FALSE))
      {
  ...
  case DACTION_MOVETOREARM:
    ...
    if (DROID_STOPPED(psDroid) || (psDroid->action == DACTION_WAITFORREARM))
    {
      ...
      if (!actionVTOLLandingPos(psDroid, &droidX, &droidY))
      {
        ...
        DebugTrace("DACTION_MOVETOREARM: couldn't find a clear tile near rearm pad - RTB\n");
  ...
  if (psDroid->action != DACTION_MOVEFIRE && psDroid->action != DACTION_ATTACK && psDroid->action != DACTION_MOVETOATTACK && psDroid->action
    != DACTION_MOVETODROIDREPAIR && psDroid->action != DACTION_DROIDREPAIR && psDroid->action != DACTION_BUILD && psDroid->action !=
    DACTION_OBSERVE && psDroid->action != DACTION_MOVETOOBSERVE)
  {
    if ((psDroid->turretRotation != 0) || (psDroid->turretPitch != 0))
      actionAlignTurret((BASE_OBJECT*)psDroid);
  }
...
#ifdef DEBUG_GROUP0
  if (psDroid->player == selectedPlayer && prevAction != psDroid->action) {}
#endif
}
...
static void actionDroidBase(DROID* psDroid, DROID_ACTION_DATA* psAction)
{
  ...
  case DACTION_ATTACK:
    ...
    // or yourself
    if ((psDroid->asWeaps[0].nStat == 0) || (psDroid->droidType == DROID_TRANSPORTER) || (psAction->psObj == (BASE_OBJECT*)psDroid))
      return;
...
pStructureType, psAction->psObj->player))))*/
      ...
      if (bMultiPlayer AND psAction->psObj->type == OBJ_DROID AND ((DROID*)psAction->psObj)->droidType == DROID_TRANSPORTER)
      {
        psDroid->action = DACTION_NONE;
        break;
      }
    ...
    if (((psDroid->order == DORDER_ATTACKTARGET || psDroid->order == DORDER_FIRESUPPORT) && secondaryGetState(psDroid, DSO_HALTTYPE, &state)
      && (state == DSS_HALT_HOLD)) || (!vtolDroid(psDroid) && orderStateObj(psDroid, DORDER_FIRESUPPORT, &psTarget) && (psTarget->type ==
      OBJ_STRUCTURE)))
    {
  ...
  case DACTION_RETURNTOPOS:
  case DACTION_FIRESUPPORT_RETREAT:
    psDroid->action = psAction->m_action;
  ...
  case DACTION_BUILD: DEBUG_ASSERT_TEXT(
      psDroid->order == DORDER_BUILD || psDroid->order == DORDER_HELPBUILD || psDroid->order == DORDER_LINEBUILD,
    ...
    if (actionDroidOnBuildPos(psDroid, psDroid->orderX, psDroid->orderY, psDroid->psTarStats))
    {
      ...
      moveDroidToNoFormation(psDroid, psDroid->actionX, psDroid->actionY);
    break;
  case DACTION_DEMOLISH: DEBUG_ASSERT_TEXT(psDroid->order == DORDER_DEMOLISH,
                                           "actionUnitBase: cannot start demolish action without a demolish order");
    psDroid->action = DACTION_MOVETODEMOLISH;
    psDroid->actionX = psAction->x;
    psDroid->actionY = psAction->y;
    DEBUG_ASSERT_TEXT((psDroid->psTarget != NULL) && (psDroid->psTarget->type == OBJ_STRUCTURE),
  ...
  case DACTION_REPAIR:
    ...
    psDroid->psActionTarget = psAction->psObj;
    DEBUG_ASSERT_TEXT((psDroid->psActionTarget != NULL) && (psDroid->psActionTarget->type == OBJ_STRUCTURE),
                      "actionUnitBase: invalid target for demolish order");
  ...
  case DACTION_FIRESUPPORT:
    ...
    if (!vtolDroid(psDroid) && !(secondaryGetState(psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_HOLD)) && // check hold
  ...
  case DACTION_SULK:
    ...
    psDroid->actionStarted = gameTime + MIN_SULK_TIME + (rand() % (MAX_SULK_TIME - MIN_SULK_TIME));
    break;
  case DACTION_DESTRUCT:
  ...
  case DACTION_DROIDREPAIR:
    ...
    moveDroidTo(psDroid, psAction->x, psAction->y);
    break;
  case DACTION_RESTORE: DEBUG_ASSERT_TEXT(psDroid->order == DORDER_RESTORE,
    ...
    moveDroidTo(psDroid, psAction->x, psAction->y);
    break;
  case DACTION_CLEARWRECK: DEBUG_ASSERT_TEXT(psDroid->order == DORDER_CLEARWRECK,
                                             "actionUnitBase: cannot start clear action without a clear order");
    psDroid->action = DACTION_MOVETOCLEAR;
    psDroid->actionX = psAction->x;
    psDroid->actionY = psAction->y;
    DEBUG_ASSERT_TEXT((psDroid->psTarget != NULL) && (psDroid->psTarget->type == OBJ_FEATURE),
                      "actionUnitBase: invalid target for demolish order");
...
{
  if (x < 0 || x >= static_cast<SDWORD>(mapWidth) || y < 0 || y >= static_cast<SDWORD>(mapHeight))
    return FALSE;

  MAPTILE* psTile = mapTile(x, y);

  if ((psTile->tileInfoBits & BITS_FPATHBLOCK) || (TILE_OCCUPIED(psTile)) || (TERRAIN_TYPE(psTile) == TER_CLIFFFACE) || (
...
BOOL actionVTOLLandingPos(DROID* psDroid, UDWORD* _px, UDWORD* _py)
{
  SDWORD endX, endY, tx, ty;
  DROID* psCurr;

  //	ASSERT(((psDroid->psActionTarget != NULL),

  /* Initial box dimensions and set iteration count to zero */
  SDWORD startX = endX = static_cast<SDWORD>(*_px) >> TILE_SHIFT;
  SDWORD startY = endY = static_cast<SDWORD>(*_py) >> TILE_SHIFT;
  UDWORD passes = 0;
  ...
  {
    /* Process whole box */
    for (SDWORD i = startX; i <= endX; i++)
    {
      for (SDWORD j = startY; j <= endY; j++)
      {
        ...
        if (i == startX OR i == endX OR j == startY OR j == endY)
        {
          ...
          if (vtolLandingTile(i, j))
          {
            ...
            *_px = (i << TILE_SHIFT) + TILE_UNITS / 2;
            *_py = (j << TILE_SHIFT) + TILE_UNITS / 2;
            result = TRUE;
            goto exit;
          }
        }
