...
BOOL mplayerSubmit(void)
{
  ...
  if (MPDPXTRA_Init() == MPDPXTRAERR_OK)
  {
    ...
    MPDPXTRA_AddScoreResult(NetPlay.players[selectedPlayer].dpid, 0, MP_WINS, stats.wins); MPDPXTRA_AddScoreResult(
      NetPlay.players[selectedPlayer].dpid, 0, MP_LOSES, stats.loses);
    ...
    MPDPXTRA_AddScoreResult(NetPlay.players[selectedPlayer].dpid, 0, MP_GAME_WIN, (stats.wins - stats2.wins)); MPDPXTRA_AddScoreResult(
      NetPlay.players[selectedPlayer].dpid, 0, MP_GAME_LOSE, (stats.loses - stats2.loses));
