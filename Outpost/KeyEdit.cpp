...
static BOOL pushedKeyMap(UDWORD key)
{
  ...
  selectedKeyMap = static_cast<KEY_MAPPING*>(widgGetFromID(psWScreen, key)->pUserData);
  ...
  for (i = KEY_1; i <= KEY_DELETE; i++)
  {
    if (keyPressed(i))
    {
      if (i != KEY_RALT // exceptions
        && i != KEY_LALT && i != KEY_RCTRL && i != KEY_LCTRL && i != KEY_RSHIFT && i != KEY_LSHIFT)
...
VOID displayKeyMap(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  auto psMapping = static_cast<KEY_MAPPING*>(psWidget->pUserData);
...
BOOL startKeyMapEditor(BOOL first)
{
  ...
  sButInit.pUserData = static_cast<void*>(psPresent);
  ...
  while (bubbleCount < mapcount - 1 AND !bAtEnd)
  {
    ...
    strcpy(test, "zzzzzzzzzzzzzzzzzzzzz");
    for (psMapping = keyMappings, psNext = nullptr, bGotOne = FALSE; psMapping; psMapping = psMapping->psNext)
    {
    ...
    if (bGotOne)
    {
      psPresent = psNext;
      bubbleCount++;
      sButInit.pUserData = static_cast<void*>(psNext);
      widgAddButton(psWScreen, &sButInit);
      // move on..
      sButInit.id++;
