...
static void highLevelDroidUpdate(DROID* psDroid, UDWORD x, UDWORD y, UDWORD state, UDWORD order,
                                 //UDWORD orderX,
                                 //UDWORD orderY, 
...
{
  DROID* pD = nullptr; // current droid we're checking
  UDWORD i;
  ...
  for (i = 0; // CRITERIA FOR PICKADROID
       ...
       && (pD != nullptr) //still here. 
       ; i++) // 
    pD = pD->psNext;

  if (pD == nullptr) // droid is no longer there or list end.
  {
    player++; // next player next time.
    player = player % MAX_PLAYERS;
    droidnum = 0;
...
static void packageCheck(UDWORD i, NETMSG* pMsg, DROID* pD)
{
  pMsg->body[i + 0] = static_cast<CHAR>(pD->player);
  pMsg->body[i + 1] = static_cast<CHAR>(pD->order); // order being executed
  ...
  pMsg->size = static_cast<UWORD>(pMsg->size + 30);
}
...
BOOL recvDroidCheck(NETMSG* m)
{
  ...
  while (i < m->size)
  {
    ...
    player = m->body[i + 0];
    ord = static_cast<DROID_ORDER>(m->body[i + 1]); // droid id.
    NetGet(m, i+2, ref);
    NetGet(m, i+6, state);
    ...
    if (DrawnInLastFrame(pD->sDisplay.frameNumber) && (pD->sDisplay.screenX < DISP_WIDTH) && (pD->sDisplay.screenY < DISP_HEIGHT))
    // check for onscreen
    {
    ...
    if (abs(static_cast<SDWORD>(x - pD->x)) < (TILE_UNITS * 2) || abs(static_cast<SDWORD>(y - pD->y)) < (TILE_UNITS * 2))
...
static void highLevelDroidUpdate(DROID* psDroid, UDWORD x, UDWORD y,
                                 ...
                                 UDWORD state, UDWORD order, BASE_OBJECT* psTarget, UDWORD numKills)
{
  // update kill rating.
  ...
  {
    if ((abs(static_cast<SDWORD>(x - psDroid->x)) > (TILE_UNITS * 2)) // if more than 2 tiles wrong.
      || (abs(static_cast<SDWORD>(y - psDroid->y)) > (TILE_UNITS * 2)))
    {
      turnOffMultiMsg(TRUE);
...
static void offscreenUpdate(DROID* psDroid, UDWORD dam, UDWORD x, UDWORD y, FRACT fx, FRACT fy, UWORD dir, DROID_ORDER order)
{
  ...
  if (order == DORDER_ATTACK || order == DORDER_MOVE || order == DORDER_RTB || order == DORDER_RTR) // move order
  {
    // calculate difference between remote and local
    xdiff = psDroid->x - static_cast<UWORD>(fx);
    ydiff = psDroid->y - static_cast<UWORD>(fy);
    distSq = (xdiff * xdiff) + (ydiff * ydiff);

    // if more than  2 squares, jump it.
    if (distSq > (2 * TILE_UNITS) * (2 * TILE_UNITS))
    {
      if ((static_cast<UDWORD>(fx) != 0) && (static_cast<UDWORD>(fy) != 0))
      {
        oldx = psDroid->x;
        oldy = psDroid->y;
        ...
        psDroid->x = static_cast<UWORD>(fx); //update move progress
  ...
  if ((order == DORDER_NONE || order == DORDER_GUARD) && !(psDroid->order == DORDER_NONE || psDroid->order == DORDER_GUARD))
  {
    turnOffMultiMsg(TRUE);
...
static STRUCTURE* pickAStructure(void)
{
  ...
  for (i = 0; ((i < snum) && (pS != nullptr)); i++)
    pS = pS->psNext;

  if (pS == nullptr) // last structure or no structures at all
  {
...
static BOOL sendStructureCheck(VOID)
{
  ...
  {
    m.body[0] = static_cast<char>(pS->player); // send struct details
    NetAdd(m, 1, pS->id);
...
BOOL recvStructureCheck(NETMSG* m)
{
    ...
    if (TILE_HAS_STRUCTURE(mapTile(x>>TILE_SHIFT,y>>TILE_SHIFT)))
    {
      ...
      else if (pS->pStructureType->type == REF_WALL)
      {
        if (psStats->type == REF_WALLCORNER)
        {
          ...
          pS = buildStructure(psStats, x, y, player,TRUE);
  ...
  if (pS)
  {
    ...
    if (m->size > 19) // capacity
    {
      ...
      {
      case REF_RESEARCH:
        cap = static_cast<UBYTE>(((RESEARCH_FACILITY*)pS->pFunctionality)->capacity);
        break;
      case REF_FACTORY:
      case REF_VTOL_FACTORY:
        //			case REF_CYBORG_FACTORY:
        cap = ((FACTORY*)pS->pFunctionality)->capacity;
        break;
      case REF_POWER_GEN:
        cap = static_cast<UBYTE>(((POWER_GEN*)pS->pFunctionality)->capacity);
        break;
      default:
...
BOOL sendPowerCheck(BOOL now)
{
  ...
  m.body[0] = static_cast<char>(selectedPlayer);
  ...
  if (titleMode != MULTIOPTION && titleMode != MULTILIMIT)
  {
    ...
    for (i = 0; i < MAX_PLAYERS; i++)
    {
      if (isHumanPlayer(i) && (i != selectedPlayer))
      {
        ...
        {
          m.body[m.size] = static_cast<UBYTE>(i);
          m.size += 1;
...
BOOL sendPing(VOID)
{
  ...
  ping.body[0] = static_cast<char>(selectedPlayer);
  ping.body[1] = 1;
  ping.size = 2;
  ping.type = NET_PING;
...
BOOL recvPing(NETMSG* ping)
{
  ...
  {
    reply.body[0] = static_cast<char>(selectedPlayer);
    reply.body[1] = 0;
    reply.size = 2;
    reply.type = NET_PING;
