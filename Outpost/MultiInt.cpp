...
extern BOOL intAddTemplateButtons(UDWORD formID, UDWORD formWidth, UDWORD formHeight, UDWORD butWidth, UDWORD butHeight, UDWORD gap,
...
// widget functions
BOOL addMultiBut(W_SCREEN* screen, UDWORD formid, UDWORD id, UDWORD x, UDWORD y, UDWORD width, UDWORD height, UDWORD tipres, UDWORD norm,
                 UDWORD hi, BOOL showmouseover);
...
static void addGames()
{
  ...
  for (i = 0; i < MaxGames; i++) // draw games 
  {
    ...
    if (NetPlay.games[i].name[0] != '\0')
    {
      ...
      {
        sButInit.x = 125;
        sButInit.y = static_cast<SHORT>(30 + ((5 + GAMES_GAMEHEIGHT) * i));
      }
      else
      {
        if (i < 6) //column 1
        ...
        else //column 2
        {
          ...
          sButInit.y = static_cast<SHORT>(30 + ((5 + GAMES_GAMEHEIGHT) * (i - 6)));
        }
      }
...
void startGameFind(void)
{
  ...
  // cancel
  addMultiBut(psWScreen,FRONTEND_BOTFORM,CON_CANCEL, 10, 5,MULTIOP_OKW,MULTIOP_OKH, STR_MUL_CANCEL, IMAGE_RETURN, IMAGE_RETURN_HI,TRUE);

  if (!safeSearch)
  {
    //refresh
    addMultiBut(psWScreen,FRONTEND_BOTFORM,MULTIOP_REFRESH, 480 - MULTIOP_OKW - 5, 5,MULTIOP_OKW,MULTIOP_OKH, STR_MUL_REFRESH,
...
static void addBlueForm(UDWORD parent, UDWORD id, STRING* txt, UDWORD x, UDWORD y, UDWORD w, UDWORD h)
{
  ...
  sFormInit.id = id;
  sFormInit.x = static_cast<UWORD>(x);
  sFormInit.y = static_cast<UWORD>(y);
  sFormInit.style = WFORM_PLAIN;
  sFormInit.width = static_cast<UWORD>(w); //190;
  sFormInit.height = static_cast<UWORD>(h); //27;
  sFormInit.pDisplay = intDisplayFeBox;
...
static void addGameOptions(BOOL bRedo)
{
  ...
  addMultiBut(psWScreen,MULTIOP_GAMETYPE,MULTIOP_CAMPAIGN,MCOL1, 2, MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_CAMPAIGN, IMAGE_CAMPAIGN,
              IMAGE_CAMPAIGN_HI,TRUE); //camp

  addMultiBut(psWScreen,MULTIOP_GAMETYPE,MULTIOP_TEAMPLAY,MCOL2, 2, MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_TEAMPLAY, IMAGE_TEAM, IMAGE_TEAM_HI,
              TRUE); //teamplay

  addMultiBut(psWScreen,MULTIOP_GAMETYPE,MULTIOP_SKIRMISH,MCOL3, 2, MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_SKIRMISH, IMAGE_SKIRMISH,
              IMAGE_SKIRMISH_HI,TRUE); //skirmish
  ...
  addMultiBut(psWScreen,MULTIOP_FOG,MULTIOP_FOG_ON,MCOL1, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_FOG_ON, IMAGE_FOG_OFF, IMAGE_FOG_OFF_HI,
              TRUE); //black stuff
  addMultiBut(psWScreen,MULTIOP_FOG,MULTIOP_FOG_OFF,MCOL2, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_FOG_OFF, IMAGE_FOG_ON, IMAGE_FOG_ON_HI,
              TRUE);
  if (game.fog)
  ...
  if (game.type != TEAMPLAY)
  {
    ...
    addMultiBut(psWScreen,MULTIOP_ALLIANCES,MULTIOP_ALLIANCE_N,MCOL1, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_ALLIANCEN, IMAGE_NOALLI,
                IMAGE_NOALLI_HI,TRUE);
    addMultiBut(psWScreen,MULTIOP_ALLIANCES,MULTIOP_ALLIANCE_Y,MCOL2, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_ALLIANCEY, IMAGE_ALLI,
                IMAGE_ALLI_HI,TRUE);
  ...
  */
  ...
  if (game.type == CAMPAIGN || game.type == SKIRMISH || game.type == TEAMPLAY)
  {
    ...
    addMultiBut(psWScreen,MULTIOP_POWER,MULTIOP_POWLEV_LOW,MCOL1, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_POWLO, IMAGE_POWLO, IMAGE_POWLO_HI,
                TRUE);
    addMultiBut(psWScreen,MULTIOP_POWER,MULTIOP_POWLEV_MED,MCOL2, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_POWMED, IMAGE_POWMED,
                IMAGE_POWMED_HI,TRUE);
    addMultiBut(psWScreen,MULTIOP_POWER,MULTIOP_POWLEV_HI, MCOL3, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_POWHI, IMAGE_POWHI, IMAGE_POWHI_HI,
                TRUE);
    ...
    addMultiBut(psWScreen,MULTIOP_BASETYPE,MULTIOP_CLEAN,MCOL1, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_CAMPCLEAN, IMAGE_NOBASE,
                IMAGE_NOBASE_HI,TRUE);
    addMultiBut(psWScreen,MULTIOP_BASETYPE,MULTIOP_BASE,MCOL2, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_CAMPBASE, IMAGE_SBASE, IMAGE_SBASE_HI,
                TRUE);
    addMultiBut(psWScreen,MULTIOP_BASETYPE,MULTIOP_DEFENCE,MCOL3, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_CAMPDEFENCE, IMAGE_LBASE,
                IMAGE_LBASE_HI,TRUE);
  ...
  if (game.type == CAMPAIGN || game.type == TEAMPLAY)
  {
    ...
    addMultiBut(psWScreen,MULTIOP_COMPUTER,MULTIOP_COMPUTER_Y,MCOL1, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_COMP_Y, IMAGE_COMPUTER_Y,
                IMAGE_COMPUTER_Y_HI,TRUE);
    addMultiBut(psWScreen,MULTIOP_COMPUTER,MULTIOP_COMPUTER_N,MCOL2, 2,MULTIOP_BUTW,MULTIOP_BUTH, STR_MUL_COMP_N, IMAGE_COMPUTER_N,
                IMAGE_COMPUTER_N_HI,TRUE);
  ...
  {
    addMultiBut(psWScreen,MULTIOP_OPTIONS,MULTIOP_HOST,MULTIOP_HOSTX,MULTIOP_HOSTY, 35, 28, STR_MUL_HOST, IMAGE_HOST, IMAGE_HOST,FALSE);
  }
  ...
  {
    addMultiBut(psWScreen,MULTIOP_OPTIONS,MULTIOP_STRUCTLIMITS,MULTIOP_STRUCTLIMITSX,MULTIOP_STRUCTLIMITSY, 35, 28, STR_MUL_STRLIM,
...
static VOID addColourChooser(UDWORD player)
{
  ...
  // add form.
  addBlueForm(MULTIOP_PLAYERS,MULTIOP_COLCHOOSER_FORM, "", 10, ((MULTIOP_PLAYERHEIGHT + 5) * player) + 4, MULTIOP_PLAYERWIDTH,
...
static BOOL SendColourRequest(UDWORD player, UBYTE col, UBYTE chosenPlayer)
{
  ...
  m.body[0] = static_cast<UBYTE>(player);
  m.body[1] = col;
  m.body[2] = chosenPlayer;
...
BOOL recvColourRequest(NETMSG* pMsg)
{
  ...
  player = static_cast<UDWORD>(pMsg->body[0]);
  col = static_cast<UDWORD>(pMsg->body[1]);
  chosenPlayer = static_cast<UDWORD>(pMsg->body[2]);
  ...
  if (players)
  {
    for (i = 0; i < game.maxPlayers; i++)
    {
      if (ingame.localOptionsReceived && NetPlay.players[i].dpid) // only draw if real player!
      {
        ...
        sButInit.style = WBUT_PLAIN;
        sButInit.x = 10;
        sButInit.y = static_cast<SHORT>(((MULTIOP_PLAYERHEIGHT + 5) * i) + 4);
      ...
      else if (game.type == SKIRMISH) // skirmish player
      {
        ...
        sFormInit.x = 10;
        sFormInit.y = static_cast<SHORT>(((MULTIOP_PLAYERHEIGHT + 5) * i) + 4);
...
static void disableMultiButs(void)
{
  ...
  if (!NetPlay.bHost)
  {
    ...
    if (game.type == CAMPAIGN)
    {
      if (game.bComputerPlayers)
        widgSetButtonState(psWScreen, MULTIOP_COMPUTER_N,WBUT_DISABLE); // pow levels
      if (!game.bComputerPlayers)
        widgSetButtonState(psWScreen, MULTIOP_COMPUTER_Y,WBUT_DISABLE);
    }
...
VOID stopJoining(void)
{
  ...
  if (ingame.localJoiningInProgress) // cancel a joined game.
  {
    sendLeavingMsg(); // say goodbye
    NETclose(); // quit running game.
...
static void processMultiopWidgets(UDWORD id)
{
  ...
  case MULTIOP_HOST:
    if (game.type != SKIRMISH)
      strcpy(sForceName, widgGetString(psWScreen, MULTIOP_FNAME));
    strcpy(game.name, widgGetString(psWScreen, MULTIOP_GNAME)); // game name
    strcpy((STRING*)sPlayer, widgGetString(psWScreen, MULTIOP_PNAME)); // pname
    strcpy(game.map, widgGetString(psWScreen, MULTIOP_MAP)); // add the name	
  ...
  if ((id >= MULTIOP_SKSLIDE) && (id <= MULTIOP_SKSLIDE_END)) // choseskirmish difficulty.
  {
    ...
    if ((id - MULTIOP_SKSLIDE == game.maxPlayers - 1) && (game.skDiff[id - MULTIOP_SKSLIDE] == 0))
    {
...
void runMultiOptions(VOID)
{
  ...
  k = getQwertyKey();
  if (k && psWScreen->psFocus == nullptr)
  {
  ...
  if (widgGetFromID(psWScreen,MULTIOP_CHATBOX))
  {
    while (getNumberConsoleMessages() > getConsoleLineInfo()) { removeTopConsoleMessage(); }
  ...
  if (multiRequestUp)
  {
    ...
    if (runMultiRequester(id, &id, (char*)&sTemp, &value))
    {
      ...
      case MULTIOP_MAP:
        ...
        game.maxPlayers = static_cast<UBYTE>(value);
        loadMapPreview();
...
BOOL startMultiOptions(BOOL bReenter)
{
  ...
  addMultiBut(psWScreen, FRONTEND_BACKDROP, MULTIOP_WHITEBOARD, MULTIOP_CHATBOXX - 15,MULTIOP_CHATBOXY + MULTIOP_CHATBOXH - 15, 9, 9, 0,
...
static VOID CurrentForce(VOID)
{
  ...
  butPerForm = ((FORCE_CURRENTWIDTH - 0 - 2) / (OBJ_BUTWIDTH + 2)) * ((FORCE_CURRENTHEIGHT - 0 - 2) / (OBJ_BUTHEIGHT + 2));
  ...
  for (pF = Force.pMembers; pF; pF = pF->psNext)
  {
    ...
    sButInit.id += 1;
    sButInit.x = static_cast<SWORD>(sButInit.x + OBJ_BUTWIDTH + 2);
    if (sButInit.x + OBJ_BUTWIDTH + 2 > FORCE_CURRENTWIDTH)
    {
      sButInit.x = 2;
      sButInit.y = static_cast<SWORD>(sButInit.y + OBJ_BUTHEIGHT + 2);
    }
...
static VOID AvailableForces(VOID)
{
  ...
  while ((psTempl != nullptr) && (numButtons < MAXTEMPLATES))
  {
  ...
  /* Calculate how many buttons will go on a single form */
  butPerForm = ((FORCE_AVAILABLEWIDTH - 0 - 2) / (OBJ_BUTWIDTH + 2)) * ((FORCE_AVAILABLEHEIGHT - 0 - 2) / (OBJ_BUTHEIGHT + 2));
  ...
  /* Put the buttons on it */
  intAddTemplateButtons(IDDES_TEMPLFORM, FORCE_AVAILABLEWIDTH, FORCE_AVAILABLEHEIGHT, OBJ_BUTWIDTH,OBJ_BUTHEIGHT, 2, nullptr);
}
...
BOOL startForceSelect(VOID)
{
  ...
  addMultiBut(psWScreen,FORCE_STATS,CON_CANCEL,FORCE_OKX,FORCE_OKY,MULTIOP_OKW,MULTIOP_OKH, STR_MUL_CANCEL, IMAGE_RETURN, IMAGE_RETURN_HI,
              FALSE); // cancel

  addMultiBut(psWScreen,FORCE_STATS,FORCE_PRESETCLEAR,FORCE_PRESETCLEARX,FORCE_PRESETCLEARY,FORCE_BUTW,FORCE_BUTH, STR_MUL_CLEAR,
              IMAGE_CLEARFORCE, IMAGE_CLEARFORCE,TRUE); // clear.	

  addMultiBut(psWScreen,FORCE_STATS,FORCE_PRESETDEFAULT,FORCE_PRESETDEFAULTX,FORCE_PRESETDEFAULTY,FORCE_BUTW,FORCE_BUTH, STR_MUL_DEFAULT,
              IMAGE_DEFAULTFORCE, IMAGE_DEFAULTFORCE,TRUE); // default.

  addMultiBut(psWScreen,FORCE_STATS,FORCE_LOAD,FORCE_LOADX,FORCE_LOADY,FORCE_BUTW,FORCE_BUTH, STR_MUL_LOAD, IMAGE_LOADFORCE,
              IMAGE_LOADFORCE,TRUE); // load

  addMultiBut(psWScreen,FORCE_STATS,FORCE_SAVE,FORCE_SAVEX,FORCE_SAVEY,FORCE_BUTW,FORCE_BUTH, STR_MUL_SAVE, IMAGE_SAVEFORCE,
              IMAGE_SAVEFORCE,TRUE); // save
...
BOOL runWhiteBoard()
{
  ...
  if (mx > WHITEX && mx < (WHITEX + WHITEW) && my > WHITEY && my < (WHITEY + WHITEH) && (mouseDown(MOUSE_LMB) || mouseReleased(MOUSE_LMB)))
  {
    if (mouseDown(MOUSE_LMB))
    {
      ...
      whiteBoard[selectedPlayer][curWhite] = static_cast<UWORD>(((y - 1) * WHITEW) + x);
    }
  ...
  if ((gameTime - lastSent) > 2000)
  {
    ...
    m.body[0] = static_cast<UBYTE>(selectedPlayer);
...
BOOL displayWhiteBoard(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    if (isHumanPlayer(i))
    {
      ...
      oldy = static_cast<UWORD>(d.quot);
      oldx = static_cast<UWORD>(d.rem);
...
BOOL addWhiteBoard()
{
  ...
  sFormInit.id = 999;
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>((WHITEX - D_W));
  sFormInit.y = static_cast<SWORD>((WHITEY - D_H));
...
BOOL addMultiEditBox(UDWORD formid, UDWORD id, UDWORD x, UDWORD y, UDWORD tip, STRING tipres[128], UDWORD icon, UDWORD iconid)
{
  ...
  sEdInit.id = id;
  sEdInit.style = WEDB_PLAIN;
  sEdInit.x = static_cast<short>(x);
  sEdInit.y = static_cast<short>(y);
  ...
  addMultiBut(psWScreen,MULTIOP_OPTIONS, iconid, (x + MULTIOP_EDITBOXW + 2), (y + 2), MULTIOP_EDITBOXH, MULTIOP_EDITBOXH, tip, icon, icon,
              FALSE);
  return TRUE;
}
                 ...
                 UDWORD hi, BOOL hiIt)
{
  ...
  sButInit.x = static_cast<short>(x);
  sButInit.y = static_cast<short>(y);
  sButInit.width = static_cast<unsigned short>(width);
  sButInit.height = static_cast<unsigned short>(height);
  if (tipres)
