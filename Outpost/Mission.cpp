...
static DROID_GROUP* g_CurrentScriptGroup = nullptr;
...
void initMission(void)
{
  ...
  mission.ETA = -1;
  mission.startTime = 0;
  mission.psGateways = nullptr;
  mission.apRLEZones = nullptr;
  mission.gwNumZones = 0;
  mission.mapHeight = 0;
  mission.mapWidth = 0;
  mission.aNumEquiv = nullptr;
  mission.apEquivZones = nullptr;
  mission.aZoneReachable = nullptr;
...
void resetVTOLLandingPos(void) { memset(asVTOLReturnPos, 0, sizeof(POINT) * MAX_PLAYERS); }
  ...
  if (mission.ETA >= 0)
  {
    ...
    if (psDroid)
    {
      ...
      if (psDroid)*/
      {
        ...
        widgSetUserData(psWScreen, IDTRANTIMER_DISPLAY, psTransporter);
...
/* get offscreen point */
void missionGetOffScreenPoint(UWORD iX, UWORD iY, UWORD* piOffX, UWORD* piOffY, UWORD* piOffZ)
{
  //	UDWORD	iMapWidth  = GetWidthOfMap()  << TILE_SHIFT,
  SDWORD iTestX = static_cast<SDWORD>(iX) - OFFSCREEN_RADIUS, iTestY = static_cast<SDWORD>(iY) - OFFSCREEN_RADIUS;

  if (iTestX > 0)
    *piOffX = static_cast<UWORD>(iTestX);
  else
    *piOffX = static_cast<UWORD>(static_cast<SDWORD>(iX) - OFFSCREEN_RADIUS);

  if (iTestY > 0)
    *piOffY = static_cast<UWORD>(iTestY);
  else
    *piOffY = static_cast<UWORD>(static_cast<SDWORD>(iY) - OFFSCREEN_RADIUS);

  *piOffZ = static_cast<UWORD>(map_Height(iX, iY) + OFFSCREEN_HEIGHT);
}
...
{
  UDWORD iMidX = (scrollMinX + scrollMaxX) / 2, iMidY = (scrollMinY + scrollMaxY) / 2;

  if (static_cast<UDWORD>(iX >> TILE_SHIFT) < iMidX)
    *piOffX = static_cast<UWORD>((scrollMinX << TILE_SHIFT) + (EDGE_SIZE * TILE_UNITS));
  else
    *piOffX = static_cast<UWORD>((scrollMaxX << TILE_SHIFT) - (EDGE_SIZE * TILE_UNITS));
  if (static_cast<UDWORD>(iY >> TILE_SHIFT) < iMidY)
    *piOffY = static_cast<UWORD>((scrollMinY << TILE_SHIFT) + (EDGE_SIZE * TILE_UNITS));
  else
    *piOffY = static_cast<UWORD>((scrollMaxY << TILE_SHIFT) - (EDGE_SIZE * TILE_UNITS));
}
...
void missionFlyTransportersIn(SDWORD iPlayer, BOOL bTrackTransporter)
{
  ...
  iZ = static_cast<UWORD>(map_Height(iX, iY) + OFFSCREEN_HEIGHT);
...
void saveMissionData(void)
{
  ...
  for (psStruct = apsStructLists[selectedPlayer]; psStruct != nullptr; psStruct = psStruct->psNext)
  {
    ...
    if (psStruct->pStructureType->type == REF_REPAIR_FACILITY AND psStruct->status == SS_BUILT)
      bRepairExists = TRUE;
  }
...
void restoreMissionData(void)
{
  ...
  mission.aMapLinePoints = nullptr;
  mission.mapWidth = 0;
  mission.mapHeight = 0;
...
void placeLimboDroids(void)
{
  ...
  for (psDroid = apsLimboDroids[selectedPlayer]; psDroid != nullptr; psDroid = psNext)
  {
    ...
    if (droidRemove(psDroid, apsLimboDroids))
    {
      ...
      pickRes = pickHalfATile(&droidX, &droidY,LOOK_FOR_EMPTY_TILE);
      if (pickRes == NO_FREE_TILE)
        DEBUG_ASSERT_TEXT(FALSE, "placeLimboUnits: Unable to find a free location");
      ...
      DEBUG_ASSERT_TEXT(FALSE, "placeLimboUnits: Unable to remove unit from Limbo list");
  }
}
...
void restoreMissionLimboData(void)
{
  ...
  DEBUG_ASSERT_TEXT(mission.apsDroidLists[selectedPlayer] == NULL, "restoreMissionLimboData: list should be empty");
}
...
void saveCampaignData(void)
{
  ...
  if (getDroidsToSafetyFlag())
  {
    ...
    while (psDroid != nullptr)
    {
      ...
      if (psDroid->droidType == DROID_TRANSPORTER)
      {
        ...
        DEBUG_ASSERT_TEXT(psDroid->psGroup != NULL, "saveCampaignData: Transporter does not have a group");

        for (psCurr = psDroid->psGroup->psList; psCurr != nullptr AND psCurr != psDroid; psCurr = psCurrNext)
        {
  ...
  if (getDroidsToSafetyFlag())
  {
    ...
    for (psDroid = mission.apsDroidLists[selectedPlayer]; psDroid != nullptr; psDroid = psDroid->psNext)
    {
      ...
      {
        //fill it with droids from the mission list
        for (psSafeDroid = mission.apsDroidLists[selectedPlayer]; psSafeDroid != nullptr; psSafeDroid = psNextSafe)
        {
          psNextSafe = psSafeDroid->psNext;
...
BOOL startMissionCampaignChange(STRING* pGame)
{
  ...
  changeProductionPlayer(static_cast<UBYTE>(selectedPlayer));
...
void processMission()
{
  ...
  for (psDroid = apsDroidLists[selectedPlayer]; psDroid != nullptr; psDroid = psNext)
  {
    ...
    if (droidRemove(psDroid, apsDroidLists))
    {
      ...
      pickRes = pickHalfATile(&droidX, &droidY,LOOK_FOR_EMPTY_TILE);
      if (pickRes == NO_FREE_TILE)
        DEBUG_ASSERT_TEXT(FALSE, "processMission: Unable to find a free location");
      psDroid->x = static_cast<UWORD>(droidX << TILE_SHIFT);
      psDroid->y = static_cast<UWORD>(droidY << TILE_SHIFT);
      if (pickRes == HALF_FREE_TILE)
      {
...
void swapMissionPointers(void)
{
  ...
  //swap psMapTiles
  pVoid = static_cast<void*>(psMapTiles);
  psMapTiles = mission.psMapTiles;
  mission.psMapTiles = static_cast<MAPTILE*>(pVoid);
  //swap map sizes
  udwTemp = mapWidth;
  mapWidth = mission.mapWidth;
  ...
  //swap gateway zones
  pVoid = static_cast<void*>(psGateways);
  psGateways = mission.psGateways;
  mission.psGateways = static_cast<struct _gateway*>(pVoid);
  ppVoid = (void**)apRLEZones;
  apRLEZones = mission.apRLEZones;
  mission.apRLEZones = (UBYTE**)ppVoid;
  udwTemp = static_cast<UDWORD>(gwNumZones);
  gwNumZones = mission.gwNumZones;
  mission.gwNumZones = static_cast<SDWORD>(udwTemp);
  pVoid = static_cast<void*>(aNumEquiv);
  aNumEquiv = mission.aNumEquiv;
  mission.aNumEquiv = static_cast<UBYTE*>(pVoid);
  ppVoid = (void**)apEquivZones;
  apEquivZones = mission.apEquivZones;
  mission.apEquivZones = (UBYTE**)ppVoid;
  pVoid = static_cast<void*>(aZoneReachable);
  aZoneReachable = mission.aZoneReachable;
  mission.aZoneReachable = static_cast<UBYTE*>(pVoid);
  //swap scroll limits
  udwTemp = scrollMinX;
  scrollMinX = mission.scrollMinX;
  ...
  for (inc = 0; inc < MAX_PLAYERS; inc++)
  {
    pVoid = static_cast<void*>(apsDroidLists[inc]);
    apsDroidLists[inc] = mission.apsDroidLists[inc];
    mission.apsDroidLists[inc] = static_cast<DROID*>(pVoid);
    pVoid = static_cast<void*>(apsStructLists[inc]);
    apsStructLists[inc] = mission.apsStructLists[inc];
    mission.apsStructLists[inc] = static_cast<STRUCTURE*>(pVoid);
    pVoid = static_cast<void*>(apsFeatureLists[inc]);
    apsFeatureLists[inc] = mission.apsFeatureLists[inc];
    mission.apsFeatureLists[inc] = static_cast<FEATURE*>(pVoid);
    pVoid = static_cast<void*>(apsFlagPosLists[inc]);
    apsFlagPosLists[inc] = mission.apsFlagPosLists[inc];
    mission.apsFlagPosLists[inc] = static_cast<FLAG_POSITION*>(pVoid);
  }
...
void endMissionExpandLimbo(void) { restoreMissionLimboData(); }
...
void aiUpdateMissionStructure(STRUCTURE* psStructure)
{
  ...
  if ((asPower[psStructure->player]->currentPower > POWER_PER_CYCLE) OR (!powerCalculated))
  {
  ...
  if (pSubject != nullptr)
  {
    ...
    else if (structureMode == REF_FACTORY)
    {
      ...
      if (pointsToAdd > psFactory->timeToBuild)
      {
        ...
        psNewDroid = buildMissionDroid((DROID_TEMPLATE*)pSubject, psStructure->x, psStructure->y, psStructure->player);
...
void missionStructureUpdate(STRUCTURE* psBuilding)
{
  ...
  if (StructIsFactory(psBuilding) OR psBuilding->pStructureType->type == REF_RESEARCH)
  {
...
void missionDroidUpdate(DROID* psDroid)
{
  ...
  if ((psDroid->droidType != DROID_TRANSPORTER) || !(orderState(psDroid, DORDER_TRANSPORTOUT) || orderState(psDroid, DORDER_TRANSPORTIN) ||
...
void missionResetDroids()
{
  ...
  {
    for (psDroid = apsDroidLists[player]; psDroid != nullptr; psDroid = psNext)
    {
      psNext = psDroid->psNext;

      //reset order - unless constructor droid that is mid-build
      //if (psDroid->droidType == DROID_CONSTRUCT AND orderStateObj(psDroid, 
      if ((psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT) AND orderStateObj(
        psDroid, DORDER_BUILD, (BASE_OBJECT**)&psStruct))
      {
  ...
  //	psDroid = psNext)
  {
    ...
    if (psDroid->x == INVALID_XY AND psDroid->y == INVALID_XY)
    {
      ...
      if (psStruct)
      {
        ...
        {
          x = static_cast<UWORD>(psFactory->psAssemblyPoint->coords.x >> TILE_SHIFT);
          y = static_cast<UWORD>(psFactory->psAssemblyPoint->coords.y >> TILE_SHIFT);
        }
        else
        {
          x = static_cast<UWORD>(psStruct->x >> TILE_SHIFT);
          y = static_cast<UWORD>(psStruct->y >> TILE_SHIFT);
        }
        pickRes = pickHalfATile(&x, &y,LOOK_FOR_EMPTY_TILE);
        if (pickRes == NO_FREE_TILE)
        {
          DEBUG_ASSERT_TEXT(FALSE, "missionResetUnits: Unable to find a free location");
          psStruct = nullptr;
        }
        else
        {
          psDroid->x = static_cast<UWORD>(x << TILE_SHIFT);
          psDroid->y = static_cast<UWORD>(y << TILE_SHIFT);
          if (pickRes == HALF_FREE_TILE)
          {
            psDroid->x += TILE_UNITS;
            psDroid->y += TILE_UNITS;
  ...
  `				removeDroid(psDroid, apsDroidLists);
      ...
      else
      {
        ...
        DEBUG_ASSERT_TEXT(FALSE, "missionResetUnits: can't place unit - cancel to continue");
        vanishDroid(psDroid);
      }
    }
  }
...
void unloadTransporter(DROID* psTransporter, UDWORD x, UDWORD y, BOOL goingHome)
{
  ...
  if (psTransporter->droidType == DROID_TRANSPORTER)
  {
    ...
    for (psDroid = psTransporter->psGroup->psList; psDroid != nullptr AND psDroid != psTransporter; psDroid = psNext)
    {
      ...
      if (!pickATileGen(&droidX, &droidY,LOOK_FOR_EMPTY_TILE, zonedPAT))
        DEBUG_ASSERT_TEXT(FALSE, "unloadTransporter: Unable to find a valid location");
      psDroid->x = static_cast<UWORD>(droidX << TILE_SHIFT);
      psDroid->y = static_cast<UWORD>(droidY << TILE_SHIFT);
      psDroid->z = map_Height(psDroid->x, psDroid->y);
      updateDroidOrientation(psDroid);
...
void missionSetReinforcementTime(UDWORD iTime) { g_iReinforceTime = iTime; }

UDWORD missionGetReinforcementTime(void) { return g_iReinforceTime; }
...
void fillTimeDisplay(STRING* psText, UDWORD time, BOOL bHours)
{
  ...
  else
  {
    if (bHours)
    {
      ...
      psText[inc++] = static_cast<UBYTE>('0' + calcTime / 10);
      psText[inc++] = static_cast<UBYTE>('0' + calcTime % 10);
...
void intUpdateMissionTimer(struct _widget* psWidget, struct _w_context* psContext)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "You've taken too long for this mission!");
    return;
  }
...
static BOOL _intAddMissionResult(BOOL result, BOOL bPlaySuccess)
{
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>(0 + D_W);
  sFormInit.y = static_cast<SWORD>(0 + D_H);
  sFormInit.width = 640;
...
DROID* buildMissionDroid(DROID_TEMPLATE* psTempl, UDWORD x, UDWORD y, UDWORD player)
{
  ...
  psNewDroid = buildDroid(psTempl, x << TILE_SHIFT, y << TILE_SHIFT, player, TRUE);
  if (!psNewDroid)
    return nullptr;
...
{
  DEBUG_ASSERT_TEXT((i >= 0) && (i < MAX_NOGO_AREAS), "getLandingZone out of range.");
...
void initNoGoAreas(void)
{
  ...
  {
    if (i != LIMBO_LANDING) { sLandingZone[i].x1 = sLandingZone[i].y1 = sLandingZone[i].x2 = sLandingZone[i].y2 = 0; }
  }
}
...
void addLandingLights(UDWORD x, UDWORD y)
{
  ...
  addEffect(&pos, EFFECT_EXPLOSION, EXPLOSION_TYPE_LAND_LIGHT,FALSE, nullptr,TRUE); //middle
  pos.x = x + 128;
  pos.z = y + 128;
  pos.y = map_Height(pos.x, pos.z) + 16;
  effectSetLandLightSpec(LL_OUTER);
  addEffect(&pos, EFFECT_EXPLOSION, EXPLOSION_TYPE_LAND_LIGHT,FALSE, nullptr,FALSE); //outer
  pos.x = x + 128;
  pos.z = y - 128;
  pos.y = map_Height(pos.x, pos.z) + 16;
  effectSetLandLightSpec(LL_OUTER);
  addEffect(&pos, EFFECT_EXPLOSION, EXPLOSION_TYPE_LAND_LIGHT,FALSE, nullptr,FALSE);
  pos.x = x - 128;
  pos.z = y + 128;
  pos.y = map_Height(pos.x, pos.z) + 16;
  effectSetLandLightSpec(LL_OUTER);
  ...
  addEffect(&pos, EFFECT_EXPLOSION, EXPLOSION_TYPE_LAND_LIGHT,FALSE, nullptr,FALSE); // inner
  pos.x = x + 64;
  pos.z = y - 64;
  pos.y = map_Height(pos.x, pos.z) + 16;
  effectSetLandLightSpec(LL_INNER);
...
BOOL withinLandingZone(UDWORD x, UDWORD y)
{
  ...
  DEBUG_ASSERT_TEXT(x < mapWidth, "withinLandingZone: x coord bigger than mapWidth");
  DEBUG_ASSERT_TEXT(y < mapHeight, "withinLandingZone: y coord bigger than mapHeight");

  for (inc = 0; inc < MAX_NOGO_AREAS; inc++)
  {
    if ((x >= static_cast<UDWORD>(sLandingZone[inc].x1) AND x <= static_cast<UDWORD>(sLandingZone[inc].x2)) AND (y >= static_cast<UDWORD>(
      sLandingZone[inc].y1) AND y <= static_cast<UDWORD>(sLandingZone[inc].y2)))
      return TRUE;
  }
  return FALSE;
}
...
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_NOGO_AREAS, "getLandingX: player {} out of range", iPlayer);
  return static_cast<UWORD>((sLandingZone[iPlayer].x1 + (sLandingZone[iPlayer].x2 - sLandingZone[iPlayer].x1) / 2) << TILE_SHIFT);
}

//returns the y coord for where the Transporter can land
UWORD getLandingY(SDWORD iPlayer)
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_NOGO_AREAS, "getLandingY: player {} out of range", iPlayer);
  return static_cast<UWORD>((sLandingZone[iPlayer].y1 + (sLandingZone[iPlayer].y2 - sLandingZone[iPlayer].y1) / 2) << TILE_SHIFT);
}
...
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_PLAYERS, "missionSetTransporterEntry: player {} too high", iPlayer);
...
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_PLAYERS, "missionSetTransporterExit: player {} too high", iPlayer);
...
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_PLAYERS, "missionGetTransporterEntry: player {} too high", iPlayer);

  *iX = static_cast<UWORD>(mission.iTranspEntryTileX[iPlayer] << TILE_SHIFT);
  *iY = static_cast<UWORD>(mission.iTranspEntryTileY[iPlayer] << TILE_SHIFT);
}

void missionGetTransporterExit(SDWORD iPlayer, UWORD* iX, UWORD* iY)
{
  DEBUG_ASSERT_TEXT(iPlayer<MAX_PLAYERS, "missionGetTransporterExit: player {} too high", iPlayer);

  *iX = static_cast<UWORD>(mission.iTranspExitTileX[iPlayer] << TILE_SHIFT);
  *iY = static_cast<UWORD>(mission.iTranspExitTileY[iPlayer] << TILE_SHIFT);
}
...
void missionTimerUpdate(void)
{
  ...
  if (!mission.cheatTime)
  {
    ...
    {
      //check if time is up
      if (static_cast<SDWORD>(gameTime - mission.startTime) > mission.time)
      {
...
void setDroidsToSafetyFlag(BOOL set) { bDroidsToSafety = set; }
BOOL getDroidsToSafetyFlag(void) { return bDroidsToSafety; }

//access functions for bPlayCountDown flag - TRUE = play coded mission count down
void setPlayCountDown(UBYTE set) { bPlayCountDown = set; }
BOOL getPlayCountDown(void) { return bPlayCountDown; }
...
void moveDroidsToSafety(DROID* psTransporter)
{
  ...
  DEBUG_ASSERT_TEXT(psTransporter->droidType == DROID_TRANSPORTER, "moveUnitsToSafety: unit not a Transporter");
...
void resetMissionWidgets(void)
{
  ...
  else if (!missionForReInforcements())
  {
    ...
    if (!psDroid)
    {
      for (psDroid = mission.apsDroidLists[selectedPlayer]; psDroid != nullptr; psDroid = psDroid->psNext)
      {
        if (psDroid->droidType == DROID_TRANSPORTER AND psDroid->action == DACTION_TRANSPORTWAITTOFLYIN)
        {
          intAddTransporterLaunch(psDroid);
          break;
        }
...
{
  DEBUG_ASSERT_TEXT(number<4, "Campaign Number too high!");
  camNumber = number;
}

UDWORD getCampaignNumber(void) { return (camNumber); }
...
void emptyTransporters(BOOL bOffWorld)
{
  ...
  for (psTransporter = mission.apsDroidLists[selectedPlayer]; psTransporter != nullptr; psTransporter = psTransporter->psNext)
  {
    if (psTransporter->droidType == DROID_TRANSPORTER)
    {
