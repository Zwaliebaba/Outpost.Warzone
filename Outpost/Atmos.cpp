...
void atmosUpdateSystem(void)
{
  ...
  if (!gamePaused() AND weather != WT_NONE)
  {
    ...
    for (i = 0; i < numberToAdd; i++)
    {
      ...
      if (pos.x > 0 AND pos.z > 0 AND pos.x < static_cast<SDWORD>((mapWidth - 1) * TILE_UNITS) AND pos.z < static_cast<SDWORD>((mapHeight -
        1) * TILE_UNITS))
      {
...
void atmosAddParticle(iVector* pos, AP_TYPE type)
{
  ...
  for (i = freeParticle, activeCount = 0; (asAtmosParts[i].status == APS_ACTIVE) AND activeCount < MAX_ATMOS_PARTICLES; i++)
  {
    activeCount++;
...
// -----------------------------------------------------------------------------
WT_CLASS atmosGetWeatherType(void) { return static_cast<WT_CLASS>(weather); }
