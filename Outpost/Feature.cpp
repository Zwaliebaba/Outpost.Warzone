...
BOOL loadFeatureStats(SBYTE* pFeatureData, UDWORD bufferSize)
{
  ...
  for (i = 0; i < numFeatureStats; i++)
  {
    ...
    psFeature->baseBreadth = static_cast<UWORD>(Breadth);
    ...
    psFeature->psImd = static_cast<iIMDShape*>(resGetData("IMD", GfxFile));
...
BOOL featureDamage(FEATURE* psFeature, UDWORD damage, UDWORD weaponClass, UDWORD weaponSubClass)
{
  UDWORD penDamage;
...
FEATURE* buildFeature(FEATURE_STATS* psStats, UDWORD x, UDWORD y, BOOL FromSave)
{
  ...
  if (FromSave == TRUE)
  {
    psFeature->x = static_cast<UWORD>(x);
    psFeature->y = static_cast<UWORD>(y);
  }
  else
  {
    psFeature->x = static_cast<UWORD>((x & (~TILE_MASK)) + psStats->baseWidth * TILE_UNITS / 2);
    psFeature->y = static_cast<UWORD>((y & (~TILE_MASK)) + psStats->baseBreadth * TILE_UNITS / 2);
  }

  /* Dump down the building wrecks at random angles - still looks shit though */
  if (psStats->subType == FEAT_BUILD_WRECK)
  {
    psFeature->direction = DirectX::XMScalarModAngle(DirectX::XMConvertToRadians(static_cast<float>(rand() % 360)));
    psFeature->gfxScaling = static_cast<UWORD>(80 + (10 - rand() % 20)); // put into define
  }
  else if (psStats->subType == FEAT_TREE)
  {
    psFeature->direction = DirectX::XMScalarModAngle(DirectX::XMConvertToRadians(static_cast<float>(rand() % 360)));
    psFeature->gfxScaling = static_cast<UWORD>(100 + (14 - rand() % 28));
  }
  else
  {
    psFeature->direction = 0.0f;
  ...
  for (width = 0; width <= psStats->baseWidth; width++)
  {
    for (breadth = 0; breadth <= psStats->baseBreadth; breadth++)
    {
      ...
      if (width != psStats->baseWidth && breadth != psStats->baseBreadth)
      {
        ...
        if (psStats->subType == FEAT_GEN_ARTE OR psStats->subType == FEAT_OIL_DRUM OR psStats->subType == FEAT_BUILD_WRECK)
      ...
      {
        psTile->height = static_cast<UBYTE>(height / ELEVATION_SCALE);
...
void featureRelease(FEATURE* psFeature) { gridRemoveObject((BASE_OBJECT*)psFeature); }
...
void removeFeature(FEATURE* psDel)
{
  ...
  if (psDel->psStats->subType == FEAT_GEN_ARTE)
  {
    ...
    addEffect(&pos, EFFECT_EXPLOSION, EXPLOSION_TYPE_DISCOVERY,FALSE, nullptr, 0);
...
void destroyFeature(FEATURE* psDel)
{
  ...
  if (psDel->visible[selectedPlayer] AND psDel->psStats->damageable)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      addEffect(&pos, EFFECT_EXPLOSION, static_cast<EFFECT_TYPE>(explosionSize),FALSE, nullptr, 0);
    }
          ...
          for (breadth = 0; breadth < psDel->psStats->baseBreadth; breadth++)
          {
            ...
            if (TERRAIN_TYPE(psTile) != TER_WATER)
            {
              if (TERRAIN_TYPE(psTile) != TER_CLIFFFACE)
              {
                ...
                psTile->texture = static_cast<UWORD>(texture);
              }
              else
              {
    ...
    pos.z = psDel->y;
    pos.y = map_Height(pos.x, pos.z);
    addEffect(&pos, EFFECT_DESTRUCTION, DESTRUCTION_TYPE_FEATURE,FALSE, nullptr, 0);
