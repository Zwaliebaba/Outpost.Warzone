...
void printComponentInfo(COMP_BASE_STATS* psStats)
{
  ...
  OPRINTF(ConsoleString,
          (ConsoleString,"%s ref %d tl %d\n" "   bPwr %d bPnts %d wt %d hp %d sp %d bdy %d imd %p\n", getStatName(psStats), psStats->ref,
            psStats->techLevel, psStats->buildPower, psStats->buildPoints, psStats->weight, psStats->hitPoints, psStats->systemPoints,
            psStats->body, psStats->pIMD));
}
...
void printWeaponInfo(WEAPON_STATS* psStats)
{
  ...
  OPRINTF(ConsoleString,
          (ConsoleString,"   sRng %d lRng %d mRng %d %s\n" "   sHt %d lHt %d pause %d dam %d\n", psStats->shortRange, proj_GetLongRange(
            psStats,0), psStats->minRange, proj_Direct(psStats) ? "direct" : "indirect", weaponShortHit(psStats,static_cast<UBYTE>(
            selectedPlayer)), weaponLongHit(psStats, static_cast<UBYTE>(selectedPlayer)), weaponFirePause(psStats,static_cast<UBYTE>(
            selectedPlayer)), weaponDamage(psStats, static_cast<UBYTE>(selectedPlayer))));
  OPRINTF(ConsoleString,
          (ConsoleString,"   rad %d radHt %d radDam %d\n" "   inTime %d inDam %d inRad %d\n", psStats->radius, psStats->radiusHit, psStats->
            radiusDamage, psStats->incenTime, psStats->incenDamage, psStats->incenRadius));
  OPRINTF(ConsoleString,
          (ConsoleString,"   flSpd %d inHt %d %s\n", psStats->flightSpeed, psStats->indirectHeight, psStats->fireOnMove ? "fireOnMove" :
            "not fireOnMove"));
  ...
  OPRINTF(ConsoleString,
          (ConsoleString,"   %srotate recoil %d\n" "   dLife %d radLife %d\n", psStats->rotate ? "" : "not ", psStats->recoilValue, psStats
            ->directLife, psStats->radiusLife));
}
...
void printDroidInfo(DROID* psDroid)
{
  ...
  OPRINTF(ConsoleString,
          (ConsoleString,"   wt %d bSpeed %d sRng %d sPwr %d ECM %d bdy %d\n", psDroid->weight, psDroid->baseSpeed, psDroid->sensorRange,
  ...
  for (i = 0; i < DROID_MAXCOMP; i++)
  {
    ...
    case COMP_BODY:
      ...
      else { OPRINTF(ConsoleString, (ConsoleString,"ZNULL BODY\n")); }
      break;
    case COMP_BRAIN:
      break;
    case COMP_PROPULSION:
      ...
      else { OPRINTF(ConsoleString, (ConsoleString,"ZNULL PROPULSION\n")); }
      break;
    case COMP_ECM:
      if (psDroid->asBits[i].nStat > 0)
      {
        ...
        OPRINTF(ConsoleString,
                (ConsoleString,"   pwr %d loc %d imd %p\n", ecmPower(psECMStats, psDroid->player), psECMStats->location, psECMStats->
                  pMountGraphic));
      }
      else { OPRINTF(ConsoleString, (ConsoleString,"ZNULL ECM\n")); }
      break;
    case COMP_SENSOR:
      if (psDroid->asBits[i].nStat > 0)
      {
        ...
        OPRINTF(ConsoleString, (ConsoleString,"   rng %d pwr %d loc %d imd %p\n",
                  ...
                  sensorRange(psSensStats,psDroid->player), sensorPower(psSensStats,psDroid->player), psSensStats->location, psSensStats->
                  pMountGraphic));
      }
      else { OPRINTF(ConsoleString, (ConsoleString,"ZNULL SENSOR\n")); }
      break;
    case COMP_CONSTRUCT:
      if (psDroid->asBits[i].nStat > 0)
      {
        ...
        OPRINTF(ConsoleString,
                (ConsoleString,"   cPnts %d imd %p\n", constructorPoints(psConstStats, psDroid->player), psConstStats->pMountGraphic));
      }
      break;
    case COMP_REPAIRUNIT:
      if (psDroid->asBits[i].nStat > 0)
      {
        ...
        OPRINTF(ConsoleString, (ConsoleString,"   repPnts %d loc %d imd %p\n",
                  ...
                  repairPoints(psRepairStats, psDroid->player), psRepairStats->location, psRepairStats->pMountGraphic));
      }
      break;
    default:
      break;
