...
typedef enum _mouse_state
{
  ...
  // Normal game mode
  MS_GRABSTART,
  // LMB is down - possibly the start of a grab
  MS_GRAB,
  // Creating a grab box
  MS_GRABBED,
  // Grab box created but not copied to paste box
  MS_PASTESTART,
  // LMB is down - possibly the start of a move
  MS_PASTE,
  // Paste mode
} MOUSE_STATE;static MOUSE_STATE mState;
...
typedef struct _paste_box
{
  ...
  MAPTILE* psTiles;
} PASTE_BOX;PASTE_BOX sPasteBox, sUndoBox; // Store the undo box as a paste box.
...
BOOL ed2dProcessInput(void)
{
  ...
  if (keyDown(KEY_RCTRL)) { worldX = worldY * 2; }
  ...
  if ((keyDown(KEY_LCTRL) || keyDown(KEY_RCTRL)) && keyPressed(KEY_V) && sPasteBox.psTiles != NULL)
  {
  ...
  case MS_GRABSTART:
    ...
    if (mState == MS_GRABSTART && (worldX >> TILE_SHIFT != dragSX || worldY >> TILE_SHIFT != dragSY))
    {
      mState = MS_GRAB;
    ...
    else if (mouseDown(MOUSE_RMB))
    {
      disp2DToWorld(mouseX(), mouseY(), &worldX, &worldY);
      if (showTerrain) {}
      else { mapTile(worldX >> TILE_SHIFT, worldY >> TILE_SHIFT)->texture = (UWORD)currTile; }
    }
    break;
  ...
  case MS_GRAB:
    ...
    if (mouseReleased(MOUSE_LMB)) { mState = MS_GRABBED; }
    break;
  case MS_GRABBED:
  case MS_PASTESTART:
  ...
  case MS_PASTE:
    ...
    if (mouseDown(MOUSE_LMB))
    {
      ...
      if (tileX > sPasteBox.width >> 1) { selSX = tileX - (sPasteBox.width >> 1); }
      else { selSX = 0; }
      if (tileY > sPasteBox.height >> 1) { selSY = tileY - (sPasteBox.height >> 1); }
      else { selSY = 0; }
      ...
      else { selEX = selSX + sPasteBox.width - 1; }
      if (selSY + sPasteBox.height >= mapHeight)
      {
        selSY = mapHeight - sPasteBox.height;
        selEY = mapHeight - 1;
      }
      else { selEY = selSY + sPasteBox.height - 1; }
    }
    break;
  }

  if (keyPressed(KEY_T)) { showTerrain = !showTerrain; }
  ...
  {
    /* Flip on X axis */
    if (keyPressed(KEY_X)) { flipBoxX(&sPasteBox); }
    /* Flip on Y axis */
    if (keyPressed(KEY_Y)) { flipBoxY(&sPasteBox); }
    /* Rotate */
    if (keyPressed(KEY_Z))
    {
      rotBox(&sPasteBox);
  ...
  {
    if (keyPressed(KEY_MINUS)) { if (currTile > 0) { currTile = (currTile & TILE_NUMMASK) - 1; } }
    if (keyPressed(KEY_EQUALS)) { if ((currTile & TILE_NUMMASK) < maxTexTile) { currTile = (currTile & TILE_NUMMASK) + 1; } }
    if (keyPressed(KEY_X)) { currTile = (currTile & TILE_NUMMASK) | (currTile & ~TILE_NUMMASK) ^ TILE_XFLIP; }
    if (keyPressed(KEY_Y)) { currTile = (currTile & TILE_NUMMASK) | (currTile & ~TILE_NUMMASK) ^ TILE_YFLIP; }
    if (keyPressed(KEY_Z))
    {
      if ((currTile & TILE_ROTMASK) < (3 << TILE_ROTSHIFT))
      {
        currTile = (currTile & ~TILE_ROTMASK) | ((currTile & TILE_ROTMASK) + (1 << TILE_ROTSHIFT));
      }
      else { currTile = (currTile & ~TILE_ROTMASK); }
    }
  }
...
void ed2dDisplay(void)
{
  ...
  if (mState == MS_PASTE)
  {
    ...
    DEBUG_ASSERT_TEXT(sx0 >= (SDWORD)viewX && sx1 <= ((SDWORD)viewX + TILES_ACROSS) &&
      sy0 >= (SDWORD)viewY && sy1 <= ((SDWORD)viewY + TILES_DOWN) && sx1 > sx0 && sy1 > sy0, "paste Box: clipping failed");

    /* Display the tiles */
    for (y = py0; y < py1; y++)
    {
      disp2DFromWorld(sx0 << TILE_SHIFT, (sy0 + y - py0) << TILE_SHIFT, (SDWORD*)&sDestRect.left, (SDWORD*)&sDestRect.top);
...
static BOOL getBox(PASTE_BOX* psBox, UDWORD x, UDWORD y, UDWORD width, UDWORD height)
{
  ...
  DEBUG_ASSERT_TEXT(x + width <= mapWidth, "getBox: box off map");
  DEBUG_ASSERT_TEXT(y + height <= mapHeight, "getBox: box off map");
...
static void putBox(PASTE_BOX* psBox, UDWORD x, UDWORD y)
{
  ...
  DEBUG_ASSERT_TEXT(x + psBox->width <= mapWidth, "putBox: box off map");
  DEBUG_ASSERT_TEXT(y + psBox->height <= mapHeight, "putBox: box off map");
...
BOOL ed2dLoadMapFile(void)
{
  ...
  screenFlipToGDI();
  if (!GetOpenFileName(&sOFN)) { goto error; }

  /* Load in the chosen file data */
  if (!loadFile(aFileName, &pFileData, &fileSize)) { goto error; }
...
BOOL ed2dSaveMapFile(void)
{
  ...
  /* make sure we're on the gdi screen */
  screenFlipToGDI();
  if (!GetSaveFileName(&sOFN)) { goto error; }

  /* Write the data to the file */
  if (!writeMapFile(aFileName)) { goto error; }
