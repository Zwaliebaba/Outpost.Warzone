...
using Pixel = int; /* 1-channel frame buffer assumed */
...
BOOL gwProcessMap(void)
{
  ...
  do
  {
    ...
    if (psCurr->zone1 == 0)
    {
      psCurr->zone1 = static_cast<UBYTE>(gatewayZone);
      // the gateway is always in it's own zone1
      gwSetGatewayZone(psCurr, gatewayZone);
    }
    else
      psCurr->zone2 = static_cast<UBYTE>(gatewayZone);
  ...
  for (y = 0; y < gwMapHeight(); y += 1)
  {
    ...
    {
      if (gwTileIsWater(x, y) && gwGetZone(x, y) == 0)
      {
        // check the zones havn't overflowed
        if (currZone > UBYTE_MAX)
        {
      ...
      else if (!gwFloodBlock(x, y) && gwGetZone(x, y) == 0)
      {
        // check the zones havn't overflowed
        if (currZone > UBYTE_MAX)
        {
...
BOOL gwCreateBlankZoneMap(void)
{
  ...
  for (i = 0; i < gwMapHeight(); i++)
  {
    *apRLEZones[i] = static_cast<UBYTE>(gwMapWidth());
    *(apRLEZones[i] + 1) = 0;
  }
...
void gwCompressLine(SDWORD line, UBYTE* pBuffer)
{
  ...
  while (bufPos < gwMapWidth())
  {
    ...
    *(apRLEZones[line] + rlePos) = static_cast<UBYTE>(count);
    *(apRLEZones[line] + rlePos + 1) = static_cast<UBYTE>(zone);
...
void gwSetZone(SDWORD x, SDWORD y, SDWORD zone)
{
  ...
  aBuffer[x] = static_cast<UBYTE>(zone);
...
BOOL gwFloodBlock(SDWORD x, SDWORD y)
{
  if ((x < 0) || (x >= gwMapWidth()) || (y < 0) || (y >= gwMapHeight())) { return TRUE; }

  return giIsGateway(x, y) || (!bGwWaterFlood && (giIsClifface(x, y) || giIsWater(x, y))) || (bGwWaterFlood && !giIsWater(x, y));
}
...
BOOL gwFloodBlock(SDWORD x, SDWORD y)
{
  ...
  return gateway || (!bGwWaterFlood && ((type == TER_CLIFFFACE) || (type == TER_WATER))) || (bGwWaterFlood && (type != TER_WATER));
}
