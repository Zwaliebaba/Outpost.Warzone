...
static SDWORD visObjRadius(BASE_OBJECT* psObject)
{
  ...
  case OBJ_FEATURE:
    radius = psObject->sDisplay.imd->radius;
    break;
  default: ASSERT((FALSE,"visObjRadius: unknown object type"));
    radius = 0;
    break;
  }
...
static SDWORD visObjHeight(BASE_OBJECT* psObject)
{
  ...
  case OBJ_FEATURE:
    height = psObject->sDisplay.imd->ymax;
    break;
  default: ASSERT((FALSE,"visObjHeight: unknown object type"));
    height = 0;
    break;
  }
...
static BOOL rayTerrainCallback(SDWORD x, SDWORD y, SDWORD dist)
{
  ...
  ASSERT((x >= 0 && x < (static_cast<SDWORD>(mapWidth) << TILE_SHIFT) &&
...
void visTilesUpdate(BASE_OBJECT* psObj, BOOL SpreadLoad)
{
  ...
  case OBJ_STRUCTURE: // Only done when structure initialy built.
    ...
    break;
  default: ASSERT((FALSE, "visTilesUpdate: visibility checking is only implemented for" "units and structures"));
    return;
  }
...
BOOL visibleObject(BASE_OBJECT* psViewer, BASE_OBJECT* psTarget)
{
  ...
  case OBJ_STRUCTURE:
    ...
    break;
  default: ASSERT((FALSE, "visibleObject: visibility checking is only implemented for" "units and structures"));
    return FALSE;
    break;
  }
...
void visGetRayObjects(SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2)
{
  ...
  for (i = 0; i < numRayObjects; i++)
  {
    ...
    ASSERT((furthest != -1, "visGetRayObjects: reordering failed"));
