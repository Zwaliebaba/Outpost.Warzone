 ...
 */
void visGetTestObjects(SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2, BASE_OBJECT* psSource, BASE_OBJECT* psTarget);
...
static SDWORD visObjRadius(BASE_OBJECT* psObject)
{
  ...
  case OBJ_FEATURE:
    radius = psObject->sDisplay.imd->radius;
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "visObjRadius: unknown object type");
    radius = 0;
    break;
  }
...
static SDWORD visObjHeight(BASE_OBJECT* psObject)
{
  ...
  case OBJ_FEATURE:
    height = psObject->sDisplay.imd->ymax;
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "visObjHeight: unknown object type");
    height = 0;
    break;
  }
...
static BOOL rayTerrainCallback(SDWORD x, SDWORD y, SDWORD dist)
{
  ...
  DEBUG_ASSERT_TEXT(x >= 0 && x < (static_cast<SDWORD>(mapWidth) << TILE_SHIFT) &&
    y >= 0 && y < (static_cast<SDWORD>(mapHeight) << TILE_SHIFT), "rayTerrainCallback: coords off map");
  ...
  if (dist == 0)
  {
    ...
    Neuron::DebugTrace("rayTerrainCallback: dist == 0, will divide by zero\n");
    dist = 1;
  }
  ...
  if (newG >= currG)
  {
    ...
    // new - ask alex M
    if ((selectedPlayer != rayPlayer) AND (bMultiPlayer && game.type == TEAMPLAY && aiCheckAlliances(selectedPlayer, rayPlayer)))
    ...
    if (getRevealStatus())
    {
      if ((static_cast<UDWORD>(rayPlayer) == selectedPlayer) OR
        // new - ask AM
...
static BOOL rayLOSCallback(SDWORD x, SDWORD y, SDWORD dist)
{
  ...
  DEBUG_ASSERT_TEXT(x >= 0 && x < (static_cast<SDWORD>(mapWidth) << TILE_SHIFT) &&
    y >= 0 && y < (static_cast<SDWORD>(mapHeight) << TILE_SHIFT), "rayLOSCallback: coords off map");
  ...
  {
    psTile = mapTile(x >> TILE_SHIFT, y >> TILE_SHIFT);
    if ((psTile->tileInfoBits & BITS_WALL) && !TILE_HAS_SMALLSTRUCTURE(psTile))
    {
      ...
      lastH = map_Height(static_cast<UDWORD>(x), static_cast<UDWORD>(y));
  }
  else
    lastH = map_Height(static_cast<UDWORD>(x), static_cast<UDWORD>(y));
  lastD = dist;
...
{
  DEBUG_ASSERT_TEXT(psObj->type == OBJ_DROID, "visTilesPending : Only implemented for droids");
...
void visTilesUpdate(BASE_OBJECT* psObj, BOOL SpreadLoad)
{
  ...
  case OBJ_STRUCTURE: // Only done when structure initialy built.
    DEBUG_ASSERT_TEXT(SpreadLoad == FALSE, "visTilesUpdate : Can only spread load for droids"); // can't spread load for structures.
    range = ((STRUCTURE*)psObj)->sensorRange;
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "visTilesUpdate: visibility checking is only implemented for" "units and structures");
    return;
  }
  ...
  if (SpreadLoad)
  {
    ...
    currRayAng = static_cast<SDWORD>(psDroid->currRayAng);
...
BOOL visibleObject(BASE_OBJECT* psViewer, BASE_OBJECT* psTarget)
{
  ...
  case OBJ_STRUCTURE:
    ...
    if ((structCBSensor((STRUCTURE*)psViewer) || structVTOLCBSensor((STRUCTURE*)psViewer)) && ((STRUCTURE*)psViewer)->psTarget == psTarget)
    {
    ...
    if ((psStruct->pStructureType->type == REF_DEFENSE) && (asWeaponStats[psStruct->asWeaps[0].nStat].surfaceToAir == SHOOT_IN_AIR))
      range = 3 * range / 2;

    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "visibleObject: visibility checking is only implemented for" "units and structures");
    return FALSE;
    break;
  }
  ...
  */
  ...
  x = static_cast<SDWORD>(psViewer->x);
  xdiff = x - static_cast<SDWORD>(psTarget->x);
  if (xdiff < 0)
    xdiff = -xdiff;
  if (xdiff > range)
  {
  ...
  y = static_cast<SDWORD>(psViewer->y);
  ydiff = y - static_cast<SDWORD>(psTarget->y);
  if (ydiff < 0)
    ydiff = -ydiff;
  if (ydiff > range)
  {
  ...
  top = (static_cast<SDWORD>(psTarget->z) + visObjHeight(psTarget) - startH);
...
BOOL visGetBlockingWall(BASE_OBJECT* psViewer, BASE_OBJECT* psTarget, STRUCTURE** ppsWall)
{
  ...
  if (numWalls == 1)
  {
    ...
    for (player = 0; player < MAX_PLAYERS; player += 1)
    {
      ...
      {
        if (((psCurr->x >> TILE_SHIFT) == tileX) && ((psCurr->y >> TILE_SHIFT) == tileY))
        {
          psWall = psCurr;
          goto found;
        }
...
void processVisibility(BASE_OBJECT* psObj)
{
  ...
  case OBJ_STRUCTURE:
    ...
    if (psECMStats && psECMStats->power > maxPower)
      psBuilding->ecmPower = static_cast<UWORD>(psECMStats->power);
    else
    {
      psBuilding->ecmPower = static_cast<UWORD>(maxPower);
      maxPower = psBuilding->ecmPower;
    }
    break;
  case OBJ_FEATURE: default:
  ...
  gridStartIterate(psObj->x, psObj->y);
  ...
  while (psViewer != nullptr)
  {
    // If we've got ranged line of sight...
    if ((psViewer->type != OBJ_FEATURE) && !currVis[psViewer->player] && visibleObject(psViewer, psObj))
    {
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    if ((visLevel < psObj->visible[i]) && (psObj->type == OBJ_DROID))
    {
      if (psObj->visible[i] <= visLevelDec)
        psObj->visible[i] = 0;
      else
        psObj->visible[i] = static_cast<UBYTE>(psObj->visible[i] - visLevelDec);
    }
    else if (visLevel > psObj->visible[i])
    {
      if (psObj->visible[i] + visLevelInc >= UBYTE_MAX)
        ...
        psObj->visible[i] = static_cast<UBYTE>(psObj->visible[i] + visLevelInc);
    }
  }
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    if ((psObj->type == OBJ_STRUCTURE OR psObj->type == OBJ_FEATURE) AND (!prevVis[i] AND psObj->visible[i]))
  ...
  if (psObj->type == OBJ_FEATURE && !prevVis[selectedPlayer] && psObj->visible[selectedPlayer])
  {
    ...
    {
      if (!TILE_HAS_STRUCTURE(mapTile(psObj->x >> TILE_SHIFT, psObj->y >> TILE_SHIFT)))
      {
...
void visGetTestObjects(SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2, BASE_OBJECT* psSource, BASE_OBJECT* psTarget)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psObj = (BASE_OBJECT*)apsStructLists[player]; psObj; psObj = psObj->psNext)
    {
      ...
      if (((static_cast<SDWORD>(psObj->x) + radius) >= x1) && ((static_cast<SDWORD>(psObj->x) - radius) <= x2) && ((static_cast<SDWORD>(
          psObj->y) + radius) >= y1) && ((static_cast<SDWORD>(psObj->y) - radius) <= y2) && psObj != psSource && psObj != psTarget &&
  ...
  {
    radius = visObjRadius(psObj);
    if (((static_cast<SDWORD>(psObj->x) + radius) >= x1) && ((static_cast<SDWORD>(psObj->x) - radius) <= x2) && ((static_cast<SDWORD>(psObj
        ->y) + radius) >= y1) && ((static_cast<SDWORD>(psObj->y) - radius) <= y2) && psObj != psSource && psObj != psTarget &&
      numTestObjects
      < MAX_RAYTEST)
      apsTestObjects[numTestObjects++] = psObj;
  }
...
void visGetRayObjects(SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2)
{
  ...
  for (i = 0; i < numTestObjects; i++)
  {
    ...
    x = static_cast<SDWORD>(psObj->x);
    y = static_cast<SDWORD>(psObj->y);
    dist = rayPointDist(x1, y1, x2, y2, x, y);
    if (dist < visObjRadius(psObj) && numRayObjects < MAX_RAYOBJ)
    {
  ...
  for (i = 0; i < numRayObjects; i++)
  {
    ...
    DEBUG_ASSERT_TEXT(furthest != -1, "visGetRayObjects: reordering failed");
...
void startSensorDisplay()
{
  ...
  psTile = psMapTiles;
  for (x = 0; x < static_cast<SDWORD>(mapWidth * mapHeight); x += 1)
  {
    psTile->inRange = 0;
    psTile += 1;
  }
