...
UDWORD adjustDirection(SDWORD present, SDWORD difference)
{
  ...
  if (sum >= 0 AND sum <= 360) { return (UDWORD)(sum); }

  if (sum < 0) { return (UDWORD)(360 + sum); }

  if (sum > 360) { return (UDWORD)(sum - 360); }
}
...
BOOL formationNew(FORMATION** ppsFormation, FORMATION_TYPE type, SDWORD x, SDWORD y, SDWORD dir)
{
  ...
  for (i = 0; i < F_MAXMEMBERS; i++)
    psNew->asMembers[i].next = static_cast<SBYTE>(i + 1);
...
void formationUpdateSpeed(FORMATION* psFormation, BASE_OBJECT* psNew)
{
  ...
  if (psNew != nullptr && psNew->type == OBJ_DROID)
  {
    psDroid = (DROID*)psNew;
  ...
  {
    if (asMembers[iUnit].psObj && asMembers[iUnit].psObj->type == OBJ_DROID)
    {
...
void formationJoin(FORMATION* psFormation, BASE_OBJECT* psObj)
{
  ...
  ASSERT((PTRVALID(psFormation, sizeof(FORMATION)), "formationJoin: invalid formation"));
  ...
  if (psFormation->rankDist < rankDist)
    psFormation->rankDist = static_cast<SWORD>(rankDist);

  size = formationObjRadius(psObj) * 4;
  if (psFormation->size < size)
    psFormation->size = static_cast<SWORD>(size);
...
void formationLeave(FORMATION* psFormation, BASE_OBJECT* psObj)
{
  ...
  ASSERT((PTRVALID(psFormation, sizeof(FORMATION)), "formationLeave: invalid formation"));
  ...
  if (unit < F_MAXMEMBERS)
  {
    ...
    asMembers[unit].psObj = nullptr;
    psFormation->free = static_cast<SBYTE>(unit);
...
void formationFindFree(FORMATION* psFormation, BASE_OBJECT* psObj, SDWORD* pX, SDWORD* pY)
{
  ...
  *pY = psFormation->y;
  ...
  for (line = 0; line < psFormation->numLines; line++)
  {
    ...
    xdiff = x - static_cast<SDWORD>(psObj->x);
    ydiff = y - static_cast<SDWORD>(psObj->y);
  ...
  asMembers[unit].line = static_cast<SBYTE>(chosenLine);
  asMembers[unit].dist = static_cast<SWORD>(chosenDist);
  ...
  if (chosenPrev == -1)
  {
    ...
    asLines[chosenLine].member = static_cast<SBYTE>(unit);
  }
  else
  {
    asMembers[unit].next = asMembers[chosenPrev].next;
    asMembers[chosenPrev].next = static_cast<SBYTE>(unit);
  }
}
...
void formationReorder(FORMATION* psFormation)
{
  ...
  for (i = 0; i < F_MAXMEMBERS; i++)
  {
    ...
    if (psObj != nullptr)
    {
      ...
      xdiff = static_cast<SDWORD>(psObj->x) - psFormation->x;
      ydiff = static_cast<SDWORD>(psObj->y) - psFormation->y;
  ...
  for (i = 0; i < numObj; i++)
  {
    ...
    else
    {
      ...
      {
        // insert at the start of the list
        asObjects[i].next = static_cast<SBYTE>(insert);
        insert = i;
      }
      else
      {
        asObjects[i].next = asObjects[prev].next;
        asObjects[prev].next = static_cast<SBYTE>(i);
      }
    }
  }
  ...
  for (i = 0; i < F_MAXMEMBERS; i++)
    psFormation->asMembers[i].next = static_cast<SBYTE>(i + 1);
...
BOOL formationGetPos(FORMATION* psFormation, BASE_OBJECT* psObj, SDWORD* pX, SDWORD* pY, BOOL bCheckLOS)
{
  SDWORD xdiff, ydiff, distSq; //,rangeSq;
  SDWORD member, x, y;
  F_MEMBER* asMembers;

  ASSERT((PTRVALID(psFormation, sizeof(FORMATION)), "formationGetPos: invalid formation pointer"));
  ...
  xdiff = psFormation->x - static_cast<SDWORD>(psObj->x);
  ydiff = psFormation->y - static_cast<SDWORD>(psObj->y);
  distSq = xdiff * xdiff + ydiff * ydiff;
  ...
  if (bCheckLOS && !fpathTileLOS(psObj->x >> TILE_SHIFT, psObj->y >> TILE_SHIFT, x >> TILE_SHIFT, y >> TILE_SHIFT))
    return FALSE;
