...
// ------------------------------------------------------------------------------------
void avSetStatus(BOOL var) { bRevealActive = var; }
...
void avUpdateTiles(void)
{
  ...
  if (endX > static_cast<SDWORD>(mapWidth - 1))
    endX = static_cast<SDWORD>(mapWidth - 1);
  if (endY > static_cast<SDWORD>(mapHeight - 1))
    endY = static_cast<SDWORD>(mapHeight - 1);
...
void processAVTile(UDWORD x, UDWORD y)
{
  ...
  if (newLevel >= psTile->illumination)
  {
    ...
    psTile->level = static_cast<UBYTE>(newLevel);
}
...
// ------------------------------------------------------------------------------------
BOOL getRevealStatus(void) { return (bRevealActive); }

// ------------------------------------------------------------------------------------
void setRevealStatus(BOOL val) { bRevealActive = val; }
// ------------------------------------------------------------------------------------
