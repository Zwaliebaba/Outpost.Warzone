...
static void intDisplayMessageButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours);
...
static void intDisplayPIEView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours);
static void intDisplayFLICView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours);
static void intDisplayTEXTView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours);
...
BOOL _intAddIntelMap(void)
{
  ...
  sFormInit.x = static_cast<SWORD>(INTMAP_X);
  sFormInit.y = static_cast<SWORD>(INTMAP_Y);
...
static BOOL intAddMessageForm(BOOL playCurrent)
{
  ...
  for (psMessage = apsMessages[selectedPlayer]; psMessage; psMessage = psMessage->psNext)
  {
  ...
  sFormInit.numMajor = numForms((OBJ_BUTWIDTH + OBJ_GAP) * numButtons, (OBJ_WIDTH - OBJ_GAP) * 2);
...
BOOL intAddMessageView(MESSAGE* psMessage)
{
  ...
  ASSERT((psMessage->type == MSG_RESEARCH, "intAddMessageView: invalid message type"));
  ...
  sFormInit.x = static_cast<SWORD>(INTMAP_RESEARCHX);
  sFormInit.y = static_cast<SWORD>(INTMAP_RESEARCHY);
  ...
  sButInit.x = static_cast<SWORD>(sFormInit.width - OPT_GAP - CLOSE_SIZE);
  sButInit.y = OPT_GAP;
  ...
  ASSERT((psMessage->type != MSG_PROXIMITY, "intAddMessageView:Invalid message type for research"));
  ...
  sFormInit.pUserData = static_cast<void*>(psMessage);
  if (!widgAddForm(psWScreen, &sFormInit))
    return FALSE;
...
void StartMessageSequences(MESSAGE* psMessage, BOOL Start)
{
  ...
  if (((VIEWDATA*)psMessage->pViewData)->type == VIEW_RPL)
  {
    ...
    psViewReplay = static_cast<VIEW_REPLAY*>(((VIEWDATA*)psMessage->pViewData)->pData);
  ...
  else if (((VIEWDATA*)psMessage->pViewData)->type == VIEW_RES)
  {
    ...
    psViewReplay = static_cast<VIEW_RESEARCH*>(((VIEWDATA*)psCurrentMsg->pViewData)->pData);

    seq_ClearSeqList();
    seq_AddSeqToList(psViewReplay->sequenceName, psViewReplay->pAudio, nullptr, FALSE, 0);
    //play first full screen video
    if (Start == TRUE)
      seq_StartNextFullScreenVideo();
  }
...
MESSAGE TimsMessage = {MSG_RESEARCH,NULL,NULL,FALSE, 0};
#endif
...
void _intIntelButtonPressed(BOOL proxMsg, UDWORD id)
{
  ...
  ASSERT((proxMsg = TRUE, "intIntelButtonPressed: Shouldn't be able to get a proximity message!"));
...
#warning DEBUG MESSAGE CODE ADDED ---- REMOVE BEFORE FINAL BUILD !!!!!!
  ButtonPresses++; if (ButtonPresses >= 2) // 2nd time only !!!
  {
...
void intCleanUpIntelMap(void)
{
  ...
  for (psMessage = apsMessages[selectedPlayer]; psMessage != nullptr; psMessage = psNext)
  {
...
void intRemoveMessageView(BOOL animated)
{
  ...
  {
    //stop the video
    psViewResearch = static_cast<VIEW_RESEARCH*>(Form->pUserData);
    seq_RenderVideoToBuffer(nullptr, psViewResearch->sequenceName, gameTime2, SEQUENCE_KILL);
...
/*Displays the buttons used on the intelligence map */
void intDisplayMessageButton(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  auto psButton = (W_CLICKFORM*)psWidget;
  auto psBuffer = static_cast<RENDERED_BUTTON*>(psButton->pUserData);
  MESSAGE* psMsg;
  BOOL Hilight = FALSE;
  ...
  OpenButtonRender(static_cast<UWORD>(xOffset + psButton->x), static_cast<UWORD>(yOffset + psButton->y), psButton->width, psButton->height);
  ...
  psMsg = static_cast<MESSAGE*>(psBuffer->Data);
  ...
  if (pResearch)
  {
    ...
    if (image > 0)
    {
      ...
      RenderToButton(IntImages, static_cast<UWORD>(image), psResGraphic, selectedPlayer, psBuffer, Down, IMDType,TOPBUTTON);
    }
    else
      RenderToButton(nullptr, 0, pResearch, selectedPlayer, psBuffer, Down, IMDType,TOPBUTTON); //ajl, changed from 0 to selectedPlayer
  }
  else
  //draw buttons for mission and general messages
  {
    if (image > 0)
    {
      if (MovieButton)
      {
        ...
        // a movie button.
        RenderImageToButton(IntImages, static_cast<UWORD>(image), psBuffer, Down,TOPBUTTON);
      }
      else
      {
        //draw the button with the relevant image
        RenderImageToButton(IntImages, static_cast<UWORD>(image + Down), psBuffer, Down,TOPBUTTON);
      }
    }
  }
...
void intDisplayPIEView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  auto Form = (W_TABFORM*)psWidget;
  auto psMessage = static_cast<MESSAGE*>(Form->pUserData);
  UDWORD x0, y0, x1, y1;
  VIEW_RESEARCH* psViewResearch;
  SWORD image = -1;
  RESEARCH* psResearch;
...
void intDisplayFLICView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  auto Form = (W_TABFORM*)psWidget;
  auto psMessage = static_cast<MESSAGE*>(Form->pUserData);
  UDWORD x0, y0, x1, y1;
...
void intDisplayTEXTView(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  auto Form = (W_TABFORM*)psWidget;
  auto psMessage = static_cast<MESSAGE*>(Form->pUserData);
...
void addVideoText(SEQ_DISPLAY* psSeqDisplay, UDWORD sequence)
{
  ...
  if (psSeqDisplay->numText > 0)
  {
    ...
    {
      seq_AddTextForVideo((UBYTE*)psSeqDisplay->ppTextMsg[i], x, y, TEXT_START_FRAME, TEXT_END_FRAME, FALSE, sequence);
      //startframe endFrame
...
void setCurrentMsg(void)
{
  ...
  psLastMsg = nullptr;
  for (psMsg = apsMessages[selectedPlayer]; psMsg != nullptr; psMsg = psMsg->psNext)
  {
    if (psMsg->type != MSG_PROXIMITY)
      psLastMsg = psMsg;
  }
...
void displayImmediateMessage(MESSAGE* psMessage) { _displayImmediateMessage(psMessage); }

// return whether a message is immediate
BOOL messageIsImmediate(void) { return immediateMessage; }

/*sets the flag*/
void setMessageImmediate(BOOL state) { immediateMessage = state; }

BOOL intAddIntelMap(void) { return _intAddIntelMap(); }

void intIntelButtonPressed(BOOL proxMsg, UDWORD id) { _intIntelButtonPressed(proxMsg, id); }
