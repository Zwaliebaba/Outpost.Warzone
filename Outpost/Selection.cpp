...
UDWORD selDroidSelection(UDWORD player, SELECTION_CLASS droidClass, SELECTIONTYPE droidType, BOOL bOnScreen);
...
UDWORD selSelectAllSameProp(UDWORD player, PROPULSION_TYPE propType, BOOL bOnScreen);
...
DROID* psOldRD = nullptr; // pointer to last selected repair unit
DROID* psOldNS = nullptr;
STRUCTURE* psOldStruct = nullptr;
...
UDWORD selDroidSelection(UDWORD player, SELECTION_CLASS droidClass, SELECTIONTYPE droidType, BOOL bOnScreen)
{
  ...
  case DS_BY_TYPE:
    ...
    case DST_ALL_SAME:
      ...
      break;
    default: ASSERT((FALSE,"Invalid selection type in uniDroidSelection"));
    }
    break;
  default: ASSERT((FALSE,"Invalid selection attempt in uniDroidSelection"));
    break;
  }
...
UDWORD selSelectAllUnits(UDWORD player, BOOL bOnScreen)
{
  ...
  /* Go thru' all */
  for (psDroid = apsDroidLists[player], count = 0; psDroid; psDroid = psDroid->psNext)
  {
...
void selNextSpecifiedUnit(UDWORD unitType)
{
  ...
  for (psCurr = apsDroidLists[selectedPlayer], psFirst = nullptr, psResult = nullptr, bLaterInList = FALSE; psCurr AND !psResult; psCurr =
       psCurr->psNext)
  {
    ...
    {
      if (psCurr->droidType == DROID_CONSTRUCT OR psCurr->droidType == DROID_CYBORG_CONSTRUCT)
        bMatch = TRUE;
    }
    else if (unitType == DROID_REPAIR)
    {
      if (psCurr->droidType == DROID_REPAIR OR psCurr->droidType == DROID_CYBORG_REPAIR)
        bMatch = TRUE;
    }
    else if (psCurr->droidType == static_cast<SDWORD>(unitType))
      bMatch = TRUE;
    if (bMatch)
    {
...
void selNextSpecifiedBuilding(UDWORD structType)
{
  ...
  ASSERT((structType>=REF_HQ AND structType<=NUM_DIFF_BUILDINGS, "Invalid structure type in selNextSpecifiedBuilding"));
...
void selCommander(SDWORD n)
{
  ...
  for (psCurr = apsDroidLists[selectedPlayer]; psCurr; psCurr = psCurr->psNext)
  {
    if (droidIsCommanderNum(psCurr, n))
    {
      ...
      setSelectedCommander(static_cast<UDWORD>(n));
      return;
    }
  }
}
