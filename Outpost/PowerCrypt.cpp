...
using PWRC_STORE = struct _pwrc_store
{
  UDWORD key; // xor key
...
void pwrcSetPlayerCryptPower(UDWORD player, UDWORD power)
{
  ...
  DEBUG_ASSERT_TEXT(player < MAX_PLAYERS, "pwrcSetPlayerCryptPower: invalid player number");

  asPCrypt[player].pad1 = static_cast<UWORD>(rand());
  asPCrypt[player].key = static_cast<UDWORD>(rand());
  asPCrypt[player].xorHigh = static_cast<UWORD>((power >> 16) ^ (asPCrypt[player].key >> 16));
  asPCrypt[player].xorLow = static_cast<UWORD>(power ^ asPCrypt[player].key);
  asPCrypt[player].pad2 = static_cast<UBYTE>(rand());
...
{
  DEBUG_ASSERT_TEXT(player < MAX_PLAYERS, "pwrcGetPlayerCryptPower: invalid player number");
...
BOOL pwrcCheckPlayerCryptPower(UDWORD player, UDWORD power)
{
  ...
  DEBUG_ASSERT_TEXT(player < MAX_PLAYERS, "pwrcCheckPlayerCryptPower: invalid player number");
  ...
  res = static_cast<UWORD>((power >> 16) ^ (asPCrypt[player].key >> 16));
  if (res != asPCrypt[player].xorHigh)
    match = FALSE;

  res = static_cast<UWORD>(power ^ asPCrypt[player].key);
...
void pwrcUpdate(void)
{
  ...
  {
    if ((i == static_cast<SDWORD>(selectedPlayer)) && !asPCrypt[i].valid && (asPCrypt[i].lastSent + PWRC_MESSAGE_PAUSE < gameTime))
    {
