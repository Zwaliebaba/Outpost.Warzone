...
using CHEAT_ENTRY = struct _cheat_entry
{
  STRING* pName;
...
CHEAT_ENTRY cheatCodes[] = {
  ...
  {"KVRZ KPXXSZ", kf_ToggleMissionTimer}, {"HPMT W^M[ZM", kf_FinishResearch}, {"\\^MPS IPM[ZMR^Q", kf_NoFaults}, //carol vorderman
  {"end of list", nullptr}
};
...
unsigned char* xorString(unsigned char* string)
{
  ...
  {
    if (*pReturn > 32)
      *pReturn = static_cast<UBYTE>(*pReturn ^ 0x3f);
    pReturn++;
  }
  return (string);
}

void setCheatCodeStatus(BOOL val) { bAllowCheatCodes = val; }

BOOL getCheatCodeStatus(void) { return (bAllowCheatCodes); }
...
BOOL attemptCheatCode(STRING* pName)
{
  ...
  while (cheatCodes[index].function != nullptr)
  {
