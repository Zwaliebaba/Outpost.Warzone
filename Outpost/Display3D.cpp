...
void drawTexturedTile(UDWORD i, UDWORD j)
{
  ...
  if ((realX < 0) OR (realY < 0) OR (realX > mapWidth - 2) OR (realY > mapHeight - 2))
  {
    psTile = &edgeTile;
    bEdgeTile = TRUE;
...
void renderProximityMsg(PROXIMITY_DISPLAY* psProxDisp)
{
  ...
  if (pViewProximity)
  {
    ...
    case PROX_ARTEFACT:
      ...
      break;
    default: ASSERT((FALSE,"Buggered proximity message type"));
      break;
    }
  }
  else
  {
    ...
    ASSERT(( ((BASE_OBJECT *)psProxDisp->psMessage->pViewData)->type == OBJ_FEATURE, "renderProximityMsg: invalid feature" ));
...
void renderDeliveryPoint(FLAG_POSITION* psPosition)
{
  ...
  ASSERT((psPosition->factoryType < NUM_FLAG_TYPES AND psPosition->factoryInc < MAX_FACTORY, "Invalid assembly point"));
...
void drawWeaponReloadBar(BASE_OBJECT* psObj, WEAPON* psWeap)
{
    ...
    vtolDroid((DROID*)psObj)))
  {
    ...
    case OBJ_STRUCTURE:
      ...
      scrR = scale * 20;
      break;
    default: ASSERT((FALSE, "drawWeaponReloadBars: invalid object type"));
      damLevel = 100;
      break;
    }
...
void drawDroidSelections(void)
{
  ...
  case BAR_DOT:
    ...
    bTinyBars = TRUE;
    break;
  default: ASSERT((FALSE,"Invalid energy bar display value"));
    break;
  }
...
iIMDShape* flattenImd(iIMDShape* imd, UDWORD structX, UDWORD structY, UDWORD direction)
{
  ...
  ASSERT((imd->npoints < MAX_FLATTEN_POINTS, "flattenImd: too many points in the PIE to flatten it"));
...
void showDroidSensorRanges(void)
{
  ...
  for (psDroid = apsDroidLists[selectedPlayer]; psDroid; psDroid = psDroid->psNext) { showSensorRange2((BASE_OBJECT*)psDroid); }

  for (psStruct = apsStructLists[selectedPlayer]; psStruct; psStruct = psStruct->psNext) { showSensorRange2((BASE_OBJECT*)psStruct); }
}
...
UDWORD getDroidRankGraphic(DROID* psDroid)
{
  ...
  case 8:
    gfxId = IMAGE_LEV_7;
    break;
  default: ASSERT((FALSE, "Weird droid level in drawDroidRank"));
    break;
  }
  /*
switch(getDroidLevel(psDroid))
