...
BOOL IsStatExpansionModule(STRUCTURE_STATS* psStats)
{
  ...
  if (psStats->type == REF_POWER_MODULE || psStats->type == REF_FACTORY_MODULE || psStats->type == REF_RESEARCH_MODULE)
    return TRUE;
  return FALSE;
}
...
void structureInitVars(void)
{
  ...
  {
    factoryModuleIMDs[i][0] = nullptr;
    factoryModuleIMDs[i][1] = nullptr;
  }
  for (i = 0; i < NUM_RESEARCH_MODULES; i++)
    researchModuleIMDs[i] = nullptr;
  for (i = 0; i < NUM_POWER_MODULES; i++)
    powerModuleIMDs[i] = nullptr;
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    for (j = 0; j < NUM_FLAG_TYPES; j++)
      factoryNumFlag[i][j] = static_cast<UBYTE>(0);
  }
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    hqExists[i] = FALSE;
  ...
  memset(&asProductionRun, 0, (sizeof(PRODUCTION_RUN) * NUM_FACTORY_TYPES * MAX_FACTORY * MAX_PROD_RUN));
  //set up at beginning of game which player will have a production list
  productionPlayer = static_cast<SBYTE>(selectedPlayer);
}
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    for (psStruct = apsStructLists[i]; psStruct != nullptr; psStruct = psStruct->psNext)
    {
      if (psStruct->pStructureType->type == REF_FACTORY)
      {
        ...
        {
          mask = static_cast<UBYTE>(1 << psFactory->psAssemblyPoint->factoryInc);
...
UBYTE getStructStrength(STRING* pStrength)
{
  ...
  if (!strcmp(pStrength, "MEDIUM"))
    return STRENGTH_MEDIUM;
  if (!strcmp(pStrength, "HARD"))
    return STRENGTH_HARD;
  if (!strcmp(pStrength, "BUNKER"))
...
BOOL loadStructureStats(SBYTE* pStructData, UDWORD bufferSize)
{
  ...
  {
    factoryModuleIMDs[module][0] = nullptr;
    factoryModuleIMDs[module][1] = nullptr;
  }
  for (module = 0; module < NUM_RESEARCH_MODULES; module++)
    researchModuleIMDs[module] = nullptr;
...
#error Invalid number of players
    ...
    else
    {
      ...
      ASSERT_TEXT(psStructure->pSensor != NULL, "loadStructureStats: should have a sensor attached to {}!", StructureName);
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    ...
    for (psCurr = apsStructLists[player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      ...
      if (displayError)
      {
        ...
        {
          ASSERT_TEXT(FALSE, "There appears to be too many {} on this map!", getStructName(&asStructureStats[inc] ));
        }
      }
    }
...
#ifdef HASH_NAMES
    StructureHash = HashString(StructureName); WeaponHash = HashString(WeaponName);
#endif
...
BOOL loadStructureFunctions(SBYTE* pFunctionData, UDWORD bufferSize)
{
...
#ifdef HASH_NAMES
    StructureHash = HashString(StructureName); FunctionHash = HashString(FunctionName);
#endif
  ...
  for (incF = 0; incF < numFunctions; incF++)
  {
    ...
    if (pFunction->type == WALL_TYPE)
    {
      ...
      ((WALL_FUNCTION*)pFunction)->pCornerStat = nullptr;
 ...
 */
BOOL structureDamage(STRUCTURE* psStructure, UDWORD damage, UDWORD weaponClass, UDWORD weaponSubClass)
{
  ...
  if (psStructure->player != selectedPlayer)
  {
    ...
    damage = static_cast<UDWORD>(modifyForDifficultyLevel(static_cast<SDWORD>(damage),TRUE));
  }
  else
  {
    // Enemy inflicting damage on player.
    damage = static_cast<UDWORD>(modifyForDifficultyLevel(static_cast<SDWORD>(damage),FALSE));
  }
...
BOOL structSetManufacture(STRUCTURE* psStruct, DROID_TEMPLATE* psTempl, UBYTE quantity)
{
  ...
  ASSERT_TEXT(psStruct->type == OBJ_STRUCTURE && (psStruct->pStructureType->type == REF_FACTORY OR psStruct->pStructureType->type == REF_CYBORG_FACTORY OR psStruct->pStructureType->type
...
static SDWORD structWallScan(BOOL aWallPresent[5][5], SDWORD x, SDWORD y)
{
  ...
  for (cx = x - 1; cx <= x + 1; cx += 1)
  {
    for (cy = y - 1; cy <= y + 1; cy += 1)
    {
      ...
      if (((cx == x && cy != y) || (cx != x && cy == y)) && aWallPresent[cx][cy])
        numWalls += 1;
    }
  }
...
SDWORD structChooseWallType(UDWORD player, UDWORD mapX, UDWORD mapY)
{
  ...
  {
    xdiff = static_cast<SDWORD>(mapX) - (static_cast<SDWORD>(psStruct->x) >> TILE_SHIFT);
    ydiff = static_cast<SDWORD>(mapY) - (static_cast<SDWORD>(psStruct->y) >> TILE_SHIFT);
    if (xdiff >= -2 && xdiff <= 2 && ydiff >= -2 && ydiff <= 2 && (psStruct->pStructureType->type == REF_WALL || psStruct->pStructureType->
      type == REF_WALLCORNER || psStruct->pStructureType->type == REF_DEFENSE))
    {
  ...
  for (x = 1; x <= 3; x += 1)
  {
    ...
    {
      // do not look at walls diagonally from this wall
      if (((x == 2 && y != 2) || (x != 2 && y == 2)) && aWallPresent[x][y])
      {
            ...
            if (psStruct->pStructureType->asFuncList[0]->type == WALL_TYPE)
            {
              ...
              if (psStruct->status == SS_BEING_BUILT)
              {
                ...
                if (psStruct != nullptr)
                {
                  ...
                  psStruct->currentBuildPts = static_cast<SWORD>(oldBuildPoints);
                }
              }
              else
              {
...
VOID buildFlatten(STRUCTURE_STATS* pStructureType, UDWORD atx, UDWORD aty, UDWORD h)
{
  ...
  for (breadth = 0; breadth <= static_cast<UBYTE>(pStructureType->baseBreadth/* + 1*/); breadth++)
  {
    for (width = 0; width <= static_cast<UBYTE>(pStructureType->baseWidth /*+ 1*/); width++)
    {
      ...
      /*				(pStructureType->type != REF_DEFENSE))*/
      {
        ...
        if (TILE_HAS_FEATURE(mapTile(x+width,y+breadth)))
          getTileFeature(x + width, y + breadth)->z = static_cast<UWORD>(h);
...
STRUCTURE* buildStructure(STRUCTURE_STATS* pStructureType, UDWORD x, UDWORD y, UDWORD player, BOOL FromSave)
{
  ...
  if (IsStatExpansionModule(pStructureType) == FALSE)
  {
    ...
    if (player == selectedPlayer)
    {
      ...
      if (pStructureType->type == REF_FACTORY OR pStructureType->type == REF_CYBORG_FACTORY OR pStructureType->type == REF_VTOL_FACTORY)
      {
        //NEVER EVER EVER WANT MORE THAN 5 FACTORIES
        if (asStructLimits[selectedPlayer][max].currentQuantity > MAX_FACTORY)
        {
          ASSERT_TEXT(FALSE, "buildStructure: trying to build too many factories");
          return nullptr;
        }
      }
      if (pStructureType->type == REF_RESEARCH)
      {
        ...
        {
          ASSERT_TEXT(FALSE, "buildStructure: trying to build too many research facilities");
          return nullptr;
        }
      }
      //HARD_CODE don't ever want more than one Las Sat structure
      if (pStructureType->psWeapStat AND pStructureType->psWeapStat->weaponSubClass == WSC_LAS_SAT AND getLasSatExists(selectedPlayer))
        return nullptr;
      ...
      if (pStructureType->type == REF_SAT_UPLINK)
      {
        ...
        {
          ASSERT_TEXT(FALSE, "buildStructure: trying to build too many Sat Uplinks");
          return nullptr;
        }
      }
    }
    ...
    if (pStructureType->type == REF_DEFENSE && mapTile(x >> TILE_SHIFT, y >> TILE_SHIFT)->tileInfoBits & BITS_WALL)
    {
    ...
    psBuilding->x = static_cast<UWORD>(x);
    psBuilding->y = static_cast<UWORD>(y);
    /*		} else {
      ...
      for (breadth = 0; breadth < pStructureType->baseBreadth; breadth++)
      {
        ...
        else
        {
          ASSERT_TEXT(!(TILE_HAS_STRUCTURE(mapTile(mapX+width,mapY+breadth))),
            "buildStructure - structure - {} already found at {}, {}", psBuilding->id, mapX+width,mapY+breadth);
        }
    ...
    if (pStructureType->type != REF_DEFENSE)
    {
      ...
      psBuilding->z = static_cast<UWORD>(mapH); //(UWORD)(map_TileHeight(mapX,mapY)+1);//jps 18july97 - shift it up a wee bit - am
    }
    else
    {
      /* Set it at the higher coord */
      getTileMaxMin(mapX, mapY, &max, &min);
      psBuilding->z = static_cast<UWORD>(max); // Got to be - don't change!!!! ALEXM
    }
    ...
    {
      psBuilding->sensorRange = static_cast<UWORD>(sensorRange(psBuilding->pStructureType->pSensor, static_cast<UBYTE>(player)));
      psBuilding->sensorPower = static_cast<UWORD>(sensorPower(psBuilding->pStructureType->pSensor, static_cast<UBYTE>(player)));
    }
    else
    {
      //give them the default sensor for droids if not
      psBuilding->sensorRange = static_cast<UWORD>(sensorRange(asSensorStats + aDefaultSensor[player], static_cast<UBYTE>(player)));
      psBuilding->sensorPower = static_cast<UWORD>(sensorPower(asSensorStats + aDefaultSensor[player], static_cast<UBYTE>(player)));
    }
    //set up the ecm stat
    if (psBuilding->pStructureType->pECM)
      psBuilding->ecmPower = static_cast<UWORD>(ecmPower(psBuilding->pStructureType->pECM, psBuilding->player));
    else
    ...
    psBuilding->armour = static_cast<UWORD>(structureArmour(pStructureType, static_cast<UBYTE>(player)));
    psBuilding->resistance = static_cast<UWORD>(structureResistance(pStructureType, static_cast<UBYTE>(player)));
    ...
    psBuilding->body = static_cast<UWORD>(structureBody(psBuilding));
...
//		if(TEST_TILE_VISIBLE(selectedPlayer,mapTile(psBuilding->x>>TILE_SHIFT,psBuilding->y>>TILE_SHIFT)))
    ...
    if (pStructureType->type == REF_RESOURCE_EXTRACTOR)
    {
      ...
      for (psFeature = apsFeatureLists[0]; psFeature != nullptr; psFeature = psFNext)
      {
        ...
        if (psFeature->psStats->subType == FEAT_OIL_RESOURCE)
        {
          if ((psFeature->x == psBuilding->x) AND (psFeature->y == psBuilding->y))
          {
            ...
            SET_TILE_STRUCTURE(mapTile(psFeature->x >> TILE_SHIFT, psFeature->y >> TILE_SHIFT));
            // if it's a tall structure then flag it in the map.
            if (psBuilding->sDisplay.imd->ymax > TALLOBJECT_YMAX)
            {
              SET_TILE_TALLSTRUCTURE(mapTile(psFeature->x >> TILE_SHIFT, psFeature->y >> TILE_SHIFT));
            }
          }
        }
      }
  ...
  else //its an upgrade
  {
    ...
    {
      if (psBuilding->pStructureType->type != REF_FACTORY AND psBuilding->pStructureType->type != REF_VTOL_FACTORY)
        return nullptr;
      ...
      if (((FACTORY*)psBuilding->pFunctionality)->capacity < SIZE_SUPER_HEAVY)
      {
        ...
        if ((bMultiPlayer && isHumanPlayer(psBuilding->player)) OR (bMultiPlayer && (game.type == SKIRMISH) && (psBuilding->player < game.
          maxPlayers)) OR !bMultiPlayer)
        //|| (psBuilding->player ==0) )
        {
          capacity = ((FACTORY*)psBuilding->pFunctionality)->capacity;
          if (capacity < NUM_FACTORY_MODULES)
          {
            if (psBuilding->pStructureType->type == REF_FACTORY) { psBuilding->sDisplay.imd = factoryModuleIMDs[capacity - 1][0]; }
            else { psBuilding->sDisplay.imd = factoryModuleIMDs[capacity - 1][1]; }
          }
          else
          {
            if (psBuilding->pStructureType->type == REF_FACTORY)
            {
              psBuilding->sDisplay.imd = factoryModuleIMDs[NUM_FACTORY_MODULES - 1][0];
            }
            else { psBuilding->sDisplay.imd = factoryModuleIMDs[NUM_FACTORY_MODULES - 1][1]; }
          }
        }
      }
    }
    ...
    if (pStructureType->type == REF_RESEARCH_MODULE)
    {
      ...
      if (((RESEARCH_FACILITY*)psBuilding->pFunctionality)->capacity < NUM_RESEARCH_MODULES)
      {
        ...
        ((RESEARCH_FACILITY*)psBuilding->pFunctionality)->researchPoints += ((RESEARCH_FUNCTION*)pStructureType->asFuncList[0])->
          researchPoints;
        bUpgraded = TRUE;
    ...
    if (pStructureType->type == REF_POWER_MODULE)
    {
      ...
      if (((POWER_GEN*)psBuilding->pFunctionality)->capacity < NUM_POWER_MODULES)
      {
        ...
        ((POWER_GEN*)psBuilding->pFunctionality)->power += ((POWER_GEN_FUNCTION*)pStructureType->asFuncList[0])->powerOutput;
        ((POWER_GEN*)psBuilding->pFunctionality)->multiplier += ((POWER_GEN_FUNCTION*)pStructureType->asFuncList[0])->powerMultiplier;
        bUpgraded = TRUE;
    ...
    if (bUpgraded)
    {
      ...
      psBuilding->body = static_cast<UWORD>(structureBody(psBuilding) * bodyDiff / 100);
  ...
  case REF_VTOL_FACTORY:
    {
      ...
      psFactory->capacity = static_cast<UBYTE>(((PRODUCTION_FUNCTION*)psBuilding->pStructureType->asFuncList[0])->capacity);
      //psFactory->propulsionType = ((PRODUCTION_FUNCTION*)psBuilding->
      psFactory->productionOutput = static_cast<UBYTE>(((PRODUCTION_FUNCTION*)psBuilding->pStructureType->asFuncList[0])->productionOutput);
      ...
      case REF_VTOL_FACTORY:
        ...
        break;
      default: ASSERT_TEXT(FALSE, "setFunctionality: Invalid factory type");
      }
      //initialise the assembly point position
  ...
  case REF_RESOURCE_EXTRACTOR:
    {
      ...
      ((RES_EXTRACTOR*)psBuilding->pFunctionality)->power = ((
        ...
        RESOURCE_FUNCTION*)psBuilding->pStructureType->asFuncList[0])->maxPower;
      //set the structure going
      //set the structure to inactive
      ((RES_EXTRACTOR*)psBuilding->pFunctionality)->active = FALSE;
      ((RES_EXTRACTOR*)psBuilding->pFunctionality)->psPowerGen = nullptr;
      break;
    }
  ...
  case REF_REPAIR_FACILITY:
    {
      ...
      {
        /* add null droid */
        grpJoin(psRepairFac->psGroup, nullptr);
      }
      //check for upgrades 
...
void assignFactoryCommandDroid(STRUCTURE* psStruct, DROID* psCommander)
{
  ...
  ASSERT_TEXT(StructIsFactory(psStruct),"assignFactoryCommandUnit: structure not a factory");
  ...
  case REF_CYBORG_FACTORY:
    ...
    break;
  default: ASSERT_TEXT(FALSE,"assignfactorycommandUnit: unknown factory type");
    typeFlag = FACTORY_FLAG;
    break;
  }
  ...
  if (psFact->psCommander != nullptr)
  {
    ...
    {
      secondarySetState(psFact->psCommander, DSO_CLEAR_PRODUCTION, 1 << (psFact->psAssemblyPoint->factoryInc + DSS_ASSPROD_SHIFT));
    }
    else if (typeFlag == CYBORG_FLAG)
    {
      secondarySetState(psFact->psCommander, DSO_CLEAR_PRODUCTION, 1 << (psFact->psAssemblyPoint->factoryInc + DSS_ASSPROD_CYBORG_SHIFT));
    }
    else
    {
      secondarySetState(psFact->psCommander, DSO_CLEAR_PRODUCTION, 1 << (psFact->psAssemblyPoint->factoryInc + DSS_ASSPROD_VTOL_SHIFT));
    }
  ...
  if (psCommander != nullptr)
  {
    ...
    ASSERT_TEXT(!missionIsOffworld(), "assignFactoryCommandDroid: cannot assign a commander to a factory when off world");
...
void clearCommandDroidFactory(DROID* psDroid)
{
  ...
  {
    if ((psCurr->pStructureType->type == REF_FACTORY) || (psCurr->pStructureType->type == REF_CYBORG_FACTORY) || (psCurr->pStructureType->
      type == REF_VTOL_FACTORY))
    {
...
BOOL placeDroid(STRUCTURE* psStructure, UDWORD* droidX, UDWORD* droidY)
{
  ...
  sx = static_cast<SWORD>(psStructure->x - psStructure->pStructureType->baseWidth * TILE_UNITS / 2);
  sx = static_cast<SWORD>(sx >> TILE_SHIFT);
  sy = static_cast<SWORD>(psStructure->y - psStructure->pStructureType->baseBreadth * TILE_UNITS / 2);
  sy = static_cast<SWORD>(sy >> TILE_SHIFT);

  /* Find the four corners of the square */
  xmin = static_cast<SWORD>(sx - 1);
  xmax = static_cast<SWORD>(sx + psStructure->pStructureType->baseWidth);
  xmid = static_cast<SWORD>(sx + (psStructure->pStructureType->baseWidth - 1) / 2);
  ymin = static_cast<SWORD>(sy - 1);
  ymax = static_cast<SWORD>(sy + psStructure->pStructureType->baseBreadth);
  if (xmin < 0)
    xmin = 0;
  if (xmax > static_cast<SDWORD>(mapWidth))
    xmax = static_cast<SWORD>(mapWidth);
  if (ymin < 0)
    ymin = 0;
  if (ymax > static_cast<SDWORD>(mapHeight))
    ymax = static_cast<SWORD>(mapHeight);
...
static void structPlaceDroid(STRUCTURE* psStructure, DROID_TEMPLATE* psTempl, DROID** ppsDroid)
{
  UDWORD x, y;
  ...
  if (placed)
  {
    ...
    psNewDroid = buildDroid(psTempl, x << TILE_SHIFT, y << TILE_SHIFT, psStructure->player, FALSE);
    if (!psNewDroid)
    {
      *ppsDroid = nullptr;
      return;
    ...
    *ppsDroid = psNewDroid;
    ...
    if ((psNewDroid->droidType == DROID_COMMAND) && (psFact->psCommander != nullptr))
    {
    ...
    if (psFact->psCommander != nullptr)
    {
      if (idfDroid(psNewDroid) || vtolDroid(psNewDroid))
      {
    ...
    else
    {
      ...
      if (!placed)
      {
        ...
        for (psFlag = apsFlagPosLists[psFact->psAssemblyPoint->player]; !((psFlag->factoryInc == psFact->psAssemblyPoint->factoryInc)
               // correct fact.
...
{
  if (getNumDroids(PlayerNumber) + getNumMissionDroids(PlayerNumber) + getNumTransporterDroids(PlayerNumber) >= (bMultiPlayer
    ? (MaxDroidsAllowedPerPlayerMultiPlayer[PlayerNumber])
...
{
  auto psFact = (FACTORY*)psStructure->pFunctionality;

  if ((droidTemplateType((DROID_TEMPLATE*)psFact->psSubject) == DROID_COMMAND) && (getNumCommandDroids(psStructure->player) >= 10))
    return TRUE;

  if ((droidTemplateType((DROID_TEMPLATE*)psFact->psSubject) == DROID_CONSTRUCT OR droidTemplateType((DROID_TEMPLATE*)psFact->psSubject) ==
    DROID_CYBORG_CONSTRUCT) && (getNumConstructorDroids(psStructure->player) >= 15))
    return TRUE;
...
BOOL CheckHaltOnMaxUnitsReached(STRUCTURE* psStructure)
{
  ...
  if (IsPlayerDroidLimitReached(psStructure->player) || maxDroidsByTypeReached(psStructure))
  {
...
void aiUpdateStructure(STRUCTURE* psStructure)
{
  ...
  if ((psStructure->asWeaps[0].nStat == 0) && (psStructure->pStructureType->type != REF_REPAIR_FACILITY))
  {
    //////
  ...
  if (psStructure->asWeaps[0].nStat > 0 AND asWeaponStats[psStructure->asWeaps[0].nStat].weaponSubClass != WSC_LAS_SAT)
  {
    ...
    if (psChosenObj != nullptr)
    {
      ...
      else if (actionTargetTurret((BASE_OBJECT*)psStructure, psChosenObj, &(psStructure->turretRotation), &(psStructure->turretPitch),
                                  psWStats, FALSE))
        combFire(psStructure->asWeaps, (BASE_OBJECT*)psStructure, psChosenObj);
    }
    else
  ...
  else if (psStructure->pStructureType->pSensor)
  {
    ...
    if ((structCBSensor(psStructure) OR structVTOLCBSensor(psStructure)) && psStructure->psTarget != nullptr)
  ...
  case REF_VTOL_FACTORY:
    {
      ...
      if (((FACTORY*)psStructure->pFunctionality)->psCommander AND ((FACTORY*)psStructure->pFunctionality)->psCommander->died)
      {
  ...
  case REF_REPAIR_FACILITY:
    {
      ...
      if ((psDroid != nullptr) && (!orderState(psDroid, DORDER_RTR) || psDroid->psTarget != (BASE_OBJECT*)psStructure))
      {
        psChosenObj = nullptr;
        psDroid = nullptr;
        psRepairFac->psObj = nullptr;
      ...
      if (psChosenObj == nullptr)
      {
        ...
        {
          if (orderStateObj(psDroid, DORDER_RTR, &psTarget) && (psTarget == (BASE_OBJECT*)psStructure) && psDroid->action ==
            DACTION_WAITFORREPAIR)
          {
            xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psStructure->x);
            ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psStructure->y);
      ...
      if (psDroid != nullptr && psDroid->action == DACTION_WAITFORREPAIR)
      {
        /* set chosen object */
        psChosenObj = (BASE_OBJECT*)psDroid;
        ...
        /*				actionDroidObjLoc( psDroid, DACTION_MOVETOREPAIRPOINT,
                    ...
                    TILE_UNITS/2 );*/
        actionDroidObjLoc(psDroid, DACTION_MOVETOREPAIRPOINT, (BASE_OBJECT*)psStructure, psStructure->x, psStructure->y);
  ...
  case REF_REARM_PAD:
    {
      ...
      if (psChosenObj == nullptr)
      {
        for (psDroid = apsDroidLists[psStructure->player]; psDroid; psDroid = psDroid->psNext)
        {
          ...
          if (vtolReadyToRearm(psDroid, psStructure) && (psChosenObj == nullptr || (((DROID*)psChosenObj)->actionStarted > psDroid->
            actionStarted)))
            psChosenObj = (BASE_OBJECT*)psDroid;
        }
        psDroid = (DROID*)psChosenObj;
        if (psDroid != nullptr) { actionDroidObj(psDroid, DACTION_MOVETOREARMPOINT, (BASE_OBJECT*)psStructure); }
      }
      else
      {
        psDroid = (DROID*)psChosenObj;
        if ((psDroid->sMove.Status == MOVEINACTIVE || psDroid->sMove.Status == MOVEHOVER) && psDroid->action == DACTION_WAITFORREARM)
        {
  ...
  if (pSubject != nullptr)
  {
    ...
    if (structureMode == REF_RESEARCH)
    {
      ...
      if (bMultiPlayer)
      {
        if (psStructure->resistance < static_cast<SWORD>(structureResistance(psStructure->pStructureType, psStructure->player)))
          return;
      }
      ...
      if (IsResearchCompleted(pPlayerRes) == 0)
      {
        ...
        pointsToAdd = (psResFacility->researchPoints * (gameTime - psResFacility->timeStarted)) / GAME_TICKS_PER_SEC;

        //check if Research is complete
        //if ((pointsToAdd + pPlayerRes->currentPoints) > psResFacility->
        //	timeToResearch)
        if ((pointsToAdd + pPlayerRes->currentPoints) > ((RESEARCH*)pSubject)->researchPoints)
        {
          ...
          {
            if (((RESEARCH*)psResFacility->psSubject)->researchPoints > ((RESEARCH*)psResFacility->psBestTopic)->researchPoints)
              psResFacility->psSubject = psResFacility->psSubject;
          }
          psResFacility->psSubject = nullptr;
          intResearchFinished(psStructure);
          researchResult(pSubject->ref - REF_RESEARCH_START, psStructure->player, TRUE);
    ...
    else if (structureMode == REF_FACTORY)
    {
      ...
      if (psFactory->powerAccrued < ((DROID_TEMPLATE*)pSubject)->powerPoints)
      {
        //wait until enough power
        return;
      }
      ...
      if ((pointsToAdd > psFactory->timeToBuild) && !IsFactoryCommanderGroupFull(psFactory) && !CheckHaltOnMaxUnitsReached(psStructure))
      {
  ...
  if (psChosenObj != nullptr)
  {
    if (structureMode == REF_REPAIR_FACILITY)
    {
      ...
      if (psDroid->action == DACTION_WAITDURINGREPAIR && actionTargetTurret((BASE_OBJECT*)psStructure, psChosenObj,
                                                                            &(psStructure->turretRotation), &(psStructure->turretPitch),
                                                                            nullptr, FALSE))
      {
...
if (psRepairFac->powerAccrued < iPower)
{
  ...
  return;
        ...
        if (psDroid->body < psDroid->originalBody)
        {
          ...
          {
            if (!(psDroid->sMove.Status == MOVEINACTIVE AND psDroid->sMove.iVertSpeed == 0))
              return;
          }
#ifdef TEST_EW
          bMultiPlayer = TRUE;
          ...
          pointsToAdd = (iDt * psRepairFac->power / GAME_TICKS_PER_SEC) - psRepairFac->currentPtsAdded;
          bFinishAction = FALSE;
        ...
        if (psDroid->body >= psDroid->originalBody)
        {
          ...
          if ((psDroid->psGroup != nullptr) && (psDroid->psGroup->type == GT_COMMAND) && (psDroid->droidType != DROID_COMMAND))
          {
          ...
          else if (psRepairFac->psDeliveryPoint != nullptr)
          {
            ...
            orderDroidLoc(psDroid, DORDER_MOVE, psRepairFac->psDeliveryPoint->coords.x, psRepairFac->psDeliveryPoint->coords.y);
            /*						orderDroidLoc( psDroid, DORDER_MOVE, psStructure->x,
                          psStructure->y +
                          (psStructure->pStructureType->baseBreadth<<TILE_SHIFT)/2 + 2*TILE_UNITS );*/
          }
        ...
        if (psStructure->visible[selectedPlayer] AND psDroid->visible[selectedPlayer])
        {
          ...
          effectSetSize(100);
          addEffect(&iVecEffect, EFFECT_EXPLOSION, EXPLOSION_TYPE_SPECIFIED, TRUE, getImdFromIndex(MI_FLAME), 0);
        }
      }
    }
    //check for rearming
    else if (structureMode == REF_REARM_PAD)
    {
      ...
      if (psDroid->died || ( //vtolHappy(psDroid) &&
        ...
        (psDroid->action != DACTION_MOVETOREARMPOINT) && (psDroid->action != DACTION_WAITDURINGREARM)))
      {
      ...
      if (psDroid->action == DACTION_WAITDURINGREARM && psDroid->sMove.Status == MOVEINACTIVE)
      {
        ...
        if (!bMultiPlayer || myResponsibility(psDroid->player))
        {
          ...
          if (psDroid->sMove.iAttackRuns != 0)
          {
            ...
            if (pointsToAdd >= pointsRequired)
            {
              ...
              psDroid->asWeaps[0].ammo = asWeaponStats[psDroid->asWeaps[0].nStat].numRounds;
        ...
        if (psDroid->body < psDroid->originalBody)
        {
          pointsToAdd = VTOL_REPAIR_FACTOR * (gameTime - psReArmPad->timeStarted) / GAME_TICKS_PER_SEC;
...
{
  if (psStruct->pStructureType->type == REF_WALL OR psStruct->pStructureType->type == REF_WALLCORNER)
    return (FALSE);
  return (TRUE);
}
...
void structureUpdate(STRUCTURE* psBuilding)
{
  ...
  if (psBuilding->visible[selectedPlayer] AND canSmoke(psBuilding))
  {
    ...
    if (percentDamage != 0)
    {
      ...
      if (gameTime > (psBuilding->lastEmission + emissionInterval))
      {
        ...
        addEffect(&dv, EFFECT_SMOKE, SMOKE_TYPE_DRIFTING_HIGH,FALSE, nullptr, 0);
  ...
  if (psBuilding->resistance < static_cast<SWORD>(iPointsRequired))
  {
  ...
  else
  {
    ...
    if (selfRepairEnabled(psBuilding->player) AND (psBuilding->body < static_cast<SWORD>(iPointsRequired)))
    {
      ...
      for the stat*/
      iPointsToAdd = (repairPoints(asRepairStats + aDefaultRepair[psBuilding->player], psBuilding->player) / 4) * ((gameTime - psBuilding->
        lastResistance) / (asRepairStats + aDefaultRepair[psBuilding->player])->time);
      ...
      if (bMultiPlayer AND ONEINTEN)
      {
        ...
        effectSetSize(30);
        addEffect(&position, EFFECT_EXPLOSION, EXPLOSION_TYPE_SPECIFIED,TRUE, getImdFromIndex(MI_PLASMA), 0);
      }

      if (iPointsToAdd)
      {
        psBuilding->body = static_cast<UWORD>(psBuilding->body + iPointsToAdd);
        psBuilding->lastResistance = gameTime;
        if (psBuilding->body > iPointsRequired)
        {
          psBuilding->body = static_cast<UWORD>(iPointsRequired);
          psBuilding->lastResistance = ACTION_START_TIME;
        }
      }
    }
...
void structureRelease(STRUCTURE* psBuilding)
{
  ...
  if (psBuilding->pFunctionality != nullptr)
  {
    ...
    if (StructIsFactory(psBuilding))
    {
      ...
      if (((FACTORY*)psBuilding->pFunctionality)->psCommander != nullptr)
...
UDWORD fillStructureList(STRUCTURE_STATS** ppList, UDWORD selectedPlayer, UDWORD limit)
{
  ...
  if (!missionIsOffworld())
  {
    ...
    {
      if (psCurr->pStructureType->type == REF_RESEARCH AND psCurr->status == SS_BUILT)
        researchModule = TRUE;
      else if (psCurr->pStructureType->type == REF_FACTORY AND psCurr->status == SS_BUILT)
        factoryModule = TRUE;
      else if (psCurr->pStructureType->type == REF_POWER_GEN AND psCurr->status == SS_BUILT)
        powerModule = TRUE;
    }
  }
  ...
  for (inc = 0; inc < numStructureStats; inc++)
  {
    ...
    if (apStructTypeLists[selectedPlayer][inc] & AVAILABLE)
    {
      ...
      if (asStructLimits[selectedPlayer][inc].currentQuantity < asStructLimits[selectedPlayer][inc].limit)
      {
        ...
        if (missionIsOffworld())
        {
          if (psBuilding->type == REF_FACTORY OR psBuilding->type == REF_POWER_GEN OR psBuilding->type == REF_RESOURCE_EXTRACTOR OR
            psBuilding->type == REF_RESEARCH OR psBuilding->type == REF_CYBORG_FACTORY OR psBuilding->type == REF_VTOL_FACTORY)
            continue;
        }
        ...
        //paranoid check!!
        if (psBuilding->type == REF_FACTORY OR psBuilding->type == REF_CYBORG_FACTORY OR psBuilding->type == REF_VTOL_FACTORY)
        {
          //NEVER EVER EVER WANT MORE THAN 5 FACTORIES
        ...
        //HARD_CODE don't ever want more than one Las Sat structure
        if (psBuilding->psWeapStat AND psBuilding->psWeapStat->weaponSubClass == WSC_LAS_SAT AND getLasSatExists(selectedPlayer))
          continue;
        //HARD_CODE don't ever want more than one Sat Uplink structure
...
BOOL validLocation(BASE_STATS* psStats, UDWORD x, UDWORD y, UDWORD player, BOOL bCheckBuildQueue)
{
  ...
  {
    site.xTL = static_cast<UWORD>(x);
    site.yTL = static_cast<UWORD>(y);
    site.xBR = static_cast<UWORD>(x + psBuilding->baseWidth - 1);
    site.yBR = static_cast<UWORD>(y + psBuilding->baseBreadth - 1);
    ...
    if (wallDrag.status != DRAG_INACTIVE)
    {
      if (psBuilding->type == REF_WALL OR psBuilding->type == REF_DEFENSE)
      {
        ...
        dx = static_cast<UWORD>(abs(static_cast<SDWORD>(mouseTileX - wallDrag.x1)));
        dy = static_cast<UWORD>(abs(static_cast<SDWORD>(mouseTileY - wallDrag.y1)));

        if (dx >= dy)
        {
          //build in x direction
          site.xTL = static_cast<UWORD>(wallDrag.x1);
          site.xBR = static_cast<UWORD>(wallDrag.x2);
          site.yTL = static_cast<UWORD>(wallDrag.y1);
          if (dy > 0)
            site.yBR = static_cast<UWORD>(site.yTL + 1);
          else
            site.yBR = site.yTL;
        ...
        else if (dx < dy)
        {
          ...
          site.yTL = static_cast<UWORD>(wallDrag.y1);
          site.yBR = static_cast<UWORD>(wallDrag.y2);
          site.xTL = static_cast<UWORD>(wallDrag.x1);
          if (dx > 0)
            site.xBR = static_cast<UWORD>(site.xTL + 1);
          else
            site.xBR = site.xTL;
  ...
  else
  {
    ...
    site.xTL = static_cast<UWORD>(x);
    site.yTL = static_cast<UWORD>(y);
    site.xBR = static_cast<UWORD>(x);
    site.yBR = static_cast<UWORD>(y);
  }
  ...
  for (i = site.xTL; i <= site.xBR AND valid; i++)
  {
    for (j = site.yTL; j <= site.yBR AND valid; j++)
    {
      ...
      if ((i < scrollMinX + 2) || (i > scrollMaxX - 5) || (j < scrollMinY + 2) || (j > scrollMaxY - 5))
      {
        valid = FALSE;
        goto failed;
      }
      ...
      if (i <= 2 OR j <= 2 OR i >= static_cast<SDWORD>(mapWidth) - 2 OR j >= static_cast<SDWORD>(mapHeight) - 2)
      {
        valid = FALSE;
        goto failed;
      }
  ...
  for (psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
  {
    if (psStruct->pStructureType->type == REF_REPAIR_FACILITY)
    {
      ...
      if ((site.xTL >= i && site.xTL <= (i + 2)) || (site.xBR >= i && site.xBR <= (i + 2)))
      {
        // now see if y extents overlap
        if ((site.yTL >= j && site.yTL <= (j + 2)) || (site.yBR >= j && site.yBR <= (j + 2)))
        {
          valid = FALSE;
          goto failed;
        }
  ...
  if (psStats->ref >= REF_STRUCTURE_START && psStats->ref < (REF_STRUCTURE_START + REF_RANGE))
  {
    ...
    case REF_MISSILE_SILO:
      ...
      for (i = site.xTL; i <= site.xBR AND valid; i++)
      {
        ...
        {
          psTile = mapTile(i, j);
          if ((TERRAIN_TYPE(psTile) == TER_WATER) || (TERRAIN_TYPE(psTile) == TER_CLIFFFACE))
            valid = FALSE;
        }
      }
      ...
      if (valid && // only do if necessary
        (!(psBuilding->type == REF_REPAIR_FACILITY OR psBuilding->type == REF_DEFENSE OR psBuilding->type == REF_WALL)))
      {
      ...
      if (valid)
      {
        ...
        if (!(psBuilding->type == REF_DEFENSE OR psBuilding->type == REF_WALL OR psBuilding->type == REF_WALLCORNER OR psBuilding->type ==
          REF_MISSILE_SILO))
        {
          /*need to check there is one tile between buildings*/
          for (i = static_cast<UWORD>(site.xTL - 1); i <= static_cast<UWORD>(site.xBR + 1); i++)
          {
            for (j = static_cast<UWORD>(site.yTL - 1); j <= static_cast<UWORD>(site.yBR + 1); j++)
            {
              //skip the actual area the structure will cover
              if (i < site.xTL OR i > site.xBR OR j < site.yTL OR j > site.yBR)
              {
                if (TILE_HAS_STRUCTURE(mapTile(i,j)))
                {
                  ...
                  if (psStruct)
                  {
                    ...
                    if ((psStruct->pStructureType->type != REF_DEFENSE) AND (psStruct->pStructureType->type != REF_WALL) AND (psStruct->
                      pStructureType->type != REF_WALLCORNER))
                    {
      ...
      if (valid)
      {
        ...
        for (i = site.xTL; i <= site.xBR AND valid; i++)
        {
          for (j = site.yTL; j <= site.yBR AND valid; j++)
          {
            ...
            {
              if ((psBuilding->type == REF_DEFENSE OR psBuilding->type == REF_WALL) && (psTile->tileInfoBits & BITS_WALL))
              {
                psStruct = getTileStructure(i, j);
    ...
    case REF_FACTORY_MODULE:
      ...
      if (TILE_HAS_STRUCTURE(mapTile(x,y)))
      {
        ...
        if (psStruct AND (psStruct->pStructureType->type == REF_FACTORY OR psStruct->pStructureType->type == REF_VTOL_FACTORY) AND psStruct
          ->status == SS_BUILT)
          valid = TRUE;
      }
      break;
    case REF_RESEARCH_MODULE:
      ...
      {
        psStruct = getTileStructure(x, y);
        if (psStruct AND psStruct->pStructureType->type == REF_RESEARCH AND psStruct->status == SS_BUILT)
          valid = TRUE;
      }
    ...
    case REF_POWER_MODULE:
      ...
      {
        psStruct = getTileStructure(x, y);
        if (psStruct AND psStruct->pStructureType->type == REF_POWER_GEN AND psStruct->status == SS_BUILT)
          valid = TRUE;
      }
      break;
    case REF_RESOURCE_EXTRACTOR:
    ...
    if (ctrlShiftDown() AND player == selectedPlayer AND bCheckBuildQueue)
    {
      ...
      if (!(psBuilding->type == REF_DEFENSE OR psBuilding->type == REF_MISSILE_SILO))
      {
        for (psDroid = apsDroidLists[player]; psDroid; psDroid = psDroid->psNext)
        {
          ...
          if (psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT)
          {
            ...
            for (order = 0; order < psDroid->listSize; order++)
            {
              ...
              {
                validCombi = FALSE;
                if (static_cast<STRUCTURE_STATS*>(psDroid->asOrderList[order].psOrderTarget)->type == REF_DEFENSE OR static_cast<
                  STRUCTURE_STATS*>(psDroid->asOrderList[order].psOrderTarget)->type == REF_MISSILE_SILO)
                  validCombi = TRUE;
                //walls can be built next to walls and defence
                if (psBuilding->type == REF_WALL OR psBuilding->type == REF_WALLCORNER AND static_cast<STRUCTURE_STATS*>(psDroid->
                  asOrderList[order].psOrderTarget)->type == REF_WALL OR static_cast<STRUCTURE_STATS*>(psDroid->asOrderList[order].
                ...
                if (!validCombi)
                {
                  ...
                  down = up + size;
                  // increase the size of a repair facility
                  if (static_cast<STRUCTURE_STATS*>(psDroid->asOrderList[order].psOrderTarget)->type == REF_REPAIR_FACILITY)
                  {
                    left -= 1;
                    up -= 1;
                    right += 1;
                  ...
                  if (((left > site.xTL - 1 AND left <= site.xBR + 1) AND (up > site.yTL - 1 AND up <= site.yBR + 1)) OR ((right > site.xTL
                    - 1 AND right <= site.xBR + 1) AND (up > site.yTL - 1 AND up <= site.yBR + 1)) OR ((left > site.xTL - 1 AND left <= site
                    .xBR + 1) AND (down > site.yTL - 1 AND down <= site.yBR + 1)) OR ((right > site.xTL - 1 AND right <= site.xBR + 1) AND (
...
BOOL getDroidDestination(BASE_STATS* psStats, UDWORD structX, UDWORD structY, UDWORD* pDroidX, UDWORD* pDroidY)
{
  ...
  start = static_cast<UWORD>(rand() % ((width + breadth) * 2));
...
BOOL removeStruct(STRUCTURE* psDel, BOOL bDestroy)
{
  ...
  if (psAssemblyPoint != nullptr)
  {
    mask = static_cast<UBYTE>(1 << psAssemblyPoint->factoryInc);
...
BOOL destroyStruct(STRUCTURE* psDel)
{
  ...
  if (psDel->visible[selectedPlayer])
  {
    ...
    for (i = 0; i < static_cast<UDWORD>(bMinor ? 2 : 4); i++) // only add two for walls - gets crazy otherwise
    {
    ...
    pos.x = psDel->x;
    pos.z = psDel->y;
    pos.y = map_Height(static_cast<UWORD>(pos.x), static_cast<UWORD>(pos.z));
    ...
    if (bMinor) // walls
    {
      ...
      addEffect(&pos, EFFECT_FIRE, FIRE_TYPE_LOCALISED,FALSE, nullptr, 0);
    }
    else if (psDel->pStructureType->type == REF_RESOURCE_EXTRACTOR) // oil resources
    {
      ...
      addEffect(&pos, EFFECT_FIRE, FIRE_TYPE_SMOKY,FALSE, nullptr, 0);
    }
    ...
    if (psDel->pStructureType->type == REF_POWER_GEN)
    {
      addEffect(&pos, EFFECT_DESTRUCTION, DESTRUCTION_TYPE_POWER_STATION,FALSE, nullptr, 0);
    ...
    /* As do wall sections */
      ...
      addEffect(&pos, EFFECT_DESTRUCTION, DESTRUCTION_TYPE_WALL_SECTION,FALSE, nullptr, 0);
    else // and everything else goes here.....
      addEffect(&pos, EFFECT_DESTRUCTION, DESTRUCTION_TYPE_STRUCTURE,FALSE, nullptr, 0);
  ...
  if (psDel->visible[selectedPlayer])
  {
    if (!resourceFound AND !(psDel->pStructureType->type == REF_WALL) AND !(psDel->pStructureType->type == REF_WALLCORNER))
    {
...
SWORD buildFoundation(STRUCTURE_STATS* psStructStats, UDWORD x, UDWORD y)
{
  ...
  return static_cast<SWORD>((foundationMin + foundationMax) / 2);
}
...
SDWORD getStructStatFromName(STRING* pName)
{
  ...
  for (inc = 0; inc < numStructureStats; inc++)
  {
    ...
    if (psStat->NameHash == HashValue) { return inc; }
  }
  return -1;
}
...
SDWORD getStructStatFromName(STRING* pName)
{
  ...
  if (!getResourceName(pName)) { return -1; }
...
BOOL checkSpecificStructExists(UDWORD structInc, UDWORD player)
{
  ...
  ASSERT_TEXT(structInc < numStructureStats, "checkSpecificStructExists: invalid structure inc");

  for (psStructure = apsStructLists[player]; psStructure != nullptr; psStructure = psStructure->psNext)
  {
    if (psStructure->status == SS_BUILT)
    {
      if ((psStructure->pStructureType->ref - REF_STRUCTURE_START) == structInc)
      {
        found = TRUE;
        break;
      }
...
void setAssemblyPoint(FLAG_POSITION* psAssemblyPoint, UDWORD x, UDWORD y, UDWORD player, BOOL bCheck)
{
  ...
  psAssemblyPoint->coords.z = map_Height(static_cast<UWORD>(x), static_cast<UWORD>(y)) + 10;
}
...
void setFlagPositionInc(void* pFunctionality, UDWORD player, UBYTE factoryType)
{
  ...
  {
    psRepair = static_cast<REPAIR_FACILITY*>(pFunctionality);
    psRepair->psDeliveryPoint->factoryInc = 0;
    psRepair->psDeliveryPoint->factoryType = factoryType;
  }
  else
  {
    psFactory = static_cast<FACTORY*>(pFunctionality);
...
void structureCompletedCallback(STRUCTURE_STATS* psStructType)
{
  ...
  if (psStructType->type == REF_FACTORY OR psStructType->type == REF_CYBORG_FACTORY OR psStructType->type == REF_VTOL_FACTORY)
...
void setHQExists(BOOL state, UDWORD player) { hqExists[player] = static_cast<UBYTE>(state); }

/*returns the status of the flag*/
BOOL getHQExists(UDWORD player) { return hqExists[player]; }

/*sets the flag to indicate a SatUplink Exists - so draw everything!*/
void setSatUplinkExists(BOOL state, UDWORD player) { satUplinkExists[player] = static_cast<UBYTE>(state); }

/*returns the status of the flag*/
BOOL getSatUplinkExists(UDWORD player) { return satUplinkExists[player]; }

/*sets the flag to indicate a Las Sat Exists - ONLY EVER WANT ONE*/
void setLasSatExists(BOOL state, UDWORD player) { lasSatExists[player] = static_cast<UBYTE>(state); }

/*returns the status of the flag*/
BOOL getLasSatExists(UDWORD player) { return lasSatExists[player]; }
...
void checkForResExtractors(STRUCTURE* psBuilding)
{
  ...
  for (i = 0; i < NUM_POWER_MODULES; i++)
  {
    if (psPowerGen->apResExtractors[i] != nullptr)
    {
      slot++;
      //make sure the derrrick is active if any oil left
      psResExtractor = (RES_EXTRACTOR*)psPowerGen->apResExtractors[i]->pFunctionality;
      if (psResExtractor->power)
        psResExtractor->active = TRUE;
    }
  }

  psResExtractor = nullptr;
  ...
  {
    for (psCurr = apsStructLists[psBuilding->player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      if (psCurr->pStructureType->type == REF_RESOURCE_EXTRACTOR)
      {
        ...
        if (!((RES_EXTRACTOR*)psCurr->pFunctionality)->active AND psCurr->status == SS_BUILT AND ((RES_EXTRACTOR*)psCurr->pFunctionality)->
          power)
        {
...
void informPowerGen(STRUCTURE* psStruct)
{
  ...
  psPowerGen = (POWER_GEN*)((RES_EXTRACTOR*)psStruct->pFunctionality)->psPowerGen->pFunctionality;
  if (psPowerGen)
  {
    for (i = 0; i < NUM_POWER_MODULES; i++)
    {
...
void releaseResExtractor(STRUCTURE* psRelease)
{
  ...
  ((RES_EXTRACTOR*)psRelease->pFunctionality)->psPowerGen = nullptr;

  //there may be spare resource extractors
  for (psCurr = apsStructLists[psRelease->player]; psCurr != nullptr; psCurr = psCurr->psNext)
  {
    //check not connected and power left and built!
    if (psCurr->pStructureType->type == REF_RESOURCE_EXTRACTOR AND psCurr != psRelease AND !((RES_EXTRACTOR*)psCurr->pFunctionality)->active
      AND ((RES_EXTRACTOR*)psCurr->pFunctionality)->power AND psCurr->status == SS_BUILT)
...
void releasePowerGen(STRUCTURE* psRelease)
{
  ...
  for (i = 0; i < NUM_POWER_MODULES; i++)
  {
    ...
    {
      ((RES_EXTRACTOR*)psPowerGen->apResExtractors[i]->pFunctionality)->active = FALSE;
      ((RES_EXTRACTOR*)psPowerGen->apResExtractors[i]->pFunctionality)->psPowerGen = nullptr;
      psPowerGen->apResExtractors[i] = nullptr;
    }
  }
  ...
  for (psCurr = apsStructLists[psRelease->player]; psCurr != nullptr; psCurr = psCurr->psNext)
  {
    if (psCurr->pStructureType->type == REF_POWER_GEN AND psCurr != psRelease AND psCurr->status == SS_BUILT)
...
{
  psBuilding->currentBuildPts = static_cast<SWORD>(psBuilding->pStructureType->buildPoints);
  ...
  case REF_RESOURCE_EXTRACTOR:
    ...
    /* GJ HACK! - add anim to deriks */
    if (psBuilding->psCurAnim == nullptr)
      psBuilding->psCurAnim = animObj_Add(psBuilding, ID_ANIM_DERIK, 0, 0);
...
void printStructureInfo(STRUCTURE* psStructure)
{
  ...
  case REF_RESOURCE_EXTRACTOR:
    ...
    #endif*/
#ifdef DEBUG
    CONPRINTF(ConsoleString, (ConsoleString,"%s - Unique ID %d", getStatName(psStructure->pStructureType), psStructure->id));
#else
  ...
  case REF_POWER_GEN:
    ...
    CONPRINTF(ConsoleString,
              (ConsoleString,"%s -  Connected %d of %d - Unique ID %d", getStatName(psStructure->pStructureType), numConnected,
                NUM_POWER_MODULES, psStructure->id));
#else
    CONPRINTF(ConsoleString,
              (ConsoleString,strresGetString(psStringRes,STR_GAM_POWCON), getStatName(psStructure->pStructureType), numConnected,
                NUM_POWER_MODULES));
#endif
    break;
  default:
#ifdef DEBUG
    CONPRINTF(ConsoleString,
              (ConsoleString,"%s - Damage %d%% - Unique ID %d", getStatName(psStructure->pStructureType), 100 - PERCENT(psStructure->body,
                structureBody(psStructure)), psStructure->id));
#else
    CONPRINTF(ConsoleString,
              (ConsoleString, getStatName(psStructure->pStructureType), 100 - PERCENT(psStructure->body, structureBody(psStructure))));
#endif
    break;
  }
}
...
BOOL validTemplateForFactory(DROID_TEMPLATE* psTemplate, STRUCTURE* psFactory)
{
  ...
  if (psTemplate->droidType == DROID_CYBORG OR psTemplate->droidType == DROID_CYBORG_SUPER OR psTemplate->droidType ==
    DROID_CYBORG_CONSTRUCT OR psTemplate->droidType == DROID_CYBORG_REPAIR)
  {
    if (psFactory->pStructureType->type != REF_CYBORG_FACTORY)
      return FALSE;
  }
  //check for VTOL droid
  else if ((asPropulsionStats + psTemplate->asParts[COMP_PROPULSION])->propulsionType == LIFT)
  {
...
BOOL electronicDamage(BASE_OBJECT* psTarget, UDWORD damage, UBYTE attackPlayer)
{
  ...
  ASSERT_TEXT(attackPlayer < MAX_PLAYERS, "electronicDamage: invalid player id");
  ...
  if (psTarget->type == OBJ_STRUCTURE)
  {
    ...
    ASSERT_TEXT(psStructure->pStructureType->resistance != 0, "electronicDamage: invalid structure for EW");
    ...
    else
    {
      ...
      psStructure->resistance = static_cast<SWORD>(psStructure->resistance - damage);
      ...
      if (psStructure->resistance < 0)
      {
        ...
        if (psStructure->player == selectedPlayer)
        {
          CONPRINTF(ConsoleString, (ConsoleString,strresGetString( psStringRes,STR_GAM_ELECDAM), getStatName(psStructure->pStructureType)));
  ...
  else if (psTarget->type == OBJ_DROID)
  {
    ...
    else
    {
      ...
      psDroid->resistance = static_cast<SWORD>(psDroid->resistance - damage);
      ...
      if (psDroid->resistance <= 0)
      {
        ...
        {
          CONPRINTF(ConsoleString, (ConsoleString,strresGetString( psStringRes,STR_GAM_ELECDAM)));
        ...
        {
          m.body[0] = DROID_GIFT;
          m.body[1] = psDroid->player;
          m.body[2] = attackPlayer;
          m.type = NET_GIFT;
          m.size = 3;
...
BOOL validStructResistance(STRUCTURE* psStruct)
{
  ...
  if (psStruct->pStructureType->resistance != 0)
  {
    ...
    if (bMultiPlayer)
    {
      ...
      case REF_HQ:
      case REF_REPAIR_FACILITY:
        if (psStruct->resistance >= static_cast<SDWORD>(structureResistance(psStruct->pStructureType, psStruct->player) / 2))
          bTarget = TRUE;
        break;
      default:
        bTarget = TRUE;
...
UDWORD structureArmour(STRUCTURE_STATS* psStats, UBYTE player)
{
  ...
  case REF_WALLCORNER:
  case REF_BLASTDOOR:
    return (psStats->armourValue + (psStats->armourValue * asWallDefenceUpgrade[player].armour) / 100);
    break;
  default:
    return (psStats->armourValue + (psStats->armourValue * asStructureUpgrade[player].armour) / 100);
    break;
  }
}
...
UDWORD structureResistance(STRUCTURE_STATS* psStats, UBYTE player)
{
  ...
  case REF_BLASTDOOR:
    ...
    break;
  default:
    return (psStats->resistance + (psStats->resistance * asStructureUpgrade[player].resistance) / 100);
    break;
  }
}
...
void factoryReward(UBYTE losingPlayer, UBYTE rewardPlayer)
{
  ...
  {
    if (apCompLists[losingPlayer][COMP_PROPULSION][inc] == AVAILABLE AND apCompLists[rewardPlayer][COMP_PROPULSION][inc] != AVAILABLE)
    {
  ...
  if (comp != 0)
  {
    ...
    {
      CONPRINTF(ConsoleString,
                (ConsoleString,"%s :- %s", strresGetString(psStringRes,STR_GAM_REWPROP), getName(asPropulsionStats[comp].pName)));
    }
    return;
  }
  ...
  {
    if (apCompLists[losingPlayer][COMP_BODY][inc] == AVAILABLE AND apCompLists[rewardPlayer][COMP_BODY][inc] != AVAILABLE)
    {
  ...
  if (comp != 0)
  {
    ...
    if (rewardPlayer == selectedPlayer)
    {
      CONPRINTF(ConsoleString, (ConsoleString,"%s :- %s", strresGetString(psStringRes,STR_GAM_REWBODY), getName(asBodyStats[comp].pName)));
    }
    return;
  }
  ...
  {
    if (apCompLists[losingPlayer][COMP_WEAPON][inc] == AVAILABLE AND apCompLists[rewardPlayer][COMP_WEAPON][inc] != AVAILABLE)
    {
  ...
  if (comp != 0)
  {
    ...
    {
      CONPRINTF(ConsoleString,
                (ConsoleString,"%s :- %s", strresGetString(psStringRes,STR_GAM_REWWEAP), getName(asWeaponStats[comp].pName)));
    }
    return;
  }
...
void repairFacilityReward(UBYTE losingPlayer, UBYTE rewardPlayer)
{
  ...
  {
    if (apCompLists[losingPlayer][COMP_REPAIRUNIT][inc] == AVAILABLE AND apCompLists[rewardPlayer][COMP_REPAIRUNIT][inc] != AVAILABLE)
    {
  ...
  if (comp != 0)
  {
    ...
    {
      CONPRINTF(ConsoleString,
                (ConsoleString,"%s :- %s", strresGetString(psStringRes,STR_GAM_REWREPA), getName(asRepairStats[comp].pName)));
    }
    return;
  }
  if (rewardPlayer == selectedPlayer)
...
{
  if ((Struct->pStructureType->type == REF_FACTORY) || (Struct->pStructureType->type == REF_CYBORG_FACTORY) || (Struct->pStructureType->type
    == REF_VTOL_FACTORY))
    return TRUE;
...
{
  if ((psCurrFlag->factoryType == FACTORY_FLAG) || (psCurrFlag->factoryType == CYBORG_FLAG) || (psCurrFlag->factoryType == VTOL_FLAG))
    return TRUE;
...
FLAG_POSITION* FindFactoryDelivery(STRUCTURE* Struct)
{
  ...
  if (StructIsFactory(Struct))
  {
    ...
    for (psCurrFlag = apsFlagPosLists[selectedPlayer]; psCurrFlag; psCurrFlag = psCurrFlag->psNext)
    {
      if (FlagIsFactory(psCurrFlag) && ((FACTORY*)Struct->pFunctionality)->psAssemblyPoint->factoryInc == psCurrFlag->factoryInc && ((
        FACTORY*)Struct->pFunctionality)->psAssemblyPoint->factoryType == psCurrFlag->factoryType)
        return psCurrFlag;
    }
  }
...
STRUCTURE* findDeliveryFactory(FLAG_POSITION* psDelPoint)
{
  ...
  for (psCurr = apsStructLists[psDelPoint->player]; psCurr != nullptr; psCurr = psCurr->psNext)
  {
    if (StructIsFactory(psCurr))
    {
      psFactory = (FACTORY*)psCurr->pFunctionality;
      if (psFactory->psAssemblyPoint->factoryInc == psDelPoint->factoryInc AND psFactory->psAssemblyPoint->factoryType == psDelPoint->
        factoryType)
        return psCurr;
    }
...
void cancelProduction(STRUCTURE* psBuilding)
{
  ...
  ASSERT_TEXT(StructIsFactory(psBuilding), "cancelProduction: structure not a factory");
  ...
  if (psBuilding->player == productionPlayer AND psFactory->psSubject)
  {
    ...
    memset(asProductionRun[psFactory->psAssemblyPoint->factoryType][psFactory->psAssemblyPoint->factoryInc], 0,
...
void holdProduction(STRUCTURE* psBuilding)
{
  ...
  ASSERT_TEXT(StructIsFactory(psBuilding), "holdProduction: structure not a factory");
...
void releaseProduction(STRUCTURE* psBuilding)
{
  ...
  ASSERT_TEXT(StructIsFactory(psBuilding), "releaseProduction: structure not a factory");
...
DROID_TEMPLATE* factoryProdUpdate(STRUCTURE* psStructure, DROID_TEMPLATE* psTemplate)
{
  ...
  ASSERT_TEXT(psStructure->player == productionPlayer, "factoryProdUpdate: called for incorrect player");
  ...
  if (psTemplate != nullptr)
  {
    ...
    for (inc = 0; inc < MAX_PROD_RUN; inc++)
    {
      if (asProductionRun[factoryType][factoryInc][inc].psTemplate == psTemplate)
      {
        ...
        if (asProductionRun[factoryType][factoryInc][inc].built < asProductionRun[factoryType][factoryInc][inc].quantity)
          return psTemplate;
        break;
      }
    }
  ...
  if (psFactory->quantity)
  {
    ...
    return (factoryProdUpdate(psStructure, nullptr));
  }
  //if got to here then nothing left to produce so clear the array
  memset(asProductionRun[factoryType][factoryInc], 0, sizeof(PRODUCTION_RUN) * MAX_PROD_RUN);
  return nullptr;
}
...
void factoryProdAdjust(STRUCTURE* psStructure, DROID_TEMPLATE* psTemplate, BOOL add)
{
  ...
  ASSERT_TEXT(psStructure->player == productionPlayer, "factoryProdAdjust: called for incorrect player");
    ...
    if (asProductionRun[factoryType][factoryInc][inc].psTemplate == psTemplate)
    {
      ...
      if (add)
      {
        ...
        if (asProductionRun[factoryType][factoryInc][inc].quantity > MAX_IN_RUN)
        {
          ...
          asProductionRun[factoryType][factoryInc][inc].psTemplate = nullptr;
  ...
  if (bCheckForCancel)
  {
    for (inc = 0; inc < MAX_PROD_RUN; inc++)
    {
      if (asProductionRun[factoryType][factoryInc][inc].psTemplate != nullptr)
        break;
    }
    if (inc == MAX_PROD_RUN)
    {
    ...
    for (factoryInc = 0; factoryInc < MAX_FACTORY; factoryInc++)
    {
      ...
      if ((factoryNumFlag[player][factoryType] & mask) == mask)
      {
        for (inc = 0; inc < MAX_PROD_RUN; inc++)
        {
          if (asProductionRun[factoryType][factoryInc][inc].psTemplate)
          {
            if (asProductionRun[factoryType][factoryInc][inc].psTemplate->droidType == DROID_COMMAND)
            {
              quantity = static_cast<UBYTE>(quantity + (asProductionRun[factoryType][factoryInc][inc].quantity - asProductionRun[
...
UWORD countAssignableFactories(UBYTE player, UWORD factoryType)
{
  ...
  ASSERT_TEXT(player == selectedPlayer, "countAssignableFactories: should only be called for selectedPlayer");
...
{
  ASSERT_TEXT(player < MAX_PLAYERS, "checkFactoryExists: invalid player");
  ASSERT_TEXT(factoryType < NUM_FACTORY_TYPES, "checkFactoryExists: invalid factoryType");
...
void checkDeliveryPoints(UDWORD version)
{
  ...
  for (inc = 0; inc < MAX_PLAYERS; inc++)
  {
    ...
    {
      for (psStruct = apsStructLists[inc]; psStruct != nullptr; psStruct = psStruct->psNext)
      {
        if (StructIsFactory(psStruct))
        {
          ...
          {
            ASSERT_TEXT(psFactory->psAssemblyPoint != NULL, "checkDeliveryPoints: no delivery point for factory");
          }
          else
          {
            setAssemblyPoint(psFactory->psAssemblyPoint, psFactory->psAssemblyPoint->coords.x, psFactory->psAssemblyPoint->coords.y, inc,
                             TRUE);
          }
        }
        else if (psStruct->pStructureType->type == REF_REPAIR_FACILITY)
        {
          ...
          {
            setAssemblyPoint(psRepair->psDeliveryPoint, psRepair->psDeliveryPoint->coords.x, psRepair->psDeliveryPoint->coords.y, inc,
                             TRUE);
          }
        }
      }
...
void factoryLoopAdjust(STRUCTURE* psStruct, BOOL add)
{
  ...
  ASSERT_TEXT(StructIsFactory(psStruct), "factoryLoopAdjust: structure is not a factory");
  ASSERT_TEXT(psStruct->player == selectedPlayer, "factoryLoopAdjust: should only be called for selectedPlayer");
...
void checkResExtractorsActive(void)
{
  ...
  for (inc = 0; inc < MAX_PLAYERS; inc++)
  {
    for (psStruct = apsStructLists[inc]; psStruct != nullptr; psStruct = psStruct->psNext)
    {
      if (psStruct->pStructureType->type == REF_RESOURCE_EXTRACTOR)
      {
        if (((RES_EXTRACTOR*)psStruct->pFunctionality)->active) { ((RES_EXTRACTOR*)psStruct->pFunctionality)->timeLastUpdated = gameTime; }
      }
    }
  }
}
...
BOOL structSensorDroidWeapon(STRUCTURE* psStruct, DROID* psDroid)
{
  ...
  if (structStandardSensor(psStruct) AND (psDroid->asWeaps[0].nStat > 0 AND !proj_Direct(asWeaponStats + psDroid->asWeaps[0].nStat)) AND !
    vtolDroid(psDroid))
    return TRUE;
  //CB Sensor Tower + indirect weapon droid (non VTOL)
  //if (structCBSensor(psStruct) AND (psDroid->numWeaps AND 
  if (structCBSensor(psStruct) AND (psDroid->asWeaps[0].nStat > 0 AND !proj_Direct(asWeaponStats + psDroid->asWeaps[0].nStat)) AND !
    vtolDroid(psDroid))
    return TRUE;
  //VTOL Intercept Sensor Tower + any weapon VTOL droid
  //else if (structVTOLSensor(psStruct) AND psDroid->numWeaps AND
  if (structVTOLSensor(psStruct) AND psDroid->asWeaps[0].nStat > 0 AND vtolDroid(psDroid))
    return TRUE;
  //VTOL CB Sensor Tower + any weapon VTOL droid 
  //else if (structVTOLCBSensor(psStruct) AND psDroid->numWeaps AND
  if (structVTOLCBSensor(psStruct) AND psDroid->asWeaps[0].nStat > 0 AND vtolDroid(psDroid))
    return TRUE;
...
BOOL structCBSensor(STRUCTURE* psStruct)
{
  if (psStruct->pStructureType->pSensor)
  {
    ...
    if ((psStruct->pStructureType->pSensor->type == INDIRECT_CB_SENSOR OR psStruct->pStructureType->pSensor->type == SUPER_SENSOR) AND
...
BOOL structStandardSensor(STRUCTURE* psStruct)
{
  ...
  {
    /*Super Sensor works as any type*/
    if ((psStruct->pStructureType->pSensor->type == STANDARD_SENSOR OR psStruct->pStructureType->pSensor->type == SUPER_SENSOR) AND psStruct
      ->pStructureType->pSensor->location == LOC_TURRET)
      return TRUE;
  }
  return FALSE;
}
...
BOOL structVTOLSensor(STRUCTURE* psStruct)
{
  ...
  {
    //Super Sensor works as any type
    if ((psStruct->pStructureType->pSensor->type == VTOL_INTERCEPT_SENSOR OR psStruct->pStructureType->pSensor->type == SUPER_SENSOR) AND
      psStruct->pStructureType->pSensor->location == LOC_TURRET)
      return TRUE;
  }
  return FALSE;
...
BOOL structVTOLCBSensor(STRUCTURE* psStruct)
{
  ...
  {
    //Super Sensor works as any type
    if ((psStruct->pStructureType->pSensor->type == VTOL_CB_SENSOR OR psStruct->pStructureType->pSensor->type == SUPER_SENSOR) AND psStruct
      ->pStructureType->pSensor->location == LOC_TURRET)
      return TRUE;
  }
  return FALSE;
...
{
  if ((psStruct->pStructureType->type == REF_REARM_PAD) && (((REARM_PAD*)psStruct->pFunctionality)->psObj == nullptr || vtolHappy(
    (DROID*)((REARM_PAD*)psStruct->pFunctionality)->psObj)))
    return TRUE;
...
STRUCTURE* findNearestReArmPad(DROID* psDroid, STRUCTURE* psTarget, BOOL bClear)
{
  ...
  if (psTarget != nullptr)
  {
    cx = static_cast<SDWORD>(psTarget->x);
    cy = static_cast<SDWORD>(psTarget->y);
  }
  else
  {
    cx = static_cast<SDWORD>(psDroid->x);
    cy = static_cast<SDWORD>(psDroid->y);
  }
  ...
  {
    if ((psStruct->pStructureType->type == REF_REARM_PAD) && (psTarget == nullptr || psTarget->cluster == psStruct->cluster) && (!bClear ||
      clearRearmPad(psStruct)))
    {
      xdiff = static_cast<SDWORD>(psStruct->x) - cx;
      ydiff = static_cast<SDWORD>(psStruct->y) - cy;
      currdist = xdiff * xdiff + ydiff * ydiff;
...
void ensureRearmPadClear(STRUCTURE* psStruct, DROID* psDroid)
{
  ...
  {
    if ((psCurr != psDroid) && (psCurr->x >> TILE_SHIFT == tx) && (psCurr->y >> TILE_SHIFT == ty) && (vtolDroid(psCurr)))
...
BOOL vtolOnRearmPad(STRUCTURE* psStruct, DROID* psDroid)
{
  ...
  for (psCurr = apsDroidLists[psDroid->player]; psCurr; psCurr = psCurr->psNext)
  {
    if ((psCurr != psDroid) && (psCurr->x >> TILE_SHIFT == tx) && (psCurr->y >> TILE_SHIFT == ty))
    {
      found = TRUE;
      break;
    }
...
STRUCTURE* giftSingleStructure(STRUCTURE* psStructure, UBYTE attackPlayer, BOOL bFromScript)
{
  ...
  if (!bMultiPlayer)
  {
    ...
    {
      ASSERT_TEXT(FALSE, "giftSingleStructure: EW attack by selectedPlayer on a structure");
      return nullptr;
    }
  }
  ...
  if (bMultiPlayer)
  {
    ...
    if (!electronicReward(psStructure, attackPlayer))
    {
      ...
      psStructure->resistance = static_cast<UWORD>(structureResistance(psStructure->pStructureType, psStructure->player));
      ...
      {
        if (psCurr->psTarget == (BASE_OBJECT*)psStructure OR psCurr->psActionTarget == (BASE_OBJECT*)psStructure)
  ...
  if (psModule)
  {
    ...
    {
    case REF_POWER_GEN:
      capacity = static_cast<UBYTE>(((POWER_GEN*)psStructure->pFunctionality)->capacity);
      break;
    case REF_RESEARCH:
      capacity = static_cast<UBYTE>(((RESEARCH_FACILITY*)psStructure->pFunctionality)->capacity);
      break;
    case REF_FACTORY:
    case REF_VTOL_FACTORY:
      capacity = ((FACTORY*)psStructure->pFunctionality)->capacity;
  ...
  if (psNewStruct)
  {
    ...
    if (capacity)
    {
      ...
      case REF_RESEARCH:
        ...
        buildStructure(psModule, psNewStruct->x, psNewStruct->y, attackPlayer, FALSE);
        break;
      case REF_FACTORY:
      case REF_VTOL_FACTORY:
...
BOOL checkStructureStats(void)
{
  ...
  for (structInc = 0; structInc < numStructureStats; structInc++)
  {
    ...
    else
    {
      ...
      {
        ASSERT_TEXT(FALSE, "checkStructureStats:Invalid functions attached to structure {}", asStructureStats[structInc].pName);
        return FALSE;
      }
    }
  }
...
UDWORD structPowerToBuild(STRUCTURE* psStruct)
{
  ...
  if (psStat)
  {
    ...
    {
    case REF_POWER_GEN:
      capacity = static_cast<UBYTE>(((POWER_GEN*)psStruct->pFunctionality)->capacity);
      break;
    case REF_RESEARCH:
      capacity = static_cast<UBYTE>(((RESEARCH_FACILITY*)psStruct->pFunctionality)->capacity);
      break;
    case REF_FACTORY:
    case REF_VTOL_FACTORY:
...
void resetResistanceLag(STRUCTURE* psBuilding)
{
  if (bMultiPlayer)
  {
    ...
    case REF_RESEARCH:
      {
        auto psResFacility = (RESEARCH_FACILITY*)psBuilding->pFunctionality;
    ...
    case REF_CYBORG_FACTORY:
      {
        auto psFactory = (FACTORY*)psBuilding->pFunctionality;
...
{
  if ((psStruct->selected OR (bMultiPlayer && !isHumanPlayer(psStruct->player))) AND psStruct->asWeaps[0].nStat AND (asWeaponStats[psStruct
    ->asWeaps[0].nStat].weaponSubClass == WSC_LAS_SAT))
    return TRUE;
