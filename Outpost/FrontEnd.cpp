...
void frontEndNewGame(void)
{
  ...
  case 1:
    ...
    seq_ClearSeqList();
    seq_AddSeqToList("CAM1\\c001.mp4", nullptr, "CAM1\\c001.txa",FALSE, 0);
...
VOID addBackdrop(VOID)
{
  ...
  sFormInit.x = static_cast<SWORD>((DISP_WIDTH - HIDDEN_FRONTEND_WIDTH) / 2);
  sFormInit.y = static_cast<SWORD>((DISP_HEIGHT - HIDDEN_FRONTEND_HEIGHT) / 2);
...
VOID removeBackdrop(VOID) { widgDelete(psWScreen, FRONTEND_BACKDROP); }
...
VOID removeBottomForm(VOID) { widgDelete(psWScreen, FRONTEND_BOTFORM); }
...
VOID addTopForm(VOID)
{
  ...
  sFormInit.x = static_cast<short>((sFormInit.width / 2) - (FRONTEND_LOGOW / 2)); //115;
  sFormInit.y = static_cast<short>((sFormInit.height / 2) - (FRONTEND_LOGOH / 2)); //18;
...
VOID removeTopForm(VOID) { widgDelete(psWScreen, FRONTEND_TOPFORM); }
...
VOID addTextButton(UDWORD id, UDWORD PosX, UDWORD PosY, STRING* txt, BOOL bAlign, BOOL bGrey)
{
  ...
  sButInit.id = id;
  sButInit.x = static_cast<short>(PosX);
  sButInit.y = static_cast<short>(PosY);
...
VOID addSideText(UDWORD id, UDWORD PosX, UDWORD PosY, STRING* txt)
{
  ...
  sLabInit.x = static_cast<short>(PosX);
  sLabInit.y = static_cast<short>(PosY);
  sLabInit.width = 30;
...
VOID addText(int FontID, UDWORD FormID, UDWORD id, UDWORD PosX, UDWORD PosY, STRING* txt, UDWORD attachID, BOOL* State)
{
  ...
  sLabInit.style = WLAB_PLAIN;
  sLabInit.x = static_cast<short>(PosX);
  sLabInit.y = static_cast<short>(PosY + 4);
  sLabInit.width = 16;
...
VOID displayTextOption(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  {
    if (psWidget->style & WBUT_TXTCENTRE)
    {
      //check for centering, calculate offset.
      AddCursorSnap(&InterfaceSnap, static_cast<SWORD>(fx + fw / 2), static_cast<short>(fy), psWidget->formID, psWidget->id, &FrontendBias);
    }
    else
      AddCursorSnap(&InterfaceSnap, static_cast<SWORD>(fx + 10), static_cast<short>(fy), psWidget->formID, psWidget->id, &FrontendBias);
  }
}
