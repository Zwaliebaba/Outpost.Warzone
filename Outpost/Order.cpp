...
void orderDroidStatsTwoLocAdd(DROID* psDroid, DROID_ORDER order, BASE_STATS* psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2);
...
void orderCheckGuardPosition(DROID* psDroid, SDWORD range)
{
  ...
  if (psDroid->psTarget != nullptr)
  {
    ...
    if ((!(psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR)) && psDroid->psTarget->type == OBJ_DROID &&
      orderStateLoc((DROID*)psDroid->psTarget, DORDER_MOVE, &x, &y))
    {
      // got a moving droid - check against where the unit is going
      psDroid->orderX = static_cast<UWORD>(x);
      psDroid->orderY = static_cast<UWORD>(y);
    }
    else
    {
  ...
  xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psDroid->orderX);
  ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psDroid->orderY);
  if (xdiff * xdiff + ydiff * ydiff > range * range)
  {
    turnOffMultiMsg(TRUE);
    if ((psDroid->sMove.Status != MOVEINACTIVE) && ((psDroid->action == DACTION_MOVE) || (psDroid->action == DACTION_MOVEFIRE)))
    {
      xdiff = psDroid->sMove.DestinationX - static_cast<SDWORD>(psDroid->orderX);
      ydiff = psDroid->sMove.DestinationY - static_cast<SDWORD>(psDroid->orderY);
      if (xdiff * xdiff + ydiff * ydiff > range * range)
...
BASE_OBJECT* checkForRepairRange(DROID* psDroid, DROID* psTarget)
{
  ...
  DEBUG_ASSERT_TEXT(psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR, "checkForRepairRange:Invalid droid type");

  if (psTarget != nullptr && psTarget->died)
    psTarget = nullptr;

  // if guarding a unit - always check that first
  if (orderStateObj(psDroid, DORDER_GUARD, (BASE_OBJECT**)&psCurr) && (psCurr != nullptr) && (psCurr->type == OBJ_DROID) &&
    droidIsDamaged(psCurr))
    return (BASE_OBJECT*)psCurr;

  /* Used to be a define. Now uses their sensor range */
  //back to the define!
  if ((psTarget != nullptr) && (psTarget->type == OBJ_DROID) && (psTarget->player == psDroid->player))
  ...
  {
    //check for damage
    if (droidIsDamaged(psCurr) && visibleObject((BASE_OBJECT*)psDroid, (BASE_OBJECT*)psCurr))
    {
      //check for within range
      xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psCurr->x);
      ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psCurr->y);
...
BASE_OBJECT* checkForDamagedStruct(DROID* psDroid, STRUCTURE* psTarget)
{
  ...
  DEBUG_ASSERT_TEXT(psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType ==
    DROID_CYBORG_CONSTRUCT, "checkForDamagedStruct:Invalid unit type");

  if (psTarget != nullptr && psTarget->died)
    psTarget = nullptr;

  if ((psTarget != nullptr) && (psTarget->type == OBJ_STRUCTURE) && (psTarget->player == psDroid->player))
    psCurr = psTarget->psNext;
  else
  ...
  {
    //check for damage
    if (structIsDamaged(psCurr) && visibleObject((BASE_OBJECT*)psDroid, (BASE_OBJECT*)psCurr))
    {
      //check for within range
      xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psCurr->x);
      ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psCurr->y);
...
void orderUpdateDroid(DROID* psDroid)
{
  ...
  case DORDER_NONE:
    ...
    if (psDroid->droidType != DROID_COMMAND && psDroid->psGroup != nullptr && psDroid->psGroup->type == GT_COMMAND && (psDroid->psTarStats
      != (BASE_STATS*)structGetDemolishStat())) // stop the constructor auto repairing when it is about to demolish
    ...
    else if (psDroid->droidType == DROID_TRANSPORTER)
    {
      ...
      if (psDroid->player == selectedPlayer AND getDroidsToSafetyFlag() AND !missionDroidsRemaining(selectedPlayer))
      {
    ...
    else if ((psDroid->player == selectedPlayer) && (psDroid->psTarStats != (BASE_STATS*)structGetDemolishStat()) &&
      // stop the constructor auto repairing when it is about to demolish
      secondaryGetState(psDroid, DSO_HALTTYPE, &state) && !vtolDroid(psDroid))
    {
    ...
    else if ((psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR) && !orderState(psDroid, DORDER_GUARD))
    {
      psObj = checkForRepairRange(psDroid, nullptr);
      if (psObj)
      {
    ...
    else if ((psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT) && !orderState(psDroid, DORDER_GUARD)
      && (psDroid->psTarStats != (BASE_STATS*)structGetDemolishStat()))
    {
      psObj = checkForDamagedStruct(psDroid, nullptr);
      if (psObj)
      {
        if (!bMultiPlayer || myResponsibility(psDroid->player))
  ...
  case DORDER_TRANSPORTIN:
    if ((psDroid->action == DACTION_NONE) && (psDroid->sMove.Status == MOVEINACTIVE))
    {
      /* clear order */
      psDroid->order = DORDER_NONE;
      psDroid->psTarget = nullptr;
  ...
  case DORDER_SCOUT_ATTACKWALL:
    ...
    else if ((((psDroid->action != DACTION_ATTACK) && (psDroid->action != DACTION_MOVETOATTACK) && (psDroid->action !=
      DACTION_ROTATETOATTACK)) || (psDroid->psActionTarget != psDroid->psTarget)) && actionInRange(psDroid, psDroid->psTarget))
  ...
  case DORDER_PATROL:
    ...
    if ((psDroid->action == DACTION_MOVE) && (secondaryGetState(psDroid, DSO_ATTACK_LEVEL, &state) && (state == DSS_ALEV_ALWAYS)) &&
    ...
    else if (psDroid->action == DACTION_NONE)
    {
      ...
      if (xdiff * xdiff + ydiff * ydiff < SCOUT_DIST * SCOUT_DIST)
      {
        if (psDroid->order == DORDER_PATROL)
        {
          ...
          psDroid->orderX2 = static_cast<UWORD>(temp);
          temp = psDroid->orderY;
          psDroid->orderY = psDroid->orderY2;
          psDroid->orderY2 = static_cast<UWORD>(temp);
          actionDroidLoc(psDroid, DACTION_MOVE, psDroid->orderX, psDroid->orderY);
        }
        else
          psDroid->order = DORDER_NONE;
    ...
    else if ((psDroid->action == DACTION_ATTACK) || (psDroid->action == DACTION_MOVETOATTACK) || (psDroid->action == DACTION_ROTATETOATTACK)
      || (psDroid->action == DACTION_OBSERVE) || (psDroid->action == DACTION_MOVETOOBSERVE))
    {
      // attacking something - see if the droid has gone too far
      xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psDroid->actionX);
      ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psDroid->actionY);
  ...
  case DORDER_CLEARWRECK:
    if (psDroid->action == DACTION_NONE || psDroid->psTarget == nullptr)
    {
      psDroid->order = DORDER_NONE;
      actionDroid(psDroid, DACTION_NONE);
    ...
    break;
  case DORDER_REARM:
    if ((psDroid->psTarget == nullptr) || (psDroid->psActionTarget == nullptr))
    {
  ...
  case DORDER_ATTACKTARGET:
    ...
    else if (((psDroid->action == DACTION_MOVE) || (psDroid->action == DACTION_MOVEFIRE)) && actionVisibleTarget(psDroid, psDroid->psTarget)
      && !vtolDroid(psDroid))
    {
      // moved near enough to attack change to attack action
      actionDroidObj(psDroid, DACTION_ATTACK, psDroid->psTarget);
    }
    else if ((psDroid->action == DACTION_MOVETOATTACK) && !vtolDroid(psDroid) && !actionVisibleTarget(psDroid, psDroid->psTarget))
    {
      // lost sight of the target while chasing it - change to a move action so
      // that the unit will fire on other things while moving
      actionDroidLoc(psDroid, DACTION_MOVE, psDroid->psTarget->x, psDroid->psTarget->y);
    }
    else if (!vtolDroid(psDroid) && (psDroid->psTarget == psDroid->psActionTarget) && actionInRange(psDroid, psDroid->psTarget) &&
      visGetBlockingWall((BASE_OBJECT*)psDroid, psDroid->psTarget, &psWall) && (psWall->player != psDroid->player))
    {
      // there is a wall in the way - attack that
      actionDroidObj(psDroid, DACTION_ATTACK, (BASE_OBJECT*)psWall);
    }
    else if ((psDroid->action == DACTION_NONE) || (psDroid->action == DACTION_CLEARREARMPAD))
    {
      ...
      else if (!vtolDroid(psDroid) || allVtolsRearmed(psDroid))
        actionDroidObj(psDroid, DACTION_ATTACK, psDroid->psTarget);
    }
    break;
  case DORDER_BUILD:
    if (psDroid->action == DACTION_BUILD && psDroid->psTarget == nullptr)
    {
      psDroid->order = DORDER_NONE;
      actionDroid(psDroid, DACTION_NONE);
    }
  ...
  case DORDER_EMBARK:
    ...
    else
    {
      ...
      else if ((abs(static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psDroid->psTarget->x)) < TILE_UNITS) && (abs(
        static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psDroid->psTarget->y)) < TILE_UNITS))
      {
  ...
  case DORDER_DISEMBARK:
    ...
    if (bMultiPlayer)
    {
      ...
      if (psDroid->droidType == DROID_TRANSPORTER)
      {
        ...
        if (psDroid->action == DACTION_NONE AND psDroid->sMove.Status == MOVEINACTIVE AND psDroid->sMove.iVertSpeed == 0)
        {
  ...
  case DORDER_LEAVEMAP:
    if ((psDroid->x < TILE_UNITS * 2) || (psDroid->x > (mapWidth - 2) * TILE_UNITS) || (psDroid->y < TILE_UNITS * 2) || (psDroid->y > (
      mapHeight - 2) * TILE_UNITS) || (psDroid->action == DACTION_NONE))
    {
      psDroid->order = DORDER_NONE;
  ...
  case DORDER_RTR_SPECIFIED:
    ...
    else if (psDroid->action == DACTION_NONE)
    {
      ...
      DEBUG_ASSERT_TEXT(PTRVALID(psStruct, sizeof(STRUCTURE)), "orderUpdateUnit: invalid structure pointer");
      psRepairFac = (REPAIR_FACILITY*)psStruct->pFunctionality;
      DEBUG_ASSERT_TEXT(PTRVALID(psRepairFac, sizeof(REPAIR_FACILITY)), "orderUpdateUnit: invalid repair facility pointer");

      xdiff = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psDroid->psTarget->x);
      ydiff = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psDroid->psTarget->y);
    ...
    else if (psDroid->order == DORDER_RTR && (psDroid->action == DACTION_MOVE || psDroid->action == DACTION_MOVEFIRE) && ((psDroid->id % 50)
      == (frameGetFrameNumber() % 50)))
    {
  ...
  case DORDER_LINEBUILD:
    if (psDroid->action == DACTION_NONE || (psDroid->action == DACTION_BUILD && psDroid->psTarget == nullptr))
    {
      // finished building the current structure
      if (psDroid->orderX >> TILE_SHIFT == psDroid->orderX2 >> TILE_SHIFT && psDroid->orderY >> TILE_SHIFT == psDroid->orderY2 >>
        TILE_SHIFT)
      {
      ...
      {
        DEBUG_ASSERT_TEXT(FALSE, "orderUpdateUnit: LINEBUILD order on diagonal line");
        break;
      }
  ...
  case DORDER_FIRESUPPORT:
    ...
    {
      psFireTarget = nullptr;
      if (psDroid->psTarget->type == OBJ_DROID)
      {
        psSpotter = (DROID*)psDroid->psTarget;
        if ((psSpotter->action == DACTION_OBSERVE) || (psSpotter->droidType == DROID_COMMAND && psSpotter->action == DACTION_ATTACK))
      ...
      if (psFireTarget)
      {
        ...
        if (vtolDroid(psDroid))
        {
          if (!vtolEmpty(psDroid) && ((psDroid->action == DACTION_MOVETOREARM) || (psDroid->action == DACTION_WAITFORREARM)) && (psDroid->
        ...
        if (bAttack && (!droidAttacking(psDroid) || psFireTarget != psDroid->psActionTarget))
        {
          //get the droid to attack
          actionDroidObj(psDroid, DACTION_ATTACK, psFireTarget);
        }
      }
      else if (vtolDroid(psDroid) && (psDroid->action != DACTION_NONE) && (psDroid->action != DACTION_FIRESUPPORT))
        moveToRearm(psDroid);
      else if ((psDroid->action != DACTION_FIRESUPPORT) && (psDroid->action != DACTION_FIRESUPPORT_RETREAT))
  ...
  case DORDER_GUARD:
        ...
        else*/
    {
      ...
      if ((psDroid->action == DACTION_NONE) || (psDroid->action == DACTION_MOVE) || (psDroid->action == DACTION_MOVEFIRE))
      {
        // not doing anything, make sure the droid is close enough
        // to the thing it is defending
        //if ((psDroid->droidType != DROID_REPAIR) &&
        if ((!(psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR)) && psDroid->psTarget != nullptr && psDroid
          ->psTarget->type == OBJ_DROID && ((DROID*)psDroid->psTarget)->droidType == DROID_COMMAND)
        {
      ...
      else if (psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR)
      {
        // repairing something, make sure the droid doesn't go too far
        orderCheckGuardPosition(psDroid, REPAIR_MAXDIST);
      }
      else if (psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT)
      {
        // repairing something, make sure the droid doesn't go too far
      ...
      else
      {
        ...
        if (!vtolRearming(psDroid))
        {
          ...
          if (psDroid->psTarget != nullptr && psDroid->psTarget->type == OBJ_DROID && ((DROID*)psDroid->psTarget)->droidType ==
            DROID_COMMAND)
          {
            // guarding a commander, allow more space
    ...
    if ((psDroid->droidType != DROID_COMMAND) && psDroid->psGroup && (psDroid->psGroup->type == GT_COMMAND))
    {
      if ((psDroid->psGroup->psCommander->action == DACTION_ATTACK) && (psDroid->psGroup->psCommander->psActionTarget != nullptr))
      {
        psObj = psDroid->psGroup->psCommander->psActionTarget;
        if (psDroid->action == DACTION_ATTACK || psDroid->action == DACTION_MOVETOATTACK)
        {
      ...
      if (!orderStateObj(psDroid, DORDER_GUARD, &psObj) || (psObj != (BASE_OBJECT*)psDroid->psGroup->psCommander))
        orderDroidObj(psDroid, DORDER_GUARD, (BASE_OBJECT*)psDroid->psGroup->psCommander);
    }

    //repair droids default to repairing droids within a given range
    psObj = nullptr;
    if ((psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR) &&
    ...
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "orderUpdateUnit: unknown order");
  }
...
void orderCmdGroupBase(DROID_GROUP* psGroup, DROID_ORDER_DATA* psData)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psGroup, sizeof(DROID_GROUP)), "cmdUnitOrderGroupBase: invalid unit group");
  ...
  if (psData->order == DORDER_RECOVER)
  {
    ...
    {
      xdiff = static_cast<SDWORD>(psCurr->x) - static_cast<SDWORD>(psData->psObj->x);
      ydiff = static_cast<SDWORD>(psCurr->y) - static_cast<SDWORD>(psData->psObj->y);
...
void orderCheckFireSupportPos(DROID* psSensor, DROID_ORDER_DATA* psOrder)
{
  ...
  {
    if (!vtolDroid(psCurr) && orderStateObj(psCurr, DORDER_FIRESUPPORT, &psTarget) && (psTarget == (BASE_OBJECT*)psSensor) && (
      secondaryGetState(psCurr, DSO_HALTTYPE, &state) && (state != DSS_HALT_HOLD)))
    {
      // got a unit doing fire support
      fsnum += 1;
      fsx += static_cast<SDWORD>(psCurr->x);
      fsy += static_cast<SDWORD>(psCurr->y);
    }
  }
  ...
  if (fsnum != 0)
  {
    ...
    xdiff = fsx - static_cast<SDWORD>(psSensor->x);
    ydiff = fsy - static_cast<SDWORD>(psSensor->y);
    if (xdiff * xdiff + ydiff * ydiff < (TILE_UNITS * 5) * (TILE_UNITS * 5))
      goto done;

    sensorVX = static_cast<SDWORD>(psOrder->x) - static_cast<SDWORD>(psSensor->x);
    sensorVY = static_cast<SDWORD>(psOrder->y) - static_cast<SDWORD>(psSensor->y);
    fsVX = fsx - static_cast<SDWORD>(psSensor->x);
    fsVY = fsy - static_cast<SDWORD>(psSensor->y);
...
static void orderPlayFireSupportAudio(BASE_OBJECT* psObj)
{
  ...
  case OBJ_DROID:
    psDroid = (DROID*)psObj;
    DEBUG_ASSERT_TEXT(PTRVALID(psObj, sizeof(DROID)), "orderPlayFireSupportAudio: invalid droid pointer");
  ...
  case OBJ_STRUCTURE: DEBUG_ASSERT_TEXT(PTRVALID(psObj, sizeof(STRUCTURE)), "orderPlayFireSupportAudio: invalid structure pointer");
...
void orderDroidBase(DROID* psDroid, DROID_ORDER_DATA* psOrder)
{
  ...
  // deal with a droid receiving a primary order
  if (secondaryGotPrimaryOrder(psDroid, static_cast<DROID_ORDER>(psOrder->order)))
    psOrder->order = DORDER_NONE;

  // if this is a command droid - all it's units do the same thing
  if ((psDroid->droidType == DROID_COMMAND) && (psDroid->psGroup != nullptr) && (psDroid->psGroup->type == GT_COMMAND) && (psOrder->order !=
      DORDER_GUARD) && //(psOrder->psObj == NULL)) &&
    (psOrder->order != DORDER_RTR) && (psOrder->order != DORDER_RECYCLE) && (psOrder->order != DORDER_MOVE))
  {
    ...
    if ((psOrder->order == DORDER_RECOVER) && (psDroid->psGroup->psList != nullptr))
      psOrder->order = DORDER_NONE;
  }
  ...
  case DORDER_SCOUT:
    ...
    if (vtolDroid(psDroid) && (fpathGroundBlockingTile(psOrder->x >> TILE_SHIFT, psOrder->y >> TILE_SHIFT) || !TEST_TILE_VISIBLE(
      psDroid->player, mapTile(psOrder->x >> TILE_SHIFT,psOrder->y >> TILE_SHIFT))))
      break;
    //in multiPlayer, cannot move Transporter to blocking tile either
    if (game.maxPlayers > 0)
    {
      if (psDroid->droidType == DROID_TRANSPORTER AND (fpathGroundBlockingTile(psOrder->x >> TILE_SHIFT, psOrder->y >> TILE_SHIFT) || !
  ...
  case DORDER_ATTACKTARGET:
    ...
    if ((psDroid->asWeaps[0].nStat == 0) || (psDroid->droidType == DROID_TRANSPORTER))
      break;
    ...
    if ((psDroid->order == DORDER_GUARD) && (psOrder->order == DORDER_ATTACKTARGET))
    {
    ...
    else
    {
      ...
      if (game.maxPlayers > 0 AND electronicDroid(psDroid) AND psOrder->psObj->type == OBJ_DROID AND ((DROID*)psOrder->psObj)->droidType ==
        DROID_TRANSPORTER)
        break;
      psDroid->psTarget = psOrder->psObj;
      psDroid->order = psOrder->order;

      if (vtolDroid(psDroid) || actionInsideMinRange(psDroid, psDroid->psTarget) || ((psOrder->order == DORDER_ATTACKTARGET) &&
        secondaryGetState(psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_HOLD)))
        actionDroidObj(psDroid, DACTION_ATTACK, psDroid->psTarget);
      else
        actionDroidLoc(psDroid, DACTION_MOVE, psOrder->psObj->x, psOrder->psObj->y);
  ...
  case DORDER_BUILD:
    ...
    if (!(psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT))
      break;
    DEBUG_ASSERT_TEXT(PTRVALID(psOrder->psStats, sizeof(STRUCTURE_STATS)), "orderUnitBase: invalid structure stats pointer");
  ...
  case DORDER_BUILDMODULE:
    ...
    //								&actionX,&actionY))
    {
      ...
      DEBUG_ASSERT_TEXT(psDroid->psTarStats != NULL, "orderUnitBase: should have found a module stats");
  ...
  case DORDER_RUN:
    psDroid->order = psOrder->order;
    if ((psOrder->order == DORDER_RUN) && ((psOrder->x != 0) || (psOrder->y != 0)))
    {
      psDroid->orderX = psOrder->x;
      psDroid->orderY = psOrder->y;
    }
    ...
    {
      psDroid->orderX = static_cast<UWORD>(asRunData[psDroid->player].sPos.x);
      psDroid->orderY = static_cast<UWORD>(asRunData[psDroid->player].sPos.y);
    }
    actionDroidLoc(psDroid, DACTION_MOVE, psDroid->orderX, psDroid->orderY);
    break;
  case DORDER_DESTRUCT:
  ...
  case DORDER_RTR_SPECIFIED:
    ...
    if (psOrder->psObj == nullptr)
    {
      ...
      for (psStruct = apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
      {
        if ((psStruct->pStructureType->type == REF_REPAIR_FACILITY) || ((psStruct->pStructureType->type == REF_HQ) && (psRepairFac ==
          nullptr)))
        {
          /* get droid->facility distance squared */
          iDX = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psStruct->x);
          iDY = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psStruct->y);
    ...
    if (psRepairFac != nullptr)
    {
      if (psRepairFac->pStructureType->type == REF_REPAIR_FACILITY)
      {
        ...
        else { actionDroidObjLoc(psDroid, DACTION_MOVE, (BASE_OBJECT*)psRepairFac, psDroid->orderX, psDroid->orderY); }
      }
      else
  ...
  case DORDER_RECYCLE:
    ...
    for (psStruct = apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
    {
      ...
      if (psStruct->pStructureType->type == REF_FACTORY OR psStruct->pStructureType->type == REF_CYBORG_FACTORY OR psStruct->pStructureType
        ->type == REF_VTOL_FACTORY OR psStruct->pStructureType->type == REF_REPAIR_FACILITY)
      {
        /* get droid->facility distance squared */
        iDX = static_cast<SDWORD>(psDroid->x) - static_cast<SDWORD>(psStruct->x);
        iDY = static_cast<SDWORD>(psDroid->y) - static_cast<SDWORD>(psStruct->y);
         ...
         * nearer than previously chosen facility
         */
        if (psFactory == nullptr || (iFactoryDistSq > iStructDistSq))
        {
          /* first facility found */
          psFactory = psStruct;
    ...
    if (psFactory != nullptr)
    {
      ...
      psDroid->orderY = static_cast<UWORD>(psFactory->y + (psFactory->pStructureType->baseBreadth << TILE_SHIFT) / 2 + TILE_UNITS / 2);
      psDroid->psTarget = (BASE_OBJECT*)psFactory;
      actionDroidObjLoc(psDroid, DACTION_MOVE, (BASE_OBJECT*)psFactory, psDroid->orderX, psDroid->orderY);
    }
  ...
  case DORDER_RESTORE:
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "orderDroidBase: invalid object type for Restore order");
      break;
    }
  ...
  case DORDER_REARM:
    ...
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "orderUnitBase: unknown order");
    break;
  }
...
void orderDroid(DROID* psDroid, DROID_ORDER order)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnit: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_NONE ||
  ...
  if (bMultiPlayer)
    SendDroidInfo(psDroid, order, 0, 0, nullptr);
}
...
void orderDroidLoc(DROID* psDroid, DROID_ORDER order, UDWORD x, UDWORD y)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitLoc: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_NONE ||
    order == DORDER_MOVE || order == DORDER_GUARD || order == DORDER_SCOUT || order == DORDER_RUN || order == DORDER_PATROL || order ==
    DORDER_TRANSPORTOUT || order == DORDER_TRANSPORTIN || order == DORDER_TRANSPORTRETURN || order == DORDER_DISEMBARK, "orderUnitLoc: Invalid order for location");

  orderClearDroidList(psDroid);

  if (bMultiPlayer) //ajl
  {
    SendDroidInfo(psDroid, order, x, y, nullptr);
    turnOffMultiMsg(TRUE); // msgs off.
  }

  memset(&sOrder, 0, sizeof(DROID_ORDER_DATA));
  sOrder.order = order;
  sOrder.x = static_cast<UWORD>(x);
  sOrder.y = static_cast<UWORD>(y);
  orderDroidBase(psDroid, &sOrder);
...
void orderDroidObj(DROID* psDroid, DROID_ORDER order, BASE_OBJECT* psObj)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitObj: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_NONE ||
...
BOOL orderStateObj(DROID* psDroid, DROID_ORDER order, BASE_OBJECT** ppsObj)
{
  ...
  case DORDER_LINEBUILD:
  case DORDER_HELPBUILD:
    if (psDroid->order == DORDER_BUILD || psDroid->order == DORDER_HELPBUILD || psDroid->order == DORDER_LINEBUILD)
      match = TRUE;
    break;
  case DORDER_ATTACK:
  case DORDER_FIRESUPPORT:
  ...
  case DORDER_BUILD:
  case DORDER_LINEBUILD:
    if (psDroid->action == DACTION_BUILD || psDroid->action == DACTION_BUILDWANDER)
    {
    ...
    break;
  case DORDER_HELPBUILD:
    if (psDroid->action == DACTION_BUILD || psDroid->action == DACTION_BUILDWANDER || psDroid->action == DACTION_MOVETOBUILD)
    {
      *ppsObj = psDroid->psTarget;
      return TRUE;
    }
...
void orderDroidStatsLoc(DROID* psDroid, DROID_ORDER order, BASE_STATS* psStats, UDWORD x, UDWORD y)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitStatsLoc: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_BUILD, "orderUnitStatsLoc: Invalid order for location");
...
void orderDroidStatsTwoLoc(DROID* psDroid, DROID_ORDER order, BASE_STATS* psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitStatsTwoLoc: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_LINEBUILD, "orderUnitStatsTwoLoc: Invalid order for location");
  DEBUG_ASSERT_TEXT(x1 == x2 || y1 == y2, "orderUnitStatsTwoLoc: Invalid locations for LINEBUILD");

  memset(&sOrder, 0, sizeof(DROID_ORDER_DATA));
  sOrder.order = order;
  sOrder.x = static_cast<UWORD>(x1);
  sOrder.y = static_cast<UWORD>(y1);
  sOrder.x2 = static_cast<UWORD>(x2);
  sOrder.y2 = static_cast<UWORD>(y2);
  sOrder.psStats = psStats;
  orderDroidBase(psDroid, &sOrder);
}
...
void orderDroidStatsTwoLocAdd(DROID* psDroid, DROID_ORDER order, BASE_STATS* psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitStatsTwoLocAdd: Invalid unit pointer");
  DEBUG_ASSERT_TEXT(order == DORDER_LINEBUILD, "orderUnitStatsTwoLocAdd: Invalid order for location");
  DEBUG_ASSERT_TEXT(x1 == x2 || y1 == y2, "orderUnitStatsTwoLocAdd: Invalid locations for LINEBUILD");
...
BOOL orderStateStatsLoc(DROID* psDroid, DROID_ORDER order, BASE_STATS** ppsStats, UDWORD* pX, UDWORD* pY)
{
  ...
  case DORDER_BUILD:
  case DORDER_LINEBUILD:
    if (psDroid->action == DACTION_MOVETOBUILD || psDroid->action == DACTION_BUILD_FOUNDATION || psDroid->action ==
      DACTION_FOUNDATION_WANDER)
    {
...
void orderDroidAdd(DROID* psDroid, DROID_ORDER_DATA* psOrder)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "orderUnitAdd: invalid unit pointer");
  ...
  if (psDroid->listSize == 0 && (psDroid->order == DORDER_NONE || psDroid->order == DORDER_GUARD))
  ...
  else
  {
    ...
    if (psOrder->order == DORDER_BUILD OR psOrder->order == DORDER_LINEBUILD)
      psDroid->asOrderList[psDroid->listSize].psOrderTarget = static_cast<void*>(psOrder->psStats);
    else
      psDroid->asOrderList[psDroid->listSize].psOrderTarget = static_cast<void*>(psOrder->psObj);
  ...
  if (!bOrderEffectDisplayed AND (psOrder->order != DORDER_BUILD OR psOrder->order != DORDER_LINEBUILD OR psOrder->order !=
...
BOOL orderDroidList(DROID* psDroid)
{
  ...
  if (psDroid->listSize > 0)
  {
    ...
    case DORDER_HELPBUILD:
    case DORDER_BUILDMODULE:
      sOrder.psObj = static_cast<BASE_OBJECT*>(psDroid->asOrderList[0].psOrderTarget);
      break;
    case DORDER_BUILD:
    case DORDER_LINEBUILD:
      sOrder.psObj = nullptr;
      ...
      break;
    default: DEBUG_ASSERT_TEXT(FALSE, "orderDroidList: Invalid order");
      return FALSE;
    }
    ...
    if (bMultiPlayer AND !(sOrder.order == DORDER_BUILD OR sOrder.order == DORDER_LINEBUILD))
      SendDroidInfo(psDroid, static_cast<DROID_ORDER>(sOrder.order), sOrder.x, sOrder.y, sOrder.psObj);
...
void orderCheckList(DROID* psDroid)
{
  ...
  while (i < psDroid->listSize)
  {
    ...
    if (psDroid->asOrderList[i].order == DORDER_ATTACK OR psDroid->asOrderList[i].order == DORDER_REPAIR OR psDroid->asOrderList[i].order ==
      DORDER_OBSERVE OR psDroid->asOrderList[i].order == DORDER_DROIDREPAIR OR psDroid->asOrderList[i].order == DORDER_FIRESUPPORT OR
      psDroid->asOrderList[i].order == DORDER_CLEARWRECK OR psDroid->asOrderList[i].order == DORDER_DEMOLISH OR psDroid->asOrderList[i].
      order == DORDER_HELPBUILD OR psDroid->asOrderList[i].order == DORDER_BUILDMODULE)
    {
      if (static_cast<BASE_OBJECT*>(psDroid->asOrderList[i].psOrderTarget) && static_cast<BASE_OBJECT*>(psDroid->asOrderList[i].
        psOrderTarget)->died)
      {
        // copy any other orders down the stack
        psDroid->listSize -= 1;
        memmove(psDroid->asOrderList + i, psDroid->asOrderList + i + 1, (psDroid->listSize - i) * sizeof(ORDER_LIST));
        memset(psDroid->asOrderList + psDroid->listSize, 0, sizeof(ORDER_LIST));
      }
      else
        i++;
...
BOOL orderDroidObjAdd(DROID* psDroid, DROID_ORDER order, BASE_OBJECT* psObj)
{
  ...
  if (order != DORDER_ATTACK && order != DORDER_REPAIR && order != DORDER_OBSERVE && order != DORDER_DROIDREPAIR && order !=
    DORDER_FIRESUPPORT && order != DORDER_CLEARWRECK && order != DORDER_DEMOLISH && order != DORDER_HELPBUILD && order !=
    DORDER_BUILDMODULE)
    return FALSE;
...
DROID_ORDER chooseOrderLoc(DROID* psDroid, UDWORD x, UDWORD y)
{
  ...
  else if (secondaryGetState(psDroid, DSO_PATROL, &state) && state == DSS_PATROL_SET)
  {
    order = DORDER_PATROL;
...
void orderSelectedLocAdd(UDWORD player, UDWORD x, UDWORD y, BOOL add)
{
  ...
  if (!add && bMultiPlayer && SendGroupOrderSelected(static_cast<UBYTE>(player), x, y, nullptr))
  {
    // turn off multiplay messages,since we've send a group one instead.
    turnOffMultiMsg(TRUE);
  }
  ...
  {
    if (psCurr->selected && (psCurr->droidType != DROID_COMMAND) && (psCurr->psGroup != nullptr) && psCurr->psGroup->type == GT_COMMAND)
...
void orderSelectedLoc(UDWORD player, UDWORD x, UDWORD y) { orderSelectedLocAdd(player, x, y, FALSE); }
...
DROID_ORDER chooseOrderObj(DROID* psDroid, BASE_OBJECT* psObj)
{
  ...
  if (psDroid->droidType == DROID_TRANSPORTER)
  {
    ...
    if (bMultiPlayer)
    {
      //default to no order
      order = DORDER_NONE;
      if (psObj->player == psDroid->player && psObj->type == OBJ_STRUCTURE)
      {
        psStruct = (STRUCTURE*)psObj;
        DEBUG_ASSERT_TEXT(PTRVALID(psObj, sizeof(STRUCTURE)), "chooseOrderObj: invalid structure pointer");
  ...
  if (psObj->type == OBJ_DROID AND ((DROID*)psObj)->droidType == DROID_TRANSPORTER AND psObj->player == psDroid->player)
  {
    order = DORDER_EMBARK;
  ...
  else if (psObj->type == OBJ_FEATURE AND (((FEATURE*)psObj)->psStats->subType == FEAT_GEN_ARTE || ((FEATURE*)psObj)->psStats->subType ==
    FEAT_OIL_DRUM))
  {
    if (vtolDroid(psDroid))
      order = DORDER_NONE;
  ...
  else if (psDroid->asWeaps[0].nStat > 0 && psObj->player != psDroid->player && !aiCheckAlliances(psObj->player, psDroid->player))
  {
  ...
  else if (psDroid->droidType == DROID_SENSOR && psObj->player != psDroid->player && !aiCheckAlliances(psObj->player, psDroid->player))
  {
    //check for standard sensor or VTOL intercept sensor
    if (asSensorStats[psDroid->asBits[COMP_SENSOR].nStat].type == STANDARD_SENSOR OR asSensorStats[psDroid->asBits[COMP_SENSOR].nStat].type
      == VTOL_INTERCEPT_SENSOR OR asSensorStats[psDroid->asBits[COMP_SENSOR].nStat].type == SUPER_SENSOR)
    {
  ...
  else if (psObj->player == psDroid->player && psObj->type == OBJ_DROID && ((DROID*)psObj)->droidType == DROID_COMMAND && psDroid->droidType
    != DROID_COMMAND && psDroid->droidType != DROID_CONSTRUCT && psDroid->droidType != DROID_CYBORG_CONSTRUCT)
  {
    //		if (!vtolDroid(psDroid))
    {
  ...
  //repair droid
  else if (psObj->player == psDroid->player && psObj->type == OBJ_DROID &&
    //psDroid->droidType == DROID_REPAIR &&
    (psDroid->droidType == DROID_REPAIR OR psDroid->droidType == DROID_CYBORG_REPAIR) && droidIsDamaged((DROID*)psObj))
    order = DORDER_DROIDREPAIR;
    // guarding constructor droids
  else if (psObj->player == psDroid->player && psObj->type == OBJ_DROID && (((DROID*)psObj)->droidType == DROID_CONSTRUCT OR ((DROID*)psObj)
    ->droidType == DROID_CYBORG_CONSTRUCT OR ((DROID*)psObj)->droidType == DROID_SENSOR) && (psDroid->droidType == DROID_WEAPON OR psDroid->
    droidType == DROID_CYBORG) && proj_Direct(asWeaponStats + psDroid->asWeaps[0].nStat))
  {
    order = DORDER_GUARD;
    assignSensorTarget(psObj);
    psDroid->selected = FALSE;
  }
  else if (psObj->player == psDroid->player && psObj->type == OBJ_STRUCTURE)
  {
    psStruct = (STRUCTURE*)psObj;
    DEBUG_ASSERT_TEXT(PTRVALID(psObj, sizeof(STRUCTURE)), "chooseOrderObj: invalid structure pointer");
    ...
    if (psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT)
    {
      ...
      if (intDemolishSelectMode())
      {
        ...
        if (psStruct->status != SS_BUILT AND checkDroidsBuilding(psStruct))
        {
          order = DORDER_NONE;
          psDroid->psTarStats = nullptr;
        }
    ...
    if (order == DORDER_NONE)
    {
      ...
      if (psStruct->pStructureType->type == REF_REPAIR_FACILITY && psStruct->status == SS_BUILT)
        ...
        psStruct->resistance < static_cast<SDWORD>(structureResistance(psStruct->pStructureType, psStruct->player)))
  ...
  else if ((psDroid->droidType == DROID_CONSTRUCT OR psDroid->droidType == DROID_CYBORG_CONSTRUCT) && psObj->type == OBJ_FEATURE)
  {
    psFeature = (FEATURE*)psObj;
    DEBUG_ASSERT_TEXT(PTRVALID(psObj, sizeof(FEATURE)), "chooseOrderObj: invalid feature pointer");
...
void orderSelectedObjAdd(UDWORD player, BASE_OBJECT* psObj, BOOL add)
{
  ...
  if (!add && bMultiPlayer && SendGroupOrderSelected(static_cast<UBYTE>(player), 0, 0, psObj))
  {
    // turn off multiplay messages,since we've send a group one instead.
    turnOffMultiMsg(TRUE);
  }
...
void orderSelectedObj(UDWORD player, BASE_OBJECT* psObj) { orderSelectedObjAdd(player, psObj, FALSE); }

/* order all selected droids with a location and a stat */
void orderSelectedStatsLoc(UDWORD player, DROID_ORDER order, BASE_STATS* psStats, UDWORD x, UDWORD y, BOOL add)
{
  DROID* psCurr;
...
void orderSelectedStatsTwoLoc(UDWORD player, DROID_ORDER order, BASE_STATS* psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2, BOOL add)
{
  DROID* psCurr;
...
STRUCTURE* FindAFactory(UDWORD player, UDWORD factoryType)
{
  ...
  DEBUG_ASSERT_TEXT(player < MAX_PLAYERS, "FindAFactory: invalid player number");
...
BOOL secondarySupported(DROID* psDroid, SECONDARY_ORDER sec)
{
  ...
  case DSO_FIRE_DESIGNATOR:
    if (psDroid->droidType != DROID_COMMAND)
      supported = FALSE;
    if ((sec == DSO_ASSIGN_PRODUCTION && FindAFactory(psDroid->player, REF_FACTORY) == nullptr) || (sec == DSO_ASSIGN_CYBORG_PRODUCTION &&
      FindAFactory(psDroid->player, REF_CYBORG_FACTORY) == nullptr) || (sec == DSO_ASSIGN_VTOL_PRODUCTION && FindAFactory(
      psDroid->player, REF_VTOL_FACTORY) == nullptr))
      supported = FALSE;
    //don't allow factories to be assigned to commanders during a Limbo Expand mission
    if ((sec == DSO_ASSIGN_PRODUCTION OR sec == DSO_ASSIGN_CYBORG_PRODUCTION OR sec == DSO_ASSIGN_VTOL_PRODUCTION) AND missionLimboExpand())
      supported = FALSE;
    break;
  ...
  case DSO_RECYCLE: // Only if player has got a factory.
    if ((FindAFactory(psDroid->player, REF_FACTORY) == nullptr) && (FindAFactory(psDroid->player, REF_CYBORG_FACTORY) == nullptr) && (
      FindAFactory(psDroid->player, REF_VTOL_FACTORY) == nullptr) && (FindARepairFacility() == nullptr))
      supported = FALSE;
    break;
...
STRING* secondaryPrintFactories(UDWORD state)
{
  ...
  for (i = 0; i < 5; i++)
  {
    if (state & (1 << (i + DSS_ASSPROD_SHIFT)))
      aBuff[i] = static_cast<STRING>('0' + i);
    else
      aBuff[i] = ' ';
    if (state & (1 << (i + DSS_ASSPROD_CYBORG_SHIFT)))
    {
      aBuff[i * 2 + 5] = 'c';
      aBuff[i * 2 + 6] = static_cast<STRING>('0' + i);
    }
    else
    {
      aBuff[i * 2 + 5] = ' ';
...
void secondaryCheckDamageLevel(DROID* psDroid)
{
  ...
  if (secondaryGetState(psDroid, DSO_REPAIR_LEVEL, &State))
  {
    ...
    if (State && PERCENT(psDroid->body, psDroid->originalBody) <= State)
    {
      ...
      if (psDroid->order != DORDER_RTR && psDroid->order != DORDER_RTB && !vtolRearming(psDroid))
      {
        if (vtolDroid(psDroid))
          moveToRearm(psDroid);
        else
...
BOOL secondarySetState(DROID* psDroid, SECONDARY_ORDER sec, SECONDARY_STATE State)
{
  ...
  if ((sec != DSO_RECYCLE) && psDroid->droidType == DROID_COMMAND && psDroid->psGroup != nullptr && psDroid->psGroup->type == GT_COMMAND)
  ...
  case DSO_ATTACK_LEVEL:
    ...
    if (State == DSS_ALEV_NEVER)
    {
      ...
      else if (orderState(psDroid, DORDER_GUARD) && droidAttacking(psDroid))
      {
  ...
  case DSO_ASSIGN_VTOL_PRODUCTION: DBP1(("order factories %s\n", secondaryPrintFactories(State)));
    ...
    if (psDroid->droidType == DROID_COMMAND)
    {
      ...
      for (psStruct = apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
      {
        ...
        if (factType == REF_FACTORY || factType == REF_VTOL_FACTORY || factType == REF_CYBORG_FACTORY)
        {
          ...
          if (!(CurrState & (1 << factoryInc)) && (State & (1 << factoryInc)))
            assignFactoryCommandDroid(psStruct, psDroid); // assign this factory to the command droid
          else if ((prodType == factType) && (CurrState & (1 << factoryInc)) && !(State & (1 << factoryInc)))
          {
  ...
  case DSO_RETURN_TO_LOC:
    ...
    {
      if (orderState(psDroid, DORDER_RTR) || orderState(psDroid, DORDER_RTB) || orderState(psDroid, DORDER_EMBARK))
        orderDroid(psDroid, DORDER_STOP);
    ...
    else
    {
      ...
      if (!orderState(psDroid, static_cast<DROID_ORDER>(order)))
        orderDroid(psDroid, static_cast<DROID_ORDER>(order));
    }
    break;
...
BOOL secondaryGotPrimaryOrder(DROID* psDroid, DROID_ORDER order)
{
  ...
  if (order != DORDER_NONE && order != DORDER_STOP && order != DORDER_DESTRUCT && order != DORDER_GUARD && order != DORDER_ATTACKTARGET)
  {
    //reset 2ndary order
...
#define MAX_ORDERS	4
  struct
  {
    ...
    {DSO_ATTACK_RANGE, DSS_ARANGE_MASK}, {DSO_REPAIR_LEVEL, DSS_REPLEV_MASK}, {DSO_ATTACK_LEVEL, DSS_ALEV_MASK},
  ...
  for (i = 0; i < MAX_ORDERS; i++)
  {
    ...
    secondarySetGroupState(player, group, static_cast<SECONDARY_ORDER>(aOrders[i].order), state);
  }
}
...
BOOL setFactoryState(STRUCTURE* psStruct, SECONDARY_ORDER sec, SECONDARY_STATE State)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "setFactoryState: structure is not a factory");
    return FALSE;
  }
...
BOOL getFactoryState(STRUCTURE* psStruct, SECONDARY_ORDER sec, SECONDARY_STATE* pState)
{
  ...
  if (!StructIsFactory(psStruct))
  {
    DEBUG_ASSERT_TEXT(FALSE, "getFactoryState: structure is not a factory");
    return FALSE;
  }
...
void orderStructureObj(UDWORD player, BASE_OBJECT* psObj)
{
  ...
  for (psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
  {
    if (lasSatStructSelected(psStruct))
    {
      ...
      firePause = weaponFirePause(&asWeaponStats[psStruct->asWeaps[0].nStat], static_cast<UBYTE>(player));
      damLevel = PERCENT(psStruct->body, structureBody(psStruct));
      if (damLevel < HEAVY_DAMAGE_LEVEL)
        firePause += firePause;
      if (isHumanPlayer(player) AND (gameTime - psStruct->asWeaps[0].lastFired <= firePause))
      {
        /* Too soon to fire again */
        break;
      }
      //ok to fire - so fire away
      proj_SendProjectile(&psStruct->asWeaps[0], nullptr, player, psObj->x, psObj->y, psObj->z, psObj, TRUE);
