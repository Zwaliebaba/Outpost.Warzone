...
DROIDSTORE* tempDroidList = nullptr;
...
BOOL sendVtolRearm(DROID* psDroid, STRUCTURE* psStruct, UBYTE chosen)
{
  ...
  NetAddType(msg, 0, UBYTE, psDroid->player);
  ...
  NetAddType(msg, 10, UBYTE, psDroid->sMove.iAttackRuns);
  NetAddType(msg, 11, UBYTE, psDroid->asWeaps[0].ammo);
...
BOOL sendDroidSecondary(DROID* psDroid, SECONDARY_ORDER sec, SECONDARY_STATE state)
{
  ...
  NetAdd(m, 4, sec);
  NetAdd(m, 8, state);
  m.body[12] = static_cast<char>(psDroid->player);
...
BOOL sendDroidSecondaryAll(DROID* psDroid)
{
  ...
  m.body[8] = static_cast<char>(psDroid->player);
...
BOOL sendDroidEmbark(DROID* psDroid)
{
  ...
  NetAdd(m, 0, psDroid->id);
  m.body[4] = static_cast<char>(psDroid->player);
...
BOOL SendDroidMove(DROID* pDroid, UDWORD x, UDWORD y, BOOL bFormation)
{
  ...
  if (sendit) // send the mesg if required.
  {
    ...
    NetAdd(m, 4, x); //x pos
    NetAdd(m, 8, y); //y pos
    NetAddType(m, 12, char, pDroid->player); //owner of droid(for speed!)
    NetAddType(m, 13, char, bFormation); //use a formation?
    m.size = 14;
...
BOOL SendGroupOrderSelected(UBYTE player, UDWORD x, UDWORD y, BASE_OBJECT* psObj)
{
  ...
  if (psObj == nullptr) //it's a position order
  {
    NetAdd(m, 0, x);
    NetAdd(m, 4, y);
    m.body[8] = 0; // subtype flag
...
BOOL recvGroupOrder(NETMSG* pMsg)
{
  ...
  order = static_cast<DROID_ORDER>(pMsg->body[12]);
  ...
  else // it's target is a position
  {
    ...
    destid = 0;
    desttype = static_cast<OBJECT_TYPE>(0);
  }
...
BOOL SendDroidInfo(DROID* psDroid, DROID_ORDER order, UDWORD x, UDWORD y, BASE_OBJECT* psObj)
{
  ...
  if ((x == 0) && (y == 0) && (psObj == nullptr))
  {
    NetAdd(m, 8, x); //note: x,y not needed
    NetAdd(m, 12, y);
    m.body[16] = 2;
  }
  else if (psObj == nullptr) //it's a position order
  {
    NetAdd(m, 8, x);
    NetAdd(m, 12, y);
...
BOOL recvDroidInfo(NETMSG* pMsg)
{
  ...
  else // it's a position order
  {
    ...
    ProcessDroidOrder(psDroid, order, x, y, static_cast<OBJECT_TYPE>(0), 0);
  }
  return TRUE;
}
...
static VOID ProcessDroidOrder(DROID* psDroid, DROID_ORDER order, UDWORD x, UDWORD y, OBJECT_TYPE desttype, UDWORD destid)
{
  ...
  {
    if (abs(static_cast<SDWORD>(psDroid->x - x)) < (TILE_UNITS / 2) && abs(static_cast<SDWORD>(psDroid->y - y)) < (TILE_UNITS / 2))
...
BOOL sendWholeDroid(DROID* pD, NETPLAYERID dest)
{
  ...
  sizecount += sizeof(asWeaps); // to build a template.

  NetAdd(m, sizecount, pD->x);
  sizecount += sizeof(pD->x);
  NetAdd(m, sizecount, pD->y);
  sizecount += sizeof(pD->y);
  NetAdd(m, sizecount, pD->z);
  sizecount += sizeof(pD->z);
  NetAdd(m, sizecount, pD->player);
  ...
  sizecount += sizeof(pD->id);

  NetAdd(m, sizecount, pD->NameVersion);
  sizecount += sizeof(pD->NameVersion);
  NetAdd(m, sizecount, pD->droidType);
  sizecount += sizeof(pD->droidType);

  NetAdd(m, sizecount, pD->direction);
  sizecount += sizeof(pD->direction);
  NetAdd(m, sizecount, pD->pitch);
  sizecount += sizeof(pD->pitch);
  NetAdd(m, sizecount, pD->roll);
  sizecount += sizeof(pD->roll);
  NetAdd(m, sizecount, pD->visible);
  sizecount += sizeof(pD->visible);
  NetAdd(m, sizecount, pD->inFire);
  sizecount += sizeof(pD->inFire);
  NetAdd(m, sizecount, pD->burnDamage);
  sizecount += sizeof(pD->burnDamage);

  NetAdd(m, sizecount, pD->body);
  sizecount += sizeof(pD->body);
  NetAdd(m, sizecount, pD->secondaryOrder);
  sizecount += sizeof(pD->secondaryOrder);
  NetAdd(m, sizecount, pD->order);
  sizecount += sizeof(pD->order);
  NetAdd(m, sizecount, pD->orderX);
  sizecount += sizeof(pD->orderX);
  NetAdd(m, sizecount, pD->orderY);
  sizecount += sizeof(pD->orderY);
  NetAdd(m, sizecount, pD->orderX2);
  sizecount += sizeof(pD->orderX2);
  NetAdd(m, sizecount, pD->orderY2);
  sizecount += sizeof(pD->orderY2);

  if (pD->psTarget)
  {
    NetAdd(m, sizecount, pD->psTarget->id);
    sizecount += sizeof(pD->psTarget->id);
  }
  else
  {
    NetAdd(m, sizecount, noTarget);
    sizecount += sizeof(noTarget);
  }

  if (pD->psTarStats)
  {
    NetAdd(m, sizecount, pD->psTarStats->ref);
    sizecount += sizeof(pD->psTarStats->ref);
  }
  else
  {
    NetAdd(m, sizecount, noTarget);
  ...
  m.size = static_cast<UWORD>(sizecount);
...
BOOL receiveWholeDroid(NETMSG* m)
{
  ...
  // get the stuff
  NetGet(m, sizecount, dt.asParts);
  sizecount += sizeof(dt.asParts); // build a template
  NetGet(m, sizecount, dt.asWeaps);
  sizecount += sizeof(dt.asWeaps);
  NetGet(m, sizecount, x);
  sizecount += sizeof(x); // edit it.
  NetGet(m, sizecount, y);
  sizecount += sizeof(y);
  NetGet(m, sizecount, z);
  sizecount += sizeof(z);
  NetGet(m, sizecount, player);
  sizecount += sizeof(player);
  ...
  NetGet(m, sizecount, pD->NameVersion);
  sizecount += sizeof(pD->NameVersion);
  NetGet(m, sizecount, pD->droidType);
  sizecount += sizeof(pD->droidType);

  NetGet(m, sizecount, pD->direction);
  sizecount += sizeof(pD->direction);
  NetGet(m, sizecount, pD->pitch);
  sizecount += sizeof(pD->pitch);
  NetGet(m, sizecount, pD->roll);
  sizecount += sizeof(pD->roll);

  NetGet(m, sizecount, pD->visible);
  sizecount += sizeof(pD->visible);
  NetGet(m, sizecount, pD->inFire);
  sizecount += sizeof(pD->inFire);
  NetGet(m, sizecount, pD->burnDamage);
  sizecount += sizeof(pD->burnDamage);

  NetGet(m, sizecount, pD->body);
  sizecount += sizeof(pD->body);

  NetGet(m, sizecount, pD->secondaryOrder);
  sizecount += sizeof(pD->secondaryOrder);

  if (ingame.localJoiningInProgress)
  {
    NetGet(m, sizecount, pD->order);
    sizecount += sizeof(pD->order); // processed laater
    NetGet(m, sizecount, pD->orderX);
    sizecount += sizeof(pD->orderX); //later!
    NetGet(m, sizecount, pD->orderY);
    sizecount += sizeof(pD->orderY); //later!
    NetGet(m, sizecount, pD->orderX2);
    sizecount += sizeof(pD->orderX2);
    NetGet(m, sizecount, pD->orderY2);
    sizecount += sizeof(pD->orderY2);

    NetGet(m, sizecount, pD->psTarget);
    sizecount += sizeof(pD->psTarget); //later!
    NetGet(m, sizecount, pD->psTarStats);
