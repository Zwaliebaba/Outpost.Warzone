...
static WNDPROC fnOldWinProc = nullptr;

/***************************************************************************/
/* cdAudio Subclass procedure */

LRESULT APIENTRY cdAudio_CheckTrackFinished(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  if (uMsg == MM_MCINOTIFY && wParam == static_cast<WPARAM>(MCI_NOTIFY_SUCCESSFUL))
...
BOOL cdAudio_Open(void)
{
  ...
  if ((dwReturn = mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE, (DWORD)static_cast<LPVOID>(&mciOpenParms))) == 0)
  {
  ...
  fnOldWinProc = (WNDPROC)SetWindowLong(GetActiveWindow(), GWL_WNDPROC, (LPARAM)cdAudio_CheckTrackFinished);
...
BOOL cdAudio_PlayTrack(SDWORD iTrack)
{
  ...
  if (mciStatusParams.dwReturn == static_cast<DWORD>(iTrack))
...
CDPlayError: mciSendCommand(g_wDeviceID, MCI_CLOSE, 0, (DWORD)&mciParams);
  return FALSE;
}
...
BOOL cdAudio_Stop(void)
{
  ...
  if ((dwReturn = mciSendCommand(g_wDeviceID, MCI_STOP, 0, (DWORD)&mciParams)) != 0)
  {
    mciSendCommand(g_wDeviceID, MCI_CLOSE, 0, (DWORD)&mciParams);
    return FALSE;
  }
...
BOOL cdAudio_Pause(void)
{
  ...
  if ((dwReturn = mciSendCommand(g_wDeviceID, MCI_PAUSE, 0, (DWORD)&mciParams)) != 0)
  {
    mciSendCommand(g_wDeviceID, MCI_CLOSE, 0, (DWORD)&mciParams);
    return FALSE;
  }
