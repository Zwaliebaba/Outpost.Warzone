...
DROID* GetCurrTransporter(void) { return psCurrTransporter; }
...
//
BOOL intRefreshTransporter(void) { return _intRefreshTransporter(); }
...
BOOL intAddTransporter(DROID* psSelected, BOOL offWorld) { return (_intAddTransporter(psSelected, offWorld)); }
...
static BOOL _intAddTransporter(DROID* psSelected, BOOL offWorld)
{
  ...
  if (bMultiPlayer)
  {
    if (psCurrTransporter AND psCurrTransporter->died AND psCurrTransporter->died != NOT_CURRENT_LIST)
    {
  ...
  sFormInit.x = static_cast<SWORD>(TRANS_X);
  sFormInit.y = static_cast<SWORD>(TRANS_Y);
...
BOOL intAddTransporterContents(void)
{
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>(TRANSCONT_X);
  sFormInit.y = static_cast<SWORD>(TRANSCONT_Y);
...
BOOL intAddTransporterLaunch(DROID* psDroid)
{
  ...
  sLabInit.x = static_cast<SWORD>(sButInit.x + 20);
  sLabInit.y = 0;
  sLabInit.width = 16;
  sLabInit.height = 16;
...
BOOL intAddTransButtonForm(void)
{
  ...
  for (psDroid = transInterfaceDroidList(); psDroid; psDroid = psDroid->psNext)
  {
    ...
    if (psDroid->droidType == DROID_TRANSPORTER AND (psDroid->action != DACTION_TRANSPORTOUT && psDroid->action != DACTION_TRANSPORTIN))
    {
...
BOOL intAddDroidsAvailForm(void)
{
  ...
  sFormInit.x = static_cast<SWORD>(TRANSDROID_X);
  sFormInit.y = static_cast<SWORD>(TRANSDROID_Y);
  ...
  butPerForm = ((TRANSDROID_TABWIDTH - OBJ_GAP) / (OBJ_BUTWIDTH + OBJ_GAP)) * ((TRANSDROID_TABHEIGHT - OBJ_GAP) / (OBJ_BUTHEIGHT +
    OBJ_GAP));
  ...
  for (psDroid = mission.apsDroidLists[selectedPlayer]; psDroid != nullptr; psDroid = psDroid->psNext)
  {
    ...
    if (psDroid->droidType != DROID_TRANSPORTER)
    {
      ...
      sBarInit.size = static_cast<UWORD>(PERCENT(psDroid->body, psDroid->originalBody));
  ...
  if (objMajor > static_cast<UWORD>(sFormInit.numMajor - 1))
  {
    //set to last if have lost a tab
    widgSetTabs(psWScreen, IDTRANS_DROIDTAB, static_cast<UWORD>(sFormInit.numMajor - 1), objMinor);
  }
  else
  {
...
UDWORD calcRemainingCapacity(DROID* psTransporter)
{
  ...
  return static_cast<UDWORD>(capacity);
}
...
void intSetTransCapacityLabel(char* Label)
{
  ...
  if (psCurrTransporter)
  {
    ...
    Label[0] = static_cast<UBYTE>('0' + capacity / 10);
    Label[1] = static_cast<UBYTE>('0' + capacity % 10);
    //NOT ANY MORE!
...
void intProcessTransporter(UDWORD id) { _intProcessTransporter(id); }
...
static void _intProcessTransporter(UDWORD id)
{
  ...
  else if (id >= IDTRANS_CONTSTART && id <= IDTRANS_CONTEND)
  {
    ...
    if (psCurrTransporter != nullptr AND !transporterFlying(psCurrTransporter))
    {
...
void setCurrentTransporter(UDWORD id)
{
  ...
  if (psDroid)
  {
    psCurrTransporter = psDroid;
    //set the data for the transporter timer
    widgSetUserData(psWScreen, IDTRANTIMER_DISPLAY, psCurrTransporter);
  }
}
...
BOOL checkTransporterSpace(DROID* psTransporter, DROID* psAssigned)
{
  ...
  ASSERT((PTRVALID(psTransporter, sizeof(DROID)), "checkTransporterSpace: Invalid droid pointer"));
  ASSERT((PTRVALID(psAssigned, sizeof(DROID)), "checkTransporterSpace: Invalid droid pointer"));
  ASSERT((psTransporter->droidType == DROID_TRANSPORTER, "checkTransporterSpace: Droid is not a Transporter"));
  ASSERT((psTransporter->psGroup != NULL, "checkTransporterSpace: tranporter doesn't have a group"));
...
UDWORD transporterSpaceRequired(DROID* psDroid)
{
  ...
  case SIZE_SUPER_HEAVY:
    size = HEAVY_DROID;
    break;
  default: ASSERT((FALSE, "transporterSpaceRequired: Unknown Droid size"));
    size = 0;
    break;
  }
...
UDWORD transporterGetLaunchTime(void) { return g_iLaunchTime; }

void transporterSetLaunchTime(UDWORD time) { g_iLaunchTime = time; }
...
{
  ASSERT((PTRVALID(psTransporter, sizeof(DROID)), "updateTransporter: Invalid droid pointer"));
  ...
  if (psTransporter->player == selectedPlayer AND getDroidsToSafetyFlag() AND psTransporter->action != DACTION_TRANSPORTOUT)
  {
    ...
    if (!missionDroidsRemaining(selectedPlayer))
    {
      ...
      psTransporter->psTarget = nullptr;
      psTransporter->psTarStats = nullptr;
      return TRUE;
    }
  }
  ...
  if (psTransporter->sMove.Status == MOVEINACTIVE || psTransporter->sMove.Status == MOVEHOVER || (psTransporter->action ==
    ...
    getDroidsToSafetyFlag()))
  {
    ...
    if (!bFirstTransporter AND missionForReInforcements() AND psTransporter->action == DACTION_TRANSPORTIN AND psTransporter->player ==
      selectedPlayer)
    {
...
void transporterSetScriptCurrent(DROID* psTransporter) { g_psCurScriptTransporter = psTransporter; }

/* get current transporter (for script callbacks) */
DROID* transporterGetScriptCurrent(void) { return g_psCurScriptTransporter; }
...
{
  ASSERT((PTRVALID(psTransporter, sizeof(DROID)), "transporterFlying: Invalid droid pointer"));
  ASSERT((psTransporter->droidType == DROID_TRANSPORTER, "transporterFlying: Droid is not a Transporter"));
  ...
  if (psTransporter->order == DORDER_TRANSPORTOUT OR psTransporter->order == DORDER_TRANSPORTIN OR psTransporter->order ==
    ...
    (bMultiPlayer AND psTransporter->order == DORDER_NONE AND psTransporter->sMove.iVertSpeed != 0))
    return TRUE;
  return FALSE;
}

//initialise the flag to indicate the first transporter has arrived - set in startMission()
void initFirstTransporterFlag(void) { bFirstTransporter = TRUE; }
