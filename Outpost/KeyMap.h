...
using KEY_MAPPING = struct _keyMapping
{
  void (*function)(void);
  BOOL active;
  KEY_STATUS status;
...
extern KEY_MAPPING* keyAddMapping(KEY_STATUS status, KEY_CODE metaCode, KEY_CODE subcode, KEY_ACTION action, void (*pKeyMapFunc)(void),
                                  STRING* name);
...
extern BOOL keyReAssignMapping(KEY_CODE origMetaCode, KEY_CODE origSubCode, KEY_CODE newMetaCode, KEY_CODE newSubCode);
