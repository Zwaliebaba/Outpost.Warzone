...
UDWORD numBodyStats = 0;UDWORD numBrainStats = 0;UDWORD numPropulsionStats = 0;UDWORD numSensorStats = 0;UDWORD numECMStats = 0;UDWORD
numRepairStats = 0;UDWORD numProgramStats = 0;UDWORD numWeaponStats = 0;UDWORD numConstructStats = 0;UDWORD numSpecialAbility = 0;
#else
/* The number of different stats stored */
UDWORD numBodyStats;
...
#define ALLOC_STATS(numEntries, list, listSize, type) \
	DEBUG_ASSERT_TEXT((numEntries) < REF_RANGE, \
...
void statsInitVars(void)
{
  ...
  asBodyStats = nullptr;
  asBrainStats = nullptr;
  asPropulsionStats = nullptr;
  asSensorStats = nullptr;
  asECMStats = nullptr;
  asRepairStats = nullptr;
  asWeaponStats = nullptr;
  asConstructStats = nullptr;
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    for (j = 0; j < COMP_NUMCOMPONENTS; j++)
      apCompLists[i][j] = nullptr;
  }

  //store for each players Structure states
  for (i = 0; i < MAX_PLAYERS; i++)
    apStructTypeLists[i] = nullptr;
  ...
  //init the max values
  maxComponentWeight = maxBodyArmour = maxBodyPower = maxBodyPoints = maxSensorRange = maxSensorPower = maxECMPower = maxConstPoints =
    maxRepairPoints = maxWeaponRange = maxWeaponDamage = maxPropulsionSpeed = 0;
}
...
{
#ifdef HASH_NAMES
  pStat->NameHash = HashString(Name); return (TRUE);
#else
...
#define SET_STATS(stats, list, index, type, refStart) \
	DEBUG_ASSERT_TEXT(((stats)->ref >= (refStart)) && ((stats)->ref < (refStart) + REF_RANGE),  \
...
BOOL statsAllocWeapons(UDWORD numStats) { ALLOC_STATS(numStats, asWeaponStats, numWeaponStats, WEAPON_STATS); }
/* Allocate Armour Stats */
/* Allocate Body Stats */
BOOL statsAllocBody(UDWORD numStats) { ALLOC_STATS(numStats, asBodyStats, numBodyStats, BODY_STATS); }
/* Allocate Brain Stats */
BOOL statsAllocBrain(UDWORD numStats) { ALLOC_STATS(numStats, asBrainStats, numBrainStats, BRAIN_STATS); }
/* Allocate Power Stats */
/* Allocate Propulsion Stats */
BOOL statsAllocPropulsion(UDWORD numStats) { ALLOC_STATS(numStats, asPropulsionStats, numPropulsionStats, PROPULSION_STATS); }
/* Allocate Sensor Stats */
BOOL statsAllocSensor(UDWORD numStats) { ALLOC_STATS(numStats, asSensorStats, numSensorStats, SENSOR_STATS); }
/* Allocate Ecm Stats */
BOOL statsAllocECM(UDWORD numStats) { ALLOC_STATS(numStats, asECMStats, numECMStats, ECM_STATS); }

/* Allocate Repair Stats */
BOOL statsAllocRepair(UDWORD numStats) { ALLOC_STATS(numStats, asRepairStats, numRepairStats, REPAIR_STATS); }

/* Allocate Program Stats */

/* Allocate Construct Stats */
BOOL statsAllocConstruct(UDWORD numStats) { ALLOC_STATS(numStats, asConstructStats, numConstructStats, CONSTRUCT_STATS); }

STRING* getStatName(void* Stat)
{
  auto psStats = static_cast<BASE_STATS*>(Stat);
...
BOOL loadWeaponStats(SBYTE* pWeaponData, UDWORD bufferSize)
{
  ...
  STRING mountGfx[MAX_NAME_SIZE], flightGfx[MAX_NAME_SIZE], hitGfx[MAX_NAME_SIZE], missGfx[MAX_NAME_SIZE], waterGfx[MAX_NAME_SIZE],
         muzzleGfx[MAX_NAME_SIZE], trailGfx[MAX_NAME_SIZE], techLevel[MAX_NAME_SIZE];
...
#define DEFAULT_FLIGHTSPEED (500)
    ...
    //set the subClass
    psStats->weaponSubClass = static_cast<WEAPON_SUBCLASS>(getWeaponSubClass(weaponSubClass));
    if (psStats->weaponSubClass == INVALID_SUBCLASS)
      return FALSE;

    //set the movement model
    psStats->movementModel = static_cast<MOVEMENT_MODEL>(getMovementModel(movement));
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: effectSize is greater than 255 for weapon {}", getStatName(psStats));
      return FALSE;
    }
    ...
    if (rotate > UBYTE_MAX)
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: rotate is greater than 255 for weapon {}", getStatName(psStats));
      return FALSE;
    }
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: minElevation is outside of limits for weapon {}", getStatName(psStats));
      return FALSE;
    }
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: maxElevation is outside of limits for weapon {}", getStatName(psStats));
      return FALSE;
    }
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: Surface to Air is outside of limits for weapon {}", getStatName(psStats));
      return FALSE;
    }
    if (surfaceToAir == 0)
      psStats->surfaceToAir = static_cast<UBYTE>(SHOOT_ON_GROUND);
    else if (surfaceToAir <= 50)
      psStats->surfaceToAir = static_cast<UBYTE>(SHOOT_IN_AIR);
    else
      psStats->surfaceToAir = static_cast<UBYTE>((SHOOT_ON_GROUND | SHOOT_IN_AIR));

    //set the attackRuns for VTOLs
    if (numAttackRuns > UBYTE_MAX)
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadWeaponStats: num of attack runs is outside of limits for weapon {}", getStatName(psStats));
      return FALSE;
    }
    ...
    if (psStats->flightSpeed > 0 && !proj_Direct(psStats))
      longRange = static_cast<UDWORD>(proj_GetLongRange(psStats, 0));
    else
...
#error Invalid number of players
    ...
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadBodyStats: unknown body size for {}", getStatName(psStats));
      return FALSE;
    }
  ...
  if (asBodyStats AND asPropulsionStats)
  {
    ...
    for (i = 0; i < numBodyStats; i++)
    {
      ...
      if (asBodyStats[i].design)
      {
        ...
        for (NumPropulsion = 0; NumPropulsion < numPropulsionStats; NumPropulsion++)
        {
          //check stat is designable
          if (asPropulsionStats[NumPropulsion].design)
          {
            setMaxComponentWeight(asPropulsionStats[NumPropulsion].weight * asBodyStats[i].weight / 100);
          }
        }
      }
    }
...
#error Invalid number of players
    ...
    if (!strcmp(location, "DEFAULT"))
      ...
      DEBUG_ASSERT_TEXT(FALSE, "Invalid Sensor location");
    ...
    if (!strcmp(type, "STANDARD"))
      ...
      DEBUG_ASSERT_TEXT(FALSE, "Invalid Sensor type");
...
#error Invalid number of players
    ...
    if (!strcmp(location, "DEFAULT"))
      ...
      DEBUG_ASSERT_TEXT(FALSE, "Invalid ECM location");
...
BOOL loadRepairStats(SBYTE* pRepairData, UDWORD bufferSize)
{
  ...
  for (i = 0; i < NumRepair; i++)
  {
    ...
    if (!strcmp(location, "DEFAULT"))
      ...
      DEBUG_ASSERT_TEXT(FALSE, "Invalid Repair location");
    ...
    if (psStats->time == 0)
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadRepairStats: the delay time cannot be zero for {}", psStats->pName);
      psStats->time = 1;
    }
...
BOOL loadPropulsionTypes(SBYTE* pPropTypeData, UDWORD bufferSize)
{
  ...
  for (i = 0; i < NumTypes; i++)
  {
    ...
    if (!strcmp(flightName, "GROUND"))
      ...
      DEBUG_ASSERT_TEXT(FALSE, "Invalid travel type for Propulsion");

    //don't care about this anymore! AB FRIDAY 13/11/98
    //want it back again! AB 27/11/98
    if (multiplier > UWORD_MAX)
    {
      DEBUG_ASSERT_TEXT(FALSE, "loadPropulsionTypes: power Ratio multiplier too high");
      //set to a default value since not life threatening!
      multiplier = 100;
    }
    pPropType->powerRatioMult = static_cast<UWORD>(multiplier);
...
BOOL loadBodyPropulsionIMDs(SBYTE* pData, UDWORD bufferSize)
{
  ...
  DEBUG_ASSERT_TEXT(asBodyStats != NULL, "Body Stats have not been set up");
  DEBUG_ASSERT_TEXT(asPropulsionStats != NULL, "Propulsion Stats have not been set up");
...
static BOOL statsGetAudioIDFromString(STRING* szStatName, STRING* szWavName, SDWORD* piWavID)
{
...
BOOL loadWeaponSounds(SBYTE* pSoundData, UDWORD bufferSize)
{
  ...
  for (i = 0; i < NumRecords; i++)
  {
    ...
    for (inc = 0; inc < static_cast<SDWORD>(numWeaponStats); inc++)
    {
#ifdef HASH_NAMES
...
{
  SDWORD NumRecords = 0, i, startID, idleID, moveOffID, moveID, hissID, shutDownID, iDum;
  STRING propulsionName[MAX_NAME_SIZE], szStart[MAX_NAME_SIZE], szIdle[MAX_NAME_SIZE], szMoveOff[MAX_NAME_SIZE], szMove[MAX_NAME_SIZE],
  ...
  for (i = 0; i < NumRecords; i++)
  {
    ...
    pPropType = asPropulsionTypes + type;
    pPropType->startID = static_cast<SWORD>(startID);
    pPropType->idleID = static_cast<SWORD>(idleID);
    pPropType->moveOffID = static_cast<SWORD>(moveOffID);
...
void statsSetWeapon(WEAPON_STATS* psStats, UDWORD index) { SET_STATS(psStats, asWeaponStats, index, WEAPON_STATS, REF_WEAPON_START); }
/* Set the stats for a particular armour type */
/* Set the stats for a particular body type */
void statsSetBody(BODY_STATS* psStats, UDWORD index) { SET_STATS(psStats, asBodyStats, index, BODY_STATS, REF_BODY_START); }
/* Set the stats for a particular brain type */
void statsSetBrain(BRAIN_STATS* psStats, UDWORD index) { SET_STATS(psStats, asBrainStats, index, BRAIN_STATS, REF_BRAIN_START); }
/* Set the stats for a particular power type */
/* Set the stats for a particular power type */
void statsSetPropulsion(PROPULSION_STATS* psStats, UDWORD index)
{
  SET_STATS(psStats, asPropulsionStats, index, PROPULSION_STATS, REF_PROPULSION_START);
}

/* Set the stats for a particular sensor type */
void statsSetSensor(SENSOR_STATS* psStats, UDWORD index) { SET_STATS(psStats, asSensorStats, index, SENSOR_STATS, REF_SENSOR_START); }
/* Set the stats for a particular ecm type */
void statsSetECM(ECM_STATS* psStats, UDWORD index) { SET_STATS(psStats, asECMStats, index, ECM_STATS, REF_ECM_START); }
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_WEAPON_START) && (ref < REF_WEAPON_START + REF_RANGE), "statsGetWeapon: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetWeapon: Reference number not found in list: {:x}", ref);
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_BODY_START) && (ref < REF_BODY_START + REF_RANGE), "statsGetBody: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetBody: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_BRAIN_START) && (ref < REF_BRAIN_START + REF_RANGE), "statsGetBrain: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetBrain: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_PROPULSION_START) && (ref < REF_PROPULSION_START +
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetPropulsion: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_SENSOR_START) && (ref < REF_SENSOR_START + REF_RANGE), "statsGetSensor: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetSensor: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_ECM_START) && (ref < REF_ECM_START + REF_RANGE), "statsGetECM: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetECM: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_REPAIR_START) && (ref < REF_REPAIR_START + REF_RANGE), "statsGetRepair: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetRepair: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
{
  UDWORD index;
  DEBUG_ASSERT_TEXT((ref >= REF_CONSTRUCT_START) && (ref < REF_CONSTRUCT_START + REF_RANGE), "statsGetConstruct: Invalid reference number: {:x}", ref);
  ...
  DEBUG_ASSERT_TEXT(FALSE, "statsGetConstruct: Reference number not found in list: {:x}", ref);
  return nullptr; // should never get here, but this stops the compiler complaining.
}
...
void storeSpeedFactor(UDWORD terrainType, UDWORD propulsionType, UDWORD speedFactor)
{
  ...
  DEBUG_ASSERT_TEXT(propulsionType < NUM_PROP_TYPES, "The propulsion type is too large");
...
{
  if (ref >= REF_BODY_START && ref < REF_BODY_START + REF_RANGE)
    return COMP_BODY;
  if (ref >= REF_BRAIN_START && ref < REF_BRAIN_START + REF_RANGE)
    return COMP_BRAIN;
  ...
  if (ref >= REF_PROPULSION_START && ref < REF_PROPULSION_START + REF_RANGE)
    return COMP_PROPULSION;
  if (ref >= REF_SENSOR_START && ref < REF_SENSOR_START + REF_RANGE)
    return COMP_SENSOR;
  if (ref >= REF_ECM_START && ref < REF_ECM_START + REF_RANGE)
    return COMP_ECM;
  ...
  if (ref >= REF_REPAIR_START && ref < REF_REPAIR_START + REF_RANGE)
  ...
  if (ref >= REF_WEAPON_START && ref < REF_WEAPON_START + REF_RANGE)
    return COMP_WEAPON;
  if (ref >= REF_CONSTRUCT_START && ref < REF_CONSTRUCT_START + REF_RANGE)
    return COMP_CONSTRUCT;
  //else
  DEBUG_ASSERT_TEXT(FALSE, "Invalid stat pointer - cannot determine Stat Type");
  return COMP_UNKNOWN;
}
...
UDWORD componentType(char* pType)
{
  ...
  if (!strcmp(pType, "CONSTRUCT"))
    return COMP_CONSTRUCT;
  DEBUG_ASSERT_TEXT(FALSE, "Unknown Component Type");
...
BOOL setTechLevel(BASE_STATS* psStats, STRING* pLevel)
{
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "Unknown Technology Level - {}", pLevel);
    return FALSE;
  }

  //store tech level in the appropriate stat
  if ((psStats->ref >= REF_BODY_START AND psStats->ref <= (REF_WEAPON_START + REF_RANGE)) OR (psStats->ref >= REF_CONSTRUCT_START AND
    psStats->ref <= (REF_CONSTRUCT_START + REF_RANGE)))
    ((COMP_BASE_STATS*)psStats)->techLevel = techLevel;
  else if (psStats->ref >= REF_STRUCTURE_START AND psStats->ref <= (REF_STRUCTURE_START + REF_RANGE))
    ((STRUCTURE_STATS*)psStats)->techLevel = techLevel;
  else if (psStats->ref >= REF_RESEARCH_START AND psStats->ref <= (REF_RESEARCH_START + REF_RANGE))
    ((RESEARCH*)psStats)->techLevel = techLevel;
  else
  {
#ifdef HASH_NAMES
    DEBUG_ASSERT_TEXT(FALSE, "Invalid stat id for {:x}", psStats->NameHash);
#else
    DEBUG_ASSERT_TEXT(FALSE, "Invalid stat id for {}", psStats->pName);
#endif
    return FALSE;
  }
  return TRUE;
...
SDWORD getWeaponSubClass(STRING* pSubClass)
{
  ...
  DEBUG_ASSERT_TEXT(FALSE, "Invalid weapon sub class - {}", pSubClass);
...
SDWORD getMovementModel(STRING* pMovement)
{
  ...
  if (!strcmp(pMovement, "SWEEP"))
    return MM_SWEEP;
  //problem if we've got to here
  DEBUG_ASSERT_TEXT(FALSE, "Invalid movement model - {}", pMovement);
...
UBYTE getWeaponEffect(STRING* pWeaponEffect)
{
  ...
  if (!strcmp(pWeaponEffect, "ANTI TANK"))
    return WE_ANTI_TANK;
  if (!strcmp(pWeaponEffect, "BUNKER BUSTER"))
    return WE_BUNKER_BUSTER;
  if (!strcmp(pWeaponEffect, "ARTILLERY ROUND"))
    return WE_ARTILLERY_ROUND;
  if (!strcmp(pWeaponEffect, "FLAMER"))
    return WE_FLAMER;
  if (!strcmp(pWeaponEffect, "ANTI AIRCRAFT"))
...
BOOL allocateName(STRING** ppStore, STRING* pName)
{
  ...
  //get the string from the id
  *ppStore = strresGetString(psStringRes, id); return TRUE;
...
{
  return (psStats->firePause - (psStats->firePause * asWeaponUpgrade[player][psStats->weaponSubClass].firePause) / 100);
}

UDWORD weaponShortHit(WEAPON_STATS* psStats, UBYTE player)
{
  return (psStats->shortHit + (psStats->shortHit * asWeaponUpgrade[player][psStats->weaponSubClass].shortHit) / 100);
}
...
{
  return (psStats->incenDamage + (psStats->incenDamage * asWeaponUpgrade[player][psStats->weaponSubClass].incenDamage) / 100);
}
...
{
  return static_cast<UWORD>(psStats->power + (psStats->power * asSensorUpgrade[player].power) / 100);
}

UDWORD sensorRange(SENSOR_STATS* psStats, UBYTE player)
{
  return static_cast<UWORD>(psStats->range + (psStats->range * asSensorUpgrade[player].range) / 100);
}

/*Access functions for the upgradeable stats of a ECM*/
UDWORD ecmPower(ECM_STATS* psStats, UBYTE player)
{
  return static_cast<UWORD>(psStats->power + (psStats->power * asECMUpgrade[player].power) / 100);
}
...
UDWORD bodyArmour(BODY_STATS* psStats, UBYTE player, UBYTE bodyType, WEAPON_CLASS weaponClass)
{
  ...
  case WC_KINETIC:
    //case WC_EXPLOSIVE:
    return (psStats->armourValue[WC_KINETIC] + (psStats->armourValue[WC_KINETIC] * asBodyUpgrade[player][bodyType].armourValue[WC_KINETIC])
      / 100);
    break;
  case WC_HEAT:
    //case WC_MISC:
  ...
  DEBUG_ASSERT_TEXT(FALSE, "bodyArmour() : Unknown weapon class");
  return 0; // Should never get here.
}
...
UDWORD getMaxComponentWeight(void) { return maxComponentWeight; }
...
UDWORD getMaxBodyArmour(void) { return maxBodyArmour; }
...
UDWORD getMaxBodyPower(void) { return maxBodyPower; }
...
UDWORD getMaxBodyPoints(void) { return maxBodyPoints; }
...
UDWORD getMaxSensorRange(void) { return maxSensorRange; }
...
UDWORD getMaxSensorPower(void) { return maxSensorPower; }
...
UDWORD getMaxECMPower(void) { return maxECMPower; }
...
UDWORD getMaxConstPoints(void) { return maxConstPoints; }
...
UDWORD getMaxRepairPoints(void) { return maxRepairPoints; }
...
UDWORD getMaxWeaponRange(void) { return maxWeaponRange; }
...
UDWORD getMaxWeaponDamage(void) { return maxWeaponDamage; }
...
UDWORD getMaxPropulsionSpeed(void) { return maxPropulsionSpeed; }
...
{
  UWORD weaponDamage, sensorRange, sensorPower, repairPoints, ecmPower, constPoints, bodyPoints, bodyPower, bodyArmour, inc;

  //init all the values
  weaponDamage = sensorRange = sensorPower = repairPoints = ecmPower = constPoints = bodyPoints = bodyPower = bodyArmour = static_cast<
    UWORD>(0);
