...
// ----------------------------------------------------------------------------------
/* Function Prototypes */
KEY_MAPPING* keyAddMapping(KEY_STATUS status, KEY_CODE metaCode, KEY_CODE subcode, KEY_ACTION action, void (*pKeyMapFunc)(void),
                           STRING* name);
...
KEY_MAPPING* keyGetMappingFromFunction(void* function)
{
  ...
  for (psMapping = keyMappings, psReturn = nullptr; psMapping AND !psReturn; psMapping = psMapping->psNext)
  {
...
using KEYMAP_MARKER = struct _keymap_Marker
{
...
void keyInitMappings(BOOL bForceDefaults)
{
  ...
  keyAddMapping(KEYMAP_ALWAYS_PROCESS,KEY_IGNORE, KEY_F1, KEYMAP_PRESSED, kf_ChooseManufacture,
                strresGetString(psStringRes, STR_RET_MANUFACTURE));
  keyAddMapping(KEYMAP_ALWAYS_PROCESS,KEY_IGNORE, KEY_F2, KEYMAP_PRESSED, kf_ChooseResearch,
                strresGetString(psStringRes, STR_RET_RESEARCH));
  ...
  keyAddMapping(KEYMAP_ALWAYS_PROCESS,KEY_IGNORE, KEY_F5, KEYMAP_PRESSED, kf_ChooseIntelligence,
                strresGetString(psStringRes, STR_RET_INTELLIGENCE));
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_F11, KEYMAP_PRESSED, kf_ToggleFormationSpeedLimiting,
                strresGetString(psStringRes, STR_BIND_SPLIM));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_F12, KEYMAP_PRESSED, kf_MoveToLastMessagePos,
                strresGetString(psStringRes, STR_BIND_PREV));
  //                                **********************************
  //                                **********************************
  //										ASSIGN GROUPS
  ...
  keyAddMapping(KEYMAP_ALWAYS,KEY_IGNORE, KEY_KP_FULLSTOP, KEYMAP_PRESSED, kf_multiAudioStart,
                strresGetString(psStringRes, STR_BIND_AUDON));
  keyAddMapping(KEYMAP_ALWAYS,KEY_IGNORE, KEY_KP_FULLSTOP, KEYMAP_RELEASED, kf_multiAudioStop,
                strresGetString(psStringRes, STR_BIND_AUDOFF));
  //
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE, KEY_LCTRL, KEY_BACKSPACE, KEYMAP_PRESSED, kf_NormalSpeed,
                strresGetString(psStringRes, STR_BIND_NORMAL_SPEED));
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_KP_STAR, KEYMAP_PRESSED, kf_JumpToResourceExtractor,
                strresGetString(psStringRes, STR_BIND_RESJ));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_JumpToRepairUnits,
                strresGetString(psStringRes, STR_BIND_REPJ));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_JumpToConstructorUnits,
                strresGetString(psStringRes, STR_BIND_CONJ));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_JumpToSensorUnits,
                strresGetString(psStringRes, STR_BIND_SENJ));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_JumpToCommandUnits,
                strresGetString(psStringRes, STR_BIND_COMJ));
  ...
  keyAddMapping(KEYMAP_ALWAYS,KEY_IGNORE, KEY_BACKQUOTE, KEYMAP_PRESSED, kf_ToggleConsoleDrop,
                strresGetString(psStringRes, STR_BIND_CONSOLE));
  //                                **********************************
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_D, KEYMAP_PRESSED, kf_JumpToUnassignedUnits,
                strresGetString(psStringRes, STR_BIND_UNITJ));
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_FULLSTOP, KEYMAP_PRESSED, kf_SetDroidRetreatHeavy,
                strresGetString(psStringRes, STR_BIND_HDAM));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE, KEY_FORWARDSLASH, KEYMAP_PRESSED, kf_SetDroidRetreatNever,
                strresGetString(psStringRes, STR_BIND_NDAM));
  //                                **********************************
  //                                **********************************
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE, KEY_LCTRL, KEY_S, KEYMAP_PRESSED, kf_SelectAllOnScreenUnits,
                strresGetString(psStringRes, STR_BIND_ASCR));
  ...
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_SelectNextFactory,
                strresGetString(psStringRes, STR_BIND_SELFACTORY));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_SelectNextResearch,
                strresGetString(psStringRes, STR_BIND_SELRESEARCH));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_SelectNextPowerStation,
                strresGetString(psStringRes, STR_BIND_SELPOWER));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_SelectNextCyborgFactory,
                strresGetString(psStringRes, STR_BIND_SELCYBORG));
  keyAddMapping(KEYMAP_ASSIGNABLE,KEY_IGNORE,KEY_MAXSCAN, KEYMAP_PRESSED, kf_ToggleReopenBuildMenu,
                strresGetString(psStringRes, STR_BIND_REOPEN_BUILD));
                           ...
                           STRING* name)
{
  ...
  DEBUG_ASSERT_TEXT(newMapping != nullptr,"Couldn't allocate memory for a key mapping");

  /* Plus one for the terminator */

  newMapping->pName = new (std::nothrow) STRING[strlen(name)+1];
  DEBUG_ASSERT_TEXT(newMapping->pName != nullptr,"Couldn't allocate the memory for the string in a mapping");
...
void keyClearMappings(void) { while (keyMappings) { keyRemoveMappingPt(keyMappings); } }
...
BOOL keyRemoveMappingPt(KEY_MAPPING* psToRemove)
{
  ...
  /* See if we can find it */
  for (psPrev = nullptr, psCurr = keyMappings; psCurr != nullptr AND psCurr != psToRemove; psPrev = psCurr, psCurr = psCurr->psNext)
  {
    /*NOP*/
  }
...
UDWORD getNumMappings(void) { return (numActiveMappings); }
...
void keyProcessMappings(BOOL bExclude)
{
  ...
  if (keyDown(KEY_LCTRL) OR keyDown(KEY_RCTRL) OR keyDown(KEY_LALT) OR keyDown(KEY_RALT) OR keyDown(KEY_LSHIFT) OR keyDown(KEY_RSHIFT))
    bMetaKeyDown = TRUE;
  else
    bMetaKeyDown = FALSE;
  ...
  for (keyToProcess = keyMappings; keyToProcess != nullptr; keyToProcess = keyToProcess->psNext)
  {
    ...
    if (keyToProcess->metaKeyCode == KEY_IGNORE AND !bMetaKeyDown AND !(keyToProcess->status == KEYMAP__DEBUG AND bDoingDebugMappings ==
      FALSE))
    {
...
BOOL checkQwertyKeys(void)
{
  ...
  if (keyDown(KEY_LALT))
  {
    ...
    if (qKey)
    {
      ...
      qwertyKeyMappings[tableEntry].psMapping = keyAddMapping(KEYMAP_ALWAYS, KEY_LSHIFT, qKey, KEYMAP_PRESSED, kf_JumpToMapMarker,
                                                              "Jump to new map marker");
      aquired = TRUE;
...
void keyShowMapping(KEY_MAPPING* psMapping)
{
  ...
  if (onlySub) { CONPRINTF(ConsoleString, (ConsoleString,"%s - %s",asciiSub,psMapping->pName)); }
  else { CONPRINTF(ConsoleString, (ConsoleString,"%s and %s - %s",asciiMeta,asciiSub,psMapping->pName)); }
}

// ----------------------------------------------------------------------------------
/* Returns the key code of the last sub key pressed - allows called functions to have a simple stack */
KEY_CODE getLastSubKey(void) { return (lastSubKey); }

// ----------------------------------------------------------------------------------
/* Returns the key code of the last meta key pressed - allows called functions to have a simple stack */
KEY_CODE getLastMetaKey(void) { return (lastMetaKey); }

// ----------------------------------------------------------------------------------
/* Allows us to enable/disable the whole mapping system */
void keyEnableProcessing(BOOL val) { bKeyProcessing = val; }
...
void keySetMappingStatus(KEY_MAPPING* psMapping, BOOL state) { psMapping->active = state; }
...
UDWORD asciiKeyCodeToTable(KEY_CODE code)
{
  ...
  return static_cast<UDWORD>(code);
}
...
void processDebugMappings(BOOL val) { bDoingDebugMappings = val; }

// ----------------------------------------------------------------------------------
/* Returns present status of debug mapping processing */
BOOL getDebugMappingStatus(void) { return (bDoingDebugMappings); }
// ----------------------------------------------------------------------------------
BOOL keyReAssignMapping(KEY_CODE origMetaCode, KEY_CODE origSubCode, KEY_CODE newMetaCode, KEY_CODE newSubCode)
{
  KEY_MAPPING* psMapping;
  ...
  for (psMapping = keyMappings, bFound = FALSE; psMapping AND !bFound; psMapping = psMapping->psNext)
  {
    /* Find the original */
...
BOOL keyReAssignMappingName(STRING* pName, KEY_CODE newMetaCode, KEY_CODE newSubCode)
{
  ...
  if (psMapping)
  {
    if (psMapping->status == KEYMAP_ASSIGNABLE)
    {
      (void)keyAddMapping(psMapping->status, newMetaCode, newSubCode, psMapping->action, psMapping->function, psMapping->pName);
