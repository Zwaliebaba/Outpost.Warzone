...
void snapInitVars(void) { EnabledFormID = 0; }
...
#ifdef SNAP_ERASABLE
  for (i = 0; i < SnapBuffer->NumSnaps; i++) { if (SnapBuffer->SnapCoords[i].FormID == 0) { Index = i; } }
#endif

  if (Index < 0)
  {
    DEBUG_ASSERT_TEXT(SnapBuffer->NumSnaps < SnapBuffer->MaxSnaps, "AddCursorSnap: MAXCURSORSNAPS Exceeded");
...
{
#ifdef SNAP_ERASABLE
  int i; for (i = 0; i < SnapBuffer->NumSnaps; i++)
  {
    if (SnapBuffer->SnapCoords[i].FormID == FormID) { SnapBuffer->SnapCoords[i].FormID = 0; }
  }
#endif
}
...
void EnableAllCursorSnaps(void) { EnabledFormID = 0; }

void DisableCursorSnapsExcept(UDWORD FormID) { EnabledFormID = FormID; }

void SetCursorSnap(CURSORSNAP* SnapBuffer, UWORD Index, SWORD PosX, SWORD PosY, UDWORD FormID, UDWORD ID)
{
  DEBUG_ASSERT_TEXT(Index < SnapBuffer->NumSnaps, "SetCursorSnap: Index out of range");
...
void GotoSnap(CURSORSNAP* SnapBuffer)
{
  ...
  {
    SetMousePos(0, SnapBuffer->SnapCoords[SnapBuffer->CurrentSnap].SnapX, SnapBuffer->SnapCoords[SnapBuffer->CurrentSnap].SnapY);
  }
}
...
void GotoDirectionalSnap(CURSORSNAP* SnapBuffer, SNAPDIRECTION Direction, SWORD CurrX, SWORD CurrY)
{
...
#else
    if (((i != CurrentSnap) || (Direction == SNAP_NEAREST)) && ((SnapBuffer->SnapCoords[i].FormID == EnabledFormID) || (EnabledFormID ==
      0)))
    {
#endif
  ...
  if (NearestSnap >= 0)
  {
    ...
    SetMousePos(0, SnapBuffer->SnapCoords[NearestSnap].SnapX, SnapBuffer->SnapCoords[NearestSnap].SnapY);
  }
}

void SnapToID(CURSORSNAP* SnapBuffer, UWORD snp) { SnapBuffer->CurrentSnap = snp; }

void SnapCursorTo(UWORD x, UWORD y) { SetMousePos(0, x, y); }

UDWORD SnapGetFormID(CURSORSNAP* SnapBuffer) { return SnapBuffer->SnapCoords[SnapBuffer->CurrentSnap].FormID; }

UDWORD SnapGetID(CURSORSNAP* SnapBuffer) { return SnapBuffer->SnapCoords[SnapBuffer->CurrentSnap].ID; }
