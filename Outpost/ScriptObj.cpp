...
BOOL scrBaseObjGet(UDWORD index)
{
  ...
  case OBJID_CLUSTERID:
    ...
    {
      ASSERT_TEXT(FALSE,"scrBaseObjGet: clusterID not valid for features");
      return FALSE;
    }
    type = VAL_INT;
  ...
  case OBJID_STRUCTSTAT:
    ...
    break;
  default: ASSERT_TEXT(FALSE, "scrBaseObjGet: unknown variable index");
    return FALSE;
    break;
  }
...
BOOL scrGroupObjGet(UDWORD index)
{
  ...
  case GROUPID_HEALTH:
    ...
    val = lgHealth;
    break;
  default: ASSERT_TEXT(FALSE, "scrGroupObjGet: unknown variable index");
    return FALSE;
    break;
  }
...
BOOL scrValDefSave(INTERP_TYPE type, UDWORD data, UBYTE* pBuffer, UDWORD* pSize)
{
  ...
  case ST_SOUND:
    ...
    *pSize = sizeof(UDWORD);
    break;
  case ST_STRUCTUREID:
  case ST_DROIDID: default: ASSERT_TEXT((type == ST_STRUCTUREID) || (type == ST_DROIDID), "scrValDefSave: unknown script variable type for save");
...
BOOL scrValDefLoad(SDWORD version, INTERP_TYPE type, UBYTE* pBuffer, UDWORD size, UDWORD* pData)
{
  ...
  case ST_GROUP:
    ...
    else if (version == 2)
    {
      ...
      pPos += sizeof(SDWORD);
      psGroup->sRunData.forceLevel = static_cast<UBYTE>(*((SDWORD*)pPos));
      pPos += sizeof(SDWORD);
      psGroup->sRunData.leadership = static_cast<UBYTE>(*((SDWORD*)pPos));
      pPos += sizeof(SDWORD);
    }
    else
    {
      ...
      psGroup->sRunData.leadership = static_cast<UBYTE>(*((SDWORD*)pPos));
      pPos += sizeof(SDWORD);
      psGroup->sRunData.healthLevel = static_cast<UBYTE>(*((SDWORD*)pPos));
      pPos += sizeof(SDWORD);
    }
