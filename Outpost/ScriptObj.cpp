...
BOOL scrBaseObjGet(UDWORD index)
{
  ...
  case OBJID_STRUCTSTAT:
    ...
    break;
  default: ASSERT((FALSE, "scrBaseObjGet: unknown variable index"));
    return FALSE;
    break;
  }
...
BOOL scrGroupObjGet(UDWORD index)
{
  ...
  case GROUPID_HEALTH:
    ...
    val = lgHealth;
    break;
  default: ASSERT((FALSE, "scrGroupObjGet: unknown variable index"));
    return FALSE;
    break;
  }
...
BOOL scrValDefSave(INTERP_TYPE type, UDWORD data, UBYTE* pBuffer, UDWORD* pSize)
{
  ...
  case ST_SOUND:
    ...
    *pSize = sizeof(UDWORD);
    break;
  case ST_STRUCTUREID:
  case ST_DROIDID: default: ASSERT(
      ( (type == ST_STRUCTUREID) || (type == ST_DROIDID), "scrValDefSave: unknown script variable type for save"));
