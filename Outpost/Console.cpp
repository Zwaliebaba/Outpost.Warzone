...
static BOOL _addConsoleMessage(STRING* messageText, CONSOLE_TEXT_JUSTIFICATION jusType)
{
  ...
  consoleStorage[messageIndex].psNext = nullptr;
...
BOOL addConsoleMessage(STRING* messageText, CONSOLE_TEXT_JUSTIFICATION jusType) { return _addConsoleMessage(messageText, jusType); }

UDWORD getNumberConsoleMessages(void) { return (numActiveMessages); }
...
void updateConsoleMessages(void)
{
  ...
  if (consoleMessages == nullptr OR mainConsole.permanent)
    return;
  ...
  if (gameTime2 - consoleMessages->timeAdded > messageDuration)
  {
    ...
    if (consoleMessages->psNext == nullptr)
    {
...
*/
void setConsoleMessageDuration(UDWORD time) { messageDuration = time; }
...
void displayConsoleMessages(void)
{
  ...
  if (consoleMessages == nullptr AND !bConsoleDropped)
  {
    /* No point - so get out */
    return;
  }
  ...
  for (psMessage = consoleMessages, numProcessed = 0; psMessage AND numProcessed < consoleVisibleLines AND MesY < (DISP_HEIGHT - linePitch);
       psMessage = psMessage->psNext)
  {
    /* Draw the text string */
    MesY = pie_DrawFormattedText((UBYTE*)psMessage->text, mainConsole.topX, MesY, mainConsole.width, psMessage->JustifyType,FALSE);
    /* Move on */
    numProcessed++;
  }
}
...
UDWORD displayOldMessages(void)
{
  ...
  */
  ...
  {
    /* Draw the text string */
    MesY = pie_DrawFormattedText((UBYTE*)consoleStorage[history[i]].text, mainConsole.topX, MesY, mainConsole.width,
                                 consoleStorage[history[i]].JustifyType,FALSE);
  }
  /* Draw the top one */
  (void)pie_DrawFormattedText((UBYTE*)consoleStorage[history[0]].text, mainConsole.topX, MesY, mainConsole.width,
  ...
  if (messageId)
  {
    ...
    if (bGotIt)
    {
      ...
      {
        /* Draw the text string */
        MesY = pie_DrawFormattedText((UBYTE*)consoleStorage[thisIndex].text, mainConsole.topX, MesY, mainConsole.width,
                                     consoleStorage[thisIndex].JustifyType,FALSE);
        count++;
        if (thisIndex)
          thisIndex--;
        else
          thisIndex = MAX_CONSOLE_MESSAGES - 1;
      }
    }
  }
  return (count);
...
void setConsoleBackdropStatus(BOOL state) { bTextBoxActive = state; }
...
*/
void enableConsoleDisplay(BOOL state) { bConsoleDisplayEnabled = state; }
...
UDWORD getConsoleLineInfo(VOID) { return consoleVisibleLines; }
...
void permitNewConsoleMessages(BOOL allow) { allowNewMessages = allow; }

BOOL getConsoleDisplayStatus(void) { return (bConsoleDisplayEnabled); }
