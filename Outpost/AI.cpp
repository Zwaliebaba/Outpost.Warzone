...
BOOL aiNearestTarget(DROID* psDroid, BASE_OBJECT** ppsObj)
{
  ...
  {
    if (asDroidNaybors[i].psObj->player != psDroid->player && (asDroidNaybors[i].psObj->type == OBJ_DROID || asDroidNaybors[i].psObj->type
      == OBJ_STRUCTURE) && asDroidNaybors[i].psObj->visible[psDroid->player] && !aiCheckAlliances(
      asDroidNaybors[i].psObj->player, psDroid->player))
    {
      ...
      if (psObj->type == OBJ_DROID)
      {
        ...
        if (bMultiPlayer)
        {
          ...
          if (!electronic OR (electronic AND ((DROID*)psObj)->droidType != DROID_TRANSPORTER))
          {
      ...
      else if (psObj->type == OBJ_STRUCTURE)
      {
        ...
        else if ((((STRUCTURE*)psObj)->pStructureType->type != REF_WALL && ((STRUCTURE*)psObj)->pStructureType->type != REF_WALLCORNER) ||
          driveModeActive() || (bMultiPlayer && game.type == SKIRMISH && !isHumanPlayer(psDroid->player)))
          psTarget = psObj;
      }
    }
  }
...
BOOL aiStructHasRange(STRUCTURE* psStruct, BASE_OBJECT* psTarget)
{
  ...
  xdiff = static_cast<SDWORD>(psStruct->x) - static_cast<SDWORD>(psTarget->x);
  ydiff = static_cast<SDWORD>(psStruct->y) - static_cast<SDWORD>(psTarget->y);
...
BOOL aiObjIsWall(BASE_OBJECT* psObj)
{
  ...
  if (((STRUCTURE*)psObj)->pStructureType->type != REF_WALL && ((STRUCTURE*)psObj)->pStructureType->type != REF_WALLCORNER)
    return FALSE;
...
BOOL aiChooseTarget(BASE_OBJECT* psObj, BASE_OBJECT** ppsTarget)
{
  UDWORD radSquared;
  ...
  case OBJ_DROID:
    ...
    if (((DROID*)psObj)->asWeaps[0].nStat == 0 && ((DROID*)psObj)->droidType != DROID_SENSOR)
    {
      // Can't attack without a weapon
      return FALSE;
    }
    radSquared = ((DROID*)psObj)->sensorRange * ((DROID*)psObj)->sensorRange;
    break;
  case OBJ_STRUCTURE:
    ...
    if ((((STRUCTURE*)psObj)->pStructureType->type == REF_DEFENSE) && (asWeaponStats[((STRUCTURE*)psObj)->asWeaps[0].nStat].surfaceToAir ==
      SHOOT_IN_AIR))
  ...
  if (psObj->type == OBJ_DROID)
  {
    if (aiNearestTarget((DROID*)psObj, &psTarget))
    {
      ...
      if (validTarget(psObj, psTarget))
      {
        ...
        if (xdiff * xdiff + ydiff * ydiff < static_cast<SDWORD>(radSquared))
        {
          *ppsTarget = psTarget;
          return TRUE;
        }
  ...
  else if (psObj->type == OBJ_STRUCTURE)
  {
    ...
    DEBUG_ASSERT_TEXT(((STRUCTURE *)psObj)->asWeaps[0].nStat > 0, "aiChooseTarget: no weapons on structure");
    ...
    if (!proj_Direct(psWStats) && (psCommander != nullptr) && aiStructHasRange((STRUCTURE*)psObj, (BASE_OBJECT*)psCommander))
    {
      ...
      if (psCommander->action == DACTION_ATTACK && psCommander->psActionTarget != nullptr)
      {
    ...
    if (psTarget == nullptr && !bCommanderBlock && !proj_Direct(psWStats))
    {
      for (psCStruct = apsStructLists[psObj->player]; psCStruct; psCStruct = psCStruct->psNext)
      {
        ...
        if (!bCBTower && structStandardSensor(psCStruct) && psCStruct->psTarget != nullptr)
        {
          /*check its a valid target*/
          if (validTarget(psObj, psCStruct->psTarget) && aiStructHasRange((STRUCTURE*)psObj, psCStruct->psTarget))
          {
            xdiff = static_cast<SDWORD>(psCStruct->psTarget->x) - static_cast<SDWORD>(psObj->x);
            ydiff = static_cast<SDWORD>(psCStruct->psTarget->y) - static_cast<SDWORD>(psObj->y);
        ...
        else if (structCBSensor(psCStruct) && psCStruct->psTarget != nullptr)
        {
          /*check its a valid target*/
          if (validTarget(psObj, psCStruct->psTarget) && aiStructHasRange((STRUCTURE*)psObj, psCStruct->psTarget))
          {
            ...
            ydiff = static_cast<SDWORD>(psCStruct->psTarget->y) - static_cast<SDWORD>(psObj->y);
            distSq = xdiff * xdiff + ydiff * ydiff;
            if ((!bCBTower || (distSq < tarDist)) && (distSq > minDist))
            {
              tarDist = distSq;
              psTarget = psCStruct->psTarget;
              bCBTower = TRUE;
    ...
    if ((psTarget == nullptr) && !bCommanderBlock)
    {
      ...
      psCurr = gridIterate();
      while (psCurr != nullptr)
      {
        //don't target features
        if (psCurr->type != OBJ_FEATURE)
        {
          if (psObj->player != psCurr->player && !aiCheckAlliances(psCurr->player, psObj->player))
          {
            /*check its a valid target*/
            if (validTarget(psObj, psCurr) && !aiObjIsWall(psCurr))
            {
              // See if in sensor range and visible
              xdiff = psCurr->x - psObj->x;
              ydiff = psCurr->y - psObj->y;
              distSq = xdiff * xdiff + ydiff * ydiff;
              if (distSq < static_cast<SDWORD>(radSquared) && psCurr->visible[psObj->player] && distSq < tarDist)
              {
                psTarget = psCurr;
                tarDist = distSq;
              }
...
BOOL aiChooseSensorTarget(BASE_OBJECT* psObj, BASE_OBJECT** ppsTarget)
{
  ...
  {
  case OBJ_DROID:
    if (asSensorStats[((DROID*)psObj)->asBits[COMP_SENSOR].nStat].location != LOC_TURRET)
    {
    ...
    break;
  case OBJ_STRUCTURE:
    if (!(structStandardSensor((STRUCTURE*)psObj) OR structVTOLSensor((STRUCTURE*)psObj)))
    {
      // to be used for Standard and VTOL intercept Turret Sensors only
      return FALSE;
    }
    radSquared = ((STRUCTURE*)psObj)->sensorRange * ((STRUCTURE*)psObj)->sensorRange;
  ...
  else
  {
    ...
    else*/
    {
      ...
      while (psCurr != nullptr)
      {
        ...
        if (psCurr->type != OBJ_FEATURE)
        {
          if (psObj->player != psCurr->player && !aiCheckAlliances(psCurr->player, psObj->player) && !aiObjIsWall(psCurr))
          {
            // See if in sensor range and visible
            xdiff = psCurr->x - psObj->x;
            ydiff = psCurr->y - psObj->y;
...
void aiUpdateDroid(DROID* psDroid)
{
  ...
  if (!orderState(psDroid, DORDER_NONE) && !orderState(psDroid, DORDER_GUARD))
    lookForTarget = FALSE;
  ...
  if ((psDroid->order == DORDER_NONE) && (psDroid->player == selectedPlayer) && !vtolDroid(psDroid) &&
    secondaryGetState(psDroid, DSO_HALTTYPE, &state) && (state == DSS_HALT_GUARD))
    lookForTarget = FALSE;
  ...
  if (orderState(psDroid, DORDER_GUARD) && (psDroid->action != DACTION_NONE))
    lookForTarget = FALSE;
  ...
  if ((psDroid->droidType == DROID_SENSOR && psDroid->player == selectedPlayer) && !bMultiPlayer)
    lookForTarget = FALSE;
  ...
  if (lookForTarget && aiChooseTarget((BASE_OBJECT*)psDroid, &psTarget))
  {
