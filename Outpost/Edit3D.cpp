...
//
void Edit3DInitVars(void) { buildState = BUILD3D_NONE; }
...
void adjustTileHeight(MAPTILE* psTile, SDWORD adjust)
{
  ...
  if (newHeight >= MIN_TILE_HEIGHT AND newHeight <= MAX_TILE_HEIGHT)
    psTile->height = static_cast<unsigned char>(newHeight);
}
...
BOOL process3DBuilding(void)
{
  ...
  if (mouseTileX < 2)
    ...
    bX = mouseTileX;
  if (mouseTileX > static_cast<SDWORD>(mapWidth - 3))
    bX = mapWidth - 3;
  else
    bX = mouseTileX;
  ...
  if (mouseTileY < 2)
    ...
    bY = mouseTileY;
  if (mouseTileY > static_cast<SDWORD>(mapHeight - 3))
    bY = mapHeight - 3;
  else
    bY = mouseTileY;

  sBuildDetails.x = buildSite.xTL = static_cast<UWORD>(bX);
  sBuildDetails.y = buildSite.yTL = static_cast<UWORD>(bY);
  buildSite.xBR = static_cast<UWORD>(buildSite.xTL + sBuildDetails.width - 1);
  buildSite.yBR = static_cast<UWORD>(buildSite.yTL + sBuildDetails.height - 1);
