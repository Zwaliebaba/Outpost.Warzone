...
static BOOL loadSaveProximity(UBYTE* pFileData, UDWORD filesize);static BOOL loadSaveProximityV(
  UBYTE* pFileData, UDWORD filesize, UDWORD numMessages, UDWORD version);static BOOL writeProximityFile(STRING* pFileName);
#endif
...
BOOL loadGame(STRING* pGameToLoad, BOOL keepObjects, BOOL freeMem, BOOL UserSaveGame)
{
  ...
  if (UserSaveGame) //always !keepObjects
  {
    ...
    if (saveGameVersion >= VERSION_14)
    {
      ...
      for (player = 0; player < MAX_PLAYERS; player++)
      {
        ...
        if (aDefaultRepair[player] != 0 AND asRepairStats[aDefaultRepair[player]].location == LOC_DEFAULT)
          enableSelfRepair(static_cast<UBYTE>(player));
  ...
  if ((gameType != GTYPE_SAVE_START) AND (gameType != GTYPE_SAVE_MIDMISSION) AND (!keepObjects))
    clearPlayerPower();
  ...
  */
  ...
  if (saveGameOnMission && UserSaveGame)
  {
    ...
    {
      missionScrollMinX = static_cast<UWORD>(mission.scrollMinX);
      missionScrollMinY = static_cast<UWORD>(mission.scrollMinY);
      missionScrollMaxX = static_cast<UWORD>(mission.scrollMaxX);
      missionScrollMaxY = static_cast<UWORD>(mission.scrollMaxY);
    }
    ...
    for (player = 0; player < MAX_PLAYERS; player++)
    {
      for (psCurr = apsDroidLists[player]; psCurr != nullptr; psCurr = psCurr->psNext)
      {
        if (!(psCurr->droidType == DROID_PERSON OR
          ...
          cyborgDroid(psCurr) OR psCurr->droidType == DROID_TRANSPORTER))
        {
  ...
  if (saveGameVersion >= VERSION_16)
  {
    ...
    {
      setNoGoArea(saveGameData.sLandingZone[inc].x1, saveGameData.sLandingZone[inc].y1, saveGameData.sLandingZone[inc].x2,
                  saveGameData.sLandingZone[inc].y2, static_cast<UBYTE>(inc));
    }
  }
  ...
  if ((gameType == GTYPE_SAVE_START) || (gameType == GTYPE_SAVE_MIDMISSION))
  {
    DEBUG_ASSERT_TEXT(gameTime == savedGameTime, "loadGame; game time modified during load");
    gameTimeReset(savedGameTime); //added 14 may 98 JPS to solve kev's problem with no firing droids
    //need to reset the event timer too - AB 14/01/99
    eventTimeReset(savedGameTime / SCR_TICKRATE);
...
BOOL saveGame(STRING* aFileName, SDWORD saveType)
{
  ...
  CreateDirectory(aFileName, nullptr);
...
UDWORD getCampaign(STRING* pGameToLoad, BOOL* bSkipCDCheck)
{
  ...
  if (psHeader->version >= VERSION_33)
  {
...
// -----------------------------------------------------------------------------------------
void game_SetValidityKey(UDWORD keys) { validityKey = validityKey | keys; }
...
BOOL gameLoadV7(UBYTE* pFileData, UDWORD filesize)
{
  ...
  if (gameType == GTYPE_SAVE_START)
  {
    ...
    if (psNewLevel->type == LDS_CAMSTART OR psNewLevel->type == LDS_BETWEEN
#ifndef COVERMOUNT
      OR psNewLevel->type == LDS_EXPAND OR psNewLevel->type == LDS_EXPAND_LIMBO
#endif
    )
      launchMission();
  }
...
BOOL gameLoadV(UBYTE* pFileData, UDWORD filesize, UDWORD version)
{
  ...
  if (version >= VERSION_18) //version 18
  {
    ...
    DEBUG_ASSERT_TEXT(strlen(date)<MAX_STR_LENGTH, "BuildDate; String error");
  ...
  if (gameType != GTYPE_SCENARIO_START)
  {
    ...
    for (inc = 0; inc < MAX_PLAYERS; inc++)
    {
      ...
      asPower[inc]->psLastPowered = nullptr;
    }
  }
...
BOOL writeGameFile(STRING* pFileName, SDWORD saveType)
{
  ...
  DEBUG_ASSERT_TEXT(saveType == GTYPE_SAVE_START || saveType == GTYPE_SAVE_MIDMISSION, "writeGameFile: invalid save type");
  ...
  {
    DEBUG_ASSERT_TEXT(FALSE, "writeGameFile:Unable to save level name - too long (max20) - {}", pLevelName);
    goto error;
  }
  ...
  psSaveGame->missionScrollMinX = static_cast<UWORD>(mission.scrollMinX);
  psSaveGame->missionScrollMinY = static_cast<UWORD>(mission.scrollMinY);
  psSaveGame->missionScrollMaxX = static_cast<UWORD>(mission.scrollMaxX);
  psSaveGame->missionScrollMaxY = static_cast<UWORD>(mission.scrollMaxY);
  ...
  psSaveGame->radarZoom = static_cast<UBYTE>(GetRadarZoom());

  //version 20
  psSaveGame->bDroidsToSafetyFlag = static_cast<UBYTE>(getDroidsToSafetyFlag());
  for (i = 0; i < MAX_PLAYERS; i++)
  ...
  psSaveGame->bPlayCountDown = static_cast<UBYTE>(getPlayCountDown());
  psSaveGame->bPlayerHasWon = static_cast<UBYTE>(testPlayerHasWon());
  psSaveGame->bPlayerHasLost = static_cast<UBYTE>(testPlayerHasLost());

  //version 30
  psSaveGame->scrGameLevel = scrGameLevel;
  psSaveGame->bExtraFailFlag = static_cast<UBYTE>(bExtraFailFlag);
  psSaveGame->bExtraVictoryFlag = static_cast<UBYTE>(bExtraVictoryFlag);
  psSaveGame->bTrackTransporter = static_cast<UBYTE>(bTrackTransporter);
...
BOOL loadSaveDroidInitV2(UBYTE* pFileData, UDWORD filesize, UDWORD quantity)
{
  ...
  for (i = 0; i < quantity; i++)
  {
    ...
    {
      DEBUG_ASSERT_TEXT(PTRVALID(psTemplate, sizeof(DROID_TEMPLATE)), "loadSaveUnitInitV2: Invalid template pointer");
...
DROID* buildDroidFromSaveDroid(SAVE_DROID* psSaveDroid, UDWORD version)
{
  ...
  if (version >= VERSION_24) //version 24
  {
    ...
    psDroid->sMove.fz = static_cast<FRACT>(psDroid->z);
  ...
  for (list = 0; list < 3; list++)
  {
    ...
    for (player = 0; player < MAX_PLAYERS; player++)
    {
      ...
      while (psDroid)
      {
        ...
        DEBUG_ASSERT_TEXT(id != 0xdddddddd, "LoadUnit found freed target");
        if (id != UDWORD_MAX)
        {
          psDroid->psTarget = getBaseObjFromId(id);
          DEBUG_ASSERT_TEXT(psDroid->psTarget != NULL, "Saved Droid psTarget getBaseObjFromId() failed");
        ...
        DEBUG_ASSERT_TEXT(id != 0xdddddddd, "LoadUnit found freed action target");
        if (id != UDWORD_MAX)
        {
          psDroid->psActionTarget = getBaseObjFromId(id);
          DEBUG_ASSERT_TEXT(psDroid->psActionTarget != NULL, "Saved Droid psActionTarget getBaseObjFromId() failed");
        ...
        DEBUG_ASSERT_TEXT(id != 0xdddddddd, "LoadUnit found freed baseStruct");
        if (id != UDWORD_MAX)
        {
          psDroid->psBaseStruct = (STRUCTURE*)getBaseObjFromId(id);
          DEBUG_ASSERT_TEXT(psDroid->psBaseStruct != NULL, "Saved Droid psBaseStruct getBaseObjFromId() failed");
          if (psDroid->psBaseStruct == nullptr)
            psDroid->action = DACTION_NONE;
        }
        else
          psDroid->psBaseStruct = nullptr; //psSaveDroid->targetID		
        if (saveGameVersion > VERSION_20)
        {
          ...
          if (((UDWORD)psDroid->psGrpNext) == UDWORD_MAX)
          {
            ...
            psDroid->psGroup = nullptr;
            psDroid->psGrpNext = nullptr;
            DEBUG_ASSERT_TEXT(id != 0xdddddddd, "LoadUnit found freed commander");
            if (id != UDWORD_MAX)
            {
              psCommander = (DROID*)getBaseObjFromId(id);
              DEBUG_ASSERT_TEXT(psCommander != NULL, "Saved Droid psCommander getBaseObjFromId() failed");
...
BOOL loadSaveDroidV11(UBYTE* pFileData, UDWORD filesize, UDWORD numDroids, UDWORD version, DROID** ppsCurrentDroidLists)
{
  ...
  for (count = 0; count < numDroids; count++, pFileData += sizeOfSaveDroid)
  {
    ...
    else if (psSaveDroid->saveType == DROID_ON_TRANSPORT)
    {
      ...
      psDroid->psBaseStruct = nullptr;
      DEBUG_ASSERT_TEXT(psCurrentTransGroup != NULL, "loadSaveUnitV9; Transporter unit without group ");
...
BOOL loadSaveDroidV19(UBYTE* pFileData, UDWORD filesize, UDWORD numDroids, UDWORD version, DROID** ppsCurrentDroidLists)
{
  ...
  for (count = 0; count < numDroids; count++, pFileData += sizeOfSaveDroid)
  {
    ...
    if (psDroid == nullptr)
      DEBUG_ASSERT_TEXT(psDroid != NULL, "unitLoad: Failed to build new unit\n");
    else if (psSaveDroid->saveType == DROID_ON_TRANSPORT)
    {
      //add the droid to the list
      psDroid->order = DORDER_NONE;
...
BOOL buildSaveDroidFromDroid(SAVE_DROID* psSaveDroid, DROID* psCurr, DROID_SAVE_TYPE saveType)
{
  ...
  psSaveDroid->droidType = static_cast<UBYTE>(psCurr->droidType);
  psSaveDroid->saveType = static_cast<UBYTE>(saveType); //identifies special load cases
  ...
  {
    DEBUG_ASSERT_TEXT(psCurr->psTarget->id != 0xdddddddd, "SaveUnit found freed target");
  ...
  {
    DEBUG_ASSERT_TEXT(psCurr->psActionTarget->id != 0xdddddddd, "SaveUnit found freed action target");
  ...
  {
    DEBUG_ASSERT_TEXT(strlen(psCurr->psTarStats->pName) < MAX_NAME_SIZE, "writeUnitFile; psTarStat pName Error");
    strcpy(psSaveDroid->tarStatName, psCurr->psTarStats->pName);
  }
  else
    strcpy(psSaveDroid->tarStatName, "");
  if ((psCurr->psBaseStruct != nullptr) && ((UDWORD)(psCurr->psBaseStruct) != UDWORD_MAX))
  {
  ...
  if ((psCurr->psGroup) && (psCurr->droidType != DROID_COMMAND))
  {
    if (psCurr->psGroup->type == GT_COMMAND)
    {
      if (psCurr->psGroup->psCommander->died <= 1)
      {
        ...
        DEBUG_ASSERT_TEXT(checkValidId(psSaveDroid->commandId), "SaveUnit pcCommander not found");
      }
      else
      {
        psSaveDroid->commandId = UDWORD_MAX;
        DEBUG_ASSERT_TEXT(FALSE, "SaveUnit pcCommander died");
      }
    }
    else
      psSaveDroid->commandId = UDWORD_MAX;
  ...
  if (psSaveDroid->sMove.psFormation != nullptr)
  {
...
BOOL writeDroidFile(STRING* pFileName, DROID** ppsCurrentDroidLists)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psCurr = ppsCurrentDroidLists[player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      totalDroids++;
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psCurr = ppsCurrentDroidLists[player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      ...
      if (psCurr->droidType == DROID_TRANSPORTER AND ppsCurrentDroidLists[player] == mission.apsDroidLists[player])
      {
        psCurr->x = INVALID_XY;
        psCurr->y = INVALID_XY;
      }
      ...
      {
        psTrans = psCurr->psGroup->psList;
        for (psTrans = psCurr->psGroup->psList; psTrans != nullptr; psTrans = psTrans->psGrpNext)
        {
...
BOOL loadSaveStructureV19(UBYTE* pFileData, UDWORD filesize, UDWORD numStructures, UDWORD version)
{
  ...
  for (count = 0; count < numStructures; count++, pFileData += sizeOfSaveStruture)
  {
    ...
    //if not a save game, don't want to overwrite any of the stats so continue
    if ((gameType != GTYPE_SAVE_START) AND (gameType != GTYPE_SAVE_MIDMISSION))
      continue;
    ...
    case REF_CYBORG_FACTORY:
      ...
      if (version >= VERSION_12)
      {
        ...
        psFactory->quantity = static_cast<UBYTE>(psSaveStructure->quantity);
        ...
        //assemblyCheck
        psFactory->psAssemblyPoint = (FLAG_POSITION*)psSaveStructure->factoryInc;
        ...
        {
          psFactory->psSubject = (BASE_STATS*)getTemplateFromMultiPlayerID(psSaveStructure->subjectInc);
          //if the build has started set the powerAccrued = 
          //powerRequired to sync the interface
          if (psFactory->timeStarted != ACTION_START_TIME AND psFactory->psSubject)
          {
            psFactory->powerAccrued = ((DROID_TEMPLATE*)psFactory->psSubject)->powerPoints;
          }
        }
      }
      break;
    case REF_RESEARCH:
      ...
      if (psResearch->timeStarted != ACTION_START_TIME AND psResearch->psSubject)
      {
    ...
    case REF_REPAIR_FACILITY: //CODE THIS SOMETIME
      if (version >= VERSION_19)
      {
        ...
        else
        {
          ...
          //if the build has started set the powerAccrued = 
          //powerRequired to sync the interface
          if (psRepair->timeStarted != ACTION_START_TIME AND psRepair->psObj)
    ...
    psStructure->body = static_cast<UWORD>(structureBody(psStructure));
    if (psSaveStructure->body < psStructure->body)
      psStructure->body = static_cast<UWORD>(psSaveStructure->body);
    //set the build status from the build points
    psStructure->currentBuildPts = static_cast<SWORD>(psStructure->pStructureType->buildPoints);
    ...
    else
    {
      ...
      case REF_RESOURCE_EXTRACTOR:
        ...
        if (((RES_EXTRACTOR*)psStructure->pFunctionality)->power != 0)
        {
          ...
          if (psStructure->psCurAnim == nullptr)
...
BOOL loadSaveStructureV(UBYTE* pFileData, UDWORD filesize, UDWORD numStructures, UDWORD version)
{
  ...
  for (count = 0; count < numStructures; count++, pFileData += sizeOfSaveStruture)
  {
    ...
    psStructure->currentBuildPts = static_cast<SWORD>(psSaveStructure->currentBuildPts);
    psStructure->currentPowerAccrued = psSaveStructure->currentPowerAccrued;
    psStructure->resistance = static_cast<SWORD>(psSaveStructure->resistance);
    ...
    case REF_REPAIR_FACILITY: //CODE THIS SOMETIME
      ...
      */
      ...
      psRepair->psObj = (BASE_OBJECT*)psSaveStructure->subjectInc;
    ...
    case REF_REARM_PAD:
      if (version >= VERSION_26) //version 26
      {
        ...
        psReArmPad->psObj = (BASE_OBJECT*)psSaveStructure->subjectInc;
...
BOOL writeStructFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psCurr = apsStructLists[player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      ...
      if (psCurr->currentBuildPts >= static_cast<SWORD>(psCurr->pStructureType->buildPoints))
      ...
      if (psCurr->pFunctionality)
      {
        ...
        case REF_VTOL_FACTORY:
          ...
          break;
        case REF_RESEARCH:
          psSaveStruct->capacity = ((RESEARCH_FACILITY*)psCurr->pFunctionality)->capacity;
          //don't need to save this - it gets set up
          //psSaveStruct->output = ((RESEARCH_FACILITY *)psCurr->
          psSaveStruct->powerAccrued = ((RESEARCH_FACILITY*)psCurr->pFunctionality)->powerAccrued;
          psSaveStruct->timeStartHold = ((RESEARCH_FACILITY*)psCurr->pFunctionality)->timeStartHold;
          if (((RESEARCH_FACILITY*)psCurr->pFunctionality)->psSubject)
          {
            ...
            DEBUG_ASSERT_TEXT(strlen(asResearch[researchId].pName)<MAX_NAME_SIZE, "writeStructData: research name too long");
          ...
          break;
        case REF_POWER_GEN:
          psSaveStruct->capacity = ((POWER_GEN*)psCurr->pFunctionality)->capacity;
          break;
        case REF_RESOURCE_EXTRACTOR:
          psSaveStruct->output = ((RES_EXTRACTOR*)psCurr->pFunctionality)->power;
          break;
        case REF_REPAIR_FACILITY: //CODE THIS SOMETIME
        ...
        case REF_REARM_PAD:
          ...
          if (psReArmPad->psObj != nullptr)
            psSaveStruct->subjectInc = psReArmPad->psObj->id;
          else
            psSaveStruct->subjectInc = UDWORD_MAX;
          break;
        default: //CODE THIS SOMETIME
          DEBUG_ASSERT_TEXT(FALSE, "Structure facility not saved");
          break;
        }
      }
...
BOOL loadStructSetPointers(void)
{
  ...
  for (list = 0; list < 2; list++)
  {
    for (player = 0; player < MAX_PLAYERS; player++)
    {
      psStruct = ppsStructLists[list][player];
      while (psStruct)
      {
        if (psStruct->pFunctionality)
        {
          ...
          case REF_VTOL_FACTORY:
            ...
            if ((UDWORD)psFactory->psCommander != UDWORD_MAX)
            {
              ...
              DEBUG_ASSERT_TEXT(psCommander != NULL, "loadStructSetPointers psCommander getBaseObjFromId() failed");
              if (psCommander == nullptr)
                psFactory->psCommander = nullptr;
              else
              {
                if (list == 1) //ie offWorld
                {
                  ...
                  ((FACTORY*)psStruct->pFunctionality)->psCommander = psCommander;
                }
                else
...
BOOL loadSaveFeatureV2(UBYTE* pFileData, UDWORD filesize, UDWORD numFeatures)
{
  ...
  for (count = 0; count < numFeatures; count++, pFileData += sizeof(SAVE_FEATURE_V2))
  {
    ...
    else
    {
      ...
      if (!getSaveObjectName(psSaveFeature->name)) { continue; }
...
BOOL writeFeatureFile(STRING* pFileName)
{
  ...
  for (psCurr = apsFeatureLists[0]; psCurr != nullptr; psCurr = psCurr->psNext)
    totalFeatures++;
...
BOOL loadSaveTemplateV7(UBYTE* pFileData, UDWORD filesize, UDWORD numTemplates)
{
  ...
  for (count = 0; count < numTemplates; count++, pFileData += sizeof(SAVE_TEMPLATE_V2))
  {
    ...
    psTemplate->droidType = static_cast<DROID_TYPE>(psSaveTemplate->droidType);
    found = TRUE;
    ...
    for (i = 0; i < psTemplate->numWeaps; i++)
    {
      ...
      psTemplate->asWeaps[i] = static_cast<UDWORD>(compInc);
    }
    if (!found)
    {
      //ignore this record
...
BOOL writeTemplateFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psCurr = apsDroidTemplates[player]; psCurr != nullptr; psCurr = psCurr->psNext)
    {
      ...
      psSaveTemplate->droidType = static_cast<UBYTE>(psCurr->droidType);
...
BOOL loadSaveCompListV9(UBYTE* pFileData, UDWORD filesize, UDWORD numRecords, UDWORD version)
{
  ...
  for (i = 0; i < numRecords; i++, pFileData += sizeof(SAVE_COMPLIST_V6))
  {
    ...
    if (psSaveCompList->state != UNAVAILABLE AND psSaveCompList->state != AVAILABLE AND psSaveCompList->state != FOUND)
    {
      //ignore this record
      continue;
    }
    ...
    apCompLists[psSaveCompList->player][psSaveCompList->type][compInc] = psSaveCompList->state;
  }
...
static BOOL writeCompListFile(STRING* pFileName)
{
  ...
  // Calculate the file size
  totalComp = (numBodyStats + numWeaponStats + numConstructStats + numECMStats + numPropulsionStats + numSensorStats + numRepairStats +
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (i = 0; i < numBodyStats; i++)
    {
      ...
      psSaveCompList->player = static_cast<UBYTE>(player);
...
BOOL loadSaveStructTypeListV7(UBYTE* pFileData, UDWORD filesize, UDWORD numRecords)
{
  ...
  for (i = 0; i < numRecords; i++, pFileData += sizeof(SAVE_STRUCTLIST_V6))
  {
    ...
    if (psSaveStructList->state != UNAVAILABLE AND psSaveStructList->state != AVAILABLE AND psSaveStructList->state != FOUND)
    {
      //ignore this record
      continue;
    }
...
static BOOL writeStructTypeListFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    ...
    for (i = 0; i < numStructureStats; i++, psStats++)
    {
      ...
      psSaveStructList->player = static_cast<UBYTE>(player);
      psSaveStructList = (SAVE_STRUCTLIST*)((UBYTE*)psSaveStructList + sizeof(SAVE_STRUCTLIST));
    }
  }
  ...
  for (i = 0; i < numRecords; i++, pFileData += sizeof(SAVE_RESEARCH_V8))
  {
    ...
    for (playerInc = 0; playerInc < MAX_PLAYERS; playerInc++)
    {
      ...
      */
      ...
      psPlRes->ResearchStatus = static_cast<UBYTE>(psSaveResearch->researched[playerInc] & RESBITS);
...
static BOOL writeResearchFile(STRING* pFileName)
{
  ...
  for (i = 0; i < numResearch; i++, psStats++)
  {
    ...
    for (player = 0; player < MAX_PLAYERS; player++)
    {
      psSaveResearch->possible[player] = static_cast<UBYTE>(IsResearchPossible(&asPlayerResList[player][i]));
      psSaveResearch->researched[player] = static_cast<UBYTE>(asPlayerResList[player][i].ResearchStatus & RESBITS);
      psSaveResearch->currentPoints[player] = asPlayerResList[player][i].currentPoints;
    }
    psSaveResearch = (SAVE_RESEARCH*)((UBYTE*)psSaveResearch + sizeof(SAVE_RESEARCH));
  }
    ...
    if (psSaveMessage->type == MSG_PROXIMITY)
    {
      ...
      if (gameType == GTYPE_SAVE_MIDMISSION)
      {
        ...
        else
        {
          ...
          if (psMessage)
          {
            psViewData = getViewData(psSaveMessage->name);
...
static BOOL writeMessageFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psMessage = apsMessages[player]; psMessage != nullptr; psMessage = psMessage->psNext)
      numMessages++;
  }
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    ...
    for (psMessage = apsMessages[player]; psMessage != nullptr; psMessage = psMessage->psNext)
    {
      ...
      if (psMessage->type == MSG_PROXIMITY)
      {
        ...
        DEBUG_ASSERT_TEXT(psProx != NULL, "Save message; proximity display not found for message");
        ...
        if (psProx->type == POS_PROXDATA)
        {
          ...
          DEBUG_ASSERT_TEXT(strlen(pViewData->pName) < MAX_STR_SIZE, "writeMessageFile; viewdata pName Error");
...
BOOL loadSaveProximity(UBYTE* pFileData, UDWORD filesize)
{
  ...
  /* Check the file version */
  if (!loadSaveProximityV(pFileData, filesize, psHeader->quantity, psHeader->version)) { return FALSE; }
  return TRUE;
}
...
static BOOL writeProximityFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psProximity = apsProxDisp[player]; psProximity != NULL; psProximity = psProximity->psNext) { numProximitys++; }
  }
  ...
  for (i = 0; i < numflags; i++, pFileData += sizeOfSaveFlag)
  {
    ...
    else if ((psflag->type == POS_DELIVERY) || (psflag->type == POS_TEMPDELIVERY))
    {
      if (psflag->factoryType == FACTORY_FLAG)
        ...
        DEBUG_ASSERT_TEXT(FALSE, "loadSaveFlagV delivery flag type not recognised?");
        ...
        if (version > VERSION_18)
        {
          ...
          if (psStruct != nullptr)
          {
            if (psStruct->type != OBJ_STRUCTURE)
              DEBUG_ASSERT_TEXT(FALSE, "loadFlag found duplicate Id for repair facility");
      ...
      else
      {
        ...
        for (psStruct = apsStructLists[psflag->player]; psStruct != nullptr; psStruct = psStruct->psNext)
        {
...
static BOOL writeFlagFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    for (psflag = apsFlagPosLists[player]; psflag != nullptr; psflag = psflag->psNext)
      numflags++;
  }
    ...
    for (factoryNum = 0; factoryNum < MAX_FACTORY; factoryNum++)
    {
      for (runNum = 0; runNum < MAX_PROD_RUN; runNum++)
      {
        ...
        if (psSaveProduction->multiPlayerID != UDWORD_MAX)
          ...
          psCurrentProd->psTemplate = nullptr;
  ...
  for (factoryType = 0; factoryType < NUM_FACTORY_TYPES; factoryType++)
  {
    for (factoryNum = 0; factoryNum < MAX_FACTORY; factoryNum++)
    {
      for (runNum = 0; runNum < MAX_PROD_RUN; runNum++)
      {
        ...
        if (psCurrentProd->psTemplate != nullptr)
...
BOOL loadSaveStructLimitsV19(UBYTE* pFileData, UDWORD filesize, UDWORD numLimits)
{
  ...
  // Load in the data
  for (count = 0; count < numLimits; count++, pFileData += sizeof(SAVE_STRUCTLIMITS_V2))
  {
...
BOOL loadSaveStructLimitsV(UBYTE* pFileData, UDWORD filesize, UDWORD numLimits)
{
  ...
  // Load in the data
  for (count = 0; count < numLimits; count++, pFileData += sizeof(SAVE_STRUCTLIMITS))
  {
...
BOOL writeStructLimitsFile(STRING* pFileName)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    ...
    for (i = 0; i < numStructureStats; i++, psStructStats++)
    {
      ...
      psSaveLimit->player = static_cast<UBYTE>(player);
...
BOOL loadScriptState(STRING* pFileName)
{
  ...
  pFileName[strlen(pFileName) - 4] = static_cast<char>(0);
  strcat(pFileName, ".es");
...
UDWORD getSaveGameType(void) { return gameType; }
