 ...
 */
extern BOOL visibleObjectBlock(BASE_OBJECT* psViewer, BASE_OBJECT* psTarget, BOOL structBlock);
...
__inline BOOL visObjInRange(BASE_OBJECT* psObj1, BASE_OBJECT* psObj2, SDWORD range)
{
  ...
  xdiff = static_cast<SDWORD>(psObj1->x) - static_cast<SDWORD>(psObj2->x);
  if (xdiff < 0)
    xdiff = -xdiff;
  if (xdiff > range)
  {
  ...
  ydiff = static_cast<SDWORD>(psObj1->y) - static_cast<SDWORD>(psObj2->y);
  if (ydiff < 0)
    ydiff = -ydiff;
  if (ydiff > range)
  {
