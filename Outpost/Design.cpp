...
using DES_SYSMODE = enum _des_sysmode
{
  IDES_SENSOR,
...
using DES_COMPMODE = enum _des_compmode
{
  IDES_BRAIN,
...
using DES_PROPMODE = enum _des_propmode
{
  IDES_GROUND,
...
static BOOL intAddMultiComponentForm(UBYTE* aSensor, UDWORD numSensor, UBYTE* aECM, UDWORD numECM, UBYTE* aConstruct, UDWORD numConstruct,
...
static BOOL intAddComponentButtons(COMP_BASE_STATS* psStats, UDWORD size, UBYTE* aAvailable, UDWORD numEntries, UDWORD compID,
                                   UDWORD WhichTab);
/* Add the droid template buttons to a form */
BOOL intAddTemplateButtons(UDWORD formID, UDWORD formWidth, UDWORD formHeight, UDWORD butWidth, UDWORD butHeight, UDWORD gap,
...
extern void RenderCompositeDroid(UDWORD Index, iVector* Rotation, iVector* Position, iVector* TurretRotation, DROID* psDroid, BOOL RotXYZ);
...
BOOL _intAddDesign(BOOL bShowCentreScreen)
{
  ...
  sFormInit.x = static_cast<SWORD>(DES_CENTERFORMX); //0;
  sFormInit.y = static_cast<SWORD>(DES_CENTERFORMY); //0;
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>(DES_STATSFORMX);
  sFormInit.y = static_cast<SWORD>(DES_STATSFORMY);
  ...
  sBarInit.iRange = static_cast<UWORD>(getMaxBodyArmour()); //DBAR_BODYMAXARMOUR;
  ...
  sBarInit.iRange = static_cast<UWORD>(getMaxBodyPower()); //DBAR_BODYMAXPOWER;
  if (!widgAddBarGraph(psWScreen, &sBarInit))
    return TRUE;
  ...
  sBarInit.iRange = static_cast<UWORD>(getMaxComponentWeight()); //DBAR_MAXWEIGHT;
  if (!widgAddBarGraph(psWScreen, &sBarInit))
    return TRUE;
...
void desSetupDesignTemplates(void)
{
  ...
  while ((psTempl != nullptr) && (i < MAXTEMPLATES))
  {
     ...
     */
    if (psTempl->droidType != DROID_TRANSPORTER && psTempl->droidType != DROID_CYBORG && psTempl->droidType != DROID_CYBORG_SUPER && psTempl
      ->droidType != DROID_CYBORG_CONSTRUCT && psTempl->droidType != DROID_CYBORG_REPAIR &&
...
static BOOL _intAddTemplateForm(DROID_TEMPLATE* psSelected)
{
  ...
  {
    if (apsTemplateList[i] != nullptr)
      numButtons++;
  }

  /* Calculate how many buttons will go on a single form */
  butPerForm = ((DES_LEFTFORMWIDTH - DES_TABTHICKNESS - DES_TABBUTGAP) / (DES_TABBUTWIDTH + DES_TABBUTGAP)) * ((DES_LEFTFORMHEIGHT -
    DES_TABTHICKNESS - DES_TABBUTGAP) / (DES_TABBUTHEIGHT + DES_TABBUTGAP));
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>(RET_X);
  sFormInit.y = static_cast<SWORD>(DESIGN_Y);
  ...
  if (!intAddTemplateButtons(IDDES_TEMPLFORM, DES_LEFTFORMWIDTH - DES_TABTHICKNESS, DES_LEFTFORMHEIGHT - DES_TABTHICKNESS, DES_TABBUTWIDTH,
                             DES_TABBUTHEIGHT, DES_TABBUTGAP, psSelected))
    return FALSE;
                           ...
                           DROID_TEMPLATE* psSelected)
{
  ...
  for (i = 0; i < MAXTEMPLATES; i++)
  {
    if (apsTemplateList[i] != nullptr)
    {
      ...
      DEBUG_ASSERT_TEXT(BufferID >= 0, "Unable to aquire stat buffer.");
      RENDERBUTTON_INUSE(&StatBuffers[BufferID]);
      StatBuffers[BufferID].Data = static_cast<void*>(psTempl);
      sButInit.pUserData = static_cast<void*>(&StatBuffers[BufferID]);
      ...
      sBarInit.size = static_cast<UWORD>(psTempl->powerPoints / POWERPOINTS_DROIDDIV);
      ...
      sBarInit.id += 1;
      sButInit.id += 1;
      sButInit.x = static_cast<SWORD>(sButInit.x + butWidth + gap);
      if (sButInit.x + butWidth + gap > formWidth)
      {
        sButInit.x = DES_LEFTFORMBUTX;
        sButInit.y = static_cast<SWORD>(sButInit.y + butHeight + gap);
      }
...
static void intSetDesignMode(DES_COMPMODE newCompMode)
{
  ...
  if (newCompMode != desCompMode)
  {
    ...
    case IDES_SYSTEM:
      intAddComponentForm(
        intNumAvailable(apCompLists[selectedPlayer][COMP_SENSOR], numSensorStats, (COMP_BASE_STATS*)asSensorStats, sizeof(SENSOR_STATS)) +
        intNumAvailable(apCompLists[selectedPlayer][COMP_ECM], numECMStats, (COMP_BASE_STATS*)asECMStats, sizeof(ECM_STATS)) +
        intNumAvailable(apCompLists[selectedPlayer][COMP_BRAIN], numBrainStats, (COMP_BASE_STATS*)asBrainStats, sizeof(BRAIN_STATS)) +
        intNumAvailable(apCompLists[selectedPlayer][COMP_CONSTRUCT], numConstructStats, (COMP_BASE_STATS*)asConstructStats,
                        sizeof(CONSTRUCT_STATS)) + intNumAvailable(apCompLists[selectedPlayer][COMP_REPAIRUNIT], numRepairStats,
                                                                   (COMP_BASE_STATS*)asRepairStats, sizeof(REPAIR_STATS)));
      intAddExtraSystemButtons(sCurrDesign.asParts[COMP_SENSOR], sCurrDesign.asParts[COMP_ECM], sCurrDesign.asParts[COMP_CONSTRUCT],
                               sCurrDesign.asParts[COMP_REPAIRUNIT], sCurrDesign.asParts[COMP_BRAIN]);
      intAddSystemButtons(IDES_SYSTEM);
      widgSetButtonState(psWScreen, IDDES_SYSTEMFORM, WBUT_LOCK);
      break;
    case IDES_TURRET:
      intAddComponentForm(intNumAvailable(apCompLists[selectedPlayer][COMP_WEAPON], numWeaponStats, (COMP_BASE_STATS*)asWeaponStats,
                                          sizeof(WEAPON_STATS)));
      weaponIndex = (sCurrDesign.numWeaps > 0) ? sCurrDesign.asWeaps[0] : 0;
      intAddComponentButtons((COMP_BASE_STATS*)asWeaponStats, sizeof(WEAPON_STATS), apCompLists[selectedPlayer][COMP_WEAPON],
    ...
    /*			intAddMultiComponentForm(apCompLists[selectedPlayer][COMP_SENSOR],
          ...
          break;*/
    case IDES_BODY:
      intAddComponentForm(intNumAvailable(apCompLists[selectedPlayer][COMP_BODY], numBodyStats, (COMP_BASE_STATS*)asBodyStats,
                                          sizeof(BODY_STATS)));
      intAddComponentButtons((COMP_BASE_STATS*)asBodyStats, sizeof(BODY_STATS), apCompLists[selectedPlayer][COMP_BODY], numBodyStats,
                             sCurrDesign.asParts[COMP_BODY],TAB_USEMAJOR);
      widgSetButtonState(psWScreen, IDDES_BODYFORM, WBUT_LOCK);
      break;
    case IDES_PROPULSION:
      intAddComponentForm(intNumAvailable(apCompLists[selectedPlayer][COMP_PROPULSION], numPropulsionStats,
                                          (COMP_BASE_STATS*)asPropulsionStats, sizeof(PROPULSION_STATS)));
      intAddComponentButtons((COMP_BASE_STATS*)asPropulsionStats, sizeof(PROPULSION_STATS), apCompLists[selectedPlayer][COMP_PROPULSION],
                             numPropulsionStats, sCurrDesign.asParts[COMP_PROPULSION], TAB_USEMAJOR);
...
{
  COMP_BASE_STATS* psStats = nullptr;
  ...
  case DROID_COMMAND:
    ...
    psStats = (COMP_BASE_STATS*)(asBrainStats + psTemplate->asParts[COMP_BRAIN]);
    break;
  case DROID_SENSOR:
    psStats = (COMP_BASE_STATS*)(asSensorStats + psTemplate->asParts[COMP_SENSOR]);
    break;
  case DROID_ECM:
    psStats = (COMP_BASE_STATS*)(asECMStats + psTemplate->asParts[COMP_ECM]);
    break;
  case DROID_CONSTRUCT:
  case DROID_CYBORG_CONSTRUCT:
    psStats = (COMP_BASE_STATS*)(asConstructStats + psTemplate->asParts[COMP_CONSTRUCT]);
    break;
  case DROID_REPAIR:
  case DROID_CYBORG_REPAIR:
    psStats = (COMP_BASE_STATS*)(asRepairStats + psTemplate->asParts[COMP_REPAIRUNIT]);
    break;
  case DROID_WEAPON:
  case DROID_PERSON:
  case DROID_CYBORG:
  ...
  case DROID_DEFAULT:
    psStats = (COMP_BASE_STATS*)(asWeaponStats + psTemplate->asWeaps[0]);
    break;
  default: DBERROR(("intSetDesignStats: unrecognised droid type"));
    return nullptr;
  }
...
STRING* GetDefaultTemplateName(DROID_TEMPLATE* psTemplate)
{
  ...
  */
  if (psTemplate->droidType == DROID_TRANSPORTER)
    return strresGetString(nullptr, HashString("Transporter"));
  /*
  ...
  if (psTemplate->asWeaps[0] != 0 || psTemplate->asParts[COMP_CONSTRUCT] != 0 || psTemplate->asParts[COMP_SENSOR] != 0 || psTemplate->
    asParts[COMP_ECM] != 0 || psTemplate->asParts[COMP_REPAIRUNIT] != 0 || psTemplate->asParts[COMP_BRAIN] != 0)
  {
...
static BOOL _intSetSystemForm(COMP_BASE_STATS* psStats)
{
  ...
  W_BARINIT sBarInit;
  W_LABINIT sLabInit;
  auto newSysMode = static_cast<DES_SYSMODE>(0);
  ...
  {
    DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(SENSOR_STATS)), "intAddSystemForm: Invalid sensor stats pointer");
    psSensor = (SENSOR_STATS*)psStats;

    /* Add the bar graphs*/
    sBarInit.id = IDDES_SENSORRANGE;
    sBarInit.iRange = static_cast<UWORD>(getMaxSensorRange()); //DBAR_SENSORMAXRANGE;
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
    sBarInit.id = IDDES_SENSORPOWER;
    sBarInit.y = DES_STATBAR_Y2; //+= DES_CLICKBARHEIGHT + DES_CLICKGAP;
    sBarInit.iRange = static_cast<UWORD>(getMaxSensorPower()); //DBAR_SENSORMAXPOWER;
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
  ...
  {
    DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(ECM_STATS)), "intAddSystemForm: Invalid ecm stats pointer");
    psECM = (ECM_STATS*)psStats;

    /* Add the bar graphs */
    sBarInit.id = IDDES_ECMPOWER;
    sBarInit.iRange = static_cast<UWORD>(getMaxECMPower()); //DBAR_ECMMAXPOWER;
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
  ...
  {
    DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(CONSTRUCT_STATS)), "intAddSystemForm: Invalid constructor stats pointer");
    ...
    sBarInit.iRange = static_cast<UWORD>(getMaxConstPoints()); //DBAR_CONSTMAXPOINTS;
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
  ...
  {
    DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(REPAIR_STATS)), "intAddSystemForm: Invalid repair stats pointer");
    psRepair = (REPAIR_STATS*)psStats;

    /* Add the bar graphs */
    sBarInit.id = IDDES_REPAIRPOINTS;
    sBarInit.pTip = strresGetString(psStringRes, STR_DES_BUILD_POINTS);
    sBarInit.iRange = static_cast<UWORD>(getMaxRepairPoints()); //DBAR_REPAIRMAXPOINTS;
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
  ...
  {
    DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(WEAPON_STATS)), "intAddSystemForm: Invalid ecm stats pointer");
    psWeapon = (WEAPON_STATS*)psStats;

    /* Add the bar graphs */
    sBarInit.id = IDDES_WEAPRANGE;
    sBarInit.iRange = static_cast<UWORD>(getMaxWeaponRange()); //DBAR_WEAPMAXRANGE;
    sBarInit.pTip = strresGetString(psStringRes, STR_DES_RANGE);
    if (!widgAddBarGraph(psWScreen, &sBarInit))
      return FALSE;
    sBarInit.id = IDDES_WEAPDAMAGE;
    sBarInit.y = DES_STATBAR_Y2; //+= DES_CLICKBARHEIGHT + DES_CLICKGAP;
    sBarInit.iRange = static_cast<UWORD>(getMaxWeaponDamage()); //DBAR_WEAPMAXDAMAGE;
...
static BOOL intSetPropulsionForm(PROPULSION_STATS* psStats)
{
  ...
  W_BARINIT sBarInit;
  W_LABINIT sLabInit;
  auto newPropMode = static_cast<DES_PROPMODE>(0);

  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(PROPULSION_STATS)), "intAddPropulsionForm: Invalid propulsion stats pointer");
  ...
  case IDES_AIR:
    ...
    sBarInit.iRange = static_cast<UWORD>(getMaxPropulsionSpeed()); //DBAR_PROPMAXSPEED;
...
static UDWORD intNumAvailable(UBYTE* aAvailable, UDWORD numEntries, COMP_BASE_STATS* asStats, UDWORD size)
{
  ...
  for (i = 0; i < numEntries; i++)
  {
    ...
    if (psCurrStats->design AND (aAvailable[i] & AVAILABLE))
      numButtons++;
...
static BOOL intAddComponentForm(UDWORD numButtons)
{
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>((RADTLX - 2));
  sFormInit.y = static_cast<SWORD>(DESIGN_Y);
  sFormInit.width = RET_FORMWIDTH;
  ...
  sFormInit.numMajor = static_cast<UWORD>(numFrm >= WFORM_MAXMAJOR ? WFORM_MAXMAJOR - 1 : numFrm);
  sFormInit.majorPos = WFORM_TABTOP;
  sFormInit.minorPos = WFORM_TABNONE;
  sFormInit.majorSize = DES_TAB_WIDTH;
  sFormInit.majorOffset = DES_TAB_LEFTOFFSET;
...
static BOOL intAddSystemButtons(SDWORD mode)
{
  ...
  case IDES_SYSTEM:
    ...
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "intAddSystemButtons: unexpected mode");
    break;
  }
                                   ...
                                   UDWORD WhichTab)
{
  ...
  if (bWeapon)
  {
    ...
    {
      psPropStats = asPropulsionStats + sCurrDesign.asParts[COMP_PROPULSION];
      DEBUG_ASSERT_TEXT(PTRVALID(psPropStats, sizeof(PROPULSION_STATS)), "intAddComponentButtons: invalid propulsion stats pointer");
  ...
  for (i = 0; i < numEntries; i++)
  {
    ...
    DEBUG_ASSERT_TEXT(BufferID >= 0, "Unable to acquire Topic buffer.");
    ...
    {
      if ((((COMMAND_DROID*)psCurrStats)->psDroid != nullptr) || ((COMMAND_DROID*)psCurrStats)->died)
  ...
  if (psTabForm)
  {
    ...
    if (numComponent < static_cast<UDWORD>(numTabs * DES_BUTSPERFORM))
...
static BOOL intAddExtraSystemButtons(UDWORD sensorIndex, UDWORD ecmIndex, UDWORD constIndex, UDWORD repairIndex, UDWORD brainIndex)
{
  ...
  COMP_BASE_STATS* psCurrStats = nullptr;
  ...
  for (buttonType = 0; buttonType < 5; buttonType++)
  {
    ...
    for (i = 0; i < numStats; i++)
    {
      ...
      {
        DEBUG_ASSERT_TEXT(FALSE, "intAddExtraSystemButtons: Too many components for the list");
        return FALSE;
      }
      ...
      if (statType(psCurrStats->ref) == COMP_BRAIN)
        ...
        System0Buffers[BufferID].Data = psCurrStats;
      sButInit.pUserData = static_cast<void*>(&System0Buffers[BufferID]);
...
static void intSetSystemStats(COMP_BASE_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(COMP_BASE_STATS)), //			 (((UBYTE *)psStats >= (UBYTE *)asCommandDroids) &&
...
static void intSetSensorStats(SENSOR_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(SENSOR_STATS)), "intSetSensorStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_SENSOR_START) &&
    (psStats->ref < REF_SENSOR_START + REF_RANGE), "intSetSensorStats: stats ref is out of range");

  /* range */
  widgSetBarSize(psWScreen, IDDES_SENSORRANGE, sensorRange(psStats, static_cast<UBYTE>(selectedPlayer)));
  /* power */
  widgSetBarSize(psWScreen, IDDES_SENSORPOWER, sensorPower(psStats, static_cast<UBYTE>(selectedPlayer)));
  /* weight */
...
static void intSetSensorShadowStats(SENSOR_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(SENSOR_STATS)), "intSetSensorShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_SENSOR_START) && (psStats->ref < REF_SENSOR_START + REF_RANGE)), "intSetSensorShadowStats: stats ref is out of range");

  if (psStats)
  {
    /* range */
    widgSetMinorBarSize(psWScreen, IDDES_SENSORRANGE, sensorRange(psStats, static_cast<UBYTE>(selectedPlayer)));
    /* power */
    widgSetMinorBarSize(psWScreen, IDDES_SENSORPOWER, sensorPower(psStats, static_cast<UBYTE>(selectedPlayer)));
    /* weight */
...
static void intSetECMStats(ECM_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(ECM_STATS)), "intSetECMStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_ECM_START) && (psStats->ref < REF_ECM_START + REF_RANGE), "intSetECMStats: stats ref is out of range");

  /* power */
  widgSetBarSize(psWScreen, IDDES_ECMPOWER, ecmPower(psStats, static_cast<UBYTE>(selectedPlayer)));
  /* weight */
...
static void intSetECMShadowStats(ECM_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(ECM_STATS)), "intSetECMShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_ECM_START) && (psStats->ref < REF_ECM_START + REF_RANGE)), "intSetECMShadowStats: stats ref is out of range");

  if (psStats)
  {
    /* power */
    widgSetMinorBarSize(psWScreen, IDDES_ECMPOWER, ecmPower(psStats, static_cast<UBYTE>(selectedPlayer)));
    /* weight */
...
static void intSetConstructStats(CONSTRUCT_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(CONSTRUCT_STATS)), "intSetConstructStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_CONSTRUCT_START) &&
    (psStats->ref < REF_CONSTRUCT_START + REF_RANGE), "intSetConstructStats: stats ref is out of range");

  /* power */
  widgSetBarSize(psWScreen, IDDES_CONSTPOINTS, constructorPoints(psStats, static_cast<UBYTE>(selectedPlayer)));
  /* weight */
...
static void intSetConstructShadowStats(CONSTRUCT_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(CONSTRUCT_STATS)), "intSetConstructShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_CONSTRUCT_START) && (psStats->ref < REF_CONSTRUCT_START + REF_RANGE)), "intSetConstructShadowStats: stats ref is out of range");

  if (psStats)
  {
    /* power */
    widgSetMinorBarSize(psWScreen, IDDES_CONSTPOINTS, constructorPoints(psStats, static_cast<UBYTE>(selectedPlayer)));
    /* weight */
...
static void intSetRepairStats(REPAIR_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(REPAIR_STATS)), "intSetRepairStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_REPAIR_START) &&
    (psStats->ref < REF_REPAIR_START + REF_RANGE), "intSetRepairStats: stats ref is out of range");

  /* power */
  widgSetBarSize(psWScreen, IDDES_REPAIRPOINTS, repairPoints(psStats, static_cast<UBYTE>(selectedPlayer)));
  /* weight */
...
static void intSetRepairShadowStats(REPAIR_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(REPAIR_STATS)), "intSetRepairShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_REPAIR_START) && (psStats->ref < REF_REPAIR_START + REF_RANGE)), "intSetRepairShadowStats: stats ref is out of range");

  if (psStats)
  {
    /* power */
    widgSetMinorBarSize(psWScreen, IDDES_REPAIRPOINTS, repairPoints(psStats, static_cast<UBYTE>(selectedPlayer)));
    /* weight */
...
static void intSetWeaponStats(WEAPON_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(WEAPON_STATS)), "intSetWeaponStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_WEAPON_START) &&
    (psStats->ref < REF_WEAPON_START + REF_RANGE), "intSetWeaponStats: stats ref is out of range");
  ...
  /* damage */
  widgSetBarSize(psWScreen, IDDES_WEAPDAMAGE, static_cast<UWORD>(weaponDamage(psStats, (UBYTE)selectedPlayer)));
  /* weight */
...
static void intSetWeaponShadowStats(WEAPON_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(WEAPON_STATS)), "intSetWeaponShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_WEAPON_START) && (psStats->ref < REF_WEAPON_START + REF_RANGE)), "intSetWeaponShadowStats: stats ref is out of range");
  ...
  if (psStats)
  {
    ...
    /* damage */
    widgSetMinorBarSize(psWScreen, IDDES_WEAPDAMAGE, static_cast<UWORD>(weaponDamage(psStats, (UBYTE)selectedPlayer)));
    /* weight */
...
static void intSetBodyStats(BODY_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(BODY_STATS)), "intSetBodyStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_BODY_START) && (psStats->ref < REF_BODY_START + REF_RANGE), "intSetBodyStats: stats ref is out of range");
  ...
  /* armour */
  //do kinetic armour
  widgSetBarSize(psWScreen, IDDES_BODYARMOUR_K, bodyArmour(psStats, static_cast<UBYTE>(selectedPlayer), DROID_BODY_UPGRADE, WC_KINETIC));
  //do heat armour
  widgSetBarSize(psWScreen, IDDES_BODYARMOUR_H, bodyArmour(psStats, static_cast<UBYTE>(selectedPlayer), DROID_BODY_UPGRADE, WC_HEAT));
  /* body points */
  /* power */
  widgSetBarSize(psWScreen, IDDES_BODYPOWER, bodyPower(psStats, static_cast<UBYTE>(selectedPlayer),DROID_BODY_UPGRADE));
...
static void intSetBodyShadowStats(BODY_STATS* psStats)
{
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(BODY_STATS)), "intSetBodyShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_BODY_START) && (psStats->ref < REF_BODY_START + REF_RANGE)), "intSetBodyShadowStats: stats ref is out of range");

  if (psStats)
  {
    /* armour - kinetic*/
    widgSetMinorBarSize(psWScreen, IDDES_BODYARMOUR_K,
                        bodyArmour(psStats, static_cast<UBYTE>(selectedPlayer), DROID_BODY_UPGRADE, WC_KINETIC));
    //armour - heat 
    widgSetMinorBarSize(psWScreen, IDDES_BODYARMOUR_H,
                        bodyArmour(psStats, static_cast<UBYTE>(selectedPlayer), DROID_BODY_UPGRADE, WC_HEAT));
    /* body points */
    /* power */
    widgSetMinorBarSize(psWScreen, IDDES_BODYPOWER, bodyPower(psStats, static_cast<UBYTE>(selectedPlayer), DROID_BODY_UPGRADE));
...
static void intSetBodyPoints(DROID_TEMPLATE* psTemplate)
{
  ...
  widgSetBarSize(psWScreen, IDDES_BODYPOINTS, calcTemplateBody(psTemplate, static_cast<UBYTE>(selectedPlayer)));
}
...
static void intSetPropulsionStats(PROPULSION_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(PTRVALID(psStats, sizeof(PROPULSION_STATS)), "intSetPropulsionStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT((psStats->ref >= REF_PROPULSION_START) &&
    (psStats->ref < REF_PROPULSION_START + REF_RANGE), "intSetPropulsionStats: stats ref is out of range");
  ...
  if (sCurrDesign.asParts[COMP_BODY] != 0) { weight = psStats->weight * (asBodyStats + sCurrDesign.asParts[COMP_BODY])->weight / 100; }
  else
  {
...
static void intSetPropulsionShadowStats(PROPULSION_STATS* psStats)
{
  ...
  DEBUG_ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(PROPULSION_STATS)), "intSetPropulsionShadowStats: Invalid stats pointer");
  DEBUG_ASSERT_TEXT(psStats == NULL ||
    ((psStats->ref >= REF_PROPULSION_START) && (psStats->ref < REF_PROPULSION_START + REF_RANGE)), "intSetPropulsionShadowStats: stats ref is out of range");

  /* Only set the shadow stats if they are the right type */
  if (psStats && ((asPropulsionTypes[psStats->propulsionType].travel == GROUND && desPropMode != IDES_GROUND) || (asPropulsionTypes[psStats
    ->propulsionType].travel == AIR && desPropMode != IDES_AIR)))
    return;
  ...
  case IDES_GROUND:
    ...
    {
      /* Road speed */
      widgSetMinorBarSize(psWScreen, IDDES_PROPROAD, intCalcSpeed(TER_ROAD, psStats));
      /* Cross country speed - grass */
      widgSetMinorBarSize(psWScreen, IDDES_PROPCOUNTRY, intCalcSpeed(TER_SANDYBRUSH, psStats));
      /* Water speed */
      widgSetMinorBarSize(psWScreen, IDDES_PROPWATER, intCalcSpeed(TER_WATER, psStats));
    }
    else
    {
      /* Reset the shadow bars */
  ...
  case IDES_AIR:
    ...
    {
      /* Air speed - terrain type doesn't matter, use ROAD */
      widgSetMinorBarSize(psWScreen, IDDES_PROPAIR, intCalcSpeed(TER_ROAD, psStats));
    }
    else
    {
      /* Reset the shadow bar */
...
static BOOL intValidTemplate(DROID_TEMPLATE* psTempl)
{
  ...
  if (psTempl->asParts[COMP_BODY] == 0)
    return FALSE;
  if (psTempl->asParts[COMP_PROPULSION] == 0)
    return FALSE;

  // Check a turret has been installed
  if (psTempl->numWeaps == 0 && psTempl->asParts[COMP_SENSOR] == 0 && psTempl->asParts[COMP_ECM] == 0 && psTempl->asParts[COMP_BRAIN] == 0
    && psTempl->asParts[COMP_REPAIRUNIT] == 0 && psTempl->asParts[COMP_CONSTRUCT] == 0)
    return FALSE;
...
BOOL MultiPlayValidTemplate(DROID_TEMPLATE* psTempl) { return (intValidTemplate(psTempl)); }
...
void intSetButtonFlash(UDWORD id, BOOL bFlash)
{
  ...
  DEBUG_ASSERT_TEXT(psWidget->type == WIDG_BUTTON, "intSetButtonFlash : Not a button");
...
{
  if ((psTemplate->droidType == DROID_DEFAULT) || (strcmp(getTemplateName(psTemplate), GetDefaultTemplateName(psTemplate)) == 0))
    return FALSE;
  return TRUE;
}
#endif
...
{
  DROID_TEMPLATE *psTempl = nullptr, *psCurr, *psPrev;
  UDWORD currID;
  UDWORD i;
#ifndef HASH_NAMES
  ...
  if (id >= IDDES_TEMPLSTART && id <= IDDES_TEMPLEND)
  {
    ...
    if (id == IDDES_TEMPLSTART)
    {
      ...
      strncpy(aCurrName, strresGetString(psStringRes, STR_DES_NEWVEH), WIDG_MAXSTR - 1);
#ifdef HASH_NAMES
    ...
    else
    {
      ...
      DEBUG_ASSERT_TEXT(psTempl != NULL, "intProcessDesign: template not found!\n");
  ...
  else if (id >= IDDES_COMPSTART && id <= IDDES_COMPEND)
  {
    ...
    case IDES_PROPULSION:
      ...
      if (!intCheckValidWeaponForProp())
      {
        ...
        if (sCurrDesign.numWeaps > 1) { DEBUG_ASSERT_TEXT(FALSE, "designScreen: More than one weapon on droid - how?"); }
        ...
        intSetSystemForm((COMP_BASE_STATS*)(asWeaponStats + sCurrDesign.asWeaps[0]));
...
#ifndef HASH_NAMES
    if (bTemplateNameCustomised == FALSE) { strcpy(sCurrDesign.aName, GetDefaultTemplateName(&sCurrDesign)); }
#endif
...
#ifdef HASH_NAMES
#else
      strncpy(sCurrDesign.aName, widgGetString(psWScreen, IDDES_NAMEBOX), DROID_MAXNAME);
...
void intDisplayStatForm(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
   ...
   */
  psStats = static_cast<BASE_STATS*>(Form->pUserData);

  x0 = static_cast<UWORD>(xOffset + Form->x);
  y0 = static_cast<UWORD>(yOffset + Form->y);
  ...
  templateRadius = static_cast<SWORD>(getComponentRadius(psStats));
...
void intDisplayViewForm(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  if (CurrentStatsTemplate)
  {
    ...
    templateRadius = static_cast<SWORD>(getComponentDroidTemplateRadius((DROID_TEMPLATE*)CurrentStatsTemplate));
...
BOOL saveTemplate(void)
{
  DROID_TEMPLATE *psTempl = nullptr, *psPlayerTempl, *psPrevTempl;
...
void runTemplateShadowStats(UDWORD id)
{
  ...
  if (psTempl AND psTempl != &sCurrDesign)
  {
    ...
    if (templType == droidTemplateType(&sCurrDesign))
    {
      ...
      {
      case DROID_WEAPON:
        psStats = (COMP_BASE_STATS*)(asWeaponStats + psTempl->asWeaps[0]);
        break;
      case DROID_SENSOR:
        psStats = (COMP_BASE_STATS*)(asSensorStats + psTempl->asParts[COMP_SENSOR]);
        break;
      case DROID_ECM:
        psStats = (COMP_BASE_STATS*)(asECMStats + psTempl->asParts[COMP_ECM]);
        break;
      case DROID_CONSTRUCT:
        psStats = (COMP_BASE_STATS*)(asConstructStats + psTempl->asParts[COMP_CONSTRUCT]);
        break;
      case DROID_REPAIR:
        psStats = (COMP_BASE_STATS*)(asRepairStats + psTempl->asParts[COMP_REPAIRUNIT]);
        break;
        //default:
...
static BOOL intCheckValidWeaponForProp(void) { return checkValidWeaponForProp(&sCurrDesign); }

BOOL intAddDesign(BOOL bShowCentreScreen) { return _intAddDesign(bShowCentreScreen); }

/* Set up the system clickable form of the design screen given a set of stats */
static BOOL intSetSystemForm(COMP_BASE_STATS* psStats) { return _intSetSystemForm(psStats); }

static BOOL intAddTemplateForm(DROID_TEMPLATE* psSelected) { return _intAddTemplateForm(psSelected); }
...
static UWORD weaponROF(WEAPON_STATS* psStat)
{
  ...
  if (psStat->numRounds)
  {
    ...
    {
      rof = static_cast<UWORD>(60 * GAME_TICKS_PER_SEC / psStat->reloadTime);
      //multiply by the number of salvos/shot
      rof = static_cast<UWORD>(rof * psStat->numRounds);
    }
  }
  if (!rof)
  {
    rof = static_cast<UWORD>(weaponFirePause(psStat, (UBYTE)selectedPlayer));
    if (rof != 0)
      rof = static_cast<UWORD>(60 * GAME_TICKS_PER_SEC / rof);
    //else leave it at 0
  }
  return rof;
}
