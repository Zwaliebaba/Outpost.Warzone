...
static COMP_BASE_STATS* intChooseSystemStats(DROID_TEMPLATE* psTemplate)
{
  ...
  case DROID_DEFAULT:
    ...
    break;
  default: Neuron::Fatal("intSetDesignStats: unrecognised droid type");
    return nullptr;
  }
...
STRING* GetDefaultTemplateName(DROID_TEMPLATE* psTemplate)
{
  ...
  */
  ...
  if (psTemplate->asParts[COMP_BODY] != 0)
  {
    ...
    {
      Neuron::DebugTrace("GetDefaultTemplateName: name string too long {}+{}\n",aCurrName,pStr);
      return nullptr;
    }
  ...
  for (i = 0; i < numEntries; i++)
  {
    ...
    if (WhichTab == TAB_USEMAJOR)
    {
      ...
      if (sButInit.y + DES_TABBUTHEIGHT + DES_TABBUTGAP > DES_RIGHTFORMHEIGHT - DES_MAJORSIZE)
      {
        ...
        {
          Neuron::DebugTrace("Too many buttons for component form");
          return FALSE;
        }
      }
    }
...
BOOL saveTemplate(void)
{
  ...
  if (bTemplateFound == TRUE && intValidTemplate(&sCurrDesign))
  {
    ...
    if (psTempl == nullptr)
    {
      ...
      if (psTempl == nullptr)
      {
        Neuron::DebugTrace("saveTemplate: heap alloc failed\n");
        return FALSE;
      }
