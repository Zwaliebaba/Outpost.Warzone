...
LPDIRECTDRAWSURFACE4 psTiles, psVidTiles;LPDIRECTDRAWSURFACE4 psDroids, psVidDroids;LPDIRECTDRAWSURFACE4 psExp, psVidExp;
LPDIRECTDRAWSURFACE4 psFlash, psVidFlash;LPDIRECTDRAWSURFACE4 psFlame, psVidFlame;LPDIRECTDRAWSURFACE4 psBullet, psVidBullet;
...
UDWORD maxTexTile = 0;static BOOL quitting = FALSE;static BOOL rangeCircles = FALSE;static BOOL visibleInfo = FALSE;static BOOL showStats =
  FALSE;static UDWORD godModeKeys[] = {KEY_G, KEY_O, KEY_D};static UDWORD godModeIndex = 0;void showDroidRange(DROID * psDroid);void
showGameStats(void);
...
static enum _struct_posmode
{
  ...
  SPM_POS,
  // Positioning a structure
  SPM_FINISHED,
  // Finished positioning a structure
  SPM_VALID,
...
{
  MSM_NORMAL,
  // Normal single click selection mode
  MSM_DRAGSTART,
  // Might be starting a drag operation
  MSM_DRAG,
  // Drag selection mode
} mouseSelMode;static BOOL noDrag; // Set to true when the mouse is pressed
...
} objSelMode;static BASE_OBJECT* psSelected; // The object selected if in single selection mode
...
BOOL disp2DInitialise(void)
{
  ...
  if (!surfCreateFromBMP("tiles32.bmp", &psTiles, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidTiles, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }
  ...
  if (!surfCreateFromBMP("Droids4.bmp", &psDroids, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidDroids, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }

  if (!surfLoadFromSurface(psVidDroids, psDroids)) { return FALSE; }

  /* Load in the Explosion Tiles */
  if (!surfCreateFromBMP("explosion.bmp", &psExp, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidExp, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }
  ...
  if (!surfCreateFromBMP("flash.bmp", &psFlash, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidFlash, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }

  if (!surfLoadFromSurface(psVidFlash, psFlash)) { return FALSE; }

  /* Load in the Flame Tiles */
  if (!surfCreateFromBMP("flame.bmp", &psFlame, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidFlame, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }

  if (!surfLoadFromSurface(psVidFlame, psFlame)) { return FALSE; }

  /* Load in the Flame Tiles */
  if (!surfCreateFromBMP("missile1.bmp", &psBullet, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidBullet, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }

  if (!surfLoadFromSurface(psVidBullet, psBullet)) { return FALSE; }

  /* Load in the Structure Tiles */
  if (!surfCreateFromBMP("building2D.bmp", &psStructures, &width, &height)) { return FALSE; }

  if (!surfCreate(&psVidStructures, width, height, DDSCAPS_OFFSCREENPLAIN, NULL, FALSE)) { return FALSE; }
...
void disp2DStartStructPosition(BASE_STATS* psStats)
{
  ...
  else if (psStats->ref >= REF_FEATURE_START && psStats->ref < (REF_FEATURE_START + REF_RANGE))
  {
  ...
  else if (psStats->ref >= REF_TEMPLATE_START && psStats->ref < (REF_TEMPLATE_START + REF_RANGE))
  {
...
void disp2DStopStructPosition(void) { structPosMode = SPM_NONE; }

/* See if a structure location has been found */
BOOL disp2DGetStructPosition(UDWORD* pX, UDWORD* pY)
{
  if (structPosMode != SPM_FINISHED) { return FALSE; }
...
}static BASE_OBJECT* getMouseObject(void)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    {
      if ((worldX > psCurr->x - TILE_UNITS / 2) && (worldX < psCurr->x + TILE_UNITS / 2) && (worldY > psCurr->y - TILE_UNITS / 2) && (worldY
        < psCurr->y + TILE_UNITS / 2))
      {
...
found_mdroid:
  ...
  if (psMObj == NULL)
  {
    ...
    for (i = 0; i < MAX_PLAYERS; i++)
    {
      for (psCurrStruct = apsStructLists[i]; psCurrStruct; psCurrStruct = psCurrStruct->psNext)
      {
        width = psCurrStruct->pStructureType->baseWidth * TILE_UNITS / 2;
        breadth = psCurrStruct->pStructureType->baseBreadth * TILE_UNITS / 2;
        if ((worldX > psCurrStruct->x - width) && (worldX < psCurrStruct->x + width) && (worldY > psCurrStruct->y - breadth) && (worldY <
          psCurrStruct->y + breadth))
        {
...
found_mstruct:
  ...
  if (psMObj == NULL)
  {
    ...
    for (psCurrFeat = apsFeatureLists[0]; psCurrFeat; psCurrFeat = psCurrFeat->psNext)
    {
      width = psCurrFeat->psStats->baseWidth * TILE_UNITS / 2;
      breadth = psCurrFeat->psStats->baseBreadth * TILE_UNITS / 2;
      if ((worldX > psCurrFeat->x - width) && (worldX < psCurrFeat->x + width) && (worldY > psCurrFeat->y - breadth) && (worldY < psCurrFeat
        ->y + breadth))
      {
...
BOOL process2DInput(void)
{
  ...
  )
  {
    ...
    if (keyDown(KEY_LEFTARROW) && (mapX > 0)) { mapX -= 1; }
    if (keyDown(KEY_RIGHTARROW) && (mapX < mapWidth - 1)) { mapX += 1; }
    if (keyDown(KEY_UPARROW) && (mapY > 0)) { mapY -= 1; }
    if (keyDown(KEY_DOWNARROW) && (mapY < mapHeight - 1)) { mapY += 1; }
  }

  /* If the mouse is at the edge, scroll the map */
  if (mapX > 0 && mouseX() >= 0 && mouseX() <= TILE_SIZE2D / 2) { mapX--; }
  else if (mapX < mapWidth - 1 && mouseX() >= DISP_WIDTH - TILE_SIZE2D / 2 && mouseX() <= DISP_WIDTH) { mapX++; }
  if (mapY > 0 && mouseY() <= TILE_SIZE2D / 2) { mapY--; }
  else if (mapY < mapWidth - 1 && mouseY() >= DISP_HEIGHT - TILE_SIZE2D / 2 && mouseY() <= DISP_HEIGHT) { mapY++; }
  ...
  */
  ...
  if (objSelMode == OSEL_SINGLE && psSelected->died) { objSelMode = OSEL_NONE; }

  switch (mouseSelMode)
  {
  case MSM_NORMAL:
    /* See if we are doing a structure position */
    if (structPosMode == SPM_POS
      OR structPosMode
    ==
    SPM_VALID
    )
    {
      ...
      if ((sStructPos.width % 2) == 0) { sStructPos.x = ((worldX + TILE_UNITS / 2) >> TILE_SHIFT) - sStructPos.width / 2; }
      else { sStructPos.x = (worldX >> TILE_SHIFT) - sStructPos.width / 2; }
      if ((sStructPos.height % 2) == 0) { sStructPos.y = ((worldY + TILE_UNITS / 2) >> TILE_SHIFT) - sStructPos.height / 2; }
      else { sStructPos.y = (worldY >> TILE_SHIFT) - sStructPos.height / 2; }
      //check for valid location
      if (validLocation(sStructPos.psStats, worldX >> TILE_SHIFT, worldY >> TILE_SHIFT)) { structPosMode = SPM_VALID; }
      else { structPosMode = SPM_POS; }
    ...
    if (mousePressed(MOUSE_LMB))
    {
      ...
      if (mouseReleased(MOUSE_LMB)) { noDrag = TRUE; }
    }
    else if (mousePressed(MOUSE_RMB))
    {
      psMObj = getMouseObject();
      if (psMObj) { intObjectSelected(psMObj); }
    }
    break;
  case MSM_DRAGSTART:
    ...
    if (mouseReleased(MOUSE_LMB) || noDrag)
    {
      ...
      if (psMObj)
      {
        ...
        {
          if (psMObj->player == selectedPlayer || psMObj->type == OBJ_FEATURE)
          {
        ...
        else
        {
          ...
          if (psMObj->type != OBJ_STRUCTURE && psMObj->player == selectedPlayer)
          {
          ...
          else { orderSelectedObj(selectedPlayer, psMObj); }
        }
      }
    ...
    else if (((dragCX - dragSX) * (dragCX - dragSX) + (dragCY - dragSY) * (dragCY - dragSY)) > DRAG_THRESHOLD)
    {
      /* A drag has started */
  ...
  case MSM_DRAG:
    ...
    if (mouseReleased(MOUSE_LMB))
    {
      ...
      {
        if (psCurr->x >= selX0 && psCurr->x <= selX1 && psCurr->y >= selY0 && psCurr->y <= selY1)
        {
      ...
      if (numSel > 1) { objSelMode = OSEL_MULTI; }
      else if (numSel == 1) { objSelMode = OSEL_SINGLE; }
      else { objSelMode = OSEL_NONE; }
    }
    break;
  }
  ...
  */
  ...
  if (keyPressed(KEY_R)) { rangeCircles = !rangeCircles; }

  if (keyPressed(KEY_V)) { visibleInfo = !visibleInfo; }

  if (keyPressed(KEY_S)) { showStats = !showStats; }
...
void blitTile(RECT* psDestRect, RECT* psSrcRect, UDWORD texture)
{
  ...
  case 8:
    ...
    if (psP1->y == psP2->y)
    {
      ...
      p8Dest = (UBYTE*)sDDSDDest.lpSurface + sDDSDDest.lPitch * psDestRect->top + psDestRect->left;
      destInc = sDDSDDest.lPitch - (psDestRect->right - psDestRect->left);
      p8Src = (UBYTE*)sDDSDSrc.lpSurface + sDDSDSrc.lPitch * psP1->y + psP1->x;
      if (psP1->y < psP4->y) { srcInc = sDDSDSrc.lPitch - (psP2->x - psP1->x); }
      else { srcInc = -sDDSDSrc.lPitch - (psP2->x - psP1->x); }
      /* Have to adjust start point if xDir or yDir are negative */
      if (xDir < 0) { p8Src--; }
      if (yDir < 0) { p8Src = p8Src - sDDSDSrc.lPitch; }
    ...
    else
    {
      ...
      {
        p8Src = (UBYTE*)sDDSDSrc.lpSurface + sDDSDSrc.lPitch * psP1->y + x;
        if (xDir < 0) { p8Src--; }
        if (yDir < 0) { p8Src = p8Src - sDDSDSrc.lPitch; }
  ...
  case 16:
    ...
    if (psP1->y == psP2->y)
    {
      ...
      xDir = (psP1->x > psP2->x) ? -1 : 1;
      yDir = (psP1->y > psP4->y) ? -1 : 1;
      p16Dest = (UWORD*)((UBYTE*)sDDSDDest.lpSurface + sDDSDDest.lPitch * psDestRect->top + (psDestRect->left << 1));
      destInc = sDDSDDest.lPitch - ((psDestRect->right - psDestRect->left) << 1);
      p16Src = (UWORD*)((UBYTE*)sDDSDSrc.lpSurface + sDDSDSrc.lPitch * psP1->y + (psP1->x << 1));
      if (psP1->y < psP4->y) { srcInc = sDDSDSrc.lPitch - ((psP2->x - psP1->x) << 1); }
      else { srcInc = -sDDSDSrc.lPitch - ((psP2->x - psP1->x) << 1); }
      /* Have to adjust start point if xDir or yDir are negative */
      if (xDir < 0) { p16Src--; }
      if (yDir < 0) { p16Src = (UWORD*)((UBYTE*)p16Src - sDDSDSrc.lPitch); }
      for (y = psP1->y; y != psP4->y; y += yDir)
      {
        for (x = psP1->x; x != psP2->x; x += xDir)
        {
    ...
    else
    {
      ...
      for (x = psP1->x; x != psP4->x; x += xDir)
      {
        p16Src = (UWORD*)((UBYTE*)sDDSDSrc.lpSurface + sDDSDSrc.lPitch * psP1->y + (x << 1));
        if (xDir < 0) { p16Src--; }
        if (yDir < 0) { p16Src = (UWORD*)((UBYTE*)p16Src - sDDSDSrc.lPitch); }
        for (y = psP1->y; y != psP2->y; y += yDir)
        {
...
static void display2DMap(void)
{
  ...
  if (x < (UDWORD)(TILES_ACROSS / 2)) { x = 0; }
  else
  {
    x -= (TILES_ACROSS / 2);
    if (x > (SDWORD)mapWidth - TILES_ACROSS) { x = mapWidth - TILES_ACROSS; }
  }
  if (y < (UDWORD)(TILES_DOWN / 2)) { y = 0; }
  else
  {
    y -= TILES_DOWN / 2;
    if (y > (SDWORD)mapHeight - TILES_DOWN) { y = mapHeight - TILES_DOWN; }
  }
  ...
  for (scrX = 0; scrX < DISP_WIDTH; scrX += TILE_SIZE2D)
  {
    for (scrY = 0; scrY < DISP_HEIGHT; scrY += TILE_SIZE2D)
    {
      ...
      {
        sSrcRect.left = ((psTile->texture & TILE_NUMMASK) % tilesPerLine) << SCR_TILE_SHIFT;
        sSrcRect.right = sSrcRect.left + TILE_SIZE2D;
        sSrcRect.top = ((psTile->texture & TILE_NUMMASK) / tilesPerLine) << SCR_TILE_SHIFT;
        sSrcRect.bottom = sSrcRect.top + TILE_SIZE2D;
        blitTile(&sDestRect, &sSrcRect, psTile->texture);

        /* Show the terrain type if necessary */
        if (showTerrain) { dispTerrain(scrX, scrY, TERRAIN_TYPE(psTile)); }
      }
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    for (psDroid = apsDroidLists[i]; psDroid != NULL; psDroid = psDroid->psNext)
    {
      ...
      &&
      (((psDroid->x - TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewX)) && (((psDroid->x + TILE_SIZE2D / 2) >> TILE_SHIFT) < (viewX +
        TILES_ACROSS)) && (((psDroid->y - TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewY)) && (((psDroid->y + TILE_SIZE2D / 2) >> TILE_SHIFT) < (
        viewY + TILES_DOWN))
      )

      {
          ...
          for (j = psDroid->sMove.Position; psDroid->sMove.MovementList[j].XCoordinate != -1; j++)
          {
            /*						scrX = ((((psDroid->sMove.MovementList[j].XCoordinate << TILE_SHIFT) + TILE_UNITS/2
                        ...
                        scrY = ((((psDroid->sMove.MovementList[j].YCoordinate << TILE_SHIFT) + TILE_UNITS/2
                            ...
                            >> TILE_SHIFT) - TILE_SIZE2D/2;*/
            scrX = (((psDroid->sMove.MovementList[j].XCoordinate - (viewX << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;
            scrY = (((psDroid->sMove.MovementList[j].YCoordinate - (viewY << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;
            screenDrawLine(scrX - 10, scrY, scrX + 10, scrY);
            screenDrawLine(scrX, scrY - 10, scrX, scrY + 10);
          }
        }

        scrX = (((psDroid->x - (viewX << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;
        scrY = (((psDroid->y - (viewY << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;

        if (visibleInfo)
        {
          screenTextOut(scrX - 30, scrY - 40, "0 1 2 3 4 5 6 7");
          screenTextOut(scrX - 30, scrY - 20, "%d %d %d %d %d %d %d %d", psDroid->visible[0], psDroid->visible[1], psDroid->visible[2],
                        psDroid->visible[3], psDroid->visible[4], psDroid->visible[5], psDroid->visible[6], psDroid->visible[7]);
        }

        bmpDir = psDroid->direction + 180;
        bmpDir = bmpDir >= 360 ? bmpDir - 360 : bmpDir;
        bmpDir = (360 - bmpDir) / 45;
        screenBlitTile(scrX, scrY, psVidDroids, 32, 32, bmpDir);
        ...
        {
        case MOVEINACTIVE:
          if (psDroid->selected) { screenTextOut(scrX, scrY, psDroid->pName); }
          else { screenTextOut(scrX, scrY, "%d", psDroid->player); }
          break;
        case MOVENAVIGATE:
        ...
        if (rangeCircles) { showDroidRange(psDroid); }
      }
    }
    ...
    for (psStructure = apsStructLists[i]; psStructure != NULL; psStructure = psStructure->psNext)
    {
      ...
      &&
      (((psStructure->x - width * TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewX)) && (((psStructure->x + width * TILE_SIZE2D / 2) >> TILE_SHIFT)
        < (viewX + TILES_ACROSS)) && (((psStructure->y - breadth * TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewY)) && (((psStructure->y +
        breadth * TILE_SIZE2D / 2) >> TILE_SHIFT) < (viewY + TILES_DOWN))
      )

      {
        scrX = (((psStructure->x - (viewX << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;
        scrY = (((psStructure->y - (viewY << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;

        if (visibleInfo)
        {
          screenTextOut(scrX - 30, scrY - 40, "0 1 2 3 4 5 6 7");
          screenTextOut(scrX - 30, scrY - 20, "%d %d %d %d %d %d %d %d", psStructure->visible[0], psStructure->visible[1],
                        psStructure->visible[2], psStructure->visible[3], psStructure->visible[4], psStructure->visible[5],
                        psStructure->visible[6], psStructure->visible[7]);
        }

        disp2DFromWorld(psStructure->x - (width * TILE_UNITS / 2), psStructure->y - (breadth * TILE_UNITS / 2), &scrX, &scrY);
        for (j = 0; j < breadth; j++)
        {
          for (k = 0; k < width; k++)
          {
            screenBlitTile(scrX, scrY, psVidStructures, 32, 32, psStructure->status);
            scrX += TILE_SIZE2D;
          }
        ...
        if (psStructure->selected) { screenTextOut(scrX - 15, scrY - 15, psStructure->pStructureType->pName); }
        else { screenTextOut(scrX - 15, scrY - 15, "%d", psStructure->player); }
      }
    }
  }
  ...
  for (psFeature = apsFeatureLists[0]; psFeature != NULL; psFeature = psFeature->psNext)
  {
    ...
    if ((psFeature->visible[selectedPlayer]
      OR godMode
    )
    &&
    (((psFeature->x - width * TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewX)) && (((psFeature->x + width * TILE_SIZE2D / 2) >> TILE_SHIFT) < (
      viewX + TILES_ACROSS)) && (((psFeature->y - breadth * TILE_SIZE2D / 2) >> TILE_SHIFT) >= (viewY)) && (((psFeature->y + breadth *
      TILE_SIZE2D / 2) >> TILE_SHIFT) < (viewY + TILES_DOWN))
    )

    {
      scrX = (((psFeature->x - (viewX << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;
      scrY = (((psFeature->y - (viewY << TILE_SHIFT)) * TILE_SIZE2D) >> TILE_SHIFT) - TILE_SIZE2D / 2;

      if (visibleInfo)
      {
        screenTextOut(scrX - 30, scrY - 40, "0 1 2 3 4 5 6 7");
        screenTextOut(scrX - 30, scrY - 20, "%d %d %d %d %d %d %d %d", psFeature->visible[0], psFeature->visible[1], psFeature->visible[2],
                      psFeature->visible[3], psFeature->visible[4], psFeature->visible[5], psFeature->visible[6], psFeature->visible[7]);
      }

      disp2DFromWorld(psFeature->x - (width * TILE_UNITS / 2), psFeature->y - (breadth * TILE_UNITS / 2), &scrX, &scrY);
      for (j = 0; j < breadth; j++)
      {
        for (k = 0; k < width; k++)
        {
          screenBlitTile(scrX, scrY, psVidStructures, 32, 32, 3);
          scrX += TILE_SIZE2D;
        }
        scrX -= width * TILE_SIZE2D;
        scrY += TILE_SIZE2D;
      ...
      if (psFeature->selected) { screenTextOut(scrX - 15, scrY - 15, psFeature->psStats->pName); }
    }
  }
  ...
  for (psCurr = psActiveBullets; psCurr != NULL; psCurr = psCurr->psNext)
  {
    ...
    case PROJ_INFLIGHT:
      ...
      if (proj_Direct(psStats))
      {
        if (psCurr->psSource)
        {
          ...
          bmpDir = bmpDir >= 360 ? bmpDir - 360 : bmpDir;
          bmpDir = (360 - bmpDir) / 45;
          flashFrame = ((gameTime >> 7) & 1) + (bmpDir << 1);
          screenBlitTile(scrX1 - FLASH_SIZE / 2, scrY1 - FLASH_SIZE / 2, psVidFlash, FLASH_SIZE, FLASH_SIZE, flashFrame);
        }
      }
      else
      {
        ...
        bmpDir = bmpDir >= 360 ? bmpDir - 360 : bmpDir;
        bmpDir = (360 - bmpDir) / 45;
        screenBlitTile(scrX1 - BULLET_SIZE / 2, scrY1 - BULLET_SIZE / 2, psVidBullet, BULLET_SIZE, BULLET_SIZE, bmpDir);
      }
      break;
    case PROJ_IMPACT:
      ...
      disp2DFromWorld(psCurr->x, psCurr->y, &scrX1, &scrY1);
      if (expTile < EXP_TILES) { screenBlitTile(scrX1 - EXP_SIZE / 2, scrY1 - EXP_SIZE / 2, psVidExp, EXP_SIZE, EXP_SIZE, expTile); }
      break;
    case PROJ_POSTIMPACT:
      ...
      for (i = tileX - tileRad; i < tileX + tileRad; i++)
      {
        ...
        {
          if (tileOnMap(i, j) && (i >= (SDWORD)viewX) && (i < (SDWORD)viewX + TILES_ACROSS) && (j >= (SDWORD)viewY) && (j < (SDWORD)viewY +
            TILES_DOWN))
          {
            xDiff = i - tileX;
            yDiff = j - tileY;
            //						if ((mapTile(i,j)->tileVisible[selectedPlayer] || godMode) &&
            if (((mapTile(i, j)->tileVisBits & 1 << selectedPlayer)
              OR godMode
            )
            AND((xDiff * xDiff + yDiff * yDiff) < radSquared)
            )
            {
              /* We've got us a burning tile - burn baby burn !! */
              flameFrame = ((gameTime >> 5) + i + j) % FLAME_TILES;
              screenBlitTile((i - viewX) << SCR_TILE_SHIFT, (j - viewY) << SCR_TILE_SHIFT, psVidFlame, FLAME_SIZE, FLAME_SIZE, flameFrame);
            }
          }
        }
      }
...
}void showGameStats(void)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++) { if (apsDroidLists[i] != NULL) { Count++; } }
