...
BOOL IdToDroid(UDWORD id, UDWORD player, DROID** psDroid)
{
  ...
  if (player == ANYPLAYER)
  {
    ...
    {
      d = apsDroidLists[i];
      while ((d != nullptr) && (d->id != id)) { d = d->psNext; }
      if (d)
      {
        *psDroid = d;
        return TRUE;
...
STRUCTURE* IdToStruct(UDWORD id, UDWORD player)
{
  ...
  if (player == ANYPLAYER)
  {
    for (i = 0; i < MAX_PLAYERS; i++)
    {
      for (psStr = apsStructLists[i]; ((psStr != nullptr) && (psStr->id != id)); psStr = psStr->psNext);
      if (psStr)
        return psStr;
    }
  }
  else
    for (psStr = apsStructLists[player]; ((psStr != nullptr) && (psStr->id != id)); psStr = psStr->psNext);
  return psStr;
}
...
FEATURE* IdToFeature(UDWORD id, UDWORD player)
{
  ...
  if (player == ANYPLAYER)
  {
    for (i = 0; i < MAX_PLAYERS; i++)
    {
      for (psF = apsFeatureLists[i]; ((psF != nullptr) && (psF->id != id)); psF = psF->psNext);
      if (psF)
        return psF;
    }
  }
  else
    for (psF = apsFeatureLists[player]; ((psF != nullptr) && (psF->id != id)); psF = psF->psNext);
  return psF;
}
...
DROID_TEMPLATE* IdToTemplate(UDWORD tempId, UDWORD player)
{
  ...
  if (player != ANYPLAYER)
  {
    for (psTempl = apsDroidTemplates[player]; // follow templates
         (psTempl && (psTempl->multiPlayerID != tempId)); psTempl = psTempl->psNext);
  }
  else
  {
...
DROID_TEMPLATE* NameToTemplate(CHAR* sName, UDWORD player)
{
  ...
  for (psTempl = apsDroidTemplates[player]; // follow templates
       (psTempl && (strcmp(psTempl->aName, sName) != 0)); psTempl = psTempl->psNext);
...
BOOL recvResearchStatus(NETMSG* pMsg)
{
  ...
  else // finished/cancelled research
  {
    if (buildingId == 0) // find the centre doing this research.(set building
    {
      ...
      {
        if (psBuilding->pStructureType->type == REF_RESEARCH && psBuilding->status == SS_BUILT && ((RESEARCH_FACILITY*)psBuilding->
          pFunctionality)->psSubject && ((RESEARCH_FACILITY*)psBuilding->pFunctionality)->psSubject->ref - REF_RESEARCH_START == index)
          break;
      }
    }
    else
...
BOOL sendTextMessage(char* pStr, BOOL all)
{
  ...
  m.size = static_cast<UWORD>(strlen(&(pStr[i])) + 5); // package the message up and send it.
  m.type = NET_TEXTMSG;
...
BOOL SendDestroyTemplate(DROID_TEMPLATE* t)
{
  ...
  m.body[0] = static_cast<char>(selectedPlayer); // send player number	
...
BOOL recvDestroyTemplate(NETMSG* m)
{
  ...
  psTempPrev = nullptr; // first find it.
...
BOOL recvDestroyFeature(NETMSG* pMsg)
{
  ...
  if ((pF == nullptr)) // if already a gonner.
