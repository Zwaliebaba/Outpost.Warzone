...
using POSITION_TYPE = enum _position_type
{
  POS_DELIVERY,
...
using OBJECT_POSITION = struct _object_position
{
  POSITION_OBJ;
};

using FLAG_POSITION = struct _flag_position
{
  POSITION_OBJ;
...
using STRUCT_STRENGTH = enum _struct_strength
{
  STRENGTH_SOFT,
  STRENGTH_MEDIUM,
  STRENGTH_HARD,
...
using STRUCTSTRENGTH_MODIFIER = UWORD;

//this structure is used to hold the permenant stats for each type of building
using STRUCTURE_STATS = struct _structure_stats
{
...
using STRUCT_STATES = enum _struct_states
{
  SS_BEING_BUILT,
  SS_BUILT,
  SS_BEING_DEMOLISHED,
...
using RES_EXTRACTOR = struct _res_extractor
{
...
using POWER_GEN = struct _power_gen
{
...
using REPAIR_FACILITY = struct REPAIR_FACILITY
{
...
using REARM_PAD = struct _rearm_pad
{
...
using STRUCTURE_LIMITS = struct _structure_limits
{
...
using PRODUCTION_RUN = struct _production_run
{
...
using STRUCTURE_UPGRADE = struct _structure_upgrade
{
  UWORD armour;
  UWORD body;
  UWORD resistance;
...
using WALLDEFENCE_UPGRADE = struct _wallDefence_upgrade
{
  UWORD armour;
  UWORD body;
};

using UPGRADE = struct _upgrade
{
  UWORD modifier; //% to increase the stat by
};

using RESEARCH_UPGRADE = UPGRADE;
using PRODUCTION_UPGRADE = UPGRADE;
using REPAIR_FACILITY_UPGRADE = UPGRADE;
using POWER_UPGRADE = UPGRADE;
using REARM_UPGRADE = UPGRADE;
