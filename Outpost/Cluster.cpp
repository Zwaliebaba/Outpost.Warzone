...
void clustInitialise(void)
{
  ...
  DEBUG_ASSERT_TEXT(CLUSTER_MAX <= UBYTE_MAX, "clustInitialse: invalid CLUSTER_MAX, this is a BUILD error");
  ...
  for (cluster = 1; cluster < CLUSTER_MAX; cluster++)
  {
    ...
    for (player = 0; player < MAX_PLAYERS; player++)
    {
      ...
      for (psDroid = apsDroidLists[player]; psDroid; psDroid = psDroid->psNext)
      {
        ...
        {
          DEBUG_ASSERT_TEXT((found == MAX_PLAYERS) || (droidUsage != 0), "clustValidateUsage: cluster has mixed players");
      ...
      for (psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
      {
        ...
        {
          DEBUG_ASSERT_TEXT((found == MAX_PLAYERS) || (structUsage != 0), "clustValidateUsage: cluster has mixed players");
      ...
      if (found == player)
      {
        DEBUG_ASSERT_TEXT((droidUsage == 0) || (structUsage == 0), "clustValidateUsage: cluster contains both droids and structs");

        DEBUG_ASSERT_TEXT(aClusterUsage[cluster] == droidUsage + structUsage, "clustValidateUsage: invalid cluster usage");
      }
    }
  }
}
...
void clustRemoveObject(BASE_OBJECT* psObj)
{
  ...
  DEBUG_ASSERT_TEXT(psObj->cluster < CLUSTER_MAX, "clustRemoveObject: invalid cluster number");

  // update the usage counter
  if (psObj->cluster != 0)
  {
    DEBUG_ASSERT_TEXT(aClusterUsage[psObj->cluster] > 0, "clustRemoveObject: usage array out of sync");
...
void _clustAddDroid(DROID* psDroid, SDWORD cluster)
{
  ...
  psDroid->cluster = static_cast<UBYTE>(cluster);
  for (player = 0; player < MAX_PLAYERS; player++)
  {
  ...
  {
    if (psCurr->cluster == static_cast<UBYTE>(cluster))
      continue;
...
void clustAddDroid(DROID* psDroid, SDWORD cluster) { _clustAddDroid(psDroid, cluster); }
...
void _clustAddStruct(STRUCTURE* psStruct, SDWORD cluster)
{
  ...
  aClusterUsage[cluster] += 1;
  psStruct->cluster = static_cast<UBYTE>(cluster);
  for (player = 0; player < MAX_PLAYERS; player++)
  {
  ...
  for (psCurr = apsStructLists[psStruct->player]; psCurr; psCurr = psCurr->psNext)
  {
    ...
    xdiff = static_cast<SDWORD>(psStruct->x) - static_cast<SDWORD>(psCurr->x);
    ydiff = static_cast<SDWORD>(psStruct->y) - static_cast<SDWORD>(psCurr->y);
    if (xdiff * xdiff + ydiff * ydiff < CLUSTER_DIST * CLUSTER_DIST)
      _clustAddStruct(psCurr, cluster);
  }
}

void clustAddStruct(STRUCTURE* psStruct, SDWORD cluster) { _clustAddStruct(psStruct, cluster); }
...
void clustDisplay(void)
{
  ...
  for (map = 0; map < CLUSTER_MAX; map++)
  {
    ...
    if (cluster != 0)
    {
      ...
      if (found) { CONPRINTF(ConsoleString, (ConsoleString, "%s", aBuff)); }
    }
  }
}
...
void clustUpdateObject(BASE_OBJECT* psObj)
{
  ...
  if (oldCluster != 0)
  {
    for (i = 0; i < CLUSTER_MAX; i++)
    {
      DEBUG_ASSERT_TEXT((aClusterMap[i] == 0) || (aClusterUsage[ aClusterMap[i] ] != 0), "clustUpdateObject: cluster map out of sync");

      if (aClusterMap[i] == oldCluster)
      {
        // found the old cluster - change it to the new one
        aClusterMap[i] = static_cast<UBYTE>(newCluster);
  ...
  aClusterInfo[newCluster] = static_cast<UBYTE>(psObj->player & CLUSTER_PLAYER_MASK);
  switch (psObj->type)
  {
  case OBJ_DROID:
  ...
  case OBJ_STRUCTURE:
    ...
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "clustUpdateObject: invalid object type");
    break;
  }
...
{
  DEBUG_ASSERT_TEXT((clusterID >= 0) && (clusterID < CLUSTER_MAX), "clustGetClusterFromID: invalid cluster ID");
...
BASE_OBJECT* clustIterate(void)
{
  ...
  if (psIterateObj == nullptr)
    psStart = psIterateList;
  else
    psStart = psIterateObj->psNext;

  for (psIterateObj = psStart; psIterateObj && psIterateObj->cluster != cluster; psIterateObj = psIterateObj->psNext);

  if (psIterateObj == nullptr)
    psIterateList = nullptr;
...
void clustGetCenter(BASE_OBJECT* psObj, SDWORD* px, SDWORD* py)
{
  ...
  case OBJ_STRUCTURE:
    ...
    break;
  default: DEBUG_ASSERT_TEXT(FALSE, "clustGetCenter: invalid object type");
    psList = nullptr;
    break;
  }
  ...
  for (psCurr = psList; psCurr; psCurr = psCurr->psNext)
  {
    ...
    {
      averagex += static_cast<SDWORD>(psCurr->x);
      averagey += static_cast<SDWORD>(psCurr->y);
      num += 1;
    }
  }
  ...
  {
    *px = static_cast<SDWORD>(psObj->x);
    *py = static_cast<SDWORD>(psObj->y);
  }
}
...
void clustObjectSeen(BASE_OBJECT* psObj, BASE_OBJECT* psViewer)
{
  ...
  for (player = 0; player < MAX_PLAYERS; player++)
  {
    if ((player != static_cast<SDWORD>(psObj->player)) && psObj->visible[player] && !(aClusterVisibility[psObj->cluster] & (1 << player)))
    {
      ...
      case OBJ_FEATURE:
        ...
        break;
      default: DEBUG_ASSERT_TEXT(FALSE, "clustObjectSeen: invalid object type");
        break;
      }
...
void clustObjectAttacked(BASE_OBJECT* psObj)
{
  if ((aClusterAttacked[psObj->cluster] + ATTACK_CB_PAUSE) < gameTime)
  {
    ...
    case OBJ_STRUCTURE:
      ...
      break;
    default: DEBUG_ASSERT_TEXT(FALSE, "clustObjectAttacked: invalid object type");
      break;
    }
