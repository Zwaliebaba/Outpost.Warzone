...
static HKEY ghWarzoneKey = nullptr;
...
BOOL loadConfig(BOOL bResourceAvailable)
{
  ...
  if (getWarzoneKeyNumeric("allowsubtitles", &val))
  {
    bAllowSubtitles = val;
    if (!bAllowSubtitles)
      setWarzoneKeyNumeric("subtitles",TRUE); // subtitles
  }
  ...
  if (getWarzoneKeyNumeric("sequences", &val))
    war_SetSeqMode(static_cast<SEQ_MODE>(val));
  else
  ...
  if (getWarzoneKeyNumeric("difficulty", &val))
    setDifficultyLevel(static_cast<DIFFICULTY_LEVEL>(val));
  else
  {
  ...
  if (getWarzoneKeyNumeric("type", &val))
    game.type = static_cast<UBYTE>(val);
  else
  {
    game.type = CAMPAIGN;
  ...
  if (getWarzoneKeyNumeric("base", &val))
    game.base = static_cast<UBYTE>(val);
  else
  {
    game.base = CAMP_BASE;
  ...
  if (getWarzoneKeyNumeric("limit", &val))
    game.limit = static_cast<UBYTE>(val);
  else
  {
    game.limit = NOLIMIT;
  ...
  if (getWarzoneKeyNumeric("maxPlay", &val))
    game.maxPlayers = static_cast<UBYTE>(val);
  else
  {
    game.maxPlayers = 4;
  ...
  if (getWarzoneKeyNumeric("alliance", &val))
    game.alliance = static_cast<UBYTE>(val);
  else
  {
...
BOOL saveConfig()
{
  ...
  if (getDifficultyLevel() == DL_KILLER OR getDifficultyLevel() == DL_TOUGH)
    setDifficultyLevel(DL_NORMAL);
  setWarzoneKeyNumeric("allowSubtitles", static_cast<DWORD>(bAllowSubtitles));
  setWarzoneKeyNumeric("gamma", static_cast<DWORD>(gamma * 25)); // gamma
  setWarzoneKeyNumeric("scroll", scroll_speed_accel); // scroll
  setWarzoneKeyNumeric("difficulty", getDifficultyLevel()); // level	
  setWarzoneKeyNumeric("barmode", barMode); //energybars		
  setWarzoneKeyNumeric("visfog", !war_GetFog()); // fogtype
  setWarzoneKeyNumeric("shake", static_cast<DWORD>(getShakeStatus())); // screenshake
  setWarzoneKeyNumeric("mouseflip", static_cast<DWORD>(getInvertMouseStatus())); // flipmouse
  setWarzoneKeyNumeric("sequences", static_cast<DWORD>(war_GetSeqMode())); // sequences
  setWarzoneKeyNumeric("subtitles", static_cast<DWORD>(seq_GetSubtitles())); // subtitles
  setWarzoneKeyNumeric("reopenBuild", static_cast<DWORD>(intGetReopenBuild())); // build menu
...
BOOL openWarzoneKey()
{
  ...
  result = RegCreateKeyEx( // create/open key.
    ...
    0, // reserved 
    nullptr, // address of class string 
...
BOOL getWarzoneKeyNumeric(STRING* pName, DWORD* val)
{
  ...
  // check guid exists
  if (RegQueryValueEx(ghWarzoneKey, pName, nullptr, &type, (UCHAR*)&result, &resultsize) != ERROR_SUCCESS)
    return FALSE;
  if (type == REG_DWORD)
  {
...
BOOL setWarzoneKeyBinary(STRING* pName, VOID* pData, UDWORD size)
{
  RegSetValueEx(ghWarzoneKey, pName, 0, REG_BINARY, static_cast<const BYTE*>(pData), size);
  return TRUE;
}
