...
void displayRequestOption(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  for (count = 0; count < widgUnpackUserData(psWidget->pUserData); count++)
...
VOID addMultiRequest(STRING* ToFindb, UDWORD mode, UBYTE mapCam)
{
  ...
  if (mode == MULTIOP_MAP) // if its a map, also look in the predone stuff.
  {
    ...
    {
      numButtons++;
      while (enumerateMultiMaps(tips[0], &players, FALSE, mapCam)) { numButtons++; }
    }
  }
  ...
  sFormInit.x = static_cast<SWORD>((M_REQUEST_X + D_W));
  sFormInit.y = static_cast<SWORD>((M_REQUEST_Y + D_H));
  ...
  sButInit.pUserData = static_cast<void*>(nullptr);
  ...
  if (dir != INVALID_HANDLE_VALUE)
  {
    ...
    while (count < (butPerForm * 4))
    {
      ...
      {
        sButInit.pUserData = widgPackUserData(found.cFileName[0] - '0');
      ...
      sButInit.id += 1;
      sButInit.x = static_cast<SWORD>(sButInit.x + (R_BUT_W + 4));
      if (sButInit.x + R_BUT_W + 2 > M_REQUEST_W)
      {
        sButInit.x = 17;
        sButInit.y = static_cast<SWORD>(sButInit.y + R_BUT_H + 4);
      }
  ...
  if (mode == MULTIOP_MAP)
  {
    if (enumerateMultiMaps(sTemp, &players,TRUE, mapCam))
    {
      do
      {
        ...
        sButInit.pTip = tips[sButInit.id - M_REQUEST_BUT];
        sButInit.pText = tips[sButInit.id - M_REQUEST_BUT];
        sButInit.pUserData = widgPackUserData(players);
...
BOOL runMultiRequester(UDWORD id, UDWORD* mode, STRING* chosen, UDWORD* chosenValue)
{
  ...
  if (id >= M_REQUEST_BUT && id <= M_REQUEST_BUTM) // chose a file.
  {
    ...
    *chosenValue = widgUnpackUserData(widgGetFromID(psRScreen, id)->pUserData);
    closeMultiRequester();
    *mode = context;
    return TRUE;
  }
...
void displayMultiPlayer(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  player = static_cast<int>(widgUnpackUserData(psWidget->pUserData)); //get the in game player number.
...
void displayAllianceState(struct _widget* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  player = widgUnpackUserData(psWidget->pUserData);
  ...
  psWidget->pUserData = widgPackUserData(PACKDWORD_TRI(a, b, c));
  intDisplayImageHilight(psWidget, xOffset, yOffset, pColours);
  psWidget->pUserData = widgPackUserData(player);
}
...
void addMultiPlayer(UDWORD player, UDWORD pos)
{
  ...
  if (player != selectedPlayer)
  {
    ...
    sButInit.pUserData = widgPackUserData(player);
  ...
  if (game.alliance != NO_ALLIANCES && player != selectedPlayer)
  {
    ...
    sButInit.pUserData = widgPackUserData(PACKDWORD_TRI(0, IMAGE_MULTI_VIS_HI, IMAGE_MULTI_VIS));
    widgAddButton(psWScreen, &sButInit);

    sButInit.id = MULTIMENU_GIFT_RES + player;
    sButInit.x = MULTIMENU_C5;
    sButInit.pTip = strresGetString(psStringRes, STR_ALLI_TEC);
    sButInit.pUserData = widgPackUserData(PACKDWORD_TRI(0, IMAGE_MULTI_TEK_HI, IMAGE_MULTI_TEK));
    widgAddButton(psWScreen, &sButInit);

    sButInit.id = MULTIMENU_GIFT_DRO + player;
    sButInit.x = MULTIMENU_C6;
    sButInit.pTip = strresGetString(psStringRes, STR_ALLI_DRO);
    sButInit.pUserData = widgPackUserData(PACKDWORD_TRI(0, IMAGE_MULTI_DRO_HI, IMAGE_MULTI_DRO));
    widgAddButton(psWScreen, &sButInit);

    sButInit.id = MULTIMENU_GIFT_POW + player;
    sButInit.x = MULTIMENU_C7;
    sButInit.pTip = strresGetString(psStringRes, STR_ALLI_POW);
    sButInit.pUserData = widgPackUserData(PACKDWORD_TRI(0, IMAGE_MULTI_POW_HI, IMAGE_MULTI_POW));
    widgAddButton(psWScreen, &sButInit);
...
BOOL intAddMultiMenu(VOID)
{
  ...
  sFormInit.style = WFORM_PLAIN;
  sFormInit.x = static_cast<SWORD>((MULTIMENU_FORM_X));
  sFormInit.y = static_cast<SWORD>((MULTIMENU_FORM_Y));
  sFormInit.width = MULTIMENU_FORM_W;
  sFormInit.height = static_cast<UWORD>(formHeight); //MULTIMENU_FORM_H;
  sFormInit.pDisplay = intOpenPlainForm;
  sFormInit.disableChildren = TRUE;
...
BOOL intRunMultiMenu(VOID) { return TRUE; }
...
BOOL intCheckAllianceValid(UBYTE player1, UBYTE player2)
{
  ...
  if ((player1 == player2) || aiCheckAlliances(player1, player2) || !(isHumanPlayer(player1) && isHumanPlayer(player2)))
    return FALSE;
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    if (isHumanPlayer(i))
    {
      if ((i != player1) && aiCheckAlliances(i, player1) || (i != player2) && aiCheckAlliances(i, player2))
        iAlliances++;
    }
  }
...
VOID intProcessMultiMenu(UDWORD id)
{
  ...
  {
    i = static_cast<UBYTE>(id - MULTIMENU_ALLIANCE_BASE);

    switch (alliances[selectedPlayer][i])
    {
    case ALLIANCE_BROKEN:
      if (intCheckAllianceValid(static_cast<UBYTE>(selectedPlayer), i))
        requestAlliance(static_cast<UBYTE>(selectedPlayer), i,TRUE,TRUE); // request an alliance
      break;
    case ALLIANCE_INVITATION:
      if (intCheckAllianceValid(static_cast<UBYTE>(selectedPlayer), i))
        formAlliance(static_cast<UBYTE>(selectedPlayer), i,TRUE,TRUE); // form an alliance
      break;
    case ALLIANCE_REQUESTED:
      breakAlliance(static_cast<UBYTE>(selectedPlayer), i,TRUE,TRUE); // break an alliance
      break;
  ...
  {
    i = static_cast<UBYTE>(id - MULTIMENU_CHANNEL);
    if (openchannels[i])
