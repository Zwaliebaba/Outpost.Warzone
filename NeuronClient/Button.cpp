...
void buttonSetFlash(W_BUTTON* psButton) { psButton->state |= WBUTS_FLASH; }
...
void buttonClicked(W_BUTTON* psWidget, UDWORD key)
{
  ...
  if (!(psWidget->state & (WBUTS_GREY | WBUTS_LOCKED)))
  {
    ...
    if ((!(psWidget->style & WBUT_NOPRIMARY) && key == WKEY_PRIMARY) || ((psWidget->style & WBUT_SECONDARY) && key == WKEY_SECONDARY))
    {
...
void buttonDisplay(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset, UDWORD* pColours)
{
  ...
  else if (psButton->state & WBUTS_GREY)
  {
    ...
    if (psButton->pText)
    {
      ...
      fx = x0 + (psButton->width - fw) / 2;
      fy = y0 + (psButton->height - Neuron::GetTextLineSize()) / 2 - Neuron::GetTextAboveBase();
      Neuron::SetTextColour(*(pColours + WCOL_LIGHT));
      pie_DrawText((unsigned char*)psButton->pText, fx + 1, fy + 1);
      Neuron::SetTextColour(*(pColours + WCOL_DISABLE));
      pie_DrawText((unsigned char*)psButton->pText, fx, fy);
    }
