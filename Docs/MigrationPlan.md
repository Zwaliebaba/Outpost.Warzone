...
| Was | Is |
|---|---|
| `dplay.h`, `dplobby.h`, `dplayx.lib` | `NetTransport.h`, `msquic.lib` (NuGet) |
...
| `DPSYS_*` system messages | `NETTRANS_EVENT`, drained by `NETeventHandler` |
...
a self-signed certificate generated by `NetCert.cpp` is not a question
...
- **No way to find a game.** Hosting works; joining takes a typed address,
  ...
  when the relay server answers `nettrans_FindSessions`.
...
Two rules in [AGENTS.md](../AGENTS.md) that this phase does not satisfy, both
predating it: **R14** forbids new third-party dependencies and a package
manager, and MsQuic arrives as a NuGet package; and **§1**'s naming table wants
`Transport`/`UdpTransport` types where this has `nettrans_*` free functions
matching the surrounding `NET*` code. Both were owner decisions taken before
AGENTS.md existed and neither has been revisited.
