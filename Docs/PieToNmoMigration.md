...
|---|---|---|---|
| **A** | `NeuronClient/Nmo.h` — the §4 structs, no logic, with the layout assertions. | Builds x64, `check_case.py` green | **Done.** 31 `static_assert`s (19 size, 12 layout), verified at both widths — x64, which ships, and the unmaintained x86 configurations, because width-independence is the property being asserted |
...
- `Nmo.h`'s 31 assertions — 19 on struct sizes, 12 on alignment and trivial
  copyability — compile at both pointer widths, so a file written by either
  build is the same file. x64 is what ships; the x86 check is what makes
  width-independence a tested claim rather than an intention.
...
- All 208 translation units cross-check clean (`tools/crosscheck.py`), Debug
  and Release, at both pointer widths. That now includes `NmoTest.cpp`: the
  CppUnitTest stub added on 2026-08-27 brought the test projects into the
  cross-checker, so the loader suite is syntax-checked on Linux and built and
  **run** by CI on Windows.
